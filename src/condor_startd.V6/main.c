 ...
 */
int coll_sock, alt_coll_sock, intcp_sock, inudp_sock;
DGRAM_IO_HANDLE coll_handle, alt_coll_handle;
...
extern void event_sigint	__P((int));
extern void event_sighup	__P((int));
extern void event_sigterm	__P((int));
extern void event_sigchld	__P((int));
	...
	char **argv;
{
	...
	init_params();
	resource_init();
	store_stmt(scan(def_owner), template_context);
	...
	fill_dgram_io_handle(&coll_handle, collector_host, coll_sock,
			     COLLECTOR_UDP_PORT);
	if (alt_collector_host) {
		fill_dgram_io_handle(&alt_coll_handle, alt_collector_host,
				     alt_coll_sock, COLLECTOR_UDP_PORT);
	}
...
static int
res_config_context(rip)
	resource_info_t *rip;
{
	config(MyName, rip->r_context);
}
