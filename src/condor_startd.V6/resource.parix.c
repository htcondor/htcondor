...
int event_vacate   __P((resource_id_t, job_id_t, task_id_t));
static int event_block    __P((resource_id_t, job_id_t, task_id_t));
static int event_unblock  __P((resource_id_t, job_id_t, task_id_t));
static int event_start    __P((resource_id_t, job_id_t, task_id_t));
int event_suspend  __P((resource_id_t, job_id_t, task_id_t));
int event_continue __P((resource_id_t, job_id_t, task_id_t));
int event_checkpoint     __P((resource_id_t, job_id_t, task_id_t));
int event_exited   __P((resource_id_t, job_id_t, task_id_t));
	...
	resource_info_t *rip;
{
	...
	tmp.type = FLOAT;
	tmp.f_val = rp->res.res_load;
	store_stmt(build_expr("LoadAvg", &tmp), cp);

	tmp.type = INT;
	tmp.i_val = rp->res.res_memavail;
	store_stmt(build_expr("VirtualMemory", &tmp), cp);

	tmp.type = INT;
	tmp.i_val = rp->res.res_diskavail;
	store_stmt(build_expr("Disk", &tmp), cp);

	tmp.type = INT;
	tmp.i_val = rp->res.res_idle;
	store_stmt(build_expr("KeyboardIdle",&tmp), cp);
	...
	if (rip->r_pid == NO_PID) {
		tmp.type = STRING;
		tmp.s_val = state_to_string(pinf[rip->r_rid].p_state);
	...
	tmp.type = STRING;
	if (!strncmp(rip->r_name, "aix", 3))
		tmp.s_val = "EPX-AIX";
	else
		tmp.s_val = "EPX-NK";
	store_stmt(build_expr("OpSys",&tmp), cp);

	tmp.type = STRING;
	tmp.s_val = rip->r_name;
	store_stmt(build_expr("Machine", &tmp), cp);
	...
	resource_info_t *rip;
{
	...
	char	*addrname;
	int phys_memory = -1;
	CONTEXT *config_context = rip->r_context;
	struct sockaddr_in sin;
	int len = sizeof sin;

	tmp.type = STRING;
	tmp.s_val = rip->r_name;
	store_stmt(build_expr("Name", &tmp), config_context);
	...
	tmp.s_val = host_cell;
	if (tmp.s_val) {
		store_stmt(build_expr("AFS_Cell",&tmp), config_context);
	...
	dprintf(D_ALWAYS, "config_context = %x\n", config_context);
	store_stmt(build_expr("UidDomain",&tmp), config_context);
	...
	tmp.type = STRING;
	tmp.s_val = ptr;
	store_stmt(build_expr("FilesystemDomain",&tmp), config_context);
	dprintf(D_ALWAYS, "FilesystemDomain = \"%s\"\n", tmp.s_val);

	tmp.type = INT;
	tmp.i_val = calc_ncpus(rip);
	store_stmt(build_expr("Cpus",&tmp), config_context);
	 ...
	 */
	tmp.type = STRING;
	tmp.s_val = param("NEST");
	if (!tmp.s_val)
		tmp.s_val = "NOTSET";
	store_stmt(build_expr("Nest",&tmp), config_context);
	...
	tmp.type = STRING;
	tmp.s_val = addrname;
	store_stmt(build_expr((char *)ATTR_STARTD_IP_ADDR, &tmp), config_context);
}
