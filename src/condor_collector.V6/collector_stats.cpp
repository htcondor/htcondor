...
void UpdatesStats::Init()
{
    ...
    #define ADD_SUBSYS_PROBES(pool,subsys,as) \
	   ...
	   pool.AddProbe("ForkQueriesFrom" #subsys, &ForkQueriesFrom[SUBSYSTEM_TYPE_##subsys], "ForkQueriesFrom" #subsys, as | ForkQueriesFrom[SUBSYSTEM_TYPE_ ## subsys].PubDefault);
	...
	ADD_SUBSYS_PROBES(Pool,DAEMON,IF_BASICPUB | IF_NONZERO);
	ADD_SUBSYS_PROBES(Pool,TOOL,IF_BASICPUB | IF_NONZERO);
	ADD_SUBSYS_PROBES(Pool,SUBMIT,IF_BASICPUB | IF_NONZERO);
	ADD_SUBSYS_PROBES(Pool,JOB,IF_BASICPUB | IF_NONZERO);
#define SUBSYSTEM_TYPE_UNKNOWN SUBSYSTEM_TYPE_INVALID
	ADD_SUBSYS_PROBES(Pool,UNKNOWN,IF_BASICPUB | IF_NONZERO);
	//pool.AddProbe("QueriesFromUNKNOWN", &QueriesFrom[0], "QueriesFromUNKNOWN", IF_BASICPUB | IF_NONZERO | QueriesFrom[0].PubDefault);
#endif
