...
void ViewServer::Init()
{
	...
	for (int i=0; i<DataSetCount; i++) {
		for (int j=0; j<HistoryLevels; j++) {
			DataSet[i][j].AccData=new AccHash(hashFunction,allowDuplicateKeys);
		}
	}
	GroupHash = new AccHash(hashFunction,allowDuplicateKeys);
	FileHash = new HashTable< MyString, int >(hashFunction,allowDuplicateKeys);
