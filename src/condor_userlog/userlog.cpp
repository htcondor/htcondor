...
HashTable<std::string, JobStatistics *> Stats(1024, hashFunction);
HashTable<std::string, HostStatistics *> HStats(1024, hashFunction);
int numJobStats = 0;
int numHostStats = 0;
...
read_log(const char *filename, int select_cluster, int select_proc)
{
	...
	char hash[40];
	HashTable<std::string, ExecuteEvent *> ExecRecs(1024, hashFunction);
	HashTable<std::string, CheckpointedEvent *> CkptRecs(1024, hashFunction);
