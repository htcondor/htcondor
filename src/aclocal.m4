...
EOF
 ...
 # We also check the exit status of the compiler.
 [$1 $2 $4 conf_flag.c -o conf_flag >conf_flag.out 2>&1
  _comp_status=$?
  if test $_comp_status -ne 0; then
    ...
    grep "option" <conf_flag.out >/dev/null 2>&1
    _comp_status=$?
 ...
 if test $[_cv_$3] = "yes"; then
  ...
  rm -f conf_flag.c conf_flag conf_flag.out
])
