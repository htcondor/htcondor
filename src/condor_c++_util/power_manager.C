...
PowerManager::PowerManager () throw ()
...
PowerManager::update () {
	...
	if ( change ) {
		dprintf ( D_ALWAYS, "PowerManager: Hibernation is %s\n", 
			( _interval > 0 ? "enabled" : "disabled" ) );
	}
}

bool 
PowerManager::doHibernate ( int state ) const {
	if ( _hibernator ) {
...
bool 
PowerManager::canHibernate () const {
	bool can = false;	
	if ( _hibernator ) {
...
bool 
PowerManager::wantsHibernate () const {
	bool does = false;
	if ( _hibernator ) {
		if ( canHibernate () ) {
			does = ( _interval > 0 );
...
int PowerManager::getHibernateCheckInterval () const {
	return _interval;
}