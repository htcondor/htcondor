...
ResMgr::ResMgr()
{
	...
	if ( NULL == primary ) {
		...
		dprintf (
			D_FULLDEBUG, 
			"Using network interface %s for hibernation\n",
...
ResMgr::reconfig_resources( void )
{
	...
	m_hook_mgr->reconfig();
		...
		// Refresh the hibernation configuration from the config file
...
ResMgr::checkHibernate( void )
{
	...
	if ( level > 0 ) {
        ...
        if ( !m_hibernation_manager->canHibernate () ) {
            dprintf ( 
				D_ALWAYS, 
				"ResMgr: ERROR: Ignoring HIBERNATE: Machine does "
				"not support any sleep states.\n" );
            return;
        }
        ...
        if ( !m_hibernation_manager->canWake () ) {
            ...
            dprintf ( 
				D_ALWAYS, 
				"ResMgr: ERROR: Ignoring HIBERNATE: Machine cannot "
				"be woken by its public network adapter (%s).\n",
				netif->interfaceName () );
            return;
		}        
...
ResMgr::cancelHibernateTimer ( void )
{
	...
	if( m_hibernate_tid != -1 ) {
		...
		if( rval < 0 ) {
			dprintf ( 
				D_ALWAYS, 
				"Failed to cancel hibernation timer (%d): "
				"daemonCore error\n", 
				m_hibernate_tid );
		} else {
			dprintf (
				D_FULLDEBUG, 
				"Canceled hibernation timer (%d)\n",
				m_hibernate_tid );
		}
		m_hibernate_tid = -1;
	}
...
ResMgr::disableResources ( const MyString &state_str )
{
	...
	/* set the sleep state so the plug-in will pickup on the
...
void ResMgr::setHibernating ( bool enable ) {
	m_hibernating = enable;
}
