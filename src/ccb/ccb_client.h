...
class CCBClient: public Service, public ClassyCountedPtr {
 ...
 private:
	...
	// The ccbids to reach the target, outermost first, parsed from a (possibly
	// nested) CCB contact <entry>#id0#...#idN.  We connect m_proxy_entry once and
	// hand the entry broker the whole route in a single request; the brokers
	// recurse server-side, so the client never walks the hops itself.
	...
	// broker directly; for a nested contact it connects the entry broker and hands
	// it the whole route in one CCB_REQUEST, and the brokers recurse server-side.
	// Returns a raw ReliSock that is a transparent end-to-end pipe to the daemon
	// (caller runs CEDAR over it), or nullptr.
	...
	// Start the (single) CCB_REQUEST to the entry broker over m_proxy_sock; the
	// broker forwards the remaining route inward, so the client issues just one
	// request rather than a handshake per hop.
	void startProxyHop();
