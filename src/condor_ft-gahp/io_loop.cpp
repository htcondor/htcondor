...
main_init( int argc, char ** const argv )
{
	...
	*/
	...
	char * ft_gahp_worker_thread = param("CONDOR_GAHP_WORKER");
	if (ft_gahp_worker_thread) {
		exec_name = ft_gahp_worker_thread;
		free(ft_gahp_worker_thread);
	}
	else {
		char * ft_gahp_name = param ("CONDOR_GAHP");
		ASSERT (ft_gahp_name);
		sprintf(exec_name, "%s_worker_thread", ft_gahp_name);
		free (ft_gahp_name);
	}
	...
	while ((line = stdin_buffer.GetNextLine()) != NULL) {
		...
		if (parse_gahp_command (command, &args) &&
			...
			if (strcasecmp (args.argv[0], GAHP_COMMAND_RESULTS) == 0) {
				...
				/* TODO
				 * add ft-gahp command dispatches
				 */
			} else {
...
/* DON'T NEED THIS FUNCTION, BUT NEED TO LIFT THE CODE THAT UPDATES
 * THE RESULTS AND SIGNALS
