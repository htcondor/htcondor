...
#define cALLOc		condor_calloc
#define fREe		condor_free
#define mALLOc		condor_malloc
#define mEMALIGn	condor_memalign
#define rEALLOc		condor_realloc
#define vALLOc		condor_valloc
...
#define malloc_stats	condor_malloc_stats
...
static mbinptr *av_ = NULL;

/* We can't store the contents of av_ in the data segment, so we store
   it at the start of the alternate heap allocated for us by Condor. */
int
condor_malloc_data_size()
{
	return (NAV*2+2)*sizeof(mbinptr);
}

int
condor_malloc_init(mbinptr *av)
{
	int i;
