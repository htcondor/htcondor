...
IpVerify::IpVerify() 
{
	...
	PermHashTable = new PermHashTable_t(compute_perm_hash, allowDuplicateKeys);
}
...
IpVerify::add_hash_entry(const struct in6_addr & sin6_addr, const char * user, perm_mask_t new_mask)
{
	...
	if ( PermHashTable->lookup(sin6_addr, perm) != -1 ) {
        ...
        perm = new UserPerm_t(hashFunction, allowDuplicateKeys);
...
IpVerify::fill_table(PermTypeEntry * pentry, char * list, bool allow)
{
    ...
    UserHash_t * whichUserHash = new UserHash_t(hashFunction, allowDuplicateKeys);
