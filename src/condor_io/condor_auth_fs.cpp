...
int Condor_Auth_FS::authenticate(const char * /* remoteHost */, CondorError* errstack, bool non_blocking)
{
	...
	if ( mySock_->isClient() ) {
		...
		if (new_dir.empty()) {
			// server sends null string if its mktemp failed
		...
		if (param_boolean("SEC_FS_ENFORCE_DIR_MATCH", true)) {
			std::string server_parent_dir = condor_dirname(new_dir.c_str());
			if (server_parent_dir != parent_dir) {
				dprintf(D_SECURITY, "AUTHENTICATE_FS%s: Server-requested directory '%s' isn't under expected directory '%s'\n", (remote_?"_REMOTE":""), new_dir.c_str(), parent_dir.c_str());
				errstack->push((remote_?"FS_REMOTE":"FS"), 1003,
					"Directory name not under expected parent dir.");
				goto client_reply;
			}
		}
		if (param_boolean("SEC_FS_ENFORCE_CHANNEL_BONDING", true)) {
			...
			dprintf(D_STATUS, "JEF server:%s prefix:%s\n", new_dir_basename, new_dir_prefix.c_str());
