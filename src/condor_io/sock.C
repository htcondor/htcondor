 ...
 */
int Sock :: _condor_read(SOCKET fd, char *buf, int sz, int timeout)
{
	int nr = 0, nro;
...
int Sock :: _condor_write(SOCKET fd, char *buf, int sz, int timeout) 
{
...
int Sock :: condor_read(SOCKET fd, char *buf, int sz, int timeout)
{
	int nr = 0;
	unsigned int l_in = 0 , temp = 0;
	static int l_out = 0, r_sz =0;
	char *d_in;
	static char *d_out = NULL;
	
	/// condor_read and write are now protected members of Sock. The fd parameter
	///is meaningless.
	...
	if (is_encrypt()){
		...
		if (!is_hdr_encrypt()){			
			...
			//recv sizeof encrypted data
			nr = _condor_read(fd,(char*)&temp,sizeof(int),timeout);
			...
			dprintf(D_ALWAYS,"condor_read: length of encrypted data = %d\n",l_in);

			d_in =(char*) malloc(l_in);
			nr = _condor_read(fd, d_in,l_in, timeout); 
			if ( nr < 0 ){
				dprintf(D_ALWAYS,"Sock :: condor_read: _condor_read failure\n");
				return -1;
			}	
			...
			if ( nr == l_in ){
				...
				dprintf(D_ALWAYS,"Sock :: condor_read: Could not receive all encrypted data\n");
				return -1;
			}	
		}
		else{
		   if (sz > l_out - r_sz ){
				dprintf(D_ALWAYS,"Sock::condor_read : not enough data to read\n");
				return -1;
			}	
		   memcpy(buf,d_out + r_sz, sz);
		   free(d_out);
		   d_out = NULL;
		   r_sz = 0;
		   l_out = 0;
