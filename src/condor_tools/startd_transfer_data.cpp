...
main(int argc, char *argv[])
{
	...
	if( ! scheddAddr ) {
		...
		startd = new DCStartd( scheddName, pool );
	} else {
		startd = new DCStartd( scheddAddr );
	}
	if( ! startd->locate() ) {
		fprintf( stderr, "%s: %s\n", MyName, startd->error() ); 
		exit( 1 );
	} else {
		printf("STARTD location successful. \n");

		std::string bar;
		startd->getSandboxLocation(sandbox_id, bar);
