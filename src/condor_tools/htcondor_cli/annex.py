...
class Status(Verb):
    ...
    def __init__(self, logger, **options):
        ...
        if htcondor.param.get("ANNEX_USE_OLD_CREATE", False):
            self.status_old(logger, **options)
            return
        ...
        for slot in annex_slots:
            ...
            slot_retirement = slot.get("DaemonStopTime")
            ...
            if annex_birthday is None or slot_birthday < annex_birthday:
                ...
                annex_attrs[annex_name]['first_retirement'] = slot_retirement
            ...
            if annex_birthday is None or slot_birthday > annex_birthday:
                ...
                annex_attrs[annex_name]['last_retirement'] = slot_retirement
        ...
        for annex_name, annex_status in status.items():
            ...
            if total_CPUs != 0:
                ...
                oldest_birthday = annex_attrs[annex_name]['first_birthday']
                oldest_retirement = annex_attrs[annex_name]['first_retirement']
                youngest_birthday = annex_attrs[annex_name]['last_birthday']
                youngest_retirement = annex_attrs[annex_name]['last_retirement']

                now = int(time.time())
                print(
                    f"Its oldest established request is about "
                    f"{(now - oldest_birthday)/(60*60):.2f} hours old",
                    end=""
                )
                if oldest_retirement is not None:
                    print(
                        f" and will retire in "
                        f"{(oldest_retirement - now)/(60*60):.2f} hours."
                    )
                else:
                    print(".")
            ...
            # How big is it?
            print(f"There are {node_count} nodes in the established annex.")
    ...
    def status_old(self, logger, **options):
        the_annex_name = options["annex_name"]
