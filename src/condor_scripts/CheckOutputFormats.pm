...
sub check_heading {
	...
	my %rules_heading = (
		...
		'-io' => sub {return $data{0} =~ /\s*ID\s+OWNER\s+RUNS\s+ST\s+INPUT\s+OUTPUT\s+RATE\s+MISC/},
	...
	if (defined $real_heading && $real_heading ne '-batch'){
		...
		if ($data{0} =~ /\s*OWNER\s+BATCH_NAME\s+SUBMITTED\s+DONE\s+RUN\s+IDLE\s+HOLD\s+TOTAL\s+JOB_IDS/){
			print "        PASSED: Headings are correct\n";
			return 1;
		} else {
			print "        ERROR: Headings are not correct\n";
