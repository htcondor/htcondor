...
void TransferState::Find(transferinfo*& ptr,
			 transferinfo*& trail,
			 int            child_pid)
{ 
  ptr = head;
...
*                                      bytes)                                 *
*        u_lint          priority    - the priority level of the checkpoint   *
			   ...
			   u_short         type)
{
  ...
  machine_name = gethostnamebyaddr(shadow_addr);
  strncpy(t->machine, machine_name, MAX_MACHINE_NAME_LENGTH);
  strncpy(t->filename, filename, MAX_CONDOR_FILENAME_LENGTH);
  strncpy(t->owner, owner, MAX_NAME_LENGTH);
  t->child_pid = child_pid;
  t->start_time = time(NULL);
  t->version = fs.GetVersion(t->machine, owner, filename) + 1;
			  ...
			  int&      fs_code)
{
  ...
  if (ptr == NULL)
	...
	if (status == CHILDTERM_SUCCESS)
	  ...
	  {
	    sprintf(temppath, "/tmp/%s/%s/%s.%d", ptr->machine, ptr->owner,
		    ptr->filename, ptr->version);
  ...
  if (num_transfers != 0)
    {
      cout << endl << "WARNING:" << endl;
      cout << "WARNING:" << endl;
      cout << "WARNING: incorrect count of nodes in transfer list" << endl;
      cout << "WARNING:" << endl;
      cout << "WARNING:" << endl << endl;
    }
}
