...
main( int argc, char* argv[] ) {
	...
	MyString buff;
	buff.sprintf ("-DGLOBUS_LOCATION=%s", gt42location);
	command_line.append (buff.Value());

	buff.sprintf ("-Djava.endorsed.dirs=%s/endorsed", gt42location);
	command_line.append (buff.Value());
  
	buff.sprintf ("-Dorg.globus.wsrf.container.webroot=%s", gt42location);
	command_line.append (buff.Value());
...
*/
	...
	const char *ctmp;
	buff.sprintf( "%s/lib", gt42location );
	...
	buff.sprintf( "%s/lib/common", gt42location );
