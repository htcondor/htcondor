...
//			setPollInterval(0);
	...
	if ( contains(m_commands_supported, "CACHE_PROXY_FROM_FILE", true) &&
		 contains(m_commands_supported, "UNCACHE_PROXY", true) &&
		 contains(m_commands_supported, "USE_CACHED_PROXY", true) ) {
...
GahpServer::Initialize( Proxy *proxy )
{
	...
	if ( contains(m_commands_supported, "INITIALIZE_FROM_FILE", true) ) {
...
GahpServer::CreateSecuritySession()
{
	...
	if  (contains(m_commands_supported, command, true)==false) {
		return false;
	}
...
GahpServer::doProxyCheck()
{
	...
	if ( master_proxy->proxy->expiration_time >
		...
		if ( contains(m_commands_supported, command, true) ) {
							  ...
							  char **job_id)
{
	...
	std::string ad_string;
		...
		// Check if this command is supported
	if (contains(server->m_commands_supported, command, true)==false) {
