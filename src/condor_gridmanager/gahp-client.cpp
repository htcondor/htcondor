...
int
GahpClient::nordugrid_status_all(const char *hostname, StringList &job_ids,
								 StringList &statuses)
{
	static const char* command = "NORDUGRID_STATUS_ALL";
	...
	if ( result ) {
		...
		int num_results = atoi(result->argv[3]);
		if ( result->argc != (2 * num_results) + 4 ) {
			EXCEPT( "Bad %s Result", command );
		}
		int rc = atoi(result->argv[1]);
		if ( strcasecmp(result->argv[2], NULLSTRING) ) {
		...
		for ( int i = 0; i < num_results; i++ ) {
			job_ids.append( result->argv[2*i + 4] );
			statuses.append( result->argv[2*i + 5] );
		}
		delete result;
		return rc;
	}
