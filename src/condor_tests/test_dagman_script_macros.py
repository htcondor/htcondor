...
SCRIPT DEBUG script.debug ALL POST GOOD {writePostScript} $node $JOB $RETRY $MAX_RETRIES $DAG_STATUS $FAILED_COUNT $FUTILE_COUNT $DONE_COUNT $QUEUED_COUNT $NUM_NODES $DAG_ID $JOBID $CLUSTERID $NUM_JOBS $SUCCESS $RETURN $EXIT_CODES $EXIT_FREQUENCIES $PRE_SCRIPT_RETURN $NUM_JOBS_ABORTED
SCRIPT DEBUG script.debug ALL POST BAD {writePostScript} $NoDe $JOB $RETRY $MAX_RETRIES $DAG_STATUS $FAILED_COUNT $FUTILE_COUNT $DONE_COUNT $QUEUED_COUNT $NUM_NODES $DAG_ID $JOBID $CLUSTERID $NUM_JOBS $SUCCESS $RETURN $EXIT_CODES $EXIT_FREQUENCIES $PRE_SCRIPT_RETURN $NUM_JOBS_ABORTED
        """)
