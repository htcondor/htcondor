...
@action
def invoke_no_args():
    p = run_command(["htcondor"])
    return p


@action
def submit_failure():
    p = run_command(["htcondor", "job", "submit", "does_not_exist.sub"])
    return p


@action
def submit_success(test_dir, default_condor):
...
@action
def resource_missing_args(test_dir, submit_success):
    p = run_command(["htcondor", "job", "submit", test_dir / "helloworld.sub", "--resource", "ec2"])
    return p

@action
def slurm_not_setup(test_dir):
    p = run_command(["htcondor", "job", "submit", test_dir / "helloworld.sub", "--resource", "slurm", "--runtime", "1", "--node_count", "1"])
    return p
...
class TestCurlPlugin:
    ...
    def test_resource_missing_args(test_dir, resource_missing_args):
        assert resource_missing_args.stdout == "Error: EC2 resources must specify a --runtime argument"

    def test_slurm_not_setup(test_dir, slurm_not_setup):
