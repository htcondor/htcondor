...
@action
def submit_output_job_cmd(test_dir, default_condor, dataflow_input_file, dataflow_output_file):
    ...
    executable_file = write_file(test_dir / "submit" / "dataflow-output.sh", executable_description)
    ...
    submit_file = write_file(test_dir / "submit" / "dataflow-output.sub", sub_description)
...
@action
def submit_dataflow_skip_job_cmd(test_dir, default_condor, finished_output_jobid, path_to_sleep, dataflow_input_file, dataflow_output_file):
    """
...
@action
def job_queue_events_for_dataflow_skip_job(default_condor, finished_dataflow_skip_jobid):
    return default_condor.job_queue.by_jobid[finished_dataflow_skip_jobid]

@action
def submit_dataflow_noskip_job_cmd(test_dir, default_condor, finished_output_jobid, path_to_sleep, dataflow_input_file, dataflow_output_file):
    """
    After submit_output_job_cmd() has completed, we now want to send a new job
    ...
    submit_file = write_file(test_dir / "submit" / "dataflow-noskip.sub", sub_description)
...
@action
def job_queue_events_for_dataflow_noskip_job(default_condor, finished_dataflow_noskip_jobid):
...
class TestDataflowJob:
    ...
    def test_dataflow_skip_job_completed_successfully(self, finished_dataflow_skip_jobid):
        assert finished_dataflow_skip_jobid is not None

    def test_dataflow_skip_job_events_in_correct_order(self, job_queue_events_for_dataflow_skip_job):
        assert SetAttribute("DataflowJobSkipped", "true") in job_queue_events_for_dataflow_skip_job

    def test_dataflow_noskip_job_completed_successfully(self, finished_dataflow_noskip_jobid):
        assert finished_dataflow_noskip_jobid is not None

    def test_dataflow_noskip_job_events_in_correct_order(self, job_queue_events_for_dataflow_noskip_job):
        assert SetAttribute("DataflowJobSkipped", "true") not in job_queue_events_for_dataflow_noskip_job
