...
MENU_ITEM	Menu[] = {
	...
	{ DIE,		 		"DIE",				1	},
	{ VACATE,		 	"VACATE",			5	},
	{ CKPT_and_VACATE,	"CKPT_and_VACATE",	5	},
	{ NO_EVENT,			NULL,				-1 	}
};
int		MenuSize;
int		MaxWeight;
...
main( int argc, char *argv[] )
{
	...
	if( argc == 2 ) {	// Seed supplied on the command line
		Seed = atoi( argv[1] );
	} else {			// Pick a seed 0 - 999
		Seed = ( (int)time(0) % 1000 );
	}
	srandom( Seed );
...
get_command()
{
	...
	for(;;) {
		raw = random();
		index = (int)(raw % MenuSize);
		// printf( "index = %d, weight = %d\n", index, Menu[index].weight );
		if( weighted_toss(Menu[index].weight) ) {
			// printf( "Accepted\n" );
			printf( "Sending %s\n", Menu[index].str );
			return Menu[index].ev;
		} else {
			// printf( "Rejected\n" );
		}
	}
...
int
weighted_toss( int weight )
{
	int		i;
	int		toss;

	for( i=0; i < weight; i++ ) {
		toss = random() % MaxWeight;
		// printf( "toss %d = %d\n", i, toss );
		if( toss == 0 ) {
			return TRUE;
		}
