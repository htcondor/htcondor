...
main( int argc, char ** const argv )
{
	...
	set_mySubSystem("OPENSTACK_GAHP", SUBSYSTEM_TYPE_GAHP);
	...
	while ((line = m_stdin_buffer.GetNextLine()) != NULL) {
		...
		if (parse_gahp_command (command, &args) &&
			...
			if (strcasecmp (args.argv[0], GAHP_COMMAND_RESULTS) == 0) {
				...
				openstack_commands.rewind();
				char *one_openstack_command = NULL;

				while( (one_openstack_command = openstack_commands.next() ) != NULL ) {
					commands[i++] = one_openstack_command;
				}
