...
const char *Node::status_t_names[] = {"STATUS_NOT_READY", "STATUS_READY    ",
									  "STATUS_PRERUN   ", "STATUS_SUBMITTED",
									  "STATUS_POSTRUN  ", "STATUS_DONE     ",
									  "STATUS_ERROR    ", "STATUS_FUTILE   "};
...
void Node::Dump(const Dag *dag) const {
	...
	dprintf(D_ALWAYS, "PARENTS: %s WAITING: %d CHILDREN: %s\n", parents.c_str(),
			(int)IsWaiting(), children.c_str());
}

//---------------------------------------------------------------------------
bool Node::SanityCheck() const {
	bool result = true;

	if (countedAsDone == true && _Status != STATUS_DONE) {
		dprintf(D_ALWAYS, "BADNESS 10000: countedAsDone == true but _Status != "
						  "STATUS_DONE\n");
		result = false;
	}
...
//---------------------------------------------------------------------------
bool Node::GetProcIsIdle(int proc) {
	CheckTrackingJob(proc);
	return (jobs[proc].events & IDLE_MASK) != 0;
}

//---------------------------------------------------------------------------
void Node::SetProcIsIdle(int proc, bool isIdle) {
	SetStateChangeTime();
...
//---------------------------------------------------------------------------
void Node::SetProcEvent(int proc, int event) {
	SetStateChangeTime();
...
//---------------------------------------------------------------------------
bool Node::CheckBatchFailed(int tolerance) {
	int check = tolerance;
...
bool Node::RemoveOnBatchFailure(bool config_rm_on_fail) {
	...
	case DAG::ToleranceMode::AUTO:
		break;
	case DAG::ToleranceMode::FAIL_FAST:
		rm = true;
		break;
	case DAG::ToleranceMode::WAIT:
		rm = false;
		break;
	}
...
//---------------------------------------------------------------------------
void Node::PrintProcIsIdle() {
	int proc = 0;
	for (auto [state, _] : jobs) {
		debug_printf(DEBUG_QUIET, "Node(%s)::_isIdle[%d]: %d\n", GetNodeName(),
					 proc++, (state & IDLE_MASK) != 0);
	}
}

//---------------------------------------------------------------------------
int Node::PrintParents(std::string &buf, size_t bufmax, const Dag *dag,
...
//---------------------------------------------------------------------------
int Node::PrintChildren(std::string &buf, size_t bufmax, const Dag *dag,
						const char *sep) const {
	int count = 0;
...
bool Node::MarkParentDone(Dag &dag, Node *child, node_id_t parent) {
	ASSERT(child != nullptr);

	if (!child->HasMultipleParents()) {
		ASSERT(child->HasSingleParent());
		ASSERT(child->GetParentsID() ==
			   parent); // notifier must be the recorded parent
		child->_parents_done = true;
		return true;
	}

	bool done =
		dag.edge_table.GetWaitEdge(child->GetParentsID()).MarkDone(parent);
	if (done) {
		child->_parents_done = true;
	}
...
//---------------------------------------------------------------------------
// Set `child` to STATUS_FUTILE unless already FUTILE/preDone; increments count
// on change. Returns false only when child was already FUTILE (caller should
// stop recursing that branch).
bool Node::InvalidateChild(Node *child, int &count) {
	ASSERT(child != nullptr);
	...
	if (child->GetStatus() == Node::STATUS_FUTILE) {
		...
		if (child->SetStatus(Node::STATUS_FUTILE)) {
			count++;
		} else {
			debug_printf(
				DEBUG_NORMAL, "Error: Failed to set node %s to status %s\n",
...
void Node::NotifyChildren(
	Dag &dag, const std::function<bool(Dag &dag, Node *child)> &fn) {
	if (m_children != NO_EDGE_ID) {
		if (EdgeTable::IsDirect(m_children)) {
			DagArc &direct = dag.edge_table.GetDirectArc(m_children);
			ASSERT(direct.id != NO_ID);
...
//---------------------------------------------------------------------------
// Recursively set descendants to status FUTILE (strong deps), or -- for a
// direct child reached via a weak dependency -- treat this exactly like a
// normal parent-completion notification instead of cascading FUTILE. Return the
// number of nodes newly set to FUTILE.
int Node::SetDescendantsToFutile(
	Dag &dag,
	const std::function<bool(Dag &dag, Node *child)> &on_weak_unblocked) {
	int count = 0;
...
int Node::CascadeFutile(Dag &dag) {
	int count = 0;
...
int Node::VisitChildren(
	Dag &dag,
	const std::function<int(Dag &dag, Node *parent, Node *child)> &fn) {
	int retval = 0;
...
//---------------------------------------------------------------------------
bool Node::CanAddParent(std::string &whynot) {
	switch (GetType()) {
	case NodeType::FINAL:
...
//---------------------------------------------------------------------------
bool Node::CanAddChildren(std::string &whynot) {
	switch (GetType()) {
	case NodeType::FINAL:
...
//---------------------------------------------------------------------------
bool Node::AddVar(const std::string &name, const std::string &value,
				  bool prepend) {
	for (auto &var : varsFromDag) {
		if (name == var._name) {
			debug_printf(
				DEBUG_NORMAL,
				"Warning: VAR \"%s\" is already defined in node \"%s\"\n",
...
bool Node::AddScript(Script *script) {
	...
	if (old_script) {
		debug_printf(DEBUG_NORMAL,
					 "Warning: node %s already has %s script <%s> assigned; "
					 "changing to <%s>\n",
					 GetNodeName(), type_name, old_script->GetCmd(),
					 script->GetCmd());
		delete old_script;
		old_script = nullptr;
	}
...
//---------------------------------------------------------------------------
bool Node::AddPreSkip(int exitCode, std::string &whynot) {
	...
	if (_preskip != PRE_SKIP_INVALID) {
		debug_printf(DEBUG_NORMAL,
					 "Warning: new PRE_SKIP value  %d for node %s overrides "
					 "old value %d\n",
...
//---------------------------------------------------------------------------
void Node::SetCategory(const char *categoryName,
...
//---------------------------------------------------------------------------
const char *Node::GetJobstateJobTag() {
	if (_jobTag.empty()) {
		std::string jobTagName = MultiLogFiles::loadValueFromSubFile(
			_cmdFile.data(), _directory.data(), JOB_TAG_NAME);
		if (jobTagName.empty()) {
		...
		std::string tmpJobTag = MultiLogFiles::loadValueFromSubFile(
			_cmdFile.data(), _directory.data(), jobTagName.c_str());
		if (tmpJobTag.empty()) {
			tmpJobTag = "-";
		} else {
			// Remove double-quotes
...
//---------------------------------------------------------------------------
bool Node::SetCondorID(const CondorID &cid) {
	bool ret = true;
	if (GetCluster() != -1) {
		debug_printf(
			DEBUG_NORMAL,
			"Reassigning the id of job %s from (%d.%d.%d) to (%d.%d.%d)\n",
			GetNodeName(), GetCluster(), GetProc(), GetSubProc(), cid._cluster,
			cid._proc, cid._subproc);
		ret = false;
	}
	_CondorID = cid;
	return ret;
...
bool Node::Hold(int proc) {
	...
	if ((jobs[proc].events & HOLD_MASK) != HOLD_MASK) {
		...
		dprintf(D_FULLDEBUG,
				"Received hold event for node %s, and job %d.%d is already on "
				"hold!\n",
...
//---------------------------------------------------------------------------
bool Node::Release(int proc, bool warn) {
	SetStateChangeTime();

	if (proc >= static_cast<int>(jobs.size()) ||
		(jobs[proc].events & HOLD_MASK) != HOLD_MASK) {
		if (warn) {
			dprintf(D_FULLDEBUG,
					"Received release event for node %s, but job %d.%d is not "
					"on hold\n",
...
void Node::Cleanup() {
	...
	for (auto [state, _] : jobs) {
		if (state != (EXEC_MASK | ABORT_TERM_MASK)) {
			debug_printf(DEBUG_NORMAL,
						 "Warning for node %s: unexpected job event value for "
						 "proc %d: %d!\n",
...
/*
	...
	queue don't have ABORT_TERM_MASK and jobs not found in queue do have
   ABORT_TERM_MASK return True for everything matches return False for one job
   has differring state
*/
bool Node::VerifyJobStates(std::set<int> &queuedJobs) {
	...
	if (jobs.size() == 0) {
		debug_printf(DEBUG_NORMAL,
					 "ERROR: Node %s is internally in submitted state with no "
					 "recorded job events!\n",
					 nodeName);
		return false;
	}
	...
	for (auto &[state, _] : jobs) {
		if (queuedJobs.contains(proc)) {
			if (state & ABORT_TERM_MASK) {
				debug_printf(DEBUG_NORMAL,
							 "ERROR: Node %s (%d.%d) located in queue but "
							 "internally marked as exited.\n",
							 nodeName, cluster, proc);
				good_state = false;
			}
		} else if (!(state &
					 ABORT_TERM_MASK)) { // If abort/term mask bit not set
			debug_printf(DEBUG_NORMAL,
						 "ERROR: Node %s (%d.%d) not located in queue and not "
						 "internally marked as exited.\n",
						 nodeName, cluster, proc);
			good_state = false;
		}
