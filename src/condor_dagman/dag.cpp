		...
		if (node->GetStatus() == Node::STATUS_SUBMITTED) {
			...
			if (!job_info.contains(cluster) || !node->VerifyJobStates(job_info[cluster])) {
				...
				if (!node->IsMissingJobs() && possible_race) {
					...
					debug_printf(
						DEBUG_NORMAL,
						"ERROR: DAGMan lost track of node %s (ClusterId=%d). Please resubmit DAG from produced rescue file.\n",
...
void Dag::ProcessAbortEvent(const ULogEvent* event, Node* node, bool recovery) {
	...
	if (node) {
		...
		if (node->TotalJobsFailed() == 1) {
			node->SetErrorMsg("HTCondor reported %s event for job proc (%d.%d.%d)", event->eventName(), event->cluster, event->proc,
							  event->subproc);
		...
		if (node->GetStatus() != Node::STATUS_ERROR && batch_failed) {
			...
			if (node->RemoveOnBatchFailure(config[conf::b::RemoveJobListOnFailure])) {
				...
				formatstr(rm_reason, "Node Error: DAG node %s (%d.%d.%d) got %s event.", node->GetNodeName(), event->cluster,
						  event->proc, event->subproc, event->eventName());
...
//---------------------------------------------------------------------------
void Dag::ProcessTerminatedEvent(const ULogEvent* event, Node* node, bool recovery) {
	if (node) {
		node->SetProcEvent(event->proc, ABORT_TERM_MASK);
		_totalJobsCompleted++;
		...
		if (job_failed) { // job failed or was killed by a signal
			...
			if (termEvent->normal) {
				debug_printf(DEBUG_QUIET, "Node %s job proc (%d.%d.%d) failed with status %d.\n", node->GetNodeName(),
							 event->cluster, event->proc, event->subproc, termEvent->returnValue);
			} else {
				debug_printf(DEBUG_QUIET, "Node %s job proc (%d.%d.%d) failed with signal %d.\n", node->GetNodeName(),
							 event->cluster, event->proc, event->subproc, termEvent->signalNumber);
			}

			// Set first failure information
			if (node->TotalJobsFailed() == 1) {
				if (termEvent->normal) {
					node->SetErrorMsg("Job proc (%d.%d.%d) failed with status %d", termEvent->cluster, termEvent->proc,
									  termEvent->subproc, termEvent->returnValue);
					node->SetReturnValue(termEvent->returnValue);
				} else {
					node->SetErrorMsg("Job proc (%d.%d.%d) failed with signal %d", termEvent->cluster, termEvent->proc,
	...
	if (node->AllProcsDone()) {
		...
		if (node->_scriptPost) { // Run Post script if node has one
			...
			if (node->DoRetry()) { // Can node be retries
				...
				_numNodesFutile +=
					node->SetDescendantsToFutile(*this, [](Dag& dag, Node* child) -> bool { return dag.StartIfReady(child); });
			}
...
//---------------------------------------------------------------------------
void Dag::ProcessPostTermEvent(const ULogEvent* event, Node* node, bool recovery) {
	if (node) {
...
void Dag::ProcessSubmitEvent(Node* node, bool recovery, bool& submitEventIsSane) {
	...
	if (node->IsWaiting()) {
		...
		debug_printf(DEBUG_QUIET,
					 "This may indicate log file corruption; you may want to check the log files and re-run the "
...
void Dag::ProcessIsIdleEvent(Node* node, int proc) {
	...
	if (_numIdleJobProcs > 0 && TotalSubmittedNodes() < 1) {
		debug_printf(DEBUG_NORMAL,
					 "Warning:  DAGMan thinks there are %d idle job procs, even though there are no nodes in the queue!  Setting "
...
void Dag::ProcessHeldEvent(Node* node, const ULogEvent* event) {
	...
	if (node->Hold(event->proc)) {
		if (config[conf::i::MaxJobHolds] > 0 && node->GetJobsOnHold() >= config[conf::i::MaxJobHolds]) {
			debug_printf(DEBUG_VERBOSE,
						 "Total hold count for job %d (node %s)has reached DAGMAN_MAX_JOB_HOLDS (%d); all job "
...
//---------------------------------------------------------------------------
Node* Dag::FindAllNodesByName(const char* nodeName, const char* finalSkipMsg, const char* file, int line) const {
	...
	while (dm.throttles.WithinLimit(Throttle::MAX_INT_SUBMITS, numSubmitsThisCycle)) {
		...
		if (!config[conf::b::AggressiveSubmit]) {
			...
			if (elapsed > config[conf::i::LogScanInterval]) {
				debug_printf(DEBUG_QUIET,
							 "Warning: Submit cycle elapsed time (%lld s) has exceeded log scan interval (%d s); bailing out of "
							 "submit loop\n",
		...
		if (catThrottle && catThrottle->isSet() && catThrottle->_currentJobs >= catThrottle->_maxJobs) {
			debug_printf(DEBUG_DEBUG_1, "Node %s deferred by category throttle (%s, %d)\n", node->GetNodeName(),
						 catThrottle->_category->c_str(), catThrottle->_maxJobs);
...
bool Dag::DoneCycle(bool includeFinalNode) const {
	return FinishedRunning(includeFinalNode) && !DoneSuccess(includeFinalNode) && NumNodesFailed() == 0 && !IsHalted() &&
		   !_dagIsAborted;
}
...
void Dag::TerminateNode(Node* node, bool recovery, bool bootstrap) {
	...
	if (bootstrap) {
		...
		node->NotifyChildren(*this, [](Dag& dag, Node* child) -> bool { return dag.StartIfReady(child); });
	}
}
...
*/
void Dag::SetNodeStatusFileName(const char* statusFileName, int minUpdateTime, bool alwaysUpdate) {
	if (_statusFileName) {
...
//-------------------------------------------------------------------------
void Dag::NumJobProcStates(int* n_held, int* n_idle, int* n_running, int* n_terminated) {
	...
	for (const auto& collection : all_nodes) {
		...
		for (const auto& node : *collection) {
			...
			if (node_held != node->GetJobsOnHold()) { // Held job procs check
				debug_printf(DEBUG_NORMAL,
							 "Warning: Number of counted held job processes (%d) is not equivalent to Node %s's internal count of "
							 "held job processes count (%d).\n",
	...
	if ((idle + held) != NumIdleJobProcs()) { // Idle job procs check
		debug_printf(DEBUG_NORMAL,
					 "Warning: Number of counted idle job processes (%d) is not equivalent to DAGs internal idle job processes "
					 "count (%d).\n",
...
bool Dag::EventSanityCheck(const ULogEvent* event, const Node* node, bool* result) {
	ASSERT(event);
	ASSERT(node);
...
bool Dag::SanityCheckSubmitEvent(const CondorID condorID, const Node* node) const {
	...
	formatstr(
		...
		node->GetNodeName(), condorID._cluster, condorID._proc, condorID._subproc, node->GetCluster(), node->GetProc(),
		node->GetSubProc());
...
//---------------------------------------------------------------------------
SubmitResult Dag::SubmitNodeJob(const Dagman& dm, Node* node, CondorID& condorID, std::string& err) {
	...
	if (!node->GetNoop() && node->GetDagFile() != nullptr && config[conf::b::GenerateSubdagSubmit]) {
		...
		if (dagmanUtils.runSubmitDag(dm.inheritOpts, node->GetDagFile(), node->GetDirectory(), node->GetEffectivePrio(), isRetry) !=
			0) {
...
//---------------------------------------------------------------------------
void Dag::ProcessFailedSubmit(Node* node, int max_submit_attempts, std::string err) {
	...
	if (node->GetSubmitAttempts() >= max_submit_attempts) {
		...
		debug_printf(DEBUG_NORMAL, "Job submit try %d/%d failed, will try again in >= %d second%s.\n", node->GetSubmitAttempts(),
					 max_submit_attempts, thisSubmitDelay, thisSubmitDelay == 1 ? "" : "s");
...
//---------------------------------------------------------------------------
const Dag::PinNodes* Dag::GetPinInOut(const PinList& pinList, const char* inOutStr, int pinNum) {
	--pinNum; // Pin numbers start with 1
	if (pinNum >= static_cast<int>(pinList.size())) {
		debug_printf(DEBUG_QUIET, "ERROR: pin %s number %d specified; max is %d\n", inOutStr, pinNum + 1,
					 static_cast<int>(pinList.size()));
		return nullptr;
	} else {
...
bool Dag::ConnectSplices(Dag* parentSplice, Dag* childSplice) {
	...
	if (pinOutCount != pinInCount) {
		debug_printf(DEBUG_QUIET, "ERROR: pin_in/out mismatch:  parent splice %s has %d pin_outs; child splice %s has %d pin_ins\n",
...
std::vector<Node*>* Dag::InitialRecordedNodes(void) { return &_splice_initial_nodes; }

//---------------------------------------------------------------------------
// This represents not the actual final nodes of the dag just after
// the file containing the dag had been parsed.
// You must NOT free the returned array or the contained pointers.
std::vector<Node*>* Dag::FinalRecordedNodes(void) { return &_splice_terminal_nodes; }
	...
	for (auto& [splice_name, splice] : _splices) {
		...
		for (const auto& [desc_name, desc] : splice->InlineDescriptions) {
			...
			if (!success && InlineDescriptions[desc_name] != desc) {
				...
				debug_printf(
					DEBUG_NORMAL, "WARNING: Conflicting inline descriptions using the same name '%s' between %s%s and splice %s.\n",
					desc_name.c_str(), (_spliceScope != "root") ? "splice " : "", _spliceScope.c_str(), splice_name.c_str());
			}
		}
	}
...
void Dag::AssumeOwnershipofNodes(const std::string& spliceName, OwnedMaterials* om) {
	Node* node = nullptr;
	unsigned int i;
	std::string key;
	node_id_t key_id;
...
bool Dag::Connect(std::vector<Node*>& parents, const std::vector<Node*>& children, unsigned int meta) {
	...
	if (parents.empty() || children.empty()) {
		debug_printf(DEBUG_NORMAL, "ERROR: No %s%s%s nodes provided for dependency creation\n", parents.empty() ? "parent" : "",
					 (parents.empty() && children.empty()) ? " nor " : "", children.empty() ? "child" : "");
		return false;
	}
	...
	if (parents.size() > 1) {
		...
		bool share_edge =
			check_edge_id > 0 && std::ranges::all_of(parents, [check_edge_id](Node* n) { return n->GetEdgeID() == check_edge_id; });

		bool no_edges = !share_edge && std::ranges::all_of(parents, [](Node* n) { return n->GetEdgeID() == NO_EDGE_ID; });
		...
		if (share_edge) {
			...
			for (auto c : children) {
				...
				} else if (!(meta & ARC_WEAK) && edge_table[check_edge_id].GetArc(c->GetNodeID()).IsWeak()) {
