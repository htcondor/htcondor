...
//TEMPTEMP -- what ones should get passed recursively? maybe not things like maxjobs???
	...
	//TEMPTEMP -- shit -- should this overwrite the submit file, or should we just leave the submit file alone but not call that a failure?  (I think we should do the latter -- also, maybe it's only not an error if you have specified -no_submit?)
	bool overwriteSubmit; // allow overwriting submit file w/o -force
...
//TEMPTEMP -- is it worth worrying about having more than one .condor.sub?
		...
		if ( start >= 0 && start + strlen( DAG_SUBMIT_FILE_SUFFIX) ==
			...
			//TEMPTEMP -- need to add more stuff here
			MyString cmdLine = "condor_submit_dag -no_submit -overwrite_submit ";
...
void ensureOutputFilesExist(const SubmitDagOptions &opts)
{
	...
	//TEMPTEMP if (!autoRunningRescue && opts.doRescueFrom < 1) {
	if (!autoRunningRescue && opts.doRescueFrom < 1 && !opts.overwriteSubmit) {//TEMPTEMP?
...
//TEMPTEMP?>>>
			else if (strArg.find("-over") != -1) // -overwrite_submit
			{
				opts.overwriteSubmit = true;
			}
//<<<TEMPTEMP?
			else
			{
...
int printUsage() 
{
	...
	printf("    -no_recurse TEMPTEMP\n");
	printf("    -overwrite_submit TEMPTEMP\n");
	exit(1);
}
