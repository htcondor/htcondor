...
void ScheddClassad::SetAttribute(const char *attrName, int64_t attrVal) const {
	if (SetAttributeInt(_jobId._cluster, _jobId._proc, attrName, attrVal) !=
		0) {
...
//---------------------------------------------------------------------------
void ScheddClassad::SetAttribute(const char *attrName,
								 const std::string &value) const {
	if (SetAttributeString(_jobId._cluster, _jobId._proc, attrName,
						   value.c_str()) != 0) {
		debug_printf(DEBUG_QUIET, "WARNING: failed to set attribute %s\n",
					 attrName);
		check_warning_strictness(DAG_STRICT_3);
...
//---------------------------------------------------------------------------
bool ScheddClassad::GetAttribute(const char *attrName, std::string &attrVal,
								 bool printWarning) const {
	char *val;
	if (GetAttributeStringNew(_jobId._cluster, _jobId._proc, attrName, &val) ==
		-1) {
		if (printWarning) {
...
//---------------------------------------------------------------------------
bool ScheddClassad::GetAttribute(const char *attrName, int &attrVal,
								 bool printWarning) const {
	int val = 0;
...
//---------------------------------------------------------------------------
bool ScheddClassad::GetAttributeExpr(const char *attrName,
									 std::string &attrVal) const {
	char *val;
	if (GetAttributeExprNew(_jobId._cluster, _jobId._proc, attrName, &val) ==
		-1) {
...
DagmanClassad::DagmanClassad(const CondorID &DAGManJobId, DCSchedd *schedd) {
	...
	if (DAGManJobId == defaultCondorId) {
		debug_printf(DEBUG_QUIET,
					 "No HTCondor ID available for DAGMan (running on command "
					 "line?); DAG status will not be reported to ClassAd\n");
		return;
	}
...
int DagmanClassad::Initialize(DagmanOptions &dagOpts) {
	...
	if (_valid) {
		...
		if (!GetAttribute(ATTR_JOB_BATCH_NAME, batchName, false)) {
			// Default batch name is top-level DAG's primary DAG file (base name
			// only).
		...
		if (GetAttributeInt(_jobId._cluster, _jobId._proc, ATTR_DAGMAN_JOB_ID,
							&parentDAG) != 0) {
...
void DagmanClassad::RecoverThrottles(Throttles &throttles) {
	...
	for (size_t i = 0; i < static_cast<size_t>(Throttle::_SIZE); i++) {
		int value = -1;
		GetAttribute(THROTTLE_ATTR[i], value, false);
		recovered[i] = value;
	}
...
void DagmanClassad::Update(const Dagman &dagman) {
	...
	dagman.dag->NumJobProcStates(&jobProcsHeld, &jobProcsIdle,
								 &jobProcsRunning);

	SetAttribute(ATTR_DAG_AD_UPDATE_TIME,
				 time(nullptr)); // Set the time that update occurred
...
//---------------------------------------------------------------------------
void DagmanClassad::GetRequestedAttrs(
	std::map<std::string, std::string> &inheritAttrs, const char *prefix) {
	Qmgr_connection *queue = OpenConnection();
	if (!_valid || !queue) {
		debug_printf(DEBUG_VERBOSE,
					 "Skipping ClassAd query -- %s: No ClassAd attributes will "
					 "be inherited",
					 queue ? " DagmanClassad object is invalid"
						   : "Failed to connect to local Schedd Queue");
		check_warning_strictness(DAG_STRICT_1);
		inheritAttrs.clear();
		return;
	}
