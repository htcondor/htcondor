...
ScriptExecResult ScriptQ::Run(Script *script, ScriptDeferAction act) {
	...
	if (!_dag->throttles.WithinLimit(throttle, _numScriptsRunning)) {
		...
		debug_printf(DEBUG_DEBUG_1,
					 "Max %s scripts (%d) already running; deferring %s script "
					 "of node %s\n",
					 prefix, _dag->throttles[throttle], prefix,
					 script->GetNodeName());
		deferScript = true;
	}
...
int ScriptQ::RunWaitingScripts(bool justOne) {
	...
	// Use std::ranges::stable_partition to separate ready scripts from deferred
	// ones. This is more efficient than the previous pop/push loop, especially
	// when many scripts are deferred. Complexity: O(n) vs O(n * deferred_count)
	auto defer_partition =
		std::ranges::stable_partition(_waitingQueue, [now](Script *script) {
			// Partition: ready scripts (true) come before deferred scripts
			// (false)
			return script->_nextRunTime == 0 || script->_nextRunTime <= now;
		});

	// If we are to only start one script then manually try non-deferred script
	// execution until the first success
	if (justOne) {
	...
	// Use std::stable_partition as a driver to Run Scripts that are not
	// deferred Executed scripts will be removed from the waiting queue in a
	// following erase call. NOTE: This will iterate over all non-deferred
	// scripts unfortunately (even if justOne is true)
	auto executed_end = std::stable_partition(
		_waitingQueue.begin(), defer_partition.begin(),
		[this, &scriptsRun](Script *script) {
			...
			case ScriptExecResult::EXECUTED:
				scriptsRun++;
				[[fallthrough]];
			// NOTE: Script execution error means DAGMan has faux reaped the
			// script as a failure (thus consider it as executed)
...
int ScriptQ::ScriptReaper(int pid, int status) {
	...
	if (script->_deferStatus != SCRIPT_DEFER_STATUS_NONE &&
		...
		debug_printf(DEBUG_NORMAL,
					 "Deferring %s script of node %s for %ld seconds (exit "
					 "status was %d)...\n",
					 script->GetScriptName(), script->GetNodeName(),
					 script->_deferTime, script->_deferStatus);
	} else {
		script->_done = TRUE;
