...
int submitDag(DagmanOptions& dagOpts) {
	...
	if (dagOpts[shallow::b::DoSubmit]) {
		...
		if (!dagOpts[shallow::str::ScheddDaemonAdFile].empty()) {
			SetEnv("_CONDOR_SCHEDD_DAEMON_AD_FILE", dagOpts[shallow::str::ScheddDaemonAdFile].c_str());
		}
		if (!dagOpts[shallow::str::ScheddAddressFile].empty()) {
			SetEnv("_CONDOR_SCHEDD_ADDRESS_FILE", dagOpts[shallow::str::ScheddAddressFile].c_str());
		}
