...
int printUsage(
	int iExitCode = 1); // NOTE: printUsage calls exit(1), so it doesn't return
void parseCommandLine(DagmanOptions &dagOpts, size_t argc,
...
int main(int argc, char *argv[]) {
	...
	if (!dagOpts[shallow::str::ConfigFile].empty()) {
		...
		process_config_source(dagOpts[shallow::str::ConfigFile].c_str(), 0,
							  "DAGMan config", NULL, true);
	}

	if (!param_boolean("DAGMAN_WRITE_PARTIAL_RESCUE", true)) {
		fprintf(stdout,
				"WARNING: DAGMAN_WRITE_PARTIAL_RESCUE = False.\n"
				"   The use of full Rescue DAG's is deprecated and slated\n"
				"   for removal during the lifetime of V24 feature series of "
				"HTCondor.\n\n");
	}
		...
		while (reader.NextLogicalLine(dagLine)) {
			...
			if (first && strcasecmp(first, "SUBDAG") == MATCH) {
				...
				if (strcasecmp(inlineOrExt, "EXTERNAL") != MATCH) {
					fprintf(stderr,
							"ERROR: only SUBDAG EXTERNAL is supported at this "
							"time (line: <%s>)\n",
							dagLine.c_str());
					return 1;
				}
				...
				if (tempToken && strcasecmp(tempToken, "DIR") == MATCH) {
					...
					if (!directory) {
						fprintf(stderr,
								"No directory specified in line: <%s>\n",
								dagLine.c_str());
						return 1;
					}
				}
...
int submitDag(DagmanOptions &dagOpts) {
	...
	if (dagOpts[shallow::b::DoSubmit]) {
		...
		printf("-no_submit given, not submitting DAG to HTCondor. You can do "
			   "this with:\n");
...
void parseCommandLine(DagmanOptions &dagOpts, size_t argc,
	...
	for (size_t iArg = 1; iArg < argc; iArg++) {
		...
		if (strArg[0] != '-') {
			...
			} else if (strArg.find("-help") != std::string::npos ||
					   strArg.find("-h") != std::string::npos) { // -help
				printUsage(0);

			} else {
				// Make note of overriding AppendFile config value via
				// -insert_sub_file
				if (strArg.find("-insert_s") != std::string::npos) {
					static bool flagUsed = false;
					if (!flagUsed &&
						!dagOpts[shallow::str::AppendFile].empty()) {
	...
	if (!dagOpts[shallow::str::ConfigFile].empty()) {
		std::string errMsg;
		if (!dagmanUtils.MakePathAbsolute(dagOpts[shallow::str::ConfigFile],
										  errMsg)) {
			fprintf(stderr, "%s\n", errMsg.c_str());
			exit(1);
		}
	...
	if (dagOpts[deep::i::DoRescueFrom] < 0) {
		fprintf(stderr,
				"-DoRescueFrom value must be non-negative; aborting.\n");
		printUsage();
	}
}
...
int printUsage(int iExitCode) {
	printf("Usage: condor_submit_dag [options] dag_file [dag_file_2 ... "
		   "dag_file_n]\n");
