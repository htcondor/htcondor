...
static bool parse_subdag(Dag *dag, const char *nodeTypeKeyword,
						 const char *dagFile, int lineNum,
						 const char *directory);

static bool parse_node(Dag *dag, const char *nodeName,
					   const char *submitFileOrSubmitDesc,
					   const char *nodeTypeKeyword, const char *dagFile,
					   int lineNum, const char *directory,
					   const char *inlineOrExt, const char *submitOrDagFile);

static bool pre_parse_node(std::string &nodename, const char *&submitFile);

static bool parse_script(const char *endline, Dag *dag, const char *filename,
						 int lineNumber);
static bool parse_parent(Dag *dag, const char *filename, int lineNumber);
static bool parse_retry(Dag *dag, const char *filename, int lineNumber);
static bool parse_abort(Dag *dag, const char *filename, int lineNumber);
...
static bool parse_priority(Dag *dag, const char *filename, int lineNumber);
...
static bool parse_splice(const Dagman &dm, Dag *dag, const char *filename,
						 int lineNumber);
static bool parse_node_status_file(Dag *dag, const char *filename,
								   int lineNumber);
static bool parse_save_point_file(Dag *dag, const char *filename,
								  int lineNumber);
static bool parse_reject(Dag *dag, const char *filename, int lineNumber);
static bool parse_jobstate_log(Dag *dag, const char *filename, int lineNumber);
static bool parse_pre_skip(Dag *dag, const char *filename, int lineNumber);
...
static bool parse_pin_in_out(Dag *dag, const char *filename, int lineNumber,
							 bool isPinIn);
static bool parse_include(const Dagman &dm, Dag *dag, const char *filename,
						  int lineNumber);
...
bool isReservedWord(const char *token) {
	...
	for (auto &keyword : keywords) {
		if (!strcasecmp(token, keyword)) {
			debug_printf(DEBUG_QUIET, "ERROR: token (%s) is a reserved word\n",
						 token);
			return true;
		}
	}
...
std::string makeString(char *s) { return std::string(s ? s : ""); }

//-----------------------------------------------------------------------------
void parseSetDoNameMunge(bool doit) { _mungeNames = doit; }
...
static std::string parse_inline_desc(MacroStreamYourFile &ms, int gl_opts,
									 const std::string &end, std::string &error,
									 char *endline) {
	std::string desc;
	char *line;
	...
	while ((line = ms.getline(gl_opts)) != nullptr) {
		...
		if (starts_with(line, end) &&
			(strlen(line) == end.size() || starts_with(line, end + " "))) {
			endline = line;
			found_end = true;
			break;
		}
...
//-----------------------------------------------------------------------------
bool parse(const Dagman &dm, Dag *dag, const char *filename,
	...
	src.line = 0;
	src.id = 4; // index into macro_set.sources. 4 is the first index after the
				// pre-defined ones
	int gl_opts = 3; // CONFIG_GETLINE_OPT_COMMENT_DOESNT_CONTINUE |
					 // CONFIG_GETLINE_OPT_CONTINUE_MAY_BE_COMMENTED_OUT;
	...
	while (((line = ms.getline(gl_opts)) != NULL)) {
		...
		// Note that getline will truncate leading spaces (as defined by
		// isspace()) so we don't need to do that before checking for empty
		// lines or comments.
		if (line[0] == 0)
			continue; // Ignore blank lines
		if (line[0] == COMMENT)
		...
		// parse them the same way with possibility of inline submit
		// descriptions
		if (NODE_KEYWORDS.contains(token)) {
			...
			if (pre_parse_success && get_inline_desc_end(subfile, inline_end)) {
				...
				std::string err;
				std::string desc =
					parse_inline_desc(ms, gl_opts, inline_end, err, line);
				std::string key =
					nodename + "-InlineDesc" + std::to_string(inline_count++);
				...
				token = strtok(
					line, DELIMITERS); // Continue tokenizing after end token
			}

			if (pre_parse_success) {
				parsed_line_successfully = parse_node(
					dag, nodename.c_str(), subfile, keyword.c_str(), filename,
					lineNumber, tmpDirectory.c_str(), "", "submitfile");
				std::string temp_nodename = munge_node_name(nodename.c_str());
				Node *node = dag->FindAllNodesByName(temp_nodename.c_str(), "",
													 filename, lineNumber);
				if (!node) {
					debug_printf(DEBUG_NORMAL,
								 "Error: unable to find node %s in our DAG "
								 "structure, aborting.\n",
								 nodename.c_str());
		...
		// Handle a SUBDAG spec
			...
			parsed_line_successfully = parse_subdag(
				dag, token, filename, lineNumber, tmpDirectory.c_str());
		}

		// Handle a Splice spec
		else if (strcasecmp(token, "SPLICE") == 0) {
			parsed_line_successfully =
				parse_splice(dm, dag, filename, lineNumber);
		}
		...
		// Handle a SUBMIT-DESCRIPTION spec
			...
			if (success && get_inline_desc_end(desc, desc_end)) {
				std::string err;
				std::string data =
					parse_inline_desc(ms, gl_opts, desc_end, err, line);
				if (!err.empty()) {
					debug_printf(DEBUG_NORMAL, "ERROR (line %d): %s\n",
								 lineNumber, err.c_str());
				} else {
	...
	while (((line = ms.getline(gl_opts)) != NULL)) {
		...
		// JOB, SERVICE, PROVISIONER, FINAL already parsed... slurp up any
		// inline submit lines
		if (NODE_KEYWORDS.contains(token)) {
			parsed_line_successfully = true;
			std::string nodename;
		...
		// Example Syntax is:  SCRIPT (PRE|POST|HOLD) [DEFER status time]
		// NodeName ScriptName Args ...
		else if (strcasecmp(token, "SCRIPT") == 0) {
			parsed_line_successfully =
				parse_script(endline, dag, filename, lineNumber);
		}
		...
		// Keywords explicitly not supported with old parse mode due to
		// deprecation
		else if (std::any_of(UNSUPPORTED_OLD_PARSER_KEYWORDS.begin(),
							 UNSUPPORTED_OLD_PARSER_KEYWORDS.end(),
							 [token](const char *kw) {
								 return strcasecmp(token, kw) == 0;
							 })) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s:%d %s is not supported in deprecated parse "
						 "mode (DAGMAN_USE_OLD_FILE_PARSER=True)\n",
		...
		// Example syntax is: Priority NodeName 2
			...
			parsed_line_successfully =
				parse_priority(dag, filename, lineNumber);
		}
		...
		// Example syntax is: Category NodeName Simulation
			...
			parsed_line_successfully =
				parse_category(dag, filename, lineNumber);
		}

		// Handle a MaxJobs spec
		// Example syntax is: MaxJobs Category 10
		else if (strcasecmp(token, "MAXJOBS") == 0) {
			parsed_line_successfully = parse_maxjobs(dag, filename, lineNumber);
		}
		...
		// Handle a NODE_STATUS_FILE spec
			...
			parsed_line_successfully =
				parse_node_status_file(dag, filename, lineNumber);
		}

		// Handle a SAVE_POINT_FILE spec
		else if (strcasecmp(token, "SAVE_POINT_FILE") == MATCH) {
			parsed_line_successfully =
				parse_save_point_file(dag, filename, lineNumber);
		}

		// Handle a REJECT spec
		else if (strcasecmp(token, "REJECT") == 0) {
			parsed_line_successfully = parse_reject(dag, filename, lineNumber);
		}

		// Handle a JOBSTATE_LOG spec
		else if (strcasecmp(token, "JOBSTATE_LOG") == 0) {
			parsed_line_successfully =
				parse_jobstate_log(dag, filename, lineNumber);
		}

		// Handle a PRE_SKIP
		else if (strcasecmp(token, "PRE_SKIP") == 0) {
			parsed_line_successfully =
				parse_pre_skip(dag, filename, lineNumber);
		}

		// Handle a DONE spec
		else if (strcasecmp(token, "DONE") == 0) {
			parsed_line_successfully = parse_done(dag, filename, lineNumber);
		}

		// Handle a CONNECT spec
		else if (strcasecmp(token, "CONNECT") == 0) {
			parsed_line_successfully = parse_connect(dag, filename, lineNumber);
		}

		// Handle a PIN_IN spec
		else if (strcasecmp(token, "PIN_IN") == 0) {
			parsed_line_successfully =
				parse_pin_in_out(dag, filename, lineNumber, true);
		}

		// Handle a PIN_OUT spec
		else if (strcasecmp(token, "PIN_OUT") == 0) {
			parsed_line_successfully =
				parse_pin_in_out(dag, filename, lineNumber, false);
		}

		// Handle a INCLUDE spec
		else if (strcasecmp(token, "INCLUDE") == 0) {
			parsed_line_successfully =
				parse_include(dm, dag, filename, lineNumber);
		}
...
static bool parse_subdag(Dag *dag, const char *nodeTypeKeyword,
	...
	if (!strcasecmp(inlineOrExt, "EXTERNAL")) {
		...
		const char *subfile;
		pre_parse_node(nodename, subfile);
		return parse_node(dag, nodename.c_str(), subfile, nodeTypeKeyword,
						  dagFile, lineNum, directory, " EXTERNAL", "dagfile");
	}
...
//-----------------------------------------------------------------------------
// parse just enough of the line to know if this is an inline submit file or a
// submit filename
static bool pre_parse_node(std::string &nodename, const char *&submitFile) {
	nodename.clear();
	submitFile = NULL;
	...
	// return success for pre-parse. we have two tokens, we don't yet know if
	// they have valid values though!!!
	return true;
}
...
static bool parse_node(Dag *dag, const char *nodeName,
	...
	if (!allowIllegalChars &&
		(strcspn(nodeName, ILLEGAL_CHARS) < strlen(nodeName))) {
	...
	if (nextTok) {
		if (strcasecmp(nextTok, "DIR") == 0) {
			if (strcmp(directory, "")) {
				debug_printf(DEBUG_QUIET,
							 "ERROR: DIR specification in node "
	...
	if (submitFileOrSubmitDesc) {
		if (strcasecmp(nodeTypeKeyword, "SUBDAG") == MATCH) {
			...
			// Assume JOB commands submit file ending with '.condor.sub' is a
			// DAG submit file.
			if (strcasecmp(nodeTypeKeyword, "FINAL") == MATCH) {
				nestedDagFile = submitFileOrSubmitDesc;
				nestedDagFile.replace(
					nestedDagFile.find(DAG_SUBMIT_FILE_SUFFIX),
					sizeof(DAG_SUBMIT_FILE_SUFFIX) - 1, "");
			} else {
				// This is no longer allowed for nodes (excluding final for
				// LSST)
				debug_printf(DEBUG_NORMAL, "Error: The use of the JOB keyword "
										   "for nested DAGs is prohibited.\n");
				return false;
			}
		}
	}
	...
	// looks ok, so add it
	if (!AddNode(dag, nodeName, directory, submitFileOrSubmitDesc, noop, done,
				 type, whynot)) {
	...
	if (nestedDagFile != "") {
		if (!SetNodeDagFile(dag, nodeName, nestedDagFile.c_str(), whynot)) {
			debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): %s\n", dagFile,
						 lineNum, whynot.c_str());
			return false;
		}
	}
...
//             SCRIPT [DEFER status time] (PRE|POST|HOLD) NodeName ScriptName
//             Args ...
//
//-----------------------------------------------------------------------------
static bool parse_script(const char *endline, Dag *dag, const char *filename,
						 int lineNumber) {
	const char *example =
		"SCRIPT [DEFER status time] [DEBUG file (STDOUT|STDERR|ALL)] "
		"(PRE|POST|HOLD) NodeName Script Args ...";
	...
	if (!strcasecmp(type_name, "DEFER")) {
		...
		if (!type_name) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): Missing PRE, POST, HOLD or "
						 "optional DEBUG\n",
						 filename, lineNumber);
			exampleSyntax(example);
			return false;
	...
	if (strcasecmp(type_name, "DEBUG") == MATCH) {
		...
		if (!token) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): Script DEBUG missing filename\n",
						 filename, lineNumber);
			exampleSyntax(example);
			return false;
		}
		...
		if (!token) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): Script DEBUG missing output "
						 "stream type (STDOUT,STDERR,ALL)\n",
						 filename, lineNumber);
			exampleSyntax(example);
			return false;
		}
		if (strcasecmp(token, "STDOUT") == MATCH) {
			...
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): Unknown Script output stream "
						 "type (%s) expected STDOUT, STDERR, or ALL\n",
						 filename, lineNumber, token);
			exampleSyntax(example);
			return false;
		}
	...
	Node *node;
	while ((node = dag->FindAllNodesByName(
				nodeName,
				"In parse_script(): skipping node %s because final nodes must "
				"have SCRIPT set explicitly (%s: %d)\n",
				filename, lineNumber))) {
		nodeName = NULL;
		...
		if (!script) {
			debug_printf(
				DEBUG_SILENT,
				"ERROR: Failed to make script object: out of memory!\n");
			return false;
		}
		...
		if (!node->AddScript(script)) {
			debug_printf(
				DEBUG_SILENT,
				"ERROR: %s (line %d): failed to add %s script to node %s\n",
	...
	if (nodeName) {
		debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): Unknown Node %s\n",
...
static bool parse_parent(Dag *dag, const char *filename, int lineNumber) {
	...
	const auto GetID = [](const Node *n) -> node_id_t {
		return n->GetNodeID();
	};

	std::ranges::sort(parents, std::less{}, GetID);
	const auto duplicate_parents =
		std::ranges::unique(parents, std::equal_to{}, GetID);
	parents.erase(duplicate_parents.begin(), duplicate_parents.end());

	if (nodeName == NULL) {
		debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): Expected CHILD token\n",
					 filename, lineNumber);
		exampleSyntax(example);
		return false;
	}
	...
	if (children.empty()) {
		debug_printf(DEBUG_QUIET,
					 "ERROR: %s (line %d): Missing Child Node names\n",
					 filename, lineNumber);
		exampleSyntax(example);
		return false;
	}
	...
	const auto duplicate_children =
		std::ranges::unique(children, std::equal_to{}, GetID);
	...
	if (useJoinNodes && parents.size() > 1 && children.size() > 1) {
		...
		Node *joinNode = AddNode(dag, joinNodeName.c_str(), "", "noop.sub",
								 true, false, NodeType::JOB, failReason);
		if (!joinNode) {
			debug_printf(DEBUG_QUIET,
		...
		if (!dag->Connect(parents, join)) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d) failed to add dependency to join "
						 "node %s\n",
						 filename, lineNumber,
		...
		// reset parent list to the join node and fall through to build the
		// child edges
		parents.clear();
	...
	if (!dag->Connect(parents, children)) {
		debug_printf(
			DEBUG_QUIET,
			"ERROR: %s (line %d) failed to add dependency to between nodes\n",
			filename, lineNumber);
		return false;
	}
...
// Purpose:  Parse a line of the format "Retry nodename num-times [UNLESS-EXIT
// 42]"
//
//-----------------------------------------------------------------------------
static bool parse_retry(Dag *dag, const char *filename, int lineNumber) {
	...
	char *token = strtok(NULL, DELIMITERS);
	if (token == NULL) {
		debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): Missing Retry value\n",
					 filename, lineNumber);
		exampleSyntax(example);
		return false;
	}
	...
	while ((node = dag->FindAllNodesByName(
				nodeName,
				"In parse_retry(): skipping node %s because final nodes cannot "
				"have RETRY specifications (%s: %d)\n",
				filename, lineNumber))) {
		nodeName = NULL;
		...
		if (node->GetType() == NodeType::FINAL) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): Final Node %s cannot have RETRY "
						 "specification\n",
		...
		if (node->GetType() == NodeType::SERVICE) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): SERVICE node %s cannot have "
						 "RETRY specification\n",
						 filename, lineNumber, node->GetNodeName());
			return false;
		}
...
static bool parse_abort(Dag *dag, const char *filename, int lineNumber) {
	...
	// RETURN keyword.
	int returnVal = std::numeric_limits<int>::max(); // assign value to avoid
													 // compiler warning
	...
	while ((node = dag->FindAllNodesByName(
				nodeName,
				"In parse_abort(): skipping node %s because final nodes cannot "
				"have ABORT-DAG-ON specification (%s: %d)s\n",
				filename, lineNumber))) {
		nodeName = NULL;
		...
		if (node->GetType() == NodeType::FINAL) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): Final Node %s cannot have "
...
//
// Function: parse_vars
// Purpose:  Parses a line of named variables that will be made available to a
// node's
...
//-----------------------------------------------------------------------------
static bool parse_vars(Dag *dag, const char *filename, int lineNumber) {
	const char *example = "Vars NodeName [PREPEND | APPEND] "
	...
	std::string varName;
	std::string varValue;
	bool prepend; // Bool for if variable is prepended or appended. Default is
				  // false
	...
	/*
	 *	Check to see if PREPEND or APPEND was specified before variables to be
	 *passed. If option is found then we set prepend boolean appropriately and
	 *increment the original varsStr pointer to point at char after option.
	 *-Cole Bollig
	 */

	if (starts_with(varsStr,
					"PREPEND")) { // See if PREPEND is at beginning of token
		prepend = true;
		varsStr += 7;
	} else if (starts_with(
				   varsStr,
				   "APPEND")) { // See if APPEND is at beginning of token
		prepend = false;
		varsStr += 6;
	} else {
	...
	while ((node = dag->FindAllNodesByName(
				nodeName,
				"In parse_vars(): skipping node %s because final nodes must "
				"have VARS set explicitly (%s: %d)\n",
				filename, lineNumber))) {
		nodeName = NULL;
		...
		for (numPairs = 0;; numPairs++) { // for each name="value" pair
			if (str ==
				NULL) { // this happens when the above strtok returns NULL
				break;
			}
...
static bool parse_priority(Dag *dag, const char *filename, int lineNumber) {
	...
	while ((node = dag->FindAllNodesByName(
				nodeName,
				"In parse_priority(): skipping node %s because final nodes "
				"cannot have PRIORITY specifications (%s: %d)\n",
				filename, lineNumber))) {
		nodeName = NULL;
...
static bool parse_category(Dag *dag, const char *filename, int lineNumber) {
	...
	while ((node = dag->FindAllNodesByName(
				nodeName,
				"In parse_category(): skipping node %s because final nodes "
				"cannot have CATEGORY specifications (%s: %d)\n",
				filename, lineNumber))) {
		nodeName = NULL;
...
static bool parse_splice(const Dagman &dm, Dag *dag, const char *filename,
	...
	if (spliceName.empty()) {
		debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): Missing SPLICE name\n",
					 filename, lineNumber);
		exampleSyntax(example);
		return false;
	}
	...
	*/
	...
	if (!spliceDir.Cd2TmpDir(directory.c_str(), errMsg)) {
		debug_printf(DEBUG_QUIET, "ERROR: can't change to directory %s: %s\n",
...
static bool parse_save_point_file(Dag *dag, const char *filename,
	...
	if (nodeName == NULL) {
		debug_printf(
			DEBUG_QUIET,
			"ERROR: %s (line %d): No Node specified for SAVE_POINT_FILE.\n",
			filename, lineNumber);
		exampleSyntax(example.c_str());
		return false;
	} else if (strcasecmp(nodeName, DAG::ALL_NODES.c_str()) ==
			   MATCH) { // It is redundant to use all nodes for save files
		debug_printf(DEBUG_NORMAL,
					 "ERROR: %s (line %d): SAVE_POINT_FILE does not allow "
					 "ALL_NODES option.\n",
					 filename, lineNumber);
		exampleSyntax(example.c_str());
		return false;
	...
	if (!saveFile.empty()) { // Name was specified
		...
		if (token != NULL) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): Extra token (%s) found in "
						 "SAVE_POINT_FILE declaration.\n",
						 filename, lineNumber, token);
			exampleSyntax(example.c_str());
			return false;
		}
	} else { // No name given so create one using node name and associated dag
			 // filename
		formatstr(saveFile, "%s-%s.save", nodeNameOrig,
	...
	if (node == NULL) {
		...
		if (node->GetType() == NodeType::SERVICE ||
			node->GetType() == NodeType::PROVISIONER) {
			debug_printf(
				DEBUG_QUIET,
				"ERROR: %s (line %d): SAVE_POINT_FILE can not be declared with "
				"neither SERVICE nor PROVISIONER nodes.\n",
				filename, lineNumber);
			return false;
		}
...
//           a certain code
//-----------------------------------------------------------------------------
bool parse_pre_skip(Dag *dag, const char *filename, int lineNumber) {
	...
	while ((node = dag->FindAllNodesByName(
				nodeName,
				"In parse_pre_skip(): skipping node %s because final nodes "
				"must have PRE_SKIP set explicitly (%s: %d)\n",
				filename, lineNumber))) {
		nodeName = NULL;
...
static bool parse_connect(Dag *dag, const char *filename, int lineNumber) {
	...
	if (splice1 == NULL) {
		debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): Missing splice1 name\n",
					 filename, lineNumber);
		exampleSyntax(example);
		return false;
	}
	...
	if (splice2 == NULL) {
		debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): Missing splice2 name\n",
					 filename, lineNumber);
		exampleSyntax(example);
		return false;
	}
	...
	if (!parentSplice) {
		debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): Splice %s not found!\n",
	...
	if (!Dag::ConnectSplices(parentSplice, childSplice)) {
		debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): (see previous line)\n",
					 filename, lineNumber);
		return false;
	}
...
static bool parse_pin_in_out(Dag *dag, const char *filename, int lineNumber,
	...
	if (pinNumber == NULL) {
		debug_printf(DEBUG_QUIET, "ERROR: %s (line %d): Missing pin_number\n",
					 filename, lineNumber);
		exampleSyntax(example);
		return false;
	}
...
static bool get_next_var(const char *filename, int lineNumber, char *&str,
	...
	while (isalnum(*str) || *str == '_' || *str == '+' || *str == '.') {
		if (*str == '+') {
			if (varnamestate != 0) {
				debug_printf(DEBUG_QUIET,
							 "ERROR: %s (line %d): '+' can only be first "
							 "character of macroname (%s)\n",
	...
	if (varName.length() ==
		'\0') { // no alphanumeric symbols at all were written into name,
	...
	if (varName == "+") {
		debug_printf(DEBUG_QUIET,
					 "ERROR: %s (line %d): macroname (%s) must contain at "
	...
	if (*str != '=') {
		if (varName.compare("PREPEND") == 0 || varName.compare("APPEND") == 0) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): %s not declared before all "
						 "passed variables.\n",
						 filename, lineNumber, varName.c_str());
		} else {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s (line %d): Illegal character (%c) in or "
						 "after macroname %s\n",
	...
	// now it's time to read in all the data until the next double quote, while
	// handling the two escape sequences: \\ and \"
...
bool DagProcessor::process(const Dagman &dm, Dag &dag, const std::string &file,
						   int dag_munge_id) {
	if (config[conf::b::UseOldDagParser]) { // Use old parser/processing :(
		bool is_main_dag = (dag_munge_id >= 0);
		parseSetDoNameMunge(is_main_dag ? config[conf::b::MungeNodeNames]
										: false);
	...
	if (useDagDir) {
		...
		if (!dagDir.Cd2TmpDir(tmpDir.c_str(), error)) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: Failed to change into DAG directory %s: %s\n",
						 tmpDir.c_str(), error.c_str());
			return false;
		}

		// The DAG file to parse is now relative to use so just use the file
		// name
	...
	// Verify that we are not recursively parsing a DAG file (i.e.
	// INCLUDE/SPLICE)
	const std::string full_file_path = parser.GetAbsolutePath();
	auto [_, added] = parsed_file_check.insert(full_file_path);
	if (!added) {
		debug_printf(DEBUG_QUIET,
					 "ERROR: Recursive DAG file parsing detected with %s\n",
					 full_file_path.c_str());
		return false;
	}

	bool success = false;
	std::string location; // Save 'file:line#' of command in case processing
						  // error occurs
	...
	if (parser.failed()) {
		...
		if (error) {
			...
			debug_printf(DEBUG_QUIET,
						 "ERROR: Unknown file parse failure for %s\n",
						 file.c_str());
		}
...
bool DagProcessor::ProcessCommand(const Dagman &dm, const DagCmd &cmd, Dag &dag,
								  int dag_munge_id) {
	bool all_good = true;
	...
	case DAG::CMD::SUBMIT_DESCRIPTION: {
		...
		std::ignore =
			dag.add_inline_desc(desc->GetName(), desc->GetInlineDesc());
	} break;
	case DAG::CMD::SUBDAG:
	case DAG::CMD::JOB:
	case DAG::CMD::FINAL:
	...
	case DAG::CMD::SERVICE:
		all_good =
			ProcessNode(DAG::DERIVE_CMD<NodeCommand>(cmd), dag, dag_munge_id);
		break;
	case DAG::CMD::SPLICE:
		all_good = ProcessSplice(dm, dag, DAG::DERIVE_CMD<SpliceCommand>(cmd),
								 dag_munge_id);
		break;
	case DAG::CMD::DOT: {
	...
	case DAG::CMD::INCLUDE:
		all_good =
			process(dm, dag, (DAG::DERIVE_CMD<FileCommand>(cmd))->GetFile(),
					dag_munge_id);
		break;
	case DAG::CMD::NODE_STATUS_FILE: {
		const NodeStatusCommand *status =
			DAG::DERIVE_CMD<NodeStatusCommand>(cmd);
		dag.SetNodeStatusFileName(status->GetFile().c_str(),
								  status->GetMinUpdateTime(),
								  status->AlwaysUpdate());
	} break;
	case DAG::CMD::JOBSTATE_LOG:
		dag.SetJobstateLogFileName(
			(DAG::DERIVE_CMD<FileCommand>(cmd))->GetFile().c_str());
		break;
	case DAG::CMD::REJECT:
	...
	case DAG::CMD::CATEGORY:
		all_good = ProcessCategory(DAG::DERIVE_CMD<CategoryCommand>(cmd), dag,
								   dag_munge_id);
		break;
	case DAG::CMD::PARENT_CHILD:
		all_good = ProcessDependencies(DAG::DERIVE_CMD<ParentChildCommand>(cmd),
									   dag, dag_munge_id);
		break;
	case DAG::CMD::SCRIPT:
		all_good = ProcessScript(DAG::DERIVE_CMD<ScriptCommand>(cmd), dag,
								 dag_munge_id);
		break;
	case DAG::CMD::RETRY:
		all_good =
			ProcessRetry(DAG::DERIVE_CMD<RetryCommand>(cmd), dag, dag_munge_id);
		break;
	case DAG::CMD::ABORT_DAG_ON:
		all_good = ProcessAbortDagOn(DAG::DERIVE_CMD<AbortDagCommand>(cmd), dag,
									 dag_munge_id);
		break;
	case DAG::CMD::VARS:
		all_good =
			ProcessVars(DAG::DERIVE_CMD<VarsCommand>(cmd), dag, dag_munge_id);
		break;
	case DAG::CMD::PRIORITY:
		all_good = ProcessPriority(DAG::DERIVE_CMD<PriorityCommand>(cmd), dag,
								   dag_munge_id);
		break;
	case DAG::CMD::PRE_SKIP:
		all_good = ProcessPreSkip(DAG::DERIVE_CMD<PreSkipCommand>(cmd), dag,
								  dag_munge_id);
		break;
	case DAG::CMD::TOLERANCE:
		all_good = ProcessTolerance(DAG::DERIVE_CMD<ToleranceCommand>(cmd), dag,
									dag_munge_id);
		break;
	case DAG::CMD::DONE:
		all_good =
			ProcessDone(DAG::DERIVE_CMD<DoneCommand>(cmd), dag, dag_munge_id);
		break;
	case DAG::CMD::SAVE_POINT_FILE:
		all_good = ProcessSaveFile(DAG::DERIVE_CMD<SavePointCommand>(cmd), dag,
								   dag_munge_id);
		break;
	...
	case DAG::CMD::CONNECT: {
		const auto &[s1, s2] =
			(DAG::DERIVE_CMD<ConnectCommand>(cmd))->GetSplices();
		Dag *parent = dag.LookupSplice(MakeFullName(s1, dag_munge_id));
		Dag *child = dag.LookupSplice(MakeFullName(s2, dag_munge_id));

		if (!parent) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: Connect parent splice %s does not exist\n",
						 s1.c_str());
		}
		if (!child) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: Connect child splice %s does not exist\n",
						 s2.c_str());
		}

		all_good =
			(parent && child) ? Dag::ConnectSplices(parent, child) : false;
	} break;
	case DAG::CMD::PIN_IN:
	case DAG::CMD::PIN_OUT: {
		...
		all_good =
			dag.SetPinInOut(!pin->IsPinOut(), node.c_str(), pin->GetPinNum());
	} break;
	default:
		// TODO: Fail or ignore?
		debug_printf(DEBUG_NORMAL,
					 "DAGMan does not know how to process %s command yet...\n",
...
bool DagProcessor::ProcessNode(const NodeCommand *cmd, Dag &dag,
							   int dag_munge_id) {
	...
	if (dag.LookupSplice(name)) {
		debug_printf(
			DEBUG_QUIET,
			"ERROR: Node name %s already associated with a splice in DAG\n",
			name.c_str());
		return false;
	}
	...
	if (cmd->GetCommand() == DAG::CMD::SUBDAG) {
		...
		if (cmd->GetCommand() ==
			DAG::CMD::FINAL) { // Allow this for final nodes (for LSST)
			final_node_dag = desc.substr(0, desc.rfind(DAG_SUBMIT_FILE_SUFFIX));
		} else {
			debug_printf(DEBUG_NORMAL, "Error: The use of the JOB keyword for "
									   "nested DAGs is prohibited.\n");
			return false;
		}
	...
	if (useDagDir) {
		if (cmd->HasDir()) {
			debug_printf(DEBUG_QUIET, "ERROR: Node DIR <directory> sub-command "
									  "can not be used with -UseDagDir\n");
			return false;
		}
	...
	std::unique_ptr<Node> node(
		new Node(name.c_str(), directory.c_str(), desc.c_str()));
	ASSERT(node);
	...
	if (cmd->IsDone()) {
		if (node->GetType() != NodeType::JOB) {
			debug_printf(
				DEBUG_QUIET, "WARNING: %s node %s can not be marked as DONE\n",
...
bool DagProcessor::ProcessSplice(const Dagman &dm, Dag &dag,
								 const SpliceCommand *cmd, int dag_munge_id) {
	std::string name = MakeFullName(cmd->GetName(), dag_munge_id);

	if (dag.NodeExists(name.c_str())) {
		debug_printf(
			DEBUG_QUIET,
			"ERROR: Splice name %s already associated with a node in DAG.\n",
			cmd->GetName().c_str());
		return false;
	}
	...
	if (dag.LookupSplice(name)) {
		debug_printf(DEBUG_QUIET,
					 "ERROR: Splice named %s already exists in DAG.\n",
					 cmd->GetName().c_str());
		return false;
	}

	// TODO: Have DAG hold splice DAG's by value and return reference to act
	// upon
	...
	if (cmd->HasDir() && !spliceDir.Cd2MainDir(error)) {
		debug_printf(DEBUG_QUIET,
					 "ERROR: Failed to return to original directory: %s\n",
					 error.c_str());
		return false;
	}
	...
	if (splice_dag->HasFinalNode()) {
		debug_printf(
			DEBUG_QUIET,
			"ERROR: Splice %s has a FINAL node. This is not permitted.\n",
			cmd->GetName().c_str());
		return false;
	}
	...
	if (!dag.InsertSplice(name, splice_dag.release())) {
		debug_printf(DEBUG_QUIET,
					 "ERROR: Failed to insert splice %s into DAG\n",
					 name.c_str());
		return false;
	}
...
bool DagProcessor::ProcessDependencies(const ParentChildCommand *cmd, Dag &dag,
									   int dag_munge_id) {
	std::vector<Node *> parents;
	std::vector<Node *> children;
	...
	for (const auto &n : cmd->GetParents()) {
		...
		if (splice) {
			...
			if (!node) {
				debug_printf(DEBUG_QUIET,
							 "ERROR: Unknown parent node %s specified\n",
							 n.data());
				return false;
			}
			parents.push_back(node);
	...
	for (const auto &n : cmd->GetChildren()) {
		...
		if (splice) {
			...
			if (!node) {
				debug_printf(DEBUG_QUIET,
							 "ERROR: Unknown child node %s specified\n",
							 n.data());
				return false;
			}
			children.push_back(node);
	...
	// DagArc metadata for the requested dependency strength, passed straight
	// through to Dag::Connect() -- ARC_WEAK is only meaningful on children-edge
	// arcs.
	...
	if (config[conf::b::UseJoinNodes] && parents.size() > 1 &&
		...
		if (!dag.Add(node.release())) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: Failed to add join node %s to DAG.\n",
						 name.c_str());
			return false;
		}
		...
		// Connect parents to the join node -- this is where the requested
		// dependency strength actually applies. The join node is a NOOP that
		// can't itself fail once unblocked, so join -> children below always
		// stays strong.
		std::vector<Node *> lst = {join};
		if (!dag.Connect(parents, lst, meta)) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: failed to add dependency to join node %s\n",
	...
	if (!dag.Connect(parents, children, meta)) {
		debug_printf(DEBUG_QUIET,
					 "ERROR: failed to add dependency between nodes\n");
		return false;
	}
...
bool DagProcessor::ProcessDone(const DoneCommand *cmd, Dag &dag,
							   int dag_munge_id) {
	...
	if (node) {
		if (node->GetType() != NodeType::JOB) {
			debug_printf(DEBUG_QUIET,
						 "WARNING: %s node(s) can not be referenced by the "
						 "DONE command\n",
						 DAG::GET_KEYWORD_STRING(cmd->GetCommand()));
			return !check_warning_strictness(DAG_STRICT_1, false);
		}
...
bool DagProcessor::ProcessSaveFile(const SavePointCommand *sp, Dag &dag,
								   int dag_munge_id) {
	...
	if (!node) {
		...
		debug_printf(DEBUG_QUIET,
					 "ERROR: Save file can not be applied to %s node(s)\n",
...
bool DagProcessor::ProcessVars(const VarsCommand *vars, Dag &dag,
							   int dag_munge_id) {
...
bool DagProcessor::ProcessScript(const ScriptCommand *cmd, Dag &dag,
								 int dag_munge_id) {
	std::string name = MakeFullName(cmd->GetNodeName(), dag_munge_id);
	...
	for (Node *node : dag.FindAllNodes(name)) {
		...
		// TODO: Just use the dag_command.h enum class for script type every
		// where in DAGMan
		//       I am deferring this work for now since that will touch a lot of
		//       code
		...
		if (!node->AddScript(script.release())) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: Failed to add %s script to node %s\n",
...
bool DagProcessor::ProcessRetry(const RetryCommand *retry, Dag &dag,
								int dag_munge_id) {
	...
	for (Node *node : dag.FindAllNodes(name)) {
		...
		if (node->GetType() != NodeType::JOB) {
			debug_printf(
				DEBUG_QUIET,
				"ERROR: %s node %s can not have a retry specification\n",
				DAG::GET_KEYWORD_STRING(retry->GetCommand()),
				node->GetNodeName());
			return false;
		}
...
bool DagProcessor::ProcessAbortDagOn(const AbortDagCommand *ado, Dag &dag,
									 int dag_munge_id) {
	...
	for (Node *node : dag.FindAllNodes(name)) {
		...
		if (node->GetType() == NodeType::FINAL) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: FINAL node %s cannot have an ABORT-DAG-ON "
						 "specification.\n",
						 node->GetNodeName());
			return false;
		}
...
bool DagProcessor::ProcessPreSkip(const PreSkipCommand *skip, Dag &dag,
								  int dag_munge_id) {
	...
	for (Node *node : dag.FindAllNodes(name)) {
		...
		if (!node->AddPreSkip(skip->GetExitCode(), error)) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: Failed to add pre skip to %s: %s\n",
...
bool DagProcessor::ProcessTolerance(const ToleranceCommand *cmd, Dag &dag,
									int dag_munge_id) {
	std::string name = MakeFullName(cmd->GetNodeName(), dag_munge_id);
	...
	for (Node *node : dag.FindAllNodes(name)) {
		...
		node->SetTolerance(cmd->GetTolerance(), cmd->GetMode(),
						   cmd->IsPercentage());
	}
...
bool DagProcessor::ProcessPriority(const PriorityCommand *prio, Dag &dag,
								   int dag_munge_id) {
	...
	for (Node *node : dag.FindAllNodes(name)) {
		...
		if (node->GetType() != NodeType::JOB) {
			debug_printf(DEBUG_QUIET,
						 "ERROR: %s node %s can not have a priority\n",
...
bool DagProcessor::ProcessCategory(const CategoryCommand *cat, Dag &dag,
								   int dag_munge_id) {
	for (const auto &n : cat->GetNodes()) {
		...
		for (Node *node : dag.FindAllNodes(name)) {
			...
			if (node->GetType() != NodeType::JOB) {
				debug_printf(
					DEBUG_QUIET,
					"ERROR: %s node %s can not be added to a category\n",
					DAG::GET_KEYWORD_STRING(cat->GetCommand()),
					node->GetNodeName());
				return false;
			}
