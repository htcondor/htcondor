...
extern unsigned int
	DebugHeaderOptions; // for D_FID, D_PID, D_CAT, D_NOHEADER & D_TIMESTAMP
#else
...
/*--------------------------------------------------------------------------*/
void debug_dprintf(int flags, debug_level_t level, const char *fmt, ...) {
	const DPF_IDENT ident = 0; // REMIND: maybe have something useful here?
...
void time_to_str(const struct tm *tm, std::string &tstr) {
	formatstr(tstr, "%02d/%02d/%02d %02d:%02d:%02d ", tm->tm_mon + 1,
