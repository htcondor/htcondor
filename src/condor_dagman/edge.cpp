...
edge_id_t EdgeTable::DirectOffsetToId(size_t idx) {
...
std::vector<size_t> EdgeTable::CompactDirectPool() {
...
size_t EdgeTable::ArcCount() const {
	...
	auto view = m_edges | std::views::drop(1) |
				std::views::transform([](const auto &edge) {
					return edge.size() * edge.GetRefCount();
				});
