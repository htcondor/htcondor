...
std::tuple<int, int> DagmanMetrics::GetSums() {
	auto SumRan = [](int sum, std::array<int, 3> counts) {
		return std::move(sum) + counts[METRIC::COUNT::FAILURE] +
			   counts[METRIC::COUNT::SUCCESS];
	};
	...
	int total_run = std::accumulate(nodeCounts.begin(),
									nodeCounts.end() - offset, 0, SumRan);
	int total = std::accumulate(nodeCounts.begin(), nodeCounts.end() - offset,
								0, SumTotal);
...
bool DagmanMetricsV1::Report(int exitCode, Dagman &dm) {
	...
	if (dm.config[conf::b::ReportGraphMetrics]) {
		...
		// it has the side effect of determining the width and height of the
		// graph
		int height, width;
		height = width = 0;
