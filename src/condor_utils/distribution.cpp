...
{
#if 1 // tj:2012 - msvc analyze doesn't like strncpy because it doesn't null terminate, so I'm re-writing
	// copy name, truncating to MAX_DISTRIBUTION_NAME characters 
	// and creating an all uppercase and a capitalized version as we go.
	// (this assumes that the input is all lowercase, is that wise?)
	char ch = name[0];
	distribution[0] = ch;
