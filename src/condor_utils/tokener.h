...
#if 0
template <class T>
const T * nocase_sorted_tokener_lookup_table<T>::lookup(const char * str) const {
	if (cItems <= 0) return NULL;
	CaseInsensitiveLiteral toke(str);
...
template <class T>
const T * nocase_sorted_tokener_lookup_table<T>::lookup_token(const tokener & toke) const {
	if (cItems <= 0) return NULL;
	for (int ixLower = 0, ixUpper = cItems-1; ixLower <= ixUpper;) {
		int ix = (ixLower + ixUpper) / 2;
		int diff = toke.compare_nocase(pTable[ix].key);
...
template <class T>
const T * case_sensitive_unsorted_tokener_lookup_table<T>::lookup_token(const tokener & toke) const {
	if (cItems <= 0) return NULL;
	for (int ix = 0; ix < (int)cItems; ++ix) {
		if (toke.matches(pTable[ix].key))
			return &pTable[ix];
...
template <class T>
const T * case_sensitive_sorted_tokener_lookup_table<T>::lookup_token(const tokener & toke) const {
	if (cItems <= 0) return NULL;
	for (int ixLower = 0, ixUpper = cItems-1; ixLower <= ixUpper;) {
		int ix = (ixLower + ixUpper) / 2;
		if (toke.matches(pTable[ix].key))
