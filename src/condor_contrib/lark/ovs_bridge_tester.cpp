...
int main(int argc, const char *argv[])
{
	...
	int result;
	if ((result = ovs_create_bridge(bridge_name)) && (result != EEXIST))
	{
	...
	if (result != EEXIST)
	{
		...
		if ((result = ovs_disable_stp(bridge_name)))
		{
			dprintf(D_ALWAYS, "Unable to disable stp on openvswitch bridge %s.\n", bridge_name);
			ovs_delete_bridge(bridge_name);
			return result;
		}
		if ((result = ovs_add_interface_to_bridge(bridge_name, bridge_device)))
		{
