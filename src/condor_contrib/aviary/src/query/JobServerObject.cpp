...
getValidKnownJob(const char* key, AviaryStatus &_status) {
	...
	if (id.cluster < 0 || id.proc < 0) {
		aviUtilFmt (_status.text, "Invalid job id '%s'",key);
    ...
    if ( g_jobs.end() == element ) {
		aviUtilFmt (_status.text, "Unknown local job id '%s'",key);
		dprintf(D_FULLDEBUG, "%s\n", _status.text.c_str());
					   ...
					   AviaryStatus &_status)
{
	...
	if ( ad.LookupString("JOB_AD_ERROR", str)  ) {
		aviUtilFmt(_status.text,"Error checking ClassAd for user priv on job '%s'; ",key);
		_status.text += str;
		dprintf(D_ALWAYS,"%s\n",_status.text.c_str());
		return false;
	}
	...
	switch (ftype) {
		case ERR:
			if ( !ad.LookupString(ATTR_JOB_ERROR, fname)  ) {
				aviUtilFmt (_status.text,  "No error file for job '%s'",key);
				dprintf(D_ALWAYS,"%s\n", _status.text.c_str());
				return false;
			}
			break;
		case LOG:
			if ( !ad.LookupString(ATTR_ULOG_FILE, fname)  ) {
				aviUtilFmt (_status.text,  "No log file for job '%s'",key);
				dprintf(D_ALWAYS,"%s\n", _status.text.c_str());
				return false;
			}
			break;
		case OUT:
			if ( !ad.LookupString(ATTR_JOB_OUTPUT, fname)  ) {
				aviUtilFmt (_status.text,  "No output file for job '%s'",key);
				dprintf(D_ALWAYS,"%s\n", _status.text.c_str());
				return false;
			}
			break;
		default:
			...
			aviUtilFmt (_status.text,  "Unknown file type for job '%s'",key);
			dprintf(D_ALWAYS,"%s\n", _status.text.c_str());
			return false;
	}
	...
	if (the_file.Error()) {
		aviUtilFmt (_status.text, "Error opening requested file '%s', error %d",fname.c_str(),the_file.Errno());
        dprintf(D_FULLDEBUG,"%s\n", _status.text.c_str());
        // don't give up yet...maybe it's IWD+filename
        string iwd_path;
        if ( !ad.LookupString(ATTR_JOB_IWD, iwd_path)  ) {
            aviUtilFmt (_status.text,  "No IWD found for job '%s'",key);
            dprintf(D_ALWAYS,"%s\n", _status.text.c_str());
            return false;
        }
