...
const char *const DAYDTMONYRTIMEZONEFMT = "%a, %d %b %Y %X %Z";
...
dc_soap_init(struct soap *&soap)
{
    ...
    startm =tim.tv_sec+(tim.tv_usec/1000000.0);	
	...
	if (enable_soap_ssl) {
		int ssl_port = param_integer("SOAP_SSL_PORT", 0);
 		...
 		if (ssl_port >= 0) {
			...
			if (ssl_soap->fposthdr != NULL) {
			  dprintf(D_ALWAYS, "Saving original ssl_soap->fposthdr\n");
			  ssl_soap->user = (void *) ssl_soap->fposthdr;
			  ssl_soap->fposthdr = posthdr_addfield;
			} else dprintf(D_ALWAYS, "No ssl_soap->fposthdr to save\n");
...
int serve_file(struct soap *soap, const char *name, const char *type) { 
  ...
  if (param_boolean("ENABLE_HASH_NAME", false)) {
  	  const char *plain_name = NULL;
  	  if ((plain_name = condor_soap::get_plain_name(web_root_realpath, name)) != NULL)
  	  	  name = plain_name;
  }
  ...
  */
  ...
  if (param_boolean("ENABLE_HASH_NAME", false)) {
	  ...
	  if (modtime.Stat(buf) == 0) {
		  ...
		  if (userptr != NULL) {
			  strftime(userptr->dttmstr, DTTM_SIZE, DAYDTMONYRTIMEZONEFMT,
				  gmtime(&filemtime));
