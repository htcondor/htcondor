...
TransferDaemon::TransferDaemon(MyString fquser, MyString id, TDMode status) :
	m_treqs_in_progress(hashFunction, allowDuplicateKeys)
{
	m_fquser = fquser;
	m_id = id;
	m_status = status;
...
{
	m_td_table = 
		new HashTable<MyString, TransferDaemon*>(hashFunction, allowDuplicateKeys);
	m_id_table = 
		new HashTable<MyString, MyString>(hashFunction, allowDuplicateKeys);
	m_td_pid_table = 
		new HashTable<long, TransferDaemon*>(hashFuncLong, allowDuplicateKeys);
}
