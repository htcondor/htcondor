...
Unchanged: `GANGLIA_CONFIG`, `GANGLIA_GMETRIC`, `GANGLIA_LIB`, `GANGLIA_LIB_PATH`, `GANGLIA_LIB64_PATH`, `GANGLIA_GSTAT_COMMAND`, `GANGLIA_SEND_DATA_FOR_ALL_HOSTS` — all of these already use the `GANGLIA_` prefix and so needed no rename.

Renamed (modern-mode names; legacy-mode keeps old names):

| Modern mode | Legacy mode (unchanged) |
|---|---|
| `GANGLIA_MIN_METRIC_LIFETIME` | `GANGLIAD_MIN_METRIC_LIFETIME` |
