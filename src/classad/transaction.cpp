...
Log( FILE *fp, ClassAdUnParser *unp )
{
    ...
    if(!rec.InsertAttr(ATTR_OP_TYPE,
			(Value::IntType)ClassAdCollectionInterface::ClassAdCollOp_OpenTransaction)||
    ...
    if(!rec.InsertAttr(ATTR_OP_TYPE,
			(Value::IntType)ClassAdCollectionInterface::ClassAdCollOp_CommitTransaction)){
		CondorErrMsg += "; FATAL ERROR: failed to log transaction";
        return( false );
    }
        buf = "";
...
LogCommit( FILE *fp, ClassAdUnParser *unp )
{
    ...
    if(!rec.InsertAttr(ATTR_OP_TYPE,
			...
			|| !rec.InsertAttr( "ServerPort", (Value::IntType)port )	) {
		CondorErrMsg += "FATAL ERROR: failed to log transaction";
		return( false );
	}
	unp->Unparse( buf, &rec );
...
LogAckCommit( FILE *fp, ClassAdUnParser *unp )
{
    ...
    if(!rec.InsertAttr(ATTR_OP_TYPE,
			(Value::IntType)ClassAdCollectionInterface::ClassAdCollOp_AckCommitTransaction )||
            !rec.InsertAttr( "XactionName", xactionName) ) {
		CondorErrMsg += "FATAL ERROR: failed to log transaction";
		return( false );
	}
...
LogAbort( FILE *fp, ClassAdUnParser *unp )
{
    ...
    if(!rec.InsertAttr(ATTR_OP_TYPE,
			(Value::IntType)ClassAdCollectionInterface::ClassAdCollOp_AbortTransaction)
