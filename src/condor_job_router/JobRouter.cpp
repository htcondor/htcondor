...
JobRouter::JobRouter(bool as_tool)
	: m_jobs(hashFunction,rejectDuplicateKeys)
	, m_schedd2_name(NULL)
	, m_schedd2_pool(NULL)
	, m_schedd1_name(NULL)
	, m_schedd1_pool(NULL)
...
JobRouter::config() {
	...
	RoutingTable *new_routes = new RoutingTable(hashFunction,rejectDuplicateKeys);
...
JobRouter::AllocateRoutingTable() {
	return new RoutingTable(hashFunction,rejectDuplicateKeys);
}
...
JobRouter::GetCandidateJobs() {
	...
	HashTable<std::string,std::string> constraint_list(hashFunction,rejectDuplicateKeys);
