...
JobRouter::JobRouter(bool as_tool)
	: m_jobs(5000,hashFunction,rejectDuplicateKeys)
	, m_schedd2_name(NULL)
	, m_schedd2_pool(NULL)
	, m_schedd1_name(NULL)
	, m_schedd1_pool(NULL)
...
JobRouter::config() {
	...
	RoutingTable *new_routes = new RoutingTable(200,hashFunction,rejectDuplicateKeys);
...
JobRouter::AllocateRoutingTable() {
	return new RoutingTable(200,hashFunction,rejectDuplicateKeys);
}
...
JobRouter::GetCandidateJobs() {
	...
	HashTable<std::string,std::string> constraint_list(200,hashFunction,rejectDuplicateKeys);
