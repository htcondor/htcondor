...
// Error codes used as the Result code for operations on Users and Projects
// values are taken from errno values
typedef enum {
	SC_ERR_SUCCESS           = 0,
	SC_ERR_GENERIC           = 1,  // aka EPERM
	SC_ERR_NOT_FOUND         = 2,  // aka ENOENT
	SC_ERR_BAD_CONSTRAINT    = 5,  // aka Q_INVALID_QUERY
	SC_ERR_ALREADY_DONE      = 11, // aka EAGAIN
	SC_ERR_PERMISSION_DENIED = 13, // aka EACCES
	SC_ERR_OBJECT_IN_USE     = 16, // aka EBUSY
	SC_ERR_LIMIT_EXCEEDED    = 27, // aka EFBIG

	SC_ERR_NOT_IMPLEMENTED   = 42
} schedd_error_result_t;

// these codes are used to construct the actOnJobs result ad
// that ad will have attributes of the form
// result_total_<int-action_result_t>=<count-of-jobs> when result type is AR_TOTALS
// and cluster_<N>=<int-action_result_t> or job_<N>_<M>=<int-action_result_t> when result type is AR_LONG
// condor_rm -long will print the actOnJobs result ad, and the job_router uses the AR_LONG form
// python bindings use the AR_TOTALS (but convert to human readable on the python side of the bindings)
//
typedef enum {
	AR_ERROR,    // as of 2025 the generic AR_ERROR is no longer generated by the schedd
	AR_SUCCESS,
	AR_NOT_FOUND,
	AR_BAD_STATUS,
	AR_ALREADY_DONE,
...
public:
	...
	*/
	...
	int offerResources(const std::vector< std::pair<std::string, const ClassAd*> > & resources, const std::string & submitter_name, int timeout, const char * claimID = NULL, long long disk_held_by_claim_in_mb = 0);
