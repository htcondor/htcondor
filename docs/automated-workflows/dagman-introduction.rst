...
A DAGMan workflow is described in a **DAG description file**. This file specifies
the nodes of the DAG as well as the dependencies that order them.
...
four-node DAG is described as follows in the DAG description file:
...
The *NodeName* uniquely identifies a node within the DAG description file and in
output messages, and must be unique within the DAG.
...
.. note::
    ...
    A DAG containing *DIR* specifications cannot be run in conjunction with
...
The optional *DONE* keyword identifies a node as already
...
to the execution of the node's associated jobs, depending on the script type. All scripts
...
and *POST* scripts are considered part of the DAG node structure. Additionally,
there is a *HOLD* script that runs when any job associated with the node goes into
the held state; it is not considered part of the DAG workflow and is run on a
best-effort basis. If it does not complete successfully, it has no effect on the
overall workflow and no error is reported.

.. note::

    The script executable does not have to be a shell script (Unix) or batch file
    (Windows), but should be lightweight since it runs directly on the AP.
...
- POST: Turn an execution failure of the list of jobs into a successful node
...
The optional *DEBUG* keyword will capture a script's specified standard
...
debug file. This keyword is followed by two pieces of information:
  ...
  #. *Type*: Type of output to capture. Takes one of the following options:
      #. **STDOUT**
      #. **STDERR**
...
.. note::
    ...
    useful information regarding that script's execution.
...
These macros may only be used as individual command-line arguments
...
-  ``$EXIT_CODE_LIST`` is a comma-separated list of each job's :ad-attr:`ExitCode` ordered by
   job :ad-attr:`ProcId`\'s. Removed jobs will have no value in their position.
-  ``$EXIT_CODE_COUNTS`` is an ordered, comma-separated list of the number of jobs associated
...
Each node in a DAG may use a submit description file like one that a user might
use to submit via :tool:`condor_submit`.
...
node contain only a single job in a cluster, because DAGMan treats the
entire list of jobs associated with a single node as one entity, meaning that
...
Since each node uses the same HTCondor submit description file, each node within
the DAG runs the same list of jobs, but the ``$(Cluster)``
macro produces unique file names for each of the node's outputs because each node
has its own cluster of jobs.
...
- **JOB**: The node name to which these jobs belong.
...
- **DAG_PARENT_NAMES**: Comma-separated list of node names that are parents
  of the node to which these jobs belong.
...
- :ad-attr:`DAGNodeName`: The node name to which these jobs belong.
- :ad-attr:`DAGManNodeRetry`: The node's current retry number. First execution is 0.
...
.. note::
    ...
    Depending on the number of parent nodes a node has, the attribute
