...
:index:` <single: flocking>` :index:` <single: flocking;HTCondor>`
...
:index:` <single: for flocking;configuration>`
...
configuration, set the following configuration variables:
 ...
 ``FLOCK_NEGOTIATOR_HOSTS`` :index:` <single: FLOCK_NEGOTIATOR_HOSTS>`
...
:index:` <single: HOSTALLOW_NEGOTIATOR_SCHEDD>`
...
:index:` <single: FLOCK_FROM>` is a comma separated list of machines, and it
