...
:index:` <single: Job Router>` :index:` <single: condor_job_router daemon>`
...
-  Jobs appropriate for routing to the grid must not rely on access to a
   ...
   **output**\ :index:` <single: output;submit commands>` file, as well as
...
:index:` <single: GridResource;Job Router Routing Table ClassAd attribute>`

 GridResource
    Specifies the value for the ``GridResource`` attribute that will be
    inserted into the routed copy of the job’s ClassAd.
    :index:` <single: Name;Job Router Routing Table ClassAd attribute>`
 Name
    ...
    :index:` <single: Requirements;Job Router Routing Table ClassAd attribute>`
 Requirements
    ...
    :index:` <single: MaxJobs;Job Router Routing Table ClassAd attribute>`
 MaxJobs
    An integer maximum number of jobs permitted on the route at one
    time. The default is 100.
    :index:` <single: MaxIdleJobs;Job Router Routing Table ClassAd attribute>`
 MaxIdleJobs
    ...
    *condor\_job\_router* daemon will submit jobs up to this limit, wait
    ...
    :index:` <single: FailureRateThreshold;Job Router Routing Table ClassAd attribute>`
 FailureRateThreshold
    ...
    :index:` <single: JobFailureTest;Job Router Routing Table ClassAd attribute>`
 JobFailureTest
    ...
    :index:` <single: TargetUniverse;Job Router Routing Table ClassAd attribute>`
 TargetUniverse
    ...
    :index:` <single: UseSharedX509UserProxy;Job Router Routing Table ClassAd attribute>`
 UseSharedX509UserProxy
    ...
    :index:` <single: SharedX509UserProxy;Job Router Routing Table ClassAd attribute>`
 SharedX509UserProxy
    A string representing file containing the X.509 user proxy for the
    routed job.
    :index:` <single: JobShouldBeSandboxed;Job Router Routing Table ClassAd attribute>`
 JobShouldBeSandboxed
    ...
    :index:` <single: OverrideRoutingEntry;Job Router Routing Table ClassAd attribute>`
 OverrideRoutingEntry
    ...
    :index:` <single: Set_<ATTR>;Job Router Routing Table ClassAd attribute>`
 Set\_<ATTR>
    ...
    :index:` <single: Eval_Set_<ATTR>;Job Router Routing Table ClassAd attribute>`
 Eval\_Set\_<ATTR>
    ...
    :index:` <single: Copy_<ATTR>;Job Router Routing Table ClassAd attribute>`
 Copy\_<ATTR>
    ...
    :index:` <single: Delete_<ATTR>;Job Router Routing Table ClassAd attribute>`
 Delete\_<ATTR>
    Deletes ``<ATTR>`` from the routed copy ClassAd. A value assigned to
    this attribute in the routing table entry is ignored.
    :index:` <single: EditJobInPlace;Job Router Routing Table ClassAd attribute>`
 EditJobInPlace
    ...
    :index:` <single: <Keyword>_HOOK_TRANSLATE_JOB>` and transformation rules
...
Using ``JOB_ROUTER_ENTRIES_CMD`` :index:` <single: JOB_ROUTER_ENTRIES_CMD>`,
...
``JOB_ROUTER_ENTRIES`` :index:` <single: JOB_ROUTER_ENTRIES>`. Or, leave
``JOB_ROUTER_ENTRIES`` :index:` <single: JOB_ROUTER_ENTRIES>` there and have
