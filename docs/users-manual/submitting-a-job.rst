...
the environment variables from the machine it runs on.  If it did, the
environment might change from run to run, or machine to machine, and
create non reproducible, difficult to debug problems.  Rather, HTCondor
is deliberate about what environment variables a job sees, and allows
the user to set them in the job description file.
...
can be copied into the job.  The **getenv** :index:`getenv<single: getenv; submit commands>` command does this, as described on the :doc:`/man-pages/condor_submit` manual page.
...
executing job that may be useful for the job.
...
-  ``_CONDOR_SCRATCH_DIR``\ :index:`_CONDOR_SCRATCH_DIR environment variable`\ :index:`_CONDOR_SCRATCH_DIR<single: _CONDOR_SCRATCH_DIR; environment variables>`
   ...
   are deleted by HTCondor when the job stops running on a machine.
-  ``_CONDOR_SLOT``
