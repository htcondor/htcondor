...
:subcom:`should_transfer_files` :index:`should_transfer_files<single: should_transfer_files; definition>`
...
#. YES: HTCondor transfers the input sandbox from
   ...
   :subcom:`when_to_transfer_output` :index:`when_to_transfer_output<single: when_to_transfer_output; definition>`.
...
#. ``ON_EXIT_OR_EVICT``: HTCondor behaves the same as described for the
   ...
   If :subcom:`transfer_output_files` :index:`transfer_output_files<single: transfer_output_files; definition>`.
...
#. the executable, as defined with the
   :subcom:`executable` :index:`executable<single: executable; when transfered>` command
#. the input, as defined with the
   :subcom:`input` :index:`input<single: input; when transfered>` command
#. any jar files, for the **java** universe, as defined with the
   :subcom:`jar_files` :index:`jar_files<single: jar_files; when transfered>` command

If the job requires other input files, the submit description file
should have the
:subcom:`transfer_input_files` :index:`transfer_input_files<single: transfer_input_files; adding additional>`
...
with :subcom:`initialdir` :index:`initialdir<single: initialdir; and file transfer>` is
...
:subcom:`transfer_output_files` :index:`transfer_output_files<single: transfer_output_files; when files missing>`.
...
job on hold. Setting
:subcom:`transfer_output_files` :index:`transfer_output_files<single: transfer_output_files; when empty string>`
...
Files in the
:subcom:`transfer_input_files` :index:`transfer_input_files<single: transfer_input_files; relative to access point>`
...
:subcom:`transfer_input_files` :index:`transfer_input_files<single: transfer_input_files; ways to specify>`:

#. Relative to the current working directory as the job is submitted, if
   the submit command
   :subcom:`initialdir` :index:`initialdir<single: initialdir; and transfer input>` is not
   specified.
#. Relative to the initial directory, if the submit command
   :subcom:`initialdir` :index:`initialdir<single: initialdir; and transfer input>` is
   specified.
#. Absolute file paths.
...
:subcom:`transfer_output_files` :index:`transfer_output_files<single: transfer_output_files; mixing relative and absolute>`.
...
specified by using
:subcom:`transfer_output_remaps` :index:`transfer_output_remaps<single: transfer_output_remaps; definition>`.
...
:subcom:`arguments` :index:`arguments<single: arguments; example with output>` command,
...
:subcom:`initialdir` :index:`initialdir<single: initialdir; example with paths>`, and its
...
:subcom:`input` :index:`input<single: input; example with paths>`,
:subcom:`output` :index:`output<single: output; example with paths>`,
:subcom:`error` :index:`error<single: error; example with paths>`,
:subcom:`transfer_input_files` :index:`transfer_input_files<single: transfer_input_files; example with paths>`,
...
using the
:subcom:`transfer_input_files` :index:`transfer_input_files<single: transfer_input_files; example that fails>`
command work correctly (see Example 1). However, relative paths to
files in the
:subcom:`arguments` :index:`arguments<single: arguments; example that fails>` command
...
:subcom:`public_input_files` :index:`public_input_files<single: public_input_files; example>`
...
:subcom:`transfer_input_files` :index:`transfer_input_files<single: transfer_input_files; and public input files>`,
the files specified in
:subcom:`public_input_files` :index:`public_input_files<single: public_input_files; example>`
can be relative to the submit directory, or absolute paths. You can also
specify an :subcom:`initialDir` :index:`initialDir<single: initialDir; and public input files>`,
...
dealing with the transfer of files.
 ...
 Error Creating Zero-Length Files on Submit Machine
    ...
    :subcom:`output` :index:`output<single: output; created at submit>` and
    :subcom:`error` :index:`error<single: error; created at submit>`. If these files
...
:subcom:`should_transfer_files` :index:`should_transfer_files<single: should_transfer_files; with URLs>`
and
:subcom:`when_to_transfer_output` :index:`when_to_transfer_output<single: when_to_transfer_output; with URLs>`.
In addition, the URL for any files specified with a URL are given in the
:subcom:`transfer_input_files` :index:`transfer_input_files<single: transfer_input_files; with URLs>`
...
:subcom:`output_destination` :index:`output_destination<single: output_destination; with URLs>`
...
If you need to specify a region, you may do so using :subcom:`aws_region` :index:`aws_region<single: aws_regoin; with non-AWS S3 cloud>`,,
despite the name.
