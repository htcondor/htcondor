...
STEP 1: License Agreement.
    Agree to the HTCondor license agreemtn. 

STEP 2: HTCondor Pool Configuration.
    Choose the option to create a new pool and enter a chosen name. 

STEP 3: This Machine's Roles.
    Each machine within an HTCondor pool can either submit jobs or
    execute submitted jobs, or both submit and execute jobs. To create 
    a fully operation HTCondor pool on one computer, you should select
    the option that makes your machine a submit point *and* from the 
    list of execution options, choose the option to "always run jobs". 

STEP 4: The Account Domain.
    Enter the machine's accounting (or UID) domain. 

STEP 5: E-mail Settings.
    Specify the desired email address(es); this option is less crucial
...
STEP 7: Host Permission Settings.
    Enter the name of your computer to be given access
    permissions, or use the defaults provided. 
...
STEP 8: VM Universe Setting.
    ...
    VMware Server installed, HTCondor also needs *Perl* installed. 
