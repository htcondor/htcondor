...
Check a current install of HTCondor for incompatibilities that may cause
...
These checks output warnings about incompatibilities that the tool is
not capable of testing and thus give accurate feedback. Warnings tend
 ...
 **--version** *XX.YY.ZZ*
    Run available checks as if the current installed HTCondor version
    is *XX.YY.ZZ* i.e. *25.12.1*

 **--target** *XX.YY.ZZ*
    Only run checks that would impact upgrading from the current version
    to the targeted version *XX.YY.ZZ*.
...
Check hosts installed HTCondor-CE for potential issues caused by incompatibilities
when upgrading between major versions.
...
Run checks as if the current installed version is `v23.0.13`

.. code-block:: console

    $ condor_upgrade_check --version 23.0.13

Run checks that would impact going to specific version `v25.14.2`

.. code-block:: console

    $ condor_upgrade_check --target 25.14.2
