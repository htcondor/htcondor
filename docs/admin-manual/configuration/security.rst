...
:macro-def:`TRUST_DOMAIN`
    ...
    Note that the issuer (``iss``) field is for the *server*. Each IDTOKEN also
...
:macro-def:`SEC_ENABLE_MATCH_PASSWORD_AUTHENTICATION`
    ...
    *condor_schedd* and *condor_shadow*. These identities are automatically added to the DAEMON, READ, and CLIENT authorization
...
:macro-def:`SEC_CREDENTIAL_PRODUCER`
    ...
    and credentials must be sent to stdout.
