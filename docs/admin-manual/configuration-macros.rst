...
Configuration for the *condor_startd*, not in this section.
...
``COLLECTOR_HOST`` :index:`COLLECTOR_HOST`
 ...
 ``COLLECTOR_PORT`` :index:`COLLECTOR_PORT`
...
``NEGOTIATOR_HOST`` :index:`NEGOTIATOR_HOST`
...
``CONDOR_VIEW_HOST`` :index:`CONDOR_VIEW_HOST`
...
``SPOOL`` :index:`SPOOL`
 ...
 ``SLOT<N>_EXECUTE`` :index:`SLOT<N>_EXECUTE`
...
``LOCAL_CONFIG_FILE`` :index:`LOCAL_CONFIG_FILE`
    ...
    on page \ `548 <IntroductiontoConfiguration.html#x31-1750003.3.6>`__
    ...
    page \ `1167 <SettingUpforSpecialEnvironments.html#x42-3530003.14.4>`__.
 ...
 ``REQUIRE_LOCAL_CONFIG_FILE`` :index:`REQUIRE_LOCAL_CONFIG_FILE`
...
``LOCAL_CONFIG_DIR`` :index:`LOCAL_CONFIG_DIR`
...
``USER_CONFIG_FILE`` :index:`USER_CONFIG_FILE`
...
``EMAIL_SIGNATURE`` :index:`EMAIL_SIGNATURE`
...
``RESERVED_SWAP`` :index:`RESERVED_SWAP`
...
``HISTORY_HELPER_MAX_CONCURRENCY`` :index:`HISTORY_HELPER_MAX_CONCURRENCY`
...
``HISTORY_HELPER_MAX_HISTORY`` :index:`HISTORY_HELPER_MAX_HISTORY`
...
``CLASSAD_LOG_STRICT_PARSING`` :index:`CLASSAD_LOG_STRICT_PARSING`
...
``DEFAULT_DOMAIN_NAME`` :index:`DEFAULT_DOMAIN_NAME`
...
``EMAIL_DOMAIN`` :index:`EMAIL_DOMAIN`
...
``CREATE_CORE_FILES`` :index:`CREATE_CORE_FILES`
...
``ABORT_ON_EXCEPTION`` :index:`ABORT_ON_EXCEPTION`
...
``PASSWD_CACHE_REFRESH`` :index:`PASSWD_CACHE_REFRESH`
...
``SYSAPI_GET_LOADAVG`` :index:`SYSAPI_GET_LOADAVG`
...
``NETWORK_MAX_PENDING_CONNECTS`` :index:`NETWORK_MAX_PENDING_CONNECTS`
...
``WANT_UDP_COMMAND_SOCKET`` :index:`WANT_UDP_COMMAND_SOCKET`
    ...
    *condor_collector*, the pool should be configured to use TCP
    ...
    on
    page \ `1109 <NetworkingincludessectionsonPortUsageandCCB.html#x37-3200003.9.5>`__
    for more information).

 ``ALLOW_SCRIPTS_TO_RUN_AS_EXECUTABLES`` :index:`ALLOW_SCRIPTS_TO_RUN_AS_EXECUTABLES`
    ...
    **executable**\ :index:`executable<single: executable; submit commands>` in a job
    ...
    page \ `1642 <MicrosoftWindows.html#x76-5810008.2.7>`__ for further
    description.

``OPEN_VERB_FOR_<EXT>_FILES`` :index:`OPEN_VERB_FOR_<EXT>_FILES`
    ...
    page \ `1642 <MicrosoftWindows.html#x76-5810008.2.7>`__ for a more
    complete description.

``ENABLE_CLASSAD_CACHING`` :index:`ENABLE_CLASSAD_CACHING`
...
``STRICT_CLASSAD_EVALUATION`` :index:`STRICT_CLASSAD_EVALUATION`
    ...
    on page \ `1288 <HTCondorsClassAdMechanism.html#x48-3990004.1.1>`__
    for details.

``CLASSAD_USER_LIBS`` :index:`CLASSAD_USER_LIBS`
...
``CLASSAD_USER_PYTHON_MODULES`` :index:`CLASSAD_USER_PYTHON_MODULES`
 ...
 ``CLASSAD_USER_PYTHON_LIB`` :index:`CLASSAD_USER_PYTHON_LIB`
...
``CONDOR_FSYNC`` :index:`CONDOR_FSYNC`
...
``STATISTICS_WINDOW_QUANTUM`` :index:`STATISTICS_WINDOW_QUANTUM`
...
``STATISTICS_WINDOW_QUANTUM_<collection>`` :index:`STATISTICS_WINDOW_QUANTUM_<collection>`
    The same as ``STATISTICS_WINDOW_QUANTUM``, but used to override the
    global setting for a particular statistic collection. Collection
    names currently implemented are ``DC`` or ``DAEMONCORE`` and
    ``SCHEDD`` or ``SCHEDULER``.

``TCP_KEEPALIVE_INTERVAL`` :index:`TCP_KEEPALIVE_INTERVAL`
...
``ADVERTISE_IPV4_FIRST`` :index:`ADVERTISE_IPV4_FIRST`
...
``IGNORE_TARGET_PROTOCOL_PREFERENCE`` :index:`IGNORE_TARGET_PROTOCOL_PREFERENCE`
...
``IGNORE_DNS_PROTOCOL_PREFERENCE`` :index:`IGNORE_DNS_PROTOCOL_PREFERENCE`
...
``PREFER_OUTBOUND_IPV4`` :index:`PREFER_OUTBOUND_IPV4`
...
``<SUBSYS>_CLASSAD_USER_MAP_NAMES`` :index:`<SUBSYS>_CLASSAD_USER_MAP_NAMES`
...
``CLASSAD_USER_MAPFILE_<name>`` :index:`CLASSAD_USER_MAPFILE_<name>`
...
``MAX_<SUBSYS>_LOG`` :index:`MAX_<SUBSYS>_LOG`
 ...
 ``MAX_DEFAULT_LOG`` :index:`MAX_DEFAULT_LOG`
...
``JOB_QUEUE_LOG`` :index:`JOB_QUEUE_LOG`
...
``LOCK_DEBUG_LOG_TO_APPEND`` :index:`LOCK_DEBUG_LOG_TO_APPEND`
...
``ENABLE_USERLOG_LOCKING`` :index:`ENABLE_USERLOG_LOCKING`
...
``ENABLE_USERLOG_FSYNC`` :index:`ENABLE_USERLOG_FSYNC`
...
``USERLOG_FILE_CACHE_MAX`` :index:`USERLOG_FILE_CACHE_MAX`
...
``USERLOG_FILE_CACHE_CLEAR_INTERVAL`` :index:`USERLOG_FILE_CACHE_CLEAR_INTERVAL`
...
``DEBUG_TIME_FORMAT`` :index:`DEBUG_TIME_FORMAT`
...
``<SUBSYS>_DEBUG`` :index:`<SUBSYS>_DEBUG` All of the HTCondor daemons can
    ...
    comma- or space-separated list of the following values:
     ...
     ``D_STATS``
        ...
        version 8.5.6, ``C_GAHP_DEBUG`` :index:`C_GAHP_DEBUG`
        defaults to ``D_STATS``.
     ``D_PID``
...
``MAX_<SUBSYS>_<LEVEL>_LOG`` :index:`MAX_<SUBSYS>_<LEVEL>_LOG`
    See section \ `3.5.2 <#x33-1890003.5.2>`__, the definition of
    ``MAX_<SUBSYS>_LOG``.

``TRUNC_<SUBSYS>_<LEVEL>_LOG_ON_OPEN`` :index:`TRUNC_<SUBSYS>_<LEVEL>_LOG_ON_OPEN`
...
``MAX_EVENT_LOG`` :index:`MAX_EVENT_LOG`
...
``EVENT_LOG_JOB_AD_INFORMATION_ATTRS`` :index:`EVENT_LOG_JOB_AD_INFORMATION_ATTRS`
    ...
    page \ `2383 <JobClassAdAttributes.html#x170-1234000A.2>`__), but it
...
starter, none of which use DaemonCore).
 ...
 ``ALLOW…`` :index:`ALLOW`
    ...
    See section \ `3.8.7 <Security.html#x36-2880003.8.7>`__ on Setting
    up security in HTCondor for details on these macros and how to
    configure them.

``ENABLE_RUNTIME_CONFIG`` :index:`ENABLE_RUNTIME_CONFIG`
...
``SETTABLE_ATTRS_<PERMISSION-LEVEL>`` :index:`SETTABLE_ATTRS_<PERMISSION-LEVEL>`
    ...
    *condor_config_val* command.
    Section \ `3.8.7 <Security.html#x36-2880003.8.7>`__ on Setting up
    ...
    page \ `1036 <Security.html#x36-2880003.8.7>`__ contains details
    specific to these macros.

``SHUTDOWN_GRACEFUL_TIMEOUT`` :index:`SHUTDOWN_GRACEFUL_TIMEOUT`
...
``<SUBSYS>_ATTRS`` or ``<SUBSYS>_EXPRS``
    ...
    **rank** and **requirements**.
    ...
    NOTE: The *condor_kbdd* does not send ClassAds now, so this entry
...
``USE_CLONE_TO_CREATE_PROCESSES`` :index:`USE_CLONE_TO_CREATE_PROCESSES`
...
``MAX_TIME_SKIP`` :index:`MAX_TIME_SKIP`
...
``NOT_RESPONDING_TIMEOUT`` :index:`NOT_RESPONDING_TIMEOUT`
...
``NOT_RESPONDING_WANT_CORE`` :index:`NOT_RESPONDING_WANT_CORE`
    ...
    ``NOT_RESPONDING_TIMEOUT`` on page `624 <#x33-1900003.5.3>`__, and
    ``CREATE_CORE_FILES`` on page `600 <#x33-1880003.5.1>`__ for related
    details.
...
``SOCKET_LISTEN_BACKLOG`` :index:`SOCKET_LISTEN_BACKLOG`
...
``MAX_ACCEPTS_PER_CYCLE`` :index:`MAX_ACCEPTS_PER_CYCLE`
...
on
page \ `1084 <NetworkingincludessectionsonPortUsageandCCB.html#x37-3000003.9>`__.

 ``BIND_ALL_INTERFACES`` :index:`BIND_ALL_INTERFACES`
    ...
    on
    page \ `1097 <NetworkingincludessectionsonPortUsageandCCB.html#x37-3110003.9.3>`__.

``CCB_ADDRESS`` :index:`CCB_ADDRESS`
    ...
    page \ `1104 <NetworkingincludessectionsonPortUsageandCCB.html#x37-3150003.9.4>`__.

``CCB_HEARTBEAT_INTERVAL`` :index:`CCB_HEARTBEAT_INTERVAL`
...
``CCB_POLLING_INTERVAL`` :index:`CCB_POLLING_INTERVAL`
...
``CCB_POLLING_MAX_INTERVAL`` :index:`CCB_POLLING_MAX_INTERVAL`
...
``CCB_POLLING_TIMESLICE`` :index:`CCB_POLLING_TIMESLICE`
...
``CCB_RECONNECT_FILE`` :index:`CCB_RECONNECT_FILE`
...
``SHARED_PORT_DEFAULT_ID`` :index:`SHARED_PORT_DEFAULT_ID`
...
``AUTO_INCLUDE_SHARED_PORT_IN_DAEMON_LIST`` :index:`AUTO_INCLUDE_SHARED_PORT_IN_DAEMON_LIST`
...
``MAX_FILE_DESCRIPTORS`` :index:`MAX_FILE_DESCRIPTORS`
...
``NETWORK_HOSTNAME`` :index:`NETWORK_HOSTNAME`
...
``NETWORK_INTERFACE`` :index:`NETWORK_INTERFACE`
    ...
    on
    page \ `1097 <NetworkingincludessectionsonPortUsageandCCB.html#x37-3100003.9.3>`__.

 ``PRIVATE_NETWORK_NAME`` :index:`PRIVATE_NETWORK_NAME`
...
``PRIVATE_NETWORK_INTERFACE`` :index:`PRIVATE_NETWORK_INTERFACE`
    For systems with multiple network interfaces, if this configuration
 ...
 ``TCP_FORWARDING_HOST`` :index:`TCP_FORWARDING_HOST`
...
``OUT_HIGHPORT`` :index:`OUT_HIGHPORT`
    An integer value that specifies an upper limit of given port numbers
    for HTCondor to use on outgoing connections, such that HTCondor is
    restricted to a range of port numbers. This range implies the use of
    both ``OUT_LOWPORT`` and ``OUT_HIGHPORT``. A range of port numbers
...
``NONBLOCKING_COLLECTOR_UPDATE`` :index:`NONBLOCKING_COLLECTOR_UPDATE`
...
``NEGOTIATOR_USE_NONBLOCKING_STARTD_CONTACT`` :index:`NEGOTIATOR_USE_NONBLOCKING_STARTD_CONTACT`
    A boolean value that defaults to ``True``. When ``True``, the
...
``UDP_NETWORK_FRAGMENT_SIZE`` :index:`UDP_NETWORK_FRAGMENT_SIZE`
...
``ALWAYS_REUSEADDR`` :index:`ALWAYS_REUSEADDR`
...
section \ `2.5.8 <SubmittingaJob.html#x17-370002.5.8>`__.
 ...
 ``TRUST_UID_DOMAIN`` :index:`TRUST_UID_DOMAIN`
...
``TRUST_LOCAL_UID_DOMAIN`` :index:`TRUST_LOCAL_UID_DOMAIN`
...
``SOFT_UID_DOMAIN`` :index:`SOFT_UID_DOMAIN`
...
``SLOT<N>_USER`` :index:`SLOT<N>_USER`
...
``STARTER_ALLOW_RUNAS_OWNER`` :index:`STARTER_ALLOW_RUNAS_OWNER`
...
``DEDICATED_EXECUTE_ACCOUNT_REGEXP`` :index:`DEDICATED_EXECUTE_ACCOUNT_REGEXP`
 ...
 ``EXECUTE_LOGIN_IS_DEDICATED`` :index:`EXECUTE_LOGIN_IS_DEDICATED`
 ...
 ``RESERVE_AFS_CACHE`` :index:`RESERVE_AFS_CACHE`
...
These macros control the *condor_master*.
 ...
 ``DC_DAEMON_LIST`` :index:`DC_DAEMON_LIST`
 ...
 ``<DaemonName>_ENVIRONMENT`` :index:`<DaemonName>_ENVIRONMENT`
 ...
 ``<SUBSYS>_ARGS``
    ...
    page \ `2153 <Condorsubmit.html#x149-108400012>`__), with one
...
``PREEN_INTERVAL`` :index:`PREEN_INTERVAL`
...
``PUBLISH_OBITUARIES`` :index:`PUBLISH_OBITUARIES`
...
``START_DAEMONS`` :index:`START_DAEMONS`
...
``MASTER_UPDATE_INTERVAL`` :index:`MASTER_UPDATE_INTERVAL`
...
``MASTER_CHECK_NEW_EXEC_INTERVAL`` :index:`MASTER_CHECK_NEW_EXEC_INTERVAL`
...
``MASTER_NEW_BINARY_RESTART`` :index:`MASTER_NEW_BINARY_RESTART`
...
``MASTER_NEW_BINARY_DELAY`` :index:`MASTER_NEW_BINARY_DELAY`
...
``SHUTDOWN_FAST_TIMEOUT`` :index:`SHUTDOWN_FAST_TIMEOUT`
...
``DEFAULT_MASTER_SHUTDOWN_SCRIPT`` :index:`DEFAULT_MASTER_SHUTDOWN_SCRIPT`
...
``MASTER_SHUTDOWN_<Name>`` :index:`MASTER_SHUTDOWN_<Name>`
    A full path and file name of a program that the *condor_master* is
    to execute via the Unix execl() call, or the similar Win32 _execl()
    ...
    *condor_set_shutdown* is run. The ``Name`` specified as an
    ...
    page for *condor_set_shutdown* on
    page \ `2100 <Condorsetshutdown.html#x143-103100012>`__ contains
...
``MASTER_BACKOFF_CONSTANT`` :index:`MASTER_BACKOFF_CONSTANT` and ``MASTER_<name>_BACKOFF_CONSTANT`` :index:`MASTER_<name>_BACKOFF_CONSTANT`
...
``MASTER_BACKOFF_FACTOR`` :index:`MASTER_BACKOFF_FACTOR` and ``MASTER_<name>_BACKOFF_FACTOR`` :index:`MASTER_<name>_BACKOFF_FACTOR`
    When a daemon crashes, *condor_master* uses an exponential back off
    delay before restarting it; see the discussion at the end of this
    section for a detailed discussion on how these parameters work
...
``MASTER_BACKOFF_CEILING`` :index:`MASTER_BACKOFF_CEILING` and ``MASTER_<name>_BACKOFF_CEILING`` :index:`MASTER_<name>_BACKOFF_CEILING`
    When a daemon crashes, *condor_master* uses an exponential back off
    delay before restarting it; see the discussion at the end of this
    section for a detailed discussion on how these parameters work
...
``MASTER_RECOVER_FACTOR`` :index:`MASTER_RECOVER_FACTOR` and ``MASTER_<name>_RECOVER_FACTOR`` :index:`MASTER_<name>_RECOVER_FACTOR`
...
probably will not want to change them for any reason.
 ...
 ``MASTER_NAME`` :index:`MASTER_NAME`
    ...
    :index:`SCHEDD_NAME` setting (see
    section \ `3.5.9 <#x33-1960003.5.9>`__ on
    page \ `698 <#x33-1960003.5.9>`__). Since HTCondor makes the
 ...
 ``MASTER_ATTRS`` :index:`MASTER_ATTRS`
...
``ALLOW_ADMIN_COMMANDS`` :index:`ALLOW_ADMIN_COMMANDS`
...
``MASTER_INSTANCE_LOCK`` :index:`MASTER_INSTANCE_LOCK`
...
``ADD_WINDOWS_FIREWALL_EXCEPTION`` :index:`ADD_WINDOWS_FIREWALL_EXCEPTION`
...
``WINDOWS_FIREWALL_FAILURE_RETRY`` :index:`WINDOWS_FIREWALL_FAILURE_RETRY`
...
``USE_PROCESS_GROUPS`` :index:`USE_PROCESS_GROUPS`
...
``DISCARD_SESSION_KEYRING_ON_STARTUP`` :index:`DISCARD_SESSION_KEYRING_ON_STARTUP`
    A boolean value that defaults to ``True``. When ``True``, the
...
``ENABLE_KERNEL_TUNING`` :index:`ENABLE_KERNEL_TUNING`
...
``KERNEL_TUNING_LOG`` :index:`KERNEL_TUNING_LOG`
...
``LINUX_KERNEL_TUNING_SCRIPT`` :index:`LINUX_KERNEL_TUNING_SCRIPT`
...
on
page \ `916 <PolicyConfigurationforExecuteHostsandforSubmitHosts.html#x35-2530003.7.1>`__
...
``WANT_HOLD_SUBCODE`` :index:`WANT_HOLD_SUBCODE`
...
``ADVERTISE_PSLOT_ROLLUP_INFORMATION`` :index:`ADVERTISE_PSLOT_ROLLUP_INFORMATION`
 ...
 ``STARTD_PARTITIONABLE_SLOT_ATTRS`` :index:`STARTD_PARTITIONABLE_SLOT_ATTRS`
...
``ENABLE_VERSIONED_OPSYS`` :index:`ENABLE_VERSIONED_OPSYS`
...
``STARTD_HISTORY`` :index:`STARTD_HISTORY`
...
``POLLING_INTERVAL`` :index:`POLLING_INTERVAL`
...
``UPDATE_INTERVAL`` :index:`UPDATE_INTERVAL`
...
``MAX_CLAIM_ALIVES_MISSED`` :index:`MAX_CLAIM_ALIVES_MISSED`
    ...
    :index:`ALIVE_INTERVAL` on page \ `695 <#x33-1960003.5.9>`__).
...
``STARTD_HAS_BAD_UTMP`` :index:`STARTD_HAS_BAD_UTMP`
...
``CONSOLE_DEVICES`` :index:`CONSOLE_DEVICES`
 ...
 ``KBDD_BUMP_CHECK_SIZE`` :index:`KBDD_BUMP_CHECK_SIZE`
...
``STARTD_ATTRS`` :index:`STARTD_ATTRS`
    This macro is described in section \ `3.5.3 <#x33-1900003.5.3>`__ as
    ``<SUBSYS>_ATTRS``.

``STARTD_DEBUG`` :index:`STARTD_DEBUG`
...
``STARTD_ADDRESS_FILE`` :index:`STARTD_ADDRESS_FILE`
    This macro is described in section \ `3.5.3 <#x33-1900003.5.3>`__ as
    ``<SUBSYS>_ADDRESS_FILE``

``STARTD_SHOULD_WRITE_CLAIM_ID_FILE`` :index:`STARTD_SHOULD_WRITE_CLAIM_ID_FILE`
...
``STARTD_CLAIM_ID_FILE`` :index:`STARTD_CLAIM_ID_FILE`
...
``NUM_CPUS`` :index:`NUM_CPUS`
 ...
 ``MAX_NUM_CPUS`` :index:`MAX_NUM_CPUS`
 ...
 ``COUNT_HYPERTHREAD_CPUS`` :index:`COUNT_HYPERTHREAD_CPUS`
...
``RESERVED_MEMORY`` :index:`RESERVED_MEMORY`
...
``STARTD_NAME`` :index:`STARTD_NAME`
    ...
    *condor_collector*. Different names will distinguish the two
    ...
    page \ `648 <#x33-1940003.5.7>`__ for defaults and composition of
...
``DedicatedScheduler`` :index:`DedicatedScheduler`
    ...
    on
    page \ `1188 <SettingUpforSpecialEnvironments.html#x42-3670003.14.9>`__
    details the use of a dedicated scheduler.

``STARTD_NOCLAIM_SHUTDOWN`` :index:`STARTD_NOCLAIM_SHUTDOWN`
...
``STARTD_PUBLISH_WINREG`` :index:`STARTD_PUBLISH_WINREG`
...
on
page \ `1197 <SettingUpforSpecialEnvironments.html#x42-3700003.14.10>`__
...
for details.
 ...
 ``STARTD_RESOURCE_PREFIX`` :index:`STARTD_RESOURCE_PREFIX`
...
``SLOTS_CONNECTED_TO_CONSOLE`` :index:`SLOTS_CONNECTED_TO_CONSOLE`
...
``SLOTS_CONNECTED_TO_KEYBOARD`` :index:`SLOTS_CONNECTED_TO_KEYBOARD`
    An integer which indicates how many of the machine slots the
    *condor_startd* is representing should be "connected" to the
    keyboard (for remote tty activity, as well as console activity).
    This defaults to all slots (N in a machine with N CPUs).

``DISCONNECTED_KEYBOARD_IDLE_BOOST`` :index:`DISCONNECTED_KEYBOARD_IDLE_BOOST`
...
``STARTD_SLOT_ATTRS`` :index:`STARTD_SLOT_ATTRS`
...
on
page \ `916 <PolicyConfigurationforExecuteHostsandforSubmitHosts.html#x35-2540003.7.1>`__
...
corresponding attribute names using ``slot``, instead.
 ...
 ``MAX_SLOT_TYPES`` :index:`MAX_SLOT_TYPES`
 ...
 ``SLOT_TYPE_<N>`` :index:`SLOT_TYPE_<N>`
...
``SLOT_TYPE_<N>_PARTITIONABLE`` :index:`SLOT_TYPE_<N>_PARTITIONABLE`
...
``CLAIM_PARTITIONABLE_LEFTOVERS`` :index:`CLAIM_PARTITIONABLE_LEFTOVERS`
...
``MACHINE_RESOURCE_NAMES`` :index:`MACHINE_RESOURCE_NAMES`
...
``OFFLINE_MACHINE_RESOURCE_<name>`` :index:`OFFLINE_MACHINE_RESOURCE_<name>`
...
``MACHINE_RESOURCE_INVENTORY_<name>`` :index:`MACHINE_RESOURCE_INVENTORY_<name>`
 ...
 ``ENVIRONMENT_FOR_Assigned<name>`` :index:`ENVIRONMENT_FOR_Assigned<name>`
 ...
 ``ENVIRONMENT_VALUE_FOR_UnAssigned<name>`` :index:`ENVIRONMENT_VALUE_FOR_UnAssigned<name>`
 ...
 ``MUST_MODIFY_REQUEST_EXPRS`` :index:`MUST_MODIFY_REQUEST_EXPRS`
...
``MODIFY_REQUEST_EXPR_REQUESTMEMORY`` :index:`MODIFY_REQUEST_EXPR_REQUESTMEMORY`
...
details consumption policies.
 ...
 ``CONSUMPTION_POLICY`` :index:`CONSUMPTION_POLICY`
...
``CONSUMPTION_<Resource>`` :index:`CONSUMPTION_<Resource>`
...
``JAVA_CLASSPATH_ARGUMENT`` :index:`JAVA_CLASSPATH_ARGUMENT`
...
``JAVA_CLASSPATH_SEPARATOR`` :index:`JAVA_CLASSPATH_SEPARATOR`
...
``JAVA_CLASSPATH_DEFAULT`` :index:`JAVA_CLASSPATH_DEFAULT`
...
``JAVA_EXTRA_ARGUMENTS`` :index:`JAVA_EXTRA_ARGUMENTS`
...
advertisement.
 ...
 ``STARTD_PUBLISH_DOTNET`` :index:`STARTD_PUBLISH_DOTNET`
...
``DOT_NET_VERSIONS`` :index:`DOT_NET_VERSIONS`
...
page \ `1273 <PowerManagement.html#x46-3910003.18>`__ on Power
Management for more details.

 ``HIBERNATE_CHECK_INTERVAL`` :index:`HIBERNATE_CHECK_INTERVAL`
 ...
 ``UNHIBERNATE`` :index:`UNHIBERNATE`
    ...
    :index:`COLLECTOR_PERSISTENT_AD_LOG` on
    page \ `738 <#x33-2010003.5.14>`__ detailed on
    page \ `683 <#x33-1950003.5.8>`__ explains this. The special
...
``HIBERNATION_PLUGIN`` :index:`HIBERNATION_PLUGIN`
 ...
 ``HIBERNATION_PLUGIN_ARGS`` :index:`HIBERNATION_PLUGIN_ARGS`
...
``HIBERNATION_OVERRIDE_WOL`` :index:`HIBERNATION_OVERRIDE_WOL`
...
``LINUX_HIBERNATION_METHOD`` :index:`LINUX_HIBERNATION_METHOD`
...
``OFFLINE_EXPIRE_ADS_AFTER`` :index:`OFFLINE_EXPIRE_ADS_AFTER`
 ...
 ``DOCKER_VOLUMES`` :index:`DOCKER_VOLUMES`
    ...
    full details.
    ( `3.16.2 <SettingUptheVMandDockerUniverses.html#x44-3890003.16.2>`__)
 ``DOCKER_IMAGE_CACHE_SIZE`` :index:`DOCKER_IMAGE_CACHE_SIZE`
...
``DOCKER_EXTRA_ARGUMENTS`` :index:`DOCKER_EXTRA_ARGUMENTS`
...
``OPENMPI_INSTALL_PATH`` :index:`OPENMPI_INSTALL_PATH`
...
``OPENMPI_EXCLUDE_NETWORK_INTERFACES`` :index:`OPENMPI_EXCLUDE_NETWORK_INTERFACES`
...
``START_LOCAL_UNIVERSE`` :index:`START_LOCAL_UNIVERSE`
 ...
 ``LOCAL_UNIV_EXECUTE`` :index:`LOCAL_UNIV_EXECUTE`
...
``MAX_JOBS_RUNNING`` :index:`MAX_JOBS_RUNNING`
...
``MAX_JOBS_PER_OWNER`` :index:`MAX_JOBS_PER_OWNER`
 ...
 ``MAX_RUNNING_SCHEDULER_JOBS_PER_OWNER`` :index:`MAX_RUNNING_SCHEDULER_JOBS_PER_OWNER`
 ...
 ``MAX_JOBS_PER_SUBMISSION`` :index:`MAX_JOBS_PER_SUBMISSION`
 ...
 ``MAX_SHADOW_EXCEPTIONS`` :index:`MAX_SHADOW_EXCEPTIONS`
...
``MAX_PENDING_STARTD_CONTACTS`` :index:`MAX_PENDING_STARTD_CONTACTS`
...
``CURB_MATCHMAKING`` :index:`CURB_MATCHMAKING`
...
``FILE_TRANSFER_DISK_LOAD_THROTTLE`` :index:`FILE_TRANSFER_DISK_LOAD_THROTTLE`
 ...
 ``FILE_TRANSFER_DISK_LOAD_THROTTLE_WAIT_BETWEEN_INCREMENTS`` :index:`FILE_TRANSFER_DISK_LOAD_THROTTLE_WAIT_BETWEEN_INCREMENTS`
...
``FILE_TRANSFER_DISK_LOAD_THROTTLE_SHORT_HORIZON`` :index:`FILE_TRANSFER_DISK_LOAD_THROTTLE_SHORT_HORIZON`
...
``FILE_TRANSFER_DISK_LOAD_THROTTLE_LONG_HORIZON`` :index:`FILE_TRANSFER_DISK_LOAD_THROTTLE_LONG_HORIZON`
    This rarely configured variable specifies the string name of the
...
``TRANSFER_QUEUE_USER_EXPR`` :index:`TRANSFER_QUEUE_USER_EXPR`
...
``MAX_TRANSFER_QUEUE_AGE`` :index:`MAX_TRANSFER_QUEUE_AGE`
...
``TRANSFER_IO_REPORT_INTERVAL`` :index:`TRANSFER_IO_REPORT_INTERVAL`
    ...
    *condor_schedd*. See
    page \ `2456 <SchedulerClassAdAttributes.html#x173-1237000A.5>`__
...
``SCHEDD_QUERY_WORKERS`` :index:`SCHEDD_QUERY_WORKERS`
...
``CONDOR_Q_DASH_BATCH_IS_DEFAULT`` :index:`CONDOR_Q_DASH_BATCH_IS_DEFAULT`
...
``CONDOR_Q_ONLY_MY_JOBS`` :index:`CONDOR_Q_ONLY_MY_JOBS`
...
``CONDOR_Q_SHOW_OLD_SUMMARY`` :index:`CONDOR_Q_SHOW_OLD_SUMMARY`
...
``SCHEDD_INTERVAL`` :index:`SCHEDD_INTERVAL`
...
``ABSENT_SUBMITTER_LIFETIME`` :index:`ABSENT_SUBMITTER_LIFETIME`
...
``ABSENT_SUBMITTER_UPDATE_RATE`` :index:`ABSENT_SUBMITTER_UPDATE_RATE`
...
``WINDOWED_STAT_WIDTH`` :index:`WINDOWED_STAT_WIDTH`
...
``SCHEDD_INTERVAL_TIMESLICE`` :index:`SCHEDD_INTERVAL_TIMESLICE`
...
``JOB_START_COUNT`` :index:`JOB_START_COUNT`
...
``JOB_START_DELAY`` :index:`JOB_START_DELAY`
...
``MAX_NEXT_JOB_START_DELAY`` :index:`MAX_NEXT_JOB_START_DELAY`
...
``JOB_STOP_COUNT`` :index:`JOB_STOP_COUNT`
...
``JOB_STOP_DELAY`` :index:`JOB_STOP_DELAY`
...
``JOB_IS_FINISHED_COUNT`` :index:`JOB_IS_FINISHED_COUNT`
...
``JOB_IS_FINISHED_INTERVAL`` :index:`JOB_IS_FINISHED_INTERVAL`
...
``ALIVE_INTERVAL`` :index:`ALIVE_INTERVAL`
    ...
    *condor_startd*, the *condor_schedd* tells the *condor_startd*
    ...
    ``MAX_CLAIM_ALIVES_MISSED`` :index:`MAX_CLAIM_ALIVES_MISSED`,
    described on page \ `657 <#x33-1950003.5.8>`__) the *condor_startd*
...
``STARTD_SENDS_ALIVES`` :index:`STARTD_SENDS_ALIVES`
...
``REQUEST_CLAIM_TIMEOUT`` :index:`REQUEST_CLAIM_TIMEOUT`
 ...
 ``SHADOW_SIZE_ESTIMATE`` :index:`SHADOW_SIZE_ESTIMATE`
...
``SHADOW_RENICE_INCREMENT`` :index:`SHADOW_RENICE_INCREMENT`
...
``SCHED_UNIV_RENICE_INCREMENT`` :index:`SCHED_UNIV_RENICE_INCREMENT`
...
``QUEUE_ALL_USERS_TRUSTED`` :index:`QUEUE_ALL_USERS_TRUSTED`
...
``QUEUE_SUPER_USER_MAY_IMPERSONATE`` :index:`QUEUE_SUPER_USER_MAY_IMPERSONATE`
...
``SYSTEM_JOB_MACHINE_ATTRS`` :index:`SYSTEM_JOB_MACHINE_ATTRS`
...
``SYSTEM_JOB_MACHINE_ATTRS_HISTORY_LENGTH`` :index:`SYSTEM_JOB_MACHINE_ATTRS_HISTORY_LENGTH`
...
``SCHEDD_NAME`` :index:`SCHEDD_NAME`
 ...
 ``SCHEDD_ATTRS`` :index:`SCHEDD_ATTRS`
    This macro is described in section \ `3.5.3 <#x33-1900003.5.3>`__ as
    ``<SUBSYS>_ATTRS``.

``SCHEDD_DEBUG`` :index:`SCHEDD_DEBUG`
    This macro (and other settings related to debug logging in the
    *condor_schedd*) is described in
    section \ `3.5.2 <#x33-1890003.5.2>`__ as ``<SUBSYS>_DEBUG``.

``SCHEDD_ADDRESS_FILE`` :index:`SCHEDD_ADDRESS_FILE`
    This macro is described in section \ `3.5.3 <#x33-1900003.5.3>`__ as
    ``<SUBSYS>_ADDRESS_FILE``.

``SCHEDD_EXECUTE`` :index:`SCHEDD_EXECUTE`
...
``FLOCK_INCREMENT`` :index:`FLOCK_INCREMENT`
...
``NEGOTIATE_ALL_JOBS_IN_CLUSTER`` :index:`NEGOTIATE_ALL_JOBS_IN_CLUSTER`
...
``PERIODIC_EXPR_INTERVAL`` :index:`PERIODIC_EXPR_INTERVAL`
...
``MAX_PERIODIC_EXPR_INTERVAL`` :index:`MAX_PERIODIC_EXPR_INTERVAL`
...
``PERIODIC_EXPR_TIMESLICE`` :index:`PERIODIC_EXPR_TIMESLICE`
...
``SYSTEM_PERIODIC_HOLD`` :index:`SYSTEM_PERIODIC_HOLD`
...
``SYSTEM_PERIODIC_HOLD_SUBCODE`` :index:`SYSTEM_PERIODIC_HOLD_SUBCODE`
...
``SYSTEM_PERIODIC_RELEASE`` :index:`SYSTEM_PERIODIC_RELEASE`
...
``SCHEDD_PREEMPTION_REQUIREMENTS`` :index:`SCHEDD_PREEMPTION_REQUIREMENTS`
...
``SCHEDD_PREEMPTION_RANK`` :index:`SCHEDD_PREEMPTION_RANK`
...
``ParallelSchedulingGroup`` :index:`ParallelSchedulingGroup`
...
``PER_JOB_HISTORY_DIR`` :index:`PER_JOB_HISTORY_DIR`
...
``DEDICATED_SCHEDULER_USE_FIFO`` :index:`DEDICATED_SCHEDULER_USE_FIFO`
...
``DEDICATED_SCHEDULER_WAIT_FOR_SPOOLER`` :index:`DEDICATED_SCHEDULER_WAIT_FOR_SPOOLER`
...
``DEDICATED_SCHEDULER_DELAY_FACTOR`` :index:`DEDICATED_SCHEDULER_DELAY_FACTOR`
...
``SCHEDD_SEND_VACATE_VIA_TCP`` :index:`SCHEDD_SEND_VACATE_VIA_TCP`
    A boolean value that defaults to ``True``. When ``True``, the
    *condor_schedd* daemon sends vacate signals via TCP, instead of the
    default UDP.

``SCHEDD_CLUSTER_INITIAL_VALUE`` :index:`SCHEDD_CLUSTER_INITIAL_VALUE`
...
``SCHEDD_CLUSTER_INCREMENT_VALUE`` :index:`SCHEDD_CLUSTER_INCREMENT_VALUE`
...
``SCHEDD_CLUSTER_MAXIMUM_VALUE`` :index:`SCHEDD_CLUSTER_MAXIMUM_VALUE`
...
``CKPT_SERVER_CLIENT_TIMEOUT_RETRY`` :index:`CKPT_SERVER_CLIENT_TIMEOUT_RETRY`
    An integer which specifies how long in seconds the *condor_schedd*
...
``SCHEDD_JOB_QUEUE_LOG_FLUSH_DELAY`` :index:`SCHEDD_JOB_QUEUE_LOG_FLUSH_DELAY`
...
``SCHEDD_COLLECT_STATS_FOR_<Name>`` :index:`SCHEDD_COLLECT_STATS_FOR_<Name>`
 ...
 ``SCHEDD_COLLECT_STATS_BY_<Name>`` :index:`SCHEDD_COLLECT_STATS_BY_<Name>`
 ...
 ``SCHEDD_EXPIRE_STATS_BY_<Name>`` :index:`SCHEDD_EXPIRE_STATS_BY_<Name>`
...
``SIGNIFICANT_ATTRIBUTES`` :index:`SIGNIFICANT_ATTRIBUTES`
...
``ADD_SIGNIFICANT_ATTRIBUTES`` :index:`ADD_SIGNIFICANT_ATTRIBUTES`
    A comma and/or space separated list of job ClassAd attributes that
...
``REMOVE_SIGNIFICANT_ATTRIBUTES`` :index:`REMOVE_SIGNIFICANT_ATTRIBUTES`
    A comma and/or space separated list of job ClassAd attributes that
...
``SCHEDD_AUDIT_LOG`` :index:`SCHEDD_AUDIT_LOG`
...
``SCHEDD_USE_SLOT_WEIGHT`` :index:`SCHEDD_USE_SLOT_WEIGHT`
...
``JOB_TRANSFORM_NAMES`` :index:`JOB_TRANSFORM_NAMES`
...
``SUBMIT_REQUIREMENT_<Name>`` :index:`SUBMIT_REQUIREMENT_<Name>`
...
``SUBMIT_REQUIREMENT_<Name>_REASON`` :index:`SUBMIT_REQUIREMENT_<Name>_REASON`
...
``SCHEDD_RESTART_REPORT`` :index:`SCHEDD_RESTART_REPORT`
...
``JOB_SPOOL_PERMISSIONS`` :index:`JOB_SPOOL_PERMISSIONS`
...
``CHOWN_JOB_SPOOL_FILES`` :index:`CHOWN_JOB_SPOOL_FILES`
...
``IMMUTABLE_JOB_ATTRS`` :index:`IMMUTABLE_JOB_ATTRS`
...
``SYSTEM_IMMUTABLE_JOB_ATTRS`` :index:`SYSTEM_IMMUTABLE_JOB_ATTRS`
...
``PROTECTED_JOB_ATTRS`` :index:`PROTECTED_JOB_ATTRS`
    A comma and/or space separated list of attributes provided by the
...
``SYSTEM_PROTECTED_JOB_ATTRS`` :index:`SYSTEM_PROTECTED_JOB_ATTRS`
    A predefined comma and/or space separated list of attributes that
    can only be altered by the queue super-user, once they have
    committed values. The hard-code value is empty.

``ALTERNATE_JOB_SPOOL`` :index:`ALTERNATE_JOB_SPOOL`
...
``SHADOW_DEBUG`` :index:`SHADOW_DEBUG`
    This macro (and other settings related to debug logging in the
    shadow) is described in section \ `3.5.2 <#x33-1890003.5.2>`__ as
    ``<SUBSYS>_DEBUG``.

``SHADOW_QUEUE_UPDATE_INTERVAL`` :index:`SHADOW_QUEUE_UPDATE_INTERVAL`
...
``SHADOW_LAZY_QUEUE_UPDATE`` :index:`SHADOW_LAZY_QUEUE_UPDATE`
...
``SHADOW_WORKLIFE`` :index:`SHADOW_WORKLIFE`
...
``COMPRESS_PERIODIC_CKPT`` :index:`COMPRESS_PERIODIC_CKPT`
...
``COMPRESS_VACATE_CKPT`` :index:`COMPRESS_VACATE_CKPT`
    A boolean value that when ``True``, directs the *condor_shadow* to
    instruct applications to compress vacate checkpoints when possible.
    The default is ``False``.

``PERIODIC_MEMORY_SYNC`` :index:`PERIODIC_MEMORY_SYNC`
...
``SLOW_CKPT_SPEED`` :index:`SLOW_CKPT_SPEED`
...
``SHADOW_JOB_CLEANUP_RETRY_DELAY`` :index:`SHADOW_JOB_CLEANUP_RETRY_DELAY`
...
``SHADOW_MAX_JOB_CLEANUP_RETRIES`` :index:`SHADOW_MAX_JOB_CLEANUP_RETRIES`
...
``SHADOW_CHECKPROXY_INTERVAL`` :index:`SHADOW_CHECKPROXY_INTERVAL`
...
``SHADOW_RUN_UNKNOWN_USER_JOBS`` :index:`SHADOW_RUN_UNKNOWN_USER_JOBS`
...
``SHADOW_STATS_LOG`` :index:`SHADOW_STATS_LOG`
...
``MAX_SHADOW_STATS_LOG`` :index:`MAX_SHADOW_STATS_LOG`
...
These settings affect the *condor_starter*.
 ...
 ``EXEC_TRANSFER_ATTEMPTS`` :index:`EXEC_TRANSFER_ATTEMPTS`
 ...
 ``JOB_RENICE_INCREMENT`` :index:`JOB_RENICE_INCREMENT`
 ...
 ``STARTER_LOCAL_LOGGING`` :index:`STARTER_LOCAL_LOGGING`
...
``STARTER_LOG_NAME_APPEND`` :index:`STARTER_LOG_NAME_APPEND`
...
``STARTER_DEBUG`` :index:`STARTER_DEBUG`
...
``STARTER_UPDATE_INTERVAL_TIMESLICE`` :index:`STARTER_UPDATE_INTERVAL_TIMESLICE`
...
``USER_JOB_WRAPPER`` :index:`USER_JOB_WRAPPER`
...
``USE_VISIBLE_DESKTOP`` :index:`USE_VISIBLE_DESKTOP`
...
``STARTER_JOB_ENVIRONMENT`` :index:`STARTER_JOB_ENVIRONMENT`
    ...
    submit file (see
    page \ `2161 <Condorsubmit.html#x149-108400012>`__). If the same
    environment variable is assigned by this macro and by the user in
    the submit file, the user’s setting takes precedence.

``JOB_INHERITS_STARTER_ENVIRONMENT`` :index:`JOB_INHERITS_STARTER_ENVIRONMENT`
    A boolean value that defaults to ``False``. When ``True``, it causes
...
``STARTER_UPLOAD_TIMEOUT`` :index:`STARTER_UPLOAD_TIMEOUT`
...
``ASSIGN_CPU_AFFINITY`` :index:`ASSIGN_CPU_AFFINITY`
...
``ENFORCE_CPU_AFFINITY`` :index:`ENFORCE_CPU_AFFINITY`
 ...
 ``SLOT<N>_CPU_AFFINITY`` :index:`SLOT<N>_CPU_AFFINITY`
    This configuration variable is replaced by ``ASSIGN_CPU_AFFINITY``.
    Do not enable this configuration variable unless using glidein or
    another unusual setup.
 ...
 ``ENABLE_URL_TRANSFERS`` :index:`ENABLE_URL_TRANSFERS`
...
``FILETRANSFER_PLUGINS`` :index:`FILETRANSFER_PLUGINS`
...
``RUN_FILETRANSFER_PLUGINS_WITH_ROOT`` :index:`RUN_FILETRANSFER_PLUGINS_WITH_ROOT`
 ...
 ``ENABLE_CHIRP`` :index:`ENABLE_CHIRP`
...
``ENABLE_CHIRP_UPDATES`` :index:`ENABLE_CHIRP_UPDATES`
...
``CHIRP_DELAYED_UPDATE_PREFIX`` :index:`CHIRP_DELAYED_UPDATE_PREFIX`
 ...
 ``CHIRP_DELAYED_UPDATE_MAX_ATTRS`` :index:`CHIRP_DELAYED_UPDATE_MAX_ATTRS`
...
``STARTER_RLIMIT_AS`` :index:`STARTER_RLIMIT_AS`
...
``USE_PID_NAMESPACES`` :index:`USE_PID_NAMESPACES`
...
``PER_JOB_NAMESPACES`` :index:`PER_JOB_NAMESPACES`
...
``DYNAMIC_RUN_ACCOUNT_LOCAL_GROUP`` :index:`DYNAMIC_RUN_ACCOUNT_LOCAL_GROUP`
...
``JOB_EXECDIR_PERMISSIONS`` :index:`JOB_EXECDIR_PERMISSIONS`
...
``MAX_STARTER_STATS_LOG`` :index:`MAX_STARTER_STATS_LOG`
...
``SINGULARITY_EXTRA_ARGUMENTS`` :index:`SINGULARITY_EXTRA_ARGUMENTS`
...
:index:`condor_submit configuration variables<single: condor_submit configuration variables; configuration>`
 ...
 ``DEFAULT_UNIVERSE`` :index:`DEFAULT_UNIVERSE`
...
``JOB_DEFAULT_NOTIFICATION`` :index:`JOB_DEFAULT_NOTIFICATION`
...
``JOB_DEFAULT_LEASE_DURATION`` :index:`JOB_DEFAULT_LEASE_DURATION`
...
``JOB_DEFAULT_REQUESTMEMORY`` :index:`JOB_DEFAULT_REQUESTMEMORY`
...
``DEFAULT_JOB_MAX_RETRIES`` :index:`DEFAULT_JOB_MAX_RETRIES`
...
NOTE: The ``APPEND_RANK_STANDARD`` :index:`APPEND_RANK_STANDARD`
...
do not specify their own with:
 ...
 ``DEFAULT_RANK`` :index:`DEFAULT_RANK`
...
``DEFAULT_RANK_VANILLA`` :index:`DEFAULT_RANK_VANILLA`
...
``DEFAULT_IO_BUFFER_BLOCK_SIZE`` :index:`DEFAULT_IO_BUFFER_BLOCK_SIZE`
...
``SUBMIT_SKIP_FILECHECKS`` :index:`SUBMIT_SKIP_FILECHECKS`
...
``SUBMIT_DEFAULT_SHOULD_TRANSFER_FILES`` :index:`SUBMIT_DEFAULT_SHOULD_TRANSFER_FILES`
...
``SUBMIT_SEND_RESCHEDULE`` :index:`SUBMIT_SEND_RESCHEDULE`
...
``SUBMIT_ATTRS`` :index:`SUBMIT_ATTRS`
...
``LOG_ON_NFS_IS_ERROR`` :index:`LOG_ON_NFS_IS_ERROR`
...
``SUBMIT_MAX_PROCS_IN_CLUSTER`` :index:`SUBMIT_MAX_PROCS_IN_CLUSTER`
...
``ENABLE_DEPRECATION_WARNINGS`` :index:`ENABLE_DEPRECATION_WARNINGS`
    A boolean value that defaults to ``False``. When ``True``,
...
``CRED_MIN_TIME_LEFT`` :index:`CRED_MIN_TIME_LEFT`
...
``VALID_SPOOL_FILES`` :index:`VALID_SPOOL_FILES`
...
``SYSTEM_VALID_SPOOL_FILES`` :index:`SYSTEM_VALID_SPOOL_FILES`
    A comma or space separated list of files that *condor_preen*
...
``INVALID_LOG_FILES`` :index:`INVALID_LOG_FILES`
...
These macros affect the *condor_collector*.
 ...
 ``CLASSAD_LIFETIME`` :index:`CLASSAD_LIFETIME`
 ...
 ``MASTER_CHECK_INTERVAL`` :index:`MASTER_CHECK_INTERVAL`
...
``COLLECTOR_REQUIREMENTS`` :index:`COLLECTOR_REQUIREMENTS`
...
``QUERY_TIMEOUT`` :index:`QUERY_TIMEOUT`
...
``COLLECTOR_SOCKET_BUFSIZE`` :index:`COLLECTOR_SOCKET_BUFSIZE`
 ...
 ``COLLECTOR_TCP_SOCKET_BUFSIZE`` :index:`COLLECTOR_TCP_SOCKET_BUFSIZE`
...
``POOL_HISTORY_MAX_STORAGE`` :index:`POOL_HISTORY_MAX_STORAGE`
...
``POOL_HISTORY_SAMPLING_INTERVAL`` :index:`POOL_HISTORY_SAMPLING_INTERVAL`
...
``COLLECTOR_DAEMON_STATS`` :index:`COLLECTOR_DAEMON_STATS`
    ...
    *condor_collector* could have as lost. In particular, for the first
    ...
    page \ `2482 <ClassAdAttributesAddedbytheCondorcollector.html#x178-1242000A.10>`__
    for more information on the added attributes.

 ``COLLECTOR_STATS_SWEEP`` :index:`COLLECTOR_STATS_SWEEP`
 ...
 ``COLLECTOR_DAEMON_HISTORY_SIZE`` :index:`COLLECTOR_DAEMON_HISTORY_SIZE`
 ...
 ``COLLECTOR_CLASS_HISTORY_SIZE`` :index:`COLLECTOR_CLASS_HISTORY_SIZE`
    This variable controls the size of the published update history that
...
``COLLECTOR_QUERY_WORKERS_RESERVE_FOR_HIGH_PRIO`` :index:`COLLECTOR_QUERY_WORKERS_RESERVE_FOR_HIGH_PRIO`
...
``COLLECTOR_QUERY_WORKERS_PENDING`` :index:`COLLECTOR_QUERY_WORKERS_PENDING`
...
``COLLECTOR_QUERY_MAX_WORKTIME`` :index:`COLLECTOR_QUERY_MAX_WORKTIME`
...
``HANDLE_QUERY_IN_PROC_POLICY`` :index:`HANDLE_QUERY_IN_PROC_POLICY`
 ...
 ``COLLECTOR_DEBUG`` :index:`COLLECTOR_DEBUG`
    This macro (and other macros related to debug logging in the
    *condor_collector* is described in
    section \ `3.5.2 <#x33-1890003.5.2>`__ as ``<SUBSYS>_DEBUG``.

``CONDOR_VIEW_CLASSAD_TYPES`` :index:`CONDOR_VIEW_CLASSAD_TYPES`
 ...
 ``COLLECTOR_FORWARD_FILTERING`` :index:`COLLECTOR_FORWARD_FILTERING`
...
``COLLECTOR_FORWARD_WATCH_LIST`` :index:`COLLECTOR_FORWARD_WATCH_LIST`
...
``COLLECTOR_FORWARD_INTERVAL`` :index:`COLLECTOR_FORWARD_INTERVAL`
    When ``COLLECTOR_FORWARD_FILTERING`` is set to ``True``, this
...
page \ `1137 <Monitoring.html#x40-3350003.12.2>`__ for more details.

 ``ABSENT_REQUIREMENTS`` :index:`ABSENT_REQUIREMENTS`
...
``ABSENT_EXPIRE_ADS_AFTER`` :index:`ABSENT_EXPIRE_ADS_AFTER`
...
``COLLECTOR_PERSISTENT_AD_LOG`` :index:`COLLECTOR_PERSISTENT_AD_LOG`
 ...
 ``EXPIRE_INVALIDATED_ADS`` :index:`EXPIRE_INVALIDATED_ADS`
...
:index:`condor_negotiator configuration variables<single: condor_negotiator configuration variables; configuration>`

These macros affect the *condor_negotiator*.

 ``NEGOTIATOR_NAME`` :index:`NEGOTIATOR_NAME`
    Used to give an alternative value to the ``Name`` attribute in the
 ...
 ``NEGOTIATOR_INTERVAL`` :index:`NEGOTIATOR_INTERVAL`
    Sets how often the *condor_negotiator* starts a negotiation cycle.
    It is defined in seconds and defaults to 60 (1 minute).

``NEGOTIATOR_UPDATE_INTERVAL`` :index:`NEGOTIATOR_UPDATE_INTERVAL`
...
``NEGOTIATOR_CYCLE_DELAY`` :index:`NEGOTIATOR_CYCLE_DELAY`
...
``NEGOTIATOR_TIMEOUT`` :index:`NEGOTIATOR_TIMEOUT`
...
``NEGOTIATION_CYCLE_STATS_LENGTH`` :index:`NEGOTIATION_CYCLE_STATS_LENGTH`
    ...
    page \ `2461 <NegotiatorClassAdAttributes.html#x174-1238000A.6>`__
...
``DEFAULT_PRIO_FACTOR`` :index:`DEFAULT_PRIO_FACTOR`
...
``MAX_ACCOUNTANT_DATABASE_SIZE`` :index:`MAX_ACCOUNTANT_DATABASE_SIZE`
...
``NEGOTIATOR_DISCOUNT_SUSPENDED_RESOURCES`` :index:`NEGOTIATOR_DISCOUNT_SUSPENDED_RESOURCES`
...
``NEGOTIATOR_SOCKET_CACHE_SIZE`` :index:`NEGOTIATOR_SOCKET_CACHE_SIZE`
...
``NEGOTIATOR_INFORM_STARTD`` :index:`NEGOTIATOR_INFORM_STARTD`
...
``NEGOTIATOR_PRE_JOB_RANK`` :index:`NEGOTIATOR_PRE_JOB_RANK`
 ...
 ``PREEMPTION_REQUIREMENTS`` :index:`PREEMPTION_REQUIREMENTS`
    ...
    of the *condor_startd*, or ``PREEMPT`` expressions. See section
...
``PREEMPTION_REQUIREMENTS_STABLE`` :index:`PREEMPTION_REQUIREMENTS_STABLE`
...
``PREEMPTION_RANK`` :index:`PREEMPTION_RANK`
    Resources that match a request are first sorted by
    ``NEGOTIATOR_PRE_JOB_RANK``. If there are any ties in the rank of
    the top choice, the top resources are sorted by the user-supplied
    rank in the job ClassAd, then by ``NEGOTIATOR_POST_JOB_RANK``, then
...
``NEGOTIATOR_SLOT_CONSTRAINT`` :index:`NEGOTIATOR_SLOT_CONSTRAINT`
...
``NEGOTIATOR_JOB_CONSTRAINT`` :index:`NEGOTIATOR_JOB_CONSTRAINT`
...
``NEGOTIATOR_TRIM_SHUTDOWN_THRESHOLD`` :index:`NEGOTIATOR_TRIM_SHUTDOWN_THRESHOLD`
...
``NEGOTIATOR_SLOT_POOLSIZE_CONSTRAINT`` :index:`NEGOTIATOR_SLOT_POOLSIZE_CONSTRAINT` or ``GROUP_DYNAMIC_MACH_CONSTRAINT`` :index:`GROUP_DYNAMIC_MACH_CONSTRAINT`
 ...
 ``NEGOTIATOR_DEBUG`` :index:`NEGOTIATOR_DEBUG`
    This macro (and other settings related to debug logging in the
    negotiator) is described in section \ `3.5.2 <#x33-1890003.5.2>`__
    as ``<SUBSYS>_DEBUG``.

``NEGOTIATOR_MAX_TIME_PER_SUBMITTER`` :index:`NEGOTIATOR_MAX_TIME_PER_SUBMITTER`
...
``NEGOTIATOR_MAX_TIME_PER_SCHEDD`` :index:`NEGOTIATOR_MAX_TIME_PER_SCHEDD`
    The maximum number of seconds the *condor_negotiator* will spend
...
``NEGOTIATOR_MAX_TIME_PER_CYCLE`` :index:`NEGOTIATOR_MAX_TIME_PER_CYCLE`
...
``NEGOTIATOR_MAX_TIME_PER_PIESPIN`` :index:`NEGOTIATOR_MAX_TIME_PER_PIESPIN`
    The maximum number of seconds the *condor_negotiator* will spend
    ...
    *condor_schedd* daemon. But, this can result in unfair allocation
    ...
    on
    page \ `842 <UserPrioritiesandNegotiation.html#x34-2380003.6.6>`__
    for a description of a pie slice. It defaults to 120 seconds.

``NEGOTIATOR_DEPTH_FIRST`` :index:`NEGOTIATOR_DEPTH_FIRST`
...
``USE_RESOURCE_REQUEST_COUNTS`` :index:`USE_RESOURCE_REQUEST_COUNTS`
    A boolean value that defaults to ``True``. When ``True``, the
...
``NEGOTIATOR_RESOURCE_REQUEST_LIST_SIZE`` :index:`NEGOTIATOR_RESOURCE_REQUEST_LIST_SIZE`
...
``NEGOTIATOR_MATCH_EXPRS`` :index:`NEGOTIATOR_MATCH_EXPRS`
 ...
 ``NEGOTIATOR_MATCHLIST_CACHING`` :index:`NEGOTIATOR_MATCHLIST_CACHING`
    ...
    page \ `1486 <TheGridUniverse.html#x56-4870005.3.11>`__ in the
    subsection on Advertising Grid Resources to HTCondor for an example.

 ``NEGOTIATOR_CONSIDER_PREEMPTION`` :index:`NEGOTIATOR_CONSIDER_PREEMPTION`
...
``NEGOTIATOR_CONSIDER_EARLY_PREEMPTION`` :index:`NEGOTIATOR_CONSIDER_EARLY_PREEMPTION`
...
``ALLOW_PSLOT_PREEMPTION`` :index:`ALLOW_PSLOT_PREEMPTION`
...
``STARTD_AD_REEVAL_EXPR`` :index:`STARTD_AD_REEVAL_EXPR`
...
``NEGOTIATOR_UPDATE_AFTER_CYCLE`` :index:`NEGOTIATOR_UPDATE_AFTER_CYCLE`
...
``NEGOTIATOR_READ_CONFIG_BEFORE_CYCLE`` :index:`NEGOTIATOR_READ_CONFIG_BEFORE_CYCLE`
    A boolean value that defaults to ``False``. When ``True``, the
...
``CONCURRENCY_LIMIT_DEFAULT_<NAME>`` :index:`CONCURRENCY_LIMIT_DEFAULT_<NAME>`
...
The following configuration macros affect negotiation for group users.
 ...
 ``GROUP_QUOTA_DYNAMIC_<groupname>`` :index:`GROUP_QUOTA_DYNAMIC_<groupname>`
 ...
 ``GROUP_PRIO_FACTOR_<groupname>`` :index:`GROUP_PRIO_FACTOR_<groupname>`
...
``GROUP_AUTOREGROUP_<groupname>`` :index:`GROUP_AUTOREGROUP_<groupname>`
...
``GROUP_ACCEPT_SURPLUS`` :index:`GROUP_ACCEPT_SURPLUS`
...
``GROUP_ACCEPT_SURPLUS_<groupname>`` :index:`GROUP_ACCEPT_SURPLUS_<groupname>`
...
``GROUP_QUOTA_ROUND_ROBIN_RATE`` :index:`GROUP_QUOTA_ROUND_ROBIN_RATE`
...
``GROUP_QUOTA_MAX_ALLOCATION_ROUNDS`` :index:`GROUP_QUOTA_MAX_ALLOCATION_ROUNDS`
...
``NEGOTIATOR_USE_SLOT_WEIGHTS`` :index:`NEGOTIATOR_USE_SLOT_WEIGHTS`
...
``NEGOTIATOR_USE_WEIGHTED_DEMAND`` :index:`NEGOTIATOR_USE_WEIGHTED_DEMAND`
...
``GROUP_SORT_EXPR`` :index:`GROUP_SORT_EXPR`
...
``NEGOTIATOR_ALLOW_QUOTA_OVERSUBSCRIPTION`` :index:`NEGOTIATOR_ALLOW_QUOTA_OVERSUBSCRIPTION`
    A boolean value that defaults to ``True``. When ``True``, the
...
``PROCD_MAX_SNAPSHOT_INTERVAL`` :index:`PROCD_MAX_SNAPSHOT_INTERVAL`
...
``MAX_PROCD_LOG`` :index:`MAX_PROCD_LOG`
...
``PROCD_ADDRESS`` :index:`PROCD_ADDRESS`
...
``USE_GID_PROCESS_TRACKING`` :index:`USE_GID_PROCESS_TRACKING`
    ...
    section \ `3.14.12 <SettingUpforSpecialEnvironments.html#x42-3780003.14.12>`__
    on
    page \ `1212 <SettingUpforSpecialEnvironments.html#x42-3780003.14.12>`__
    for a detailed description.

``MIN_TRACKING_GID`` :index:`MIN_TRACKING_GID`
...
``MAX_TRACKING_GID`` :index:`MAX_TRACKING_GID`
...
``BASE_CGROUP`` :index:`BASE_CGROUP`
    ...
    on
    page \ `1213 <SettingUpforSpecialEnvironments.html#x42-3790003.14.13>`__
...
``CREDD_CACHE_LOCALLY`` :index:`CREDD_CACHE_LOCALLY`
...
``CRED_SUPER_USERS`` :index:`CRED_SUPER_USERS`
    A comma and/or space separated list of user names on a given machine
...
``SKIP_WINDOWS_LOGON_NETWORK`` :index:`SKIP_WINDOWS_LOGON_NETWORK`
...
``GRIDMANAGER_MINIMUM_PROXY_TIME`` :index:`GRIDMANAGER_MINIMUM_PROXY_TIME`
...
``HOLD_JOB_IF_CREDENTIAL_EXPIRES`` :index:`HOLD_JOB_IF_CREDENTIAL_EXPIRES`
...
``GRIDMANAGER_JOB_PROBE_INTERVAL`` :index:`GRIDMANAGER_JOB_PROBE_INTERVAL`
...
``GRIDMANAGER_MAX_SUBMITTED_JOBS_PER_RESOURCE`` :index:`GRIDMANAGER_MAX_SUBMITTED_JOBS_PER_RESOURCE`
 ...
 ``GRIDMANAGER_MAX_JOBMANAGERS_PER_RESOURCE`` :index:`GRIDMANAGER_MAX_JOBMANAGERS_PER_RESOURCE`
...
``GAHP_ARGS`` :index:`GAHP_ARGS`
    Arguments to be passed to the GAHP server. This configuration
    variable is no longer used.

``GAHP_DEBUG_HIDE_SENSITIVE_DATA`` :index:`GAHP_DEBUG_HIDE_SENSITIVE_DATA`
...
``GLOBUS_GATEKEEPER_TIMEOUT`` :index:`GLOBUS_GATEKEEPER_TIMEOUT`
...
``EC2_RESOURCE_TIMEOUT`` :index:`EC2_RESOURCE_TIMEOUT`
...
``EC2_GAHP_RATE_LIMIT`` :index:`EC2_GAHP_RATE_LIMIT`
...
``GRAM_VERSION_DETECTION`` :index:`GRAM_VERSION_DETECTION`
    A boolean value that defaults to ``True``. When ``True``, the
...
``BATCH_GAHP_CHECK_STATUS_ATTEMPTS`` :index:`BATCH_GAHP_CHECK_STATUS_ATTEMPTS`
...
``GAHP_SSL_CADIR`` :index:`GAHP_SSL_CADIR`
...
``GAHP_SSL_CAFILE`` :index:`GAHP_SSL_CAFILE`
...
These macros affect the *condor_job_router* daemon.
 ...
 ``JOB_ROUTER_DEFAULTS`` :index:`JOB_ROUTER_DEFAULTS`
...
``JOB_ROUTER_ENTRIES`` :index:`JOB_ROUTER_ENTRIES`
 ...
 ``JOB_ROUTER_ENTRIES_FILE`` :index:`JOB_ROUTER_ENTRIES_FILE`
...
``JOB_ROUTER_ENTRIES_CMD`` :index:`JOB_ROUTER_ENTRIES_CMD`
...
``JOB_ROUTER_ENTRIES_REFRESH`` :index:`JOB_ROUTER_ENTRIES_REFRESH`
...
``JOB_ROUTER_LOCK`` :index:`JOB_ROUTER_LOCK`
...
``JOB_ROUTER_SOURCE_JOB_CONSTRAINT`` :index:`JOB_ROUTER_SOURCE_JOB_CONSTRAINT`
...
``JOB_ROUTER_MAX_JOBS`` :index:`JOB_ROUTER_MAX_JOBS`
...
``MAX_JOB_MIRROR_UPDATE_LAG`` :index:`MAX_JOB_MIRROR_UPDATE_LAG`
...
``JOB_ROUTER_POLLING_PERIOD`` :index:`JOB_ROUTER_POLLING_PERIOD`
...
``JOB_ROUTER_NAME`` :index:`JOB_ROUTER_NAME`
...
``JOB_ROUTER_SCHEDD1_SPOOL`` :index:`JOB_ROUTER_SCHEDD1_SPOOL`
...
``JOB_ROUTER_SCHEDD2_SPOOL`` :index:`JOB_ROUTER_SCHEDD2_SPOOL`
...
``JOB_ROUTER_SCHEDD1_NAME`` :index:`JOB_ROUTER_SCHEDD1_NAME`
...
``JOB_ROUTER_SCHEDD2_NAME`` :index:`JOB_ROUTER_SCHEDD2_NAME`
...
``JOB_ROUTER_SCHEDD1_POOL`` :index:`JOB_ROUTER_SCHEDD1_POOL`
...
``JOB_ROUTER_SCHEDD2_POOL`` :index:`JOB_ROUTER_SCHEDD2_POOL`
    The Condor pool (*condor_collector* address) of the
    *condor_schedd* to which the routed copy of the jobs are submitted.
    If not specified, defaults to the local pool.

``JOB_ROUTER_ROUND_ROBIN_SELECTION`` :index:`JOB_ROUTER_ROUND_ROBIN_SELECTION`
...
::
 ...
 ``LeaseManager.GETADS_INTERVAL`` :index:`LeaseManager.GETADS_INTERVAL`
...
``LeaseManager.UPDATE_INTERVAL`` :index:`LeaseManager.UPDATE_INTERVAL`
    An integer value, given in seconds, that controls the frequency with
    which the *condor_lease_manager* sends its ClassAds to the
    *condor_collector*. The default value is 60 seconds, with a minimum
    value of 5 seconds.

``LeaseManager.PRUNE_INTERVAL`` :index:`LeaseManager.PRUNE_INTERVAL`
    An integer value, given in seconds, that controls the frequency with
...
``LeaseManager.DEBUG_ADS`` :index:`LeaseManager.DEBUG_ADS`
...
``LeaseManager.MAX_LEASE_DURATION`` :index:`LeaseManager.MAX_LEASE_DURATION`
 ...
 ``LeaseManager.MAX_TOTAL_LEASE_DURATION`` :index:`LeaseManager.MAX_TOTAL_LEASE_DURATION`
...
``LeaseManager.DEFAULT_MAX_LEASE_DURATION`` :index:`LeaseManager.DEFAULT_MAX_LEASE_DURATION`
...
``LeaseManager.CLASSAD_LOG`` :index:`LeaseManager.CLASSAD_LOG`
...
``LeaseManager.QUERY_ADTYPE`` :index:`LeaseManager.QUERY_ADTYPE`
...
``LeaseManager.QUERY_CONSTRAINTS`` :index:`LeaseManager.QUERY_CONSTRAINTS`
...
These macros affect the Grid Monitor.
 ...
 ``ENABLE_GRID_MONITOR`` :index:`ENABLE_GRID_MONITOR`
    ...
    page \ `1453 <TheGridUniverse.html#x56-4640005.3.2>`__ for more
    information.
...
``GRID_MONITOR_HEARTBEAT_TIMEOUT`` :index:`GRID_MONITOR_HEARTBEAT_TIMEOUT`
...
``GRID_MONITOR_RETRY_DURATION`` :index:`GRID_MONITOR_RETRY_DURATION`
...
``GRID_MONITOR_NO_STATUS_TIMEOUT`` :index:`GRID_MONITOR_NO_STATUS_TIMEOUT`
...
``GRID_MONITOR_DISABLE_TIME`` :index:`GRID_MONITOR_DISABLE_TIME`
...
``GLEXEC_RETRIES`` :index:`GLEXEC_RETRIES`
...
``GLEXEC_RETRY_DELAY`` :index:`GLEXEC_RETRY_DELAY`
...
``GLEXEC_HOLD_ON_INITIAL_FAILURE`` :index:`GLEXEC_HOLD_ON_INITIAL_FAILURE`
...
:index:`DAGMan configuration: general`
 ...
 ``DAGMAN_CONFIG_FILE`` :index:`DAGMAN_CONFIG_FILE`
...
``DAGMAN_USE_STRICT`` :index:`DAGMAN_USE_STRICT`
 ...
 ``DAGMAN_STARTUP_CYCLE_DETECT`` :index:`DAGMAN_STARTUP_CYCLE_DETECT`
    A boolean value that defaults to ``False``. When ``True``, causes
...
``DAGMAN_ABORT_DUPLICATES`` :index:`DAGMAN_ABORT_DUPLICATES`
 ...
 ``DAGMAN_USE_OLD_DAG_READER`` :index:`DAGMAN_USE_OLD_DAG_READER`
 ...
 ``DAGMAN_USE_SHARED_PORT`` :index:`DAGMAN_USE_SHARED_PORT`
...
:index:`DAGMan configuration: throttling`
 ...
 ``DAGMAN_MAX_JOBS_IDLE`` :index:`DAGMAN_MAX_JOBS_IDLE`
    ...
    *condor_submit_dag* **-maxidle **\ *c*\ ommand-line argument (see
     `12 <Condorsubmitdag.html#x150-109200012>`__).

``DAGMAN_MAX_JOBS_SUBMITTED`` :index:`DAGMAN_MAX_JOBS_SUBMITTED`
...
:index:`DAGMan configuration: priority, node semantics`
 ...
 ``DAGMAN_DEFAULT_PRIORITY`` :index:`DAGMAN_DEFAULT_PRIORITY`
    An integer value defining the minimum priority of node jobs running
    under this *condor_dagman* job. Defaults to 0.

``DAGMAN_SUBMIT_DEPTH_FIRST`` :index:`DAGMAN_SUBMIT_DEPTH_FIRST`
...
``DAGMAN_ALWAYS_RUN_POST`` :index:`DAGMAN_ALWAYS_RUN_POST`
...
:index:`DAGMan configuration: submission/removal`
 ...
 ``DAGMAN_USER_LOG_SCAN_INTERVAL`` :index:`DAGMAN_USER_LOG_SCAN_INTERVAL`
 ...
 ``DAGMAN_MAX_SUBMITS_PER_INTERVAL`` :index:`DAGMAN_MAX_SUBMITS_PER_INTERVAL`
...
``DAGMAN_MAX_JOB_HOLDS`` :index:`DAGMAN_MAX_JOB_HOLDS`
...
``DAGMAN_HOLD_CLAIM_TIME`` :index:`DAGMAN_HOLD_CLAIM_TIME`
...
``DAGMAN_SUBMIT_DELAY`` :index:`DAGMAN_SUBMIT_DELAY`
...
``DAGMAN_PROHIBIT_MULTI_JOBS`` :index:`DAGMAN_PROHIBIT_MULTI_JOBS`
...
``DAGMAN_GENERATE_SUBDAG_SUBMITS`` :index:`DAGMAN_GENERATE_SUBDAG_SUBMITS`
...
``DAGMAN_REMOVE_NODE_JOBS`` :index:`DAGMAN_REMOVE_NODE_JOBS`
...
``DAGMAN_MUNGE_NODE_NAMES`` :index:`DAGMAN_MUNGE_NODE_NAMES`
...
``DAGMAN_SUPPRESS_JOB_LOGS`` :index:`DAGMAN_SUPPRESS_JOB_LOGS`
...
``DAGMAN_SUPPRESS_NOTIFICATION`` :index:`DAGMAN_SUPPRESS_NOTIFICATION`
...
``DAGMAN_CONDOR_SUBMIT_EXE`` :index:`DAGMAN_CONDOR_SUBMIT_EXE`
...
``DAGMAN_CONDOR_RM_EXE`` :index:`DAGMAN_CONDOR_RM_EXE`
...
``DAGMAN_ABORT_ON_SCARY_SUBMIT`` :index:`DAGMAN_ABORT_ON_SCARY_SUBMIT`
...
:index:`DAGMan configuration: rescue/retry`
 ...
 ``DAGMAN_AUTO_RESCUE`` :index:`DAGMAN_AUTO_RESCUE`
    A boolean value that controls whether *condor_dagman* automatically
...
``DAGMAN_MAX_RESCUE_NUM`` :index:`DAGMAN_MAX_RESCUE_NUM`
...
``DAGMAN_RESET_RETRIES_UPON_RESCUE`` :index:`DAGMAN_RESET_RETRIES_UPON_RESCUE`
...
``DAGMAN_WRITE_PARTIAL_RESCUE`` :index:`DAGMAN_WRITE_PARTIAL_RESCUE`
 ...
 ``DAGMAN_RETRY_SUBMIT_FIRST`` :index:`DAGMAN_RETRY_SUBMIT_FIRST`
...
``DAGMAN_OLD_RESCUE`` :index:`DAGMAN_OLD_RESCUE`
...
:index:`DAGMan configuration: log files`
 ...
 ``DAGMAN_LOG_ON_NFS_IS_ERROR`` :index:`DAGMAN_LOG_ON_NFS_IS_ERROR`
...
``DAGMAN_ALLOW_EVENTS`` :index:`DAGMAN_ALLOW_EVENTS`
 ...
 ``DAGMAN_IGNORE_DUPLICATE_JOB_EXECUTION`` :index:`DAGMAN_IGNORE_DUPLICATE_JOB_EXECUTION`
...
``DAGMAN_VERBOSITY`` :index:`DAGMAN_VERBOSITY`
 ...
 ``DAGMAN_DEBUG_CACHE_ENABLE`` :index:`DAGMAN_DEBUG_CACHE_ENABLE`
...
``DAGMAN_DEBUG_CACHE_SIZE`` :index:`DAGMAN_DEBUG_CACHE_SIZE`
...
``MAX_DAGMAN_LOG`` :index:`MAX_DAGMAN_LOG`
    This variable is described in section \ `3.5.2 <#x33-1890003.5.2>`__
...
:index:`DAGMan configuration: HTCondor attributes`
 ...
 ``DAGMAN_COPY_TO_SPOOL`` :index:`DAGMAN_COPY_TO_SPOOL`
 ...
 ``DAGMAN_INSERT_SUB_FILE`` :index:`DAGMAN_INSERT_SUB_FILE`
...
``DAGMAN_ON_EXIT_REMOVE`` :index:`DAGMAN_ON_EXIT_REMOVE`
...
``GSI_SKIP_HOST_CHECK`` :index:`GSI_SKIP_HOST_CHECK`
...
``GSI_SKIP_HOST_CHECK_CERT_REGEX`` :index:`GSI_SKIP_HOST_CHECK_CERT_REGEX`
...
``GSI_DAEMON_DIRECTORY`` :index:`GSI_DAEMON_DIRECTORY`
...
``GSI_AUTHZ_CONF`` :index:`GSI_AUTHZ_CONF`
...
``GSS_ASSIST_GRIDMAP_CACHE_EXPIRATION`` :index:`GSS_ASSIST_GRIDMAP_CACHE_EXPIRATION`
...
``DELEGATE_JOB_GSI_CREDENTIALS`` :index:`DELEGATE_JOB_GSI_CREDENTIALS`
...
``DELEGATE_FULL_JOB_GSI_CREDENTIALS`` :index:`DELEGATE_FULL_JOB_GSI_CREDENTIALS`
...
``DELEGATE_JOB_GSI_CREDENTIALS_LIFETIME`` :index:`DELEGATE_JOB_GSI_CREDENTIALS_LIFETIME`
...
``DELEGATE_JOB_GSI_CREDENTIALS_REFRESH`` :index:`DELEGATE_JOB_GSI_CREDENTIALS_REFRESH`
...
``GSI_DELEGATION_KEYBITS`` :index:`GSI_DELEGATION_KEYBITS`
...
``GSI_DELEGATION_CLOCK_SKEW_ALLOWABLE`` :index:`GSI_DELEGATION_CLOCK_SKEW_ALLOWABLE`
...
``GRIDMAP`` :index:`GRIDMAP`
 ...
 ``SEC_<access-level>_SESSION_DURATION`` :index:`SEC_<access-level>_SESSION_DURATION`
 ...
 ``SEC_<access-level>_SESSION_LEASE`` :index:`SEC_<access-level>_SESSION_LEASE`
...
``SEC_INVALIDATE_SESSIONS_VIA_TCP`` :index:`SEC_INVALIDATE_SESSIONS_VIA_TCP`
...
``FS_REMOTE_DIR`` :index:`FS_REMOTE_DIR`
...
``ENCRYPT_EXECUTE_DIRECTORY`` :index:`ENCRYPT_EXECUTE_DIRECTORY`
...
``ENCRYPT_EXECUTE_DIRECTORY_FILENAMES`` :index:`ENCRYPT_EXECUTE_DIRECTORY_FILENAMES`
...
``ECRYPTFS_ADD_PASSPHRASE`` :index:`ECRYPTFS_ADD_PASSPHRASE`
...
``SEC_TCP_SESSION_DEADLINE`` :index:`SEC_TCP_SESSION_DEADLINE`
...
``SEC_DEFAULT_AUTHENTICATION_TIMEOUT`` :index:`SEC_DEFAULT_AUTHENTICATION_TIMEOUT`
...
``SEC_PASSWORD_FILE`` :index:`SEC_PASSWORD_FILE`
...
``AUTH_SSL_CLIENT_CADIR`` :index:`AUTH_SSL_CLIENT_CADIR`
    The path to a directory that may contain the certificates (each in
...
``CERTIFICATE_MAPFILE_ASSUME_HASH_KEYS`` :index:`CERTIFICATE_MAPFILE_ASSUME_HASH_KEYS`
...
``KERBEROS_SERVER_KEYTAB`` :index:`KERBEROS_SERVER_KEYTAB`
...
``KERBEROS_SERVER_PRINCIPAL`` :index:`KERBEROS_SERVER_PRINCIPAL`
...
``VM_STATUS_INTERVAL`` :index:`VM_STATUS_INTERVAL`
...
``VM_GAHP_REQ_TIMEOUT`` :index:`VM_GAHP_REQ_TIMEOUT`
...
``VM_RECHECK_INTERVAL`` :index:`VM_RECHECK_INTERVAL`
...
``VM_SOFT_SUSPEND`` :index:`VM_SOFT_SUSPEND`
...
``VM_UNIV_NOBODY_USER`` :index:`VM_UNIV_NOBODY_USER`
...
``ALWAYS_VM_UNIV_USE_NOBODY`` :index:`ALWAYS_VM_UNIV_USE_NOBODY`
...
``VM_NETWORKING_TYPE`` :index:`VM_NETWORKING_TYPE`
 ...
 ``VM_NETWORKING_BRIDGE_INTERFACE`` :index:`VM_NETWORKING_BRIDGE_INTERFACE`
...
``LIBVIRT_XML_SCRIPT`` :index:`LIBVIRT_XML_SCRIPT`
...
``LIBVIRT_XML_SCRIPT_ARGS`` :index:`LIBVIRT_XML_SCRIPT_ARGS`
...
``VMWARE_NAT_NETWORKING_TYPE`` :index:`VMWARE_NAT_NETWORKING_TYPE`
    An optional string used in networking that the *condor_vm-gahp*
    inserts into the VMware configuration file to define a networking
    type. If nat networking is used, this variable’s definition takes
    precedence over one defined by ``VMWARE_NETWORKING_TYPE``.

``VMWARE_BRIDGE_NETWORKING_TYPE`` :index:`VMWARE_BRIDGE_NETWORKING_TYPE`
    An optional string used in networking that the *condor_vm-gahp*
    inserts into the VMware configuration file to define a networking
    type. If bridge networking is used, this variable’s definition takes
    precedence over one defined by ``VMWARE_NETWORKING_TYPE``.

``VMWARE_LOCAL_SETTINGS_FILE`` :index:`VMWARE_LOCAL_SETTINGS_FILE`
...
machine software.
 ...
 ``XEN_BOOTLOADER`` :index:`XEN_BOOTLOADER`
...
universe.
 ...
 ``VMP_HOST_MACHINE`` :index:`VMP_HOST_MACHINE`
    A configuration variable for the inner virtual machine, which
    specifies the host name.

``VMP_VM_LIST`` :index:`VMP_VM_LIST`
...
These macros affect the high availability operation of HTCondor.
 ...
 ``MASTER_HA_LIST`` :index:`MASTER_HA_LIST`
 ...
 ``HA_<SUBSYS>_LOCK_URL`` :index:`HA_<SUBSYS>_LOCK_URL`
...
``HA_LOCK_HOLD_TIME`` :index:`HA_LOCK_HOLD_TIME`
 ...
 ``HA_<SUBSYS>_LOCK_HOLD_TIME`` :index:`HA_<SUBSYS>_LOCK_HOLD_TIME`
...
``HA_POLL_PERIOD`` :index:`HA_POLL_PERIOD`
 ...
 ``HA_<SUBSYS>_POLL_PERIOD`` :index:`HA_<SUBSYS>_POLL_PERIOD`
...
``MASTER_<SUBSYS>_CONTROLLER`` :index:`MASTER_<SUBSYS>_CONTROLLER`
 ...
 ``HAD_USE_PRIMARY`` :index:`HAD_USE_PRIMARY`
...
``HAD_CONTROLLEE`` :index:`HAD_CONTROLLEE`
...
``HAD_CONNECTION_TIMEOUT`` :index:`HAD_CONNECTION_TIMEOUT`
...
``MAX_TRANSFER_LIFETIME`` :index:`MAX_TRANSFER_LIFETIME`
...
``HAD_UPDATE_INTERVAL`` :index:`HAD_UPDATE_INTERVAL`
...
``MAX_REPLICATION_LOG`` :index:`MAX_REPLICATION_LOG`
    Controls the maximum length in bytes to which the
...
*condor_ssh_to_job* is at
section \ `12 <Condorsshtojob.html#x144-103900012>`__.

 ``ENABLE_SSH_TO_JOB`` :index:`ENABLE_SSH_TO_JOB`
...
``SCHEDD_ENABLE_SSH_TO_JOB`` :index:`SCHEDD_ENABLE_SSH_TO_JOB`
...
``SSH_TO_JOB_<SSH-CLIENT>_CMD`` :index:`SSH_TO_JOB_<SSH-CLIENT>_CMD`
...
``SSH_TO_JOB_SSHD_CONFIG_TEMPLATE`` :index:`SSH_TO_JOB_SSHD_CONFIG_TEMPLATE`
...
``SSH_TO_JOB_SSH_KEYGEN`` :index:`SSH_TO_JOB_SSH_KEYGEN`
    A string, read by the *condor_starter* that specifies the path to
    *ssh_keygen*, the program used to create ssh keys.

``SSH_TO_JOB_SSH_KEYGEN_ARGS`` :index:`SSH_TO_JOB_SSH_KEYGEN_ARGS`
    A string, read by the *condor_starter* that specifies the
...
:index:`COLLECTOR_PERSISTENT_AD_LOG` on
page \ `738 <#x33-2010003.5.14>`__ for details on how to do this.
...
``ROOSTER_UNHIBERNATE`` :index:`ROOSTER_UNHIBERNATE`
...
``ROOSTER_UNHIBERNATE_RANK`` :index:`ROOSTER_UNHIBERNATE_RANK`
...
``ROOSTER_WAKEUP_CMD`` :index:`ROOSTER_WAKEUP_CMD`
...
see \ `1094 <NetworkingincludessectionsonPortUsageandCCB.html#x37-3090003.9.2>`__.
...
``SHARED_PORT_PORT`` :index:`SHARED_PORT_PORT`
 ...
 ``SHARED_PORT_DAEMON_AD_FILE`` :index:`SHARED_PORT_DAEMON_AD_FILE`
...
``SHARED_PORT_MAX_WORKERS`` :index:`SHARED_PORT_MAX_WORKERS`
...
``DAEMON_SOCKET_DIR`` :index:`DAEMON_SOCKET_DIR`
 ...
 ``SHARED_PORT_ARGS`` :index:`SHARED_PORT_ARGS`
...
page \ `1372 <Hooks.html#x51-4380004.4.1>`__ on Job Hooks for more
...
page \ `1504 <TheHTCondorJobRouter.html#x57-4910005.4>`__.

 ``SLOT<N>_JOB_HOOK_KEYWORD`` :index:`SLOT<N>_JOB_HOOK_KEYWORD`
...
``STARTD_JOB_HOOK_KEYWORD`` :index:`STARTD_JOB_HOOK_KEYWORD`
    For the fetch work hooks, the keyword used to define which set of
...
``<Keyword>_HOOK_FETCH_WORK`` :index:`<Keyword>_HOOK_FETCH_WORK`
...
``<Keyword>_HOOK_REPLY_FETCH`` :index:`<Keyword>_HOOK_REPLY_FETCH`
    For the fetch work hooks, the full path to the program to invoke
...
``<Keyword>_HOOK_REPLY_CLAIM`` :index:`<Keyword>_HOOK_REPLY_CLAIM`
    For the fetch work hooks, the full path to the program to invoke
 ...
 ``<Keyword>_HOOK_PREPARE_JOB`` :index:`<Keyword>_HOOK_PREPARE_JOB`
...
``<Keyword>_HOOK_UPDATE_JOB_INFO`` :index:`<Keyword>_HOOK_UPDATE_JOB_INFO`
    ...
    *condor_starter* to present an updated and augmented job ClassAd to
    ...
    page \ `1373 <Hooks.html#x51-4390004.4.1>`__ for the list of
    additional attributes included. When the job is first invoked, the
    *condor_starter* will invoke the program after
 ...
 ``<Keyword>_HOOK_EVICT_CLAIM`` :index:`<Keyword>_HOOK_EVICT_CLAIM`
    For the fetch work hooks, the full path to the program to invoke
...
``<Keyword>_HOOK_JOB_EXIT`` :index:`<Keyword>_HOOK_JOB_EXIT`
    For the fetch work hooks, the full path to the program invoked by
...
``<Keyword>_HOOK_JOB_EXIT_TIMEOUT`` :index:`<Keyword>_HOOK_JOB_EXIT_TIMEOUT`
...
``FetchWorkDelay`` :index:`FetchWorkDelay`
    ...
    *condor_startd* should wait after an invocation of
    ...
    page \ `1379 <Hooks.html#x51-4410004.4.1>`__.

``JOB_ROUTER_HOOK_KEYWORD`` :index:`JOB_ROUTER_HOOK_KEYWORD`
...
``<Keyword>_HOOK_TRANSLATE_JOB`` :index:`<Keyword>_HOOK_TRANSLATE_JOB`
...
``<Keyword>_HOOK_JOB_FINALIZE`` :index:`<Keyword>_HOOK_JOB_FINALIZE`
    A Job Router hook, the full path to the program invoked when the Job
...
``<Keyword>_HOOK_JOB_CLEANUP`` :index:`<Keyword>_HOOK_JOB_CLEANUP`
    A Job Router hook, the full path to the program invoked when the Job
...
page \ `1391 <Hooks.html#x51-4450004.4.3>`__.

``STARTD_CRON_NAME`` :index:`STARTD_CRON_NAME` and ``SCHEDD_CRON_NAME`` :index:`SCHEDD_CRON_NAME`
...
``STARTD_CRON_CONFIG_VAL`` :index:`STARTD_CRON_CONFIG_VAL` and ``SCHEDD_CRON_CONFIG_VAL`` :index:`SCHEDD_CRON_CONFIG_VAL` and ``BENCHMARKS_CONFIG_VAL`` :index:`BENCHMARKS_CONFIG_VAL`
...
``STARTD_CRON_AUTOPUBLISH`` :index:`STARTD_CRON_AUTOPUBLISH`
 ...
 ``STARTD_CRON_JOBLIST`` :index:`STARTD_CRON_JOBLIST` and ``SCHEDD_CRON_JOBLIST`` :index:`SCHEDD_CRON_JOBLIST` and ``BENCHMARKS_JOBLIST`` :index:`BENCHMARKS_JOBLIST`
...
``STARTD_CRON_<JobName>_PREFIX`` :index:`STARTD_CRON_<JobName>_PREFIX` and ``SCHEDD_CRON_<JobName>_PREFIX`` :index:`SCHEDD_CRON_<JobName>_PREFIX` and ``BENCHMARKS_<JobName>_PREFIX`` :index:`BENCHMARKS_<JobName>_PREFIX`
 ...
 ``STARTD_CRON_<JobName>_EXECUTABLE`` :index:`STARTD_CRON_<JobName>_EXECUTABLE` and ``SCHEDD_CRON_<JobName>_EXECUTABLE`` :index:`SCHEDD_CRON_<JobName>_EXECUTABLE` and ``BENCHMARKS_<JobName>_EXECUTABLE`` :index:`BENCHMARKS_<JobName>_EXECUTABLE`
 ...
 ``STARTD_CRON_<JobName>_PERIOD`` :index:`STARTD_CRON_<JobName>_PERIOD` and ``SCHEDD_CRON_<JobName>_PERIOD`` :index:`SCHEDD_CRON_<JobName>_PERIOD` and ``BENCHMARKS_<JobName>_PERIOD`` :index:`BENCHMARKS_<JobName>_PERIOD`
 ...
 ``STARTD_CRON_<JobName>_METRICS`` :index:`STARTD_CRON_<JobName>_METRICS`
 ...
 ``STARTD_CRON_<JobName>_RECONFIG`` :index:`STARTD_CRON_<JobName>_RECONFIG` and ``SCHEDD_CRON_<JobName>_RECONFIG`` :index:`SCHEDD_CRON_<JobName>_RECONFIG`
 ...
 ``STARTD_CRON_<JobName>_RECONFIG_RERUN`` :index:`STARTD_CRON_<JobName>_RECONFIG_RERUN` and ``SCHEDD_CRON_<JobName>_RECONFIG_RERUN`` :index:`SCHEDD_CRON_<JobName>_RECONFIG_RERUN`
 ...
 ``STARTD_CRON_<JobName>_JOB_LOAD`` :index:`STARTD_CRON_<JobName>_JOB_LOAD` and ``SCHEDD_CRON_<JobName>_JOB_LOAD`` :index:`SCHEDD_CRON_<JobName>_JOB_LOAD` and ``BENCHMARKS_<JobName>_JOB_LOAD`` :index:`BENCHMARKS_<JobName>_JOB_LOAD`
...
``STARTD_CRON_<JobName>_KILL`` :index:`STARTD_CRON_<JobName>_KILL` and ``SCHEDD_CRON_<JobName>_KILL`` :index:`SCHEDD_CRON_<JobName>_KILL` and ``BENCHMARKS_<JobName>_KILL`` :index:`BENCHMARKS_<JobName>_KILL`
 ...
 ``STARTD_CRON_<JobName>_ARGS`` :index:`STARTD_CRON_<JobName>_ARGS` and ``SCHEDD_CRON_<JobName>_ARGS`` :index:`SCHEDD_CRON_<JobName>_ARGS` and ``BENCHMARKS_<JobName>_ARGS`` :index:`BENCHMARKS_<JobName>_ARGS`
 ...
 ``STARTD_CRON_<JobName>_ENV`` :index:`STARTD_CRON_<JobName>_ENV` and ``SCHEDD_CRON_<JobName>_ENV`` :index:`SCHEDD_CRON_<JobName>_ENV` and ``BENCHMARKS_<JobName>_ENV`` :index:`BENCHMARKS_<JobName>_ENV`
 ...
 ``STARTD_CRON_<JobName>_CWD`` :index:`STARTD_CRON_<JobName>_CWD` and ``SCHEDD_CRON_<JobName>_CWD`` :index:`SCHEDD_CRON_<JobName>_CWD` and ``BENCHMARKS_<JobName>_CWD`` :index:`BENCHMARKS_<JobName>_CWD`
...
These macros are utilized only on Windows platforms.
 ...
 ``WINDOWS_RMDIR`` :index:`WINDOWS_RMDIR`
...
``WINDOWS_RMDIR_OPTIONS`` :index:`WINDOWS_RMDIR_OPTIONS`
...
``DEFRAG_DRAINING_MACHINES_PER_HOUR`` :index:`DEFRAG_DRAINING_MACHINES_PER_HOUR`
...
``DEFRAG_DRAINING_START_EXPR`` :index:`DEFRAG_DRAINING_START_EXPR`
...
``DEFRAG_REQUIREMENTS`` :index:`DEFRAG_REQUIREMENTS`
 ...
 ``DEFRAG_CANCEL_REQUIREMENTS`` :index:`DEFRAG_CANCEL_REQUIREMENTS`
...
``DEFRAG_WHOLE_MACHINE_EXPR`` :index:`DEFRAG_WHOLE_MACHINE_EXPR`
 ...
 ``DEFRAG_MAX_WHOLE_MACHINES`` :index:`DEFRAG_MAX_WHOLE_MACHINES`
...
``DEFRAG_MAX_CONCURRENT_DRAINING`` :index:`DEFRAG_MAX_CONCURRENT_DRAINING`
...
``DEFRAG_INTERVAL`` :index:`DEFRAG_INTERVAL`
...
``DEFRAG_UPDATE_INTERVAL`` :index:`DEFRAG_UPDATE_INTERVAL`
...
``DEFRAG_SCHEDULE`` :index:`DEFRAG_SCHEDULE`
 ...
 ``DEFRAG_STATE_FILE`` :index:`DEFRAG_STATE_FILE`
...
has.
 ...
 ``GANGLIAD_INTERVAL`` :index:`GANGLIAD_INTERVAL`
...
``GANGLIAD_VERBOSITY`` :index:`GANGLIAD_VERBOSITY`
...
``GANGLIAD_REQUIREMENTS`` :index:`GANGLIAD_REQUIREMENTS`
...
``GANGLIAD_PER_EXECUTE_NODE_METRICS`` :index:`GANGLIAD_PER_EXECUTE_NODE_METRICS`
...
``GANGLIA_GSTAT_COMMAND`` :index:`GANGLIA_GSTAT_COMMAND`
...
:index:`GANGLIA_SEND_DATA_FOR_ALL_HOSTS`
...
``GANGLIA_LIB_PATH`` :index:`GANGLIA_LIB_PATH`
...
``GANGLIA_LIB64_PATH`` :index:`GANGLIA_LIB64_PATH`
    A comma-separated list of directories within which to search for the
    ``libganglia`` executable, if ``GANGLIA_LIB``
    :index:`GANGLIA_LIB` is not configured. This is used in 64-bit
    versions of HTCondor.

``GANGLIAD_DEFAULT_CLUSTER`` :index:`GANGLIAD_DEFAULT_CLUSTER`
...
``GANGLIAD_DEFAULT_MACHINE`` :index:`GANGLIAD_DEFAULT_MACHINE`
