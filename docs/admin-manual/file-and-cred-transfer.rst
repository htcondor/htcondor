...
Note that in both ``scitokens-admin-create-key`` examples,
``--ec`` is passed to indicate that the elliptic curve encryption
algorithm (i.e. ES256) is to be used when generating the keys.
If the consumer of the SciTokens only accepts RSA-based keys,
the ``--ec`` flag can be omitted, however the local issuer credmon
must be configured to use the RSA algorithm (i.e. RS256) by setting
``LOCAL_CREDMON_PRIVATE_KEY_ALGORITHM = RS256``.
...
the user manual.  This section of the manual explains a bit about the
(26.0.x) implementation and then the relevant configuration options.
...
.. info:
    ...
    This section is **not** normative; the implementation is subject to
    change.  Certain properties of the described system may be visible to
    the submitter or the administrator, but are not documented because
    we do *not* promise that they won't change.
