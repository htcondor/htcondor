...
for %%e in ( %EXTERNALS_NEEDED% ) do ( if not exist %EXT_TRIGGERS%\%%e ( perl -w %EXTERN_DIR%\build_win32_external --extern_dir=%EXTERN_DIR% --package_name=%%e --extern_config=%cd% || exit /b 1 ) else echo The %%e external up to date. Done. )
