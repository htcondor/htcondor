...
if(UNIX)
    execute_process( COMMAND date "+%Y%m%d_%H%M%S" OUTPUT_VARIABLE BUILD_DATE OUTPUT_STRIP_TRAILING_WHITESPACE )
else()
    ...
    if( ${_date} AND ${_time} )
        ...
        set( BUILD_DATE "${_date}_${HOUR}${CMAKE_MATCH_2}" )
    else()
        set( BUILD_DATE "" )
    endif()
endif()
...
if(PRE_RELEASE)
    if(BUILDID)
        set (CONDOR_PACKAGE_NAME "${PACKAGE_NAME}-${PACKAGE_VERSION}-${BUILDID}")
    elseif(BUILD_DATE)
        set (CONDOR_PACKAGE_NAME "${PACKAGE_NAME}-${PACKAGE_VERSION}-${BUILD_DATE}")
    else()
        string( REGEX REPLACE "[: /$+]+" "_" PRE_RELEASE_STRIP "${PRE_RELEASE}" )
        string( REGEX REPLACE "_+$" "" PRE_RELEASE_STRIP "${PRE_RELEASE_STRIP}" )
        set (CONDOR_PACKAGE_NAME "${PACKAGE_NAME}-${PACKAGE_VERSION}-${PRE_RELEASE_STRIP}")
    endif(BUILDID)
else()
