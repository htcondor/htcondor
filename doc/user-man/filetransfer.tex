...
\textbf{Uploading to URLs using output file remaps}

File transfer plugins now support uploads as well as downloads. 
The \Attr{transfer\_output\_remaps} attribute can additionally be used to upload 
...
\textbf{Passing a credential for URL file transfers}

Some files served over HTTPS will require a credential in order to download.
Each credential \Expr{cred} should be placed in a file in 
\Expr{\$\_CONDOR\_CREDS/cred.use}. Then in order to use that credential for 
a download, append its name to the beginning of the URL protocol along with a + 
symbol. For example, to download the file 
https://download.com/bar using the \Expr{cred} credential, specify the 
