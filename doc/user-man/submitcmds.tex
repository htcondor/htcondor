...
This expression is available for the vanilla, java, parallel, mpi, grid,
...
for an example of a \AdAttr{periodic\_*} expression.
...
\SubmitCmd{amazon}, \SubmitCmd{gt2}, \SubmitCmd{gt4}, \SubmitCmd{condor}, 
...
For Windows platform jobs only,
a boolean value that causes a correctly configured Windows platform
machine to execute the job under the login of the submitter.
May not be used with \SubmitCmd{load\_profile}.
See section~\ref{sec:windows-run-as-owner} for administrative details.
