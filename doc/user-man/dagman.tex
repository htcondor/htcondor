...
in other words when a node within a DAG is also a DAG.
Condor DAGMan handles this situation quite easily.
(Note that DAGs can be nested to any depth.)
...
and the diamond-shaped DAG is called the outer (or top-level) DAG.
...
The Condor submit file corresponding to this DAG will be named
\File{inner.dag.condor.sub}.  (The DAGMan submit file is always
...
node, where the job is a DAG.  The syntax used for each \Arg{SUBDAG}
entry is

\Opt{SUBDAG} \Opt{EXTERNAL} \Arg{JobName} \Arg{DagFileName}
\oOptArg{DIR}{directory} \oOpt{NOOP} \oOpt{DONE}

(Note that \Opt{DIR}, \Opt{NOOP}, and \Opt{DONE}, if used, must appear
in the order shown above.)

A \Opt{SUBDAG} node is essentially the same as a "normal" node, except
that the nested DAG file is specified instead of the Condor submit file.
("SUBDAG EXTERNAL A foo.dag" is functionally equivalent to
"JOB A foo.dag.condor.sub", but SUBDAG EXTERNAL is now the preferred
syntax for specifying such a node.)  "EXTERNAL" means that the
SUBDAG is run in its own instance of \Condor{dagman}.

\Opt{NOOP} and \Opt{DONE} for \Opt{SUBDAG} nodes have the same effect
that they do for \Opt{JOB} nodes.

\begin{itemize}
\item{Nested submit file generation}

There are three ways that you can generate the \File{.condor.sub} files
for nested DAGs:

\begin{itemize}
\item Lazily (the default in 7.5.2 and later versions).
\item Eagerly (the default in versions 7.4.1 through 7.5.1).
\item Manually (the only way prior to version 7.4.1).
\end{itemize}

When the \File{.condor.sub} files are generated \Bold{lazily}, the
\File{.condor.sub} file for each nested DAG is generated immediately
before the nested DAG job is submitted (\Condor{dagman} runs
\Condor{submit\_dag} \Arg{-no\_submit} on the nested DAG file).  This
is now the default.
There are several advantages to the "lazy" mode of nested DAG submit
file creation:
\begin{itemize}
\item The DAG file for a nested DAG does not have to exist until that node
is ready to run, so the nested DAG file can be dynamically created by earlier
parts of the top-level DAG (including the PRE script of the nested
DAG node itself).
\item It is now possible to have nested DAGs within splices, which is not
possible with "eager" submit file creation (because \Condor{submit\_dag}
does not understand splices).
\end{itemize}

The main disadvantage of lazy submit file generation is that if you have
a syntax error in one of your lower-level DAG files, you won't find out
about it until the upper-level DAG tries to run that lower-level DAG.

When the \File{.condor.sub} files are generated \Bold{eagerly},
\Condor{submit\_dag} runs itself recursively (with the \Arg{-no\_submit}
option) on each nested DAG, so all of the \File{.condor.sub} files
are generated before the top-level DAG is actually submitted.  To
generate the \File{.condor.sub} files eagerly, you must now pass the
\Arg{-do\_recurse} flag to \Condor{submit\_dag}; you must also set
the \MacroNI{DAGMAN\_GENERATE\_SUBDAG\_SUBMITS} configuration variable
to \Expr{false} so that \Condor{dagman} does not re-run
\Condor{submit\_dag} at run time (if, in fact, you don't want to regenerate
the submit files).

To generate the \File{.condor.sub} files \Bold{manually}, you
must run \Condor{submit\_dag} \Arg{-no\_submit} on each lower-level
DAG file before running \Condor{submit\_dag} on the top-level DAG file;
you must also set the \MacroNI{DAGMAN\_GENERATE\_SUBDAG\_SUBMITS}
configuration variable to \Expr{false} so that \Condor{dagman} does
not re-run \Condor{submit\_dag} at run time.  The main reason for
generating the \File{.condor.sub} files manually is to set options
for the lower-level DAG that you would not otherwise be able to set
(for example, \Arg{-insert\_sub\_file}).  For instance,
...
Also note that you can combine more than one method of generating the
\File{.condor.sub} files -- for example, you might pass the
\Arg{-do\_recurse} flag to \Condor{submit\_dag}, but leave the
\MacroNI{DAGMAN\_GENERATE\_SUBDAG\_SUBMITS} configuration variable set
to the default of \Expr{true}.  Doing this would give you the benefit
of an immediate error message at submit time if you have a syntax error
in one of your nested DAG files, but the lower-level \File{.condor.sub}
...
top-level \Condor{submit\_dag} to any lower-level \Condor{submit\_dag}s
(whether the lower-level submit files are generated lazily or eagerly):
\begin{itemize}
\item \Opt{-verbose}
\item \Opt{-force}
...
already-existing lower-level DAG submit files:
\begin{itemize}
\item \Opt{-maxjobs}
\item \Opt{-maxidle}
\item \Opt{-maxpre}
...
\Condor{submit\_dag}s.

Note that the \Opt{-force} option will cause existing DAG submit files to
be overwritten without preserving any existing values.

\item{Submission of the outer (top-level) DAG}
...
When using nested DAGs, it is strongly recommended that you use
"new-style" rescue DAGs (this is the default).  Using "new-style"
rescue DAGs will automatically run the proper rescue DAG(s) if
there is a failure in your workflow.  For example, if one of the
nodes in \File{inner.dag} fails, this will produce a rescue
DAG for inner.dag (\File{inner.dag.rescue.001}, etc.).  Then,
since \File{inner.dag} failed, node B of \File{diamond.dag} will fail,
producing a rescue DAG for \File{diamond.dag}
(\File{diamond.dag.rescue.001}, etc.).  If you re-run
\emph{condor\_submit\_dag diamond.dag} the most recent outer rescue
DAG will be run, and this will re-run the inner DAG, which will
actually run the most recent inner rescue DAG.  If you use
"old-style" rescue DAGs, you would have to either rename the 
inner rescue DAG or run it manually.
