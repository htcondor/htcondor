...
of previous nodes.  (There is one case where a final node is not run --
if \Macro{DAGMAN\_STARTUP\_CYCLE\_DETECT} is set to \Expr{True}, and
a cycle is detected at startup time, the final node is not run.  However,
if \Macro{DAGMAN\_STARTUP\_CYCLE\_DETECT} is set to \Expr{False} and
a cycle is detected during the course of the run, the final node
will still be run.)
...
in determining the success or failure of the DAG.  For example, if 
some nodes of a DAG fail, but the final node succeeds, the DAG will
be considered successful.  So if you have a final node, it's important
that you're careful about whether it succeeds or fails -- you'll
probably want to use the \Env{\$DAG\_STATUS} and \Env{\$FAILED\_COUNT}
macros described below in your final node.
...
Two special macros have been introduced for use by final nodes (they
can also be used by "regular" nodes):  \Env{\$DAG\_STATUS} and
\Env{\$FAILED\_COUNT}.
...
\begin{itemize}
\item 0: OK
\item 1: error (an error condition not enumerated below)
\item 2: node failed (one or more nodes in the DAG have failed)
\item 3: aborted (the DAG has been aborted by an ABORT-DAG-ON specification)
\item 4: removed (the DAG has been \Condor{rm}'ed)
\item 5: cycle (a cycle has been found in the DAG)
\item 6: halted (the DAG has been halted (see section ~\ref{sec:DagSuspend}))
\end{itemize}

\index{DAGMan!FAILED_COUNT@\verb^$FAILED_COUNT^ value}
\Env{\$FAILED\_COUNT} is simply the number of nodes that have failed in the
DAG.

The \Env{\$DAG\_STATUS} and \Env{\$FAILED\_COUNT} macros can be used both
as PRE and POST script arguments, and in node job submit files.
For example:
...
If you have a final node in your DAG, it will \emph{always} be run at
the end of the workflow (except in the "cycle detected at startup" case
documented above).  If you don't want the final node to do anything in
certain cases, use the \Env{\$DAG\_STATUS} and \Env{\$FAILED\_COUNT}
macros to take appropriate actions.  In the example shown below, the PRE
script aborts fails if the DAG has been condor\_rm'ed, which causes the
final node to be considered failed without actually submitting the
Condor job of the node.
...
The following options that can be used with "regular" nodes are not
allowed for FINAL nodes:
\begin{itemize}
\item DONE
\item PARENT, CHILD
\item RETRY
...
Note: this feature was introduced in version 7.7.5.
