...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:DAGJobstateLog}The jobstate.log file}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The \File{jobstate.log} file might also be useful to anyone else
who wants to build an automated tool to monitor workflows.

The syntax for a \Arg{JOBSTATE\_LOG} specification is

\Opt{JOBSTATE\_LOG} \Arg{JobstateLogFileName}

No more than one \File{jobstate.log} file can be created by a single
instance of DAGMan.  If more than one \File{jobstate.log} file is
specified (via multiple specifications in the same DAG file,
specifications in splices, or specifications in multiple independent
DAGs run with a single DAGMan) the first file name specified will take
effect, and a warning will be printed in the \File{dagman.out} file
when subsequent \Opt{JOBSTATE\_LOG} specifications are parsed.

The \File{jobstate.log} file can basically be considered a filtered
version of the \File{dagman.out} file, in a machine-readable format.
It contains the actual node job events that \Condor{dagman} reads,
plus some additional meta-events.

The \File{jobstate.log} file is different from the node status file
in that it is appended to, rather than being overwritten, as the
DAG runs.  Therefore, it contains a history of the DAG run, rather
than a snapshot of the current state of the DAG.

The format of the lines in the \File{jobstate.log} file is as follows
(the timestamp is in the Unix form of seconds since the Epoch, for
easy machine-readability):
\begin{itemize}
\item Normal: \Arg{timestamp} \Arg{nodeName} \Arg{eventName}
\Arg{condorID} \Arg{jobTag} - \Arg{sequenceNumber}
\item DAGMan start: \Arg{timestamp} INTERNAL *** DAGMAN\_STARTED
\Arg{dagmanCondorID} ***
\item DAGMan exit: \Arg{timestamp} INTERNAL *** DAGMAN\_FINISHED
\Arg{exitCode} ***
\item Recovery started: \Arg{timestamp} INTERNAL *** RECOVERY\_STARTED ***
\item Recovery finished: \Arg{timestamp} INTERNAL *** RECOVERY\_FINISHED ***
\end{itemize}

The \Arg{jobTag} value is obtained from the node job's submit file (if
no jobTag value is specified, the value will default to "-").
The jobTag value is specified in a classad attribute specified by the
value of the job\_tag\_name classad attribute, like this:
...
If no job\_tag\_name value is specified, the value defaults to
"+pegasus\_site", like this:

\begin{verbatim}
+pegasus_site = "local"
\end{verbatim}

Here's an example of a very simple \File{jobstate.log} file:
...
If DAGMan goes into recovery mode, a RECOVERY\_STARTED meta-event will
be printed to the \File{jobstate.log} file.  During recovery mode, events
will only be printed to the \File{jobstate.log} file if they were not
already printed (before recovery mode started).  At the end of recovery
mode, a RECOVERY\_FINISHED or RECOVERY\_FAILURE meta-event will be
printed, as appropriate.

The sequence number is a monotonically-increasing number, starting
at one, that is associated with each attempt at running a node.
(If a node is retried, it gets a new sequence number; a submit failure
does not result in a new sequence number.)  When a rescue DAG is run,
the sequence numbers pick up from where they left off in the previous
attempt at running the DAG (note that this only applies if the rescue
DAG is run automatically or with the \Arg{-dorescuefrom} command-line
flag).

Node job events read in recovery mode will only be printed if they were
not previously printed (before recovery mode started).

Following is a list of all possible "real" events and meta-events that
can be found in the \File{jobstate.log} file.
