...
Another new feature in version 7.3.2 is default node job user logs.
In the past it was a fatal error if the submit file for a node job did
not specify a log file.  However, starting with version 7.3.2,
DAGMan specifies a default user log file for the job in this case.
The file used as the default node log is controlled by the
\MacroNI{DAGMAN\_DEFAULT\_NODE\_LOG} configuration variable (see 
section~\ref{param:DAGManDefaultNodeLog}).
