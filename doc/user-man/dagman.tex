...
As of version 8.3.7, it is possible to have single quotes within a VARS
value.  When used in arguments, single quotes can be used in two different
ways:
\begin{itemize}
\item Within an argument (in both Old and New argument syntax)
\item To delimit an argument containing whitespace (in New argument
syntax only)
\end{itemize}
There are examples of all three cases below.  Note that in New argument
syntax, to pass a single quote within an argument, it must be escaped
(for \Condor{submit} parsing) with another single quote.
