...
The diamond-shaped DAG example specifies the dependencies with
\begin{verbatim}
PARENT A CHILD B C
PARENT B C CHILD D
\end{verbatim}
...
The \Arg{SCRIPT} keyword specifies
processing that is done either before a job within
a node is submitted to HTCondor or Stork for execution
or after
a job within
a node completes its execution.
...
The \Arg{RETRY} keyword provides a
...
optional keyword \Arg{UNLESS-EXIT} (followed by an
integer exit value).
...
\index{VARS}
The \Arg{VARS} keyword provides a
...
The \Arg{FINAL} keyword specifies a job to be run at the end of
...
The keywords \Arg{DIR} and \Arg{NOOP} are not case sensitive.
