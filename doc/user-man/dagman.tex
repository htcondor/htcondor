...
#TEMPTEMP -- this isn't so true anymore, since priorities are propagated to the actual condor priority for the job
...
#TEMPTEMP -- need note about priority not being passed to condor_submit if not explicitly set
#TEMPTEMP -- although if overall DAG priority overrides non-set node priority, that overrides submit file prio
#TEMPTEMP -- note that if a node's priority is not explicitly set, that does not override a child node's priority

#TEMPTEMP -- need to add something here about the priority of the DAG itself,e specially for sub-DAGs
