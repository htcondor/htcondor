...
As of version 7.3.2, DAGMan's handling of log files has
significantly changed.  Prior to version 7.3.2, DAGMan has assembled
a list of all relevant log files at startup (by looking at all of the
node job submit files).  DAGMan then opened \emph{all} of the log files
and kept them all open for the duration of the DAG.

In version 7.3.2 and later, DAGMan doesn't even look at the node job
submit file at all until right before it's going to submit the job --
at that point it reads the submit file to find out what the job's log
file is.  Furthermore, DAGMan only monitors log files that are relevant
to the jobs in the queue at the current time, or nodes for which a
POST script is running.

There are several advantages to the new "lazy log file evaluation"
scheme:
...
\item It is now much easier to have one node of a DAG write the
submit file for a later node in the DAG.

\end{itemize}

There are two known disadvantages at this time:

\begin{itemize}

\item If a submit file is missing a log file specification, the
DAG will still fail, but the missing log file will not be noticed
at the time the DAG is submitted.

\item Because the log files are internally identified by inode
numbers, it is possible that errors could arise if log files for
a given DAG are spread across more than one device (that could
allow two unique files to have the same inode number).  We hope
