...
%TEMPTEMP -- add a section explaining ALL_NODES -- doesn't work with
% multiple DAGs on command line, etc.; how it interacts w/ splices;
% how it interacts w/ INCLUDE; especially useful in splices
%TEMPTEMP -- add ALL_NODES examples
%TEMPTEMP -- VARS A.B ... will no longer work
...
\Opt{PRE} \Arg{JobName}|ALL\_NODES \Arg{ExecutableName} \oArg{arguments}

\Opt{SCRIPT} \oOptArg{DEFER}{status time}
\Opt{POST}  \Arg{JobName}|ALL\_NODES \Arg{ExecutableName} \oArg{arguments}
...
\Opt{RETRY} \Arg{JobName}|ALL\_NODES \Arg{NumberOfRetries} \oOptArg{UNLESS-EXIT}{value}
...
\Opt{ABORT-DAG-ON} \Arg{JobName}|ALL\_NODES \Arg{AbortExitValue}
...
\Opt{VARS} \Arg{JobName}|ALL\_NODES \Arg{macroname=}\Arg{"string"} [\Arg{macroname=}\Arg{"string"\Dots}]
...
\Opt{PRIORITY} \Arg{JobName}|ALL\_NODES \Arg{PriorityValue}
...
\Opt{CATEGORY} \Arg{JobName}|ALL\_NODES \Arg{CategoryName}
...
\end{description}
%TEMPTEMP -- RETRY B+X doesn't work; ALL_NODES changes will break
% VARS B+X
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:DAGFinalNode}ALL\_NODES option}
\index{DAG input file!ALL\_NODES option}

%TEMPTEMP -- format ALL_NODES
In the following commands, a specific node name can be replaced by
the option "ALL\_NODES":
%TEMPTEMP -- add PRE_SKIP here if we implement that
\begin{itemize}
\item SCRIPT
\item RETRY
\item ABORT-DAG-ON
...
TEMPTEMP -- list commands it applies to
TEMPTEMP -- case insensitive
TEMPTEMP -- does not apply to final nodes?
TEMPTEMP -- explain how it works with splices
TEMPTEMP -- give examples
