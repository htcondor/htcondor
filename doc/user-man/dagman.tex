...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:DAG-node-status}Node status file}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{DAGMan!node status file}
\index{node status}

In order to be able to easily monitor the status of all DAG nodes, it
is possible to specify a node status file that is periodically updated
by DAGMan.
...
MinimumUpdateTime is the minimum number of seconds that must elapse
between updates to the node status file.  This setting can be used
to avoid having DAGMan spend too much time writing the node status
file if you have a large DAG.  If no value is specified, no limit
is set.  However, the node status file can be updated at most once
per DAGMAN\_USER\_LOG\_SCAN\_INTERVAL (see
section~\ref{param:DAGManUserLogScanInterval}), no matter how small
...
the file \File{my.dag.status} will be updated with the DAG node status at
intervals of 30 seconds or more.

Node status file specifications inside splices are ignored.  If you
specify multiple DAG files on the \Condor{submit\_dag} command line,
and more than one specifies a node status file, the first specification
takes precedence.
...
It is important to note that the node status file is overwritten
each time it is updated.  Therefore, the node status file only holds
the information about the \emph{current} status of each node; it
does not provide a history of the node status.

Possible node status values are as follows:

\begin{itemize}
\item STATUS\_UNREADY (at least one parent has not yet finished)
\item STATUS\_READY (all parents have finished, but not yet running)
\item STATUS\_PRERUN (pre script is running)
\item STATUS\_SUBMITTED (the node's Condor job(s) are in the queue)
\item STATUS\_POSTRUN (post script is running)
\item STATUS\_DONE (the node has completed successfully)
\item STATUS\_ERROR (the node has failed)
\end{itemize}
