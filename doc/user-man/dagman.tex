...
As of version 8.1.0, \Condor{dagman} has the capability to report
workflow metrics to one or more HTTP servers.  This capability is
currently only used for workflows run under Pegasus.  The reporting can be
disabled by setting the \Macro{CONDOR\_DEVELOPERS} configuration
macro to \Expr{NONE} or by setting the \Macro{PEGASUS\_METRICS} environment
variable to any value other than \Expr{True} (case-insenitive) or 1.
The \File{dagman.out} file will indicate whether or not metrics were
reported.

For every DAG run, a metrics file is created (whether the metrics
are actually reported or not).  This metrics file is named
\File{\textless{dag\_file\_name}\textgreater.metrics}.  In a workflow
with nested DAGs, each DAG will create its own metrics file.
...
An explanation of the data items is as follows:
\begin{itemize}
\item client: the name of the workflow software (e.g., "condor\_dagman").
\item version: the version of the client software.
\item planner: the workflow planner (read from the \File{braindump.txt} file).
\item planner\_version: the planner software version (read from the
\File{braindump.txt} file).
\item type: the type of data ("metrics").
\item wf\_uuid: the workflow ID (generated by pegasus-plan, read from
the \File{braindump.txt} file).
\item root\_wf\_uuid: the "root" workflow ID (relevant for nested
workflows) (generated by pegasus-plan, read from the \File{braindump.txt} file).
\item start\_time: the start time of the client (in epoch seconds with
millisecond precision).
\item end\_time: the end time of the client (in epoch seconds with
millisecond precision).
\item duration: the duration of the client (in seconds with millisecond
precision).
\item exitcode: the condor\_dagman exit code.
\item dagman\_id: an ID identifying the DAGMan instance
(hostname:schedd\_port:DAGMan\_cluster\_id).
\item parent\_dagman\_id: DAG ID of parent of this DAG; empty if
this DAG is not a sub-DAG.
\item rescue\_dag\_number: number of the rescue DAG being run (0 if
not running a rescue DAG).
\item jobs: the number of simple (not sub-DAG) nodes in the input DAG file.
\item jobs\_failed: the number of failed simple nodes in the workflow.
\item jobs\_succeeded: the number of successful simple nodes in the
workflow; this includes jobs that succeeded after retries.
\item dag\_jobs: the number of sub-DAG nodes in the input DAG file.
\item dag\_jobs\_failed: the number of sub-DAG nodes that failed.
\item dag\_jobs\_succeeded: the number of sub-DAG nodes that succeeded.
\item total\_jobs: the total number of jobs (simple and sub-DAG) in
the input DAG file.
\item total\_jobs\_run: the total number of nodes executed in a DAG.
Should be equal to jobs\_succeeded + jobs\_failed + dag\_jobs\_succeeded
+ dag\_jobs\_failed.
\item total\_job\_time: the sum of the time between the first
execute event and the terminated event for all non-sub DAG jobs.
\item dag\_status: the final status of the DAG, as follows:
\begin{itemize}
\item 0: OK
\item 1: error; an error condition different than those listed here
\item 2: one or more nodes in the DAG have failed
...
\item 6: halted; the DAG has been halted (see section ~\ref{sec:DagSuspend})
\end{itemize}
Note that any dag\_status other than 0 corresponds to a non-zero
exit code.
\end{itemize}

The \File{braindump.txt} file is generated by pegasus-plan; the name of
the \File{braindump.txt} file
is specified with the \Macro{PEGASUS\_BRAINDUMP\_FILE} environment
variable (if not specified, the file name defaults to to
\File{braindump.txt} in the current directory).

Note that, as of version 8.1.0, the total\_job\_time value is always zero,
...
reporting the metrics if the \Condor{dagman} job is removed, or if
the DAG "drains" from the queue because of being halted by a halt
file).

The metrics are actually reported by the
\Condor{dagman\_metrics\_reporter} executable
(see ~\pageref{man-condor-dagman-metrics-reporter} for more details).
