...
You may want to temporarily suspend a running DAG (for example, the load may
be high on your submit machine, and you may want to prevent DAGMan from
submitting any more jobs until the load goes down).  There are two ways
to suspend (and resume) a DAG:

\begin{itemize}
\item \Condor{hold}/\Condor{release}
\item A DAG halt file
\end{itemize}

When you \Condor{hold} a DAGMan job, no new node jobs will be submitted,
and no PRE or POST scripts will be run; but any node jobs already in
the Condor queue will continue undisturbed.  If you leave the DAGMan job
on hold, it will remain in the Condor queue after all of the node jobs
are finished.  To resume the DAG, \Condor{release} the DAGMan job.

Note that while the DAGMan job is on hold, no updates will be made to
the \File{dagman.out} file.

The second way of suspending a DAG is to create a file called 
\File{\textless{dag\_file}\textgreater.halt}
(for example, if your DAG file is \File{test1.dag}, your halt file would
be \File{test1.dag.halt}).  As long as such a file exists, the DAG will
be in a halted state:  no PRE scripts will be run, and no node jobs
will be submitted.  As when the DAG is held, existing node jobs will
continue undisturbed.  Unlike a held DAG, though, a halted DAG will still
run POST scripts and will still update the \File{dagman.out} file.
Furthermore, a halted DAG will not remain in the queue indefinitely;
when all of the node jobs have finished, DAGMan will create a rescue DAG
and exit.  To resume a halted DAG, simply remove the halt file.
...
\Condor{dagman} works by watching log files for events, like submission,
termination, going on hold, and so forth.  When a new job is ready to be
run, it is submitted to \Condor{schedd}, which needs to acquire a computing
resource, which in turn requires the \Condor{schedd} to contact the central
manager and get a claim on a machine.  This claim cycle can take many minutes.

One can avoid the negotiation cycle with the central manager by setting the
\Macro{DAGMAN\_HOLD\_CLAIM\_IDLE} configuration variable.  When set, the schedd
with hold a claim idle, so that the \Condor{startd} will delay in shifting from
Claimed to Preempting state (see Figure~\ref{fig:machine-states}). Thus, if
another job is presented that is suitable for the claimed resource, then the
\Condor{schedd} will submit the job directly to the \Condor{startd}, avoiding
the overhead of a negotiation cycle.  This results in a speed up of job
completion.  The use of this configuration variable is envisaged to be most
suitable for ``linear'' or ``shish-kabob'' DAGs in pools with long negotiation
times.

By default, \Macro{DAGMAN\_HOLD\_CLAIM\_IDLE} is $20$, meaning that the claim
will be held idle for 20 seconds, during which time a new job can be submitted
directly to the \Condor{startd} for which the claim is held. Setting it to 0
will mean that claims are not held idle for the running DAGman. If a node has
no children, the \Macro{DAGMAN\_HOLD\_CLAIM\_IDLE} will be ignored and set to 0; then the KeepClaimIdle attribute will be undefined in the JobAd of the node. 
