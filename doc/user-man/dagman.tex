...
such that the user or a script can easily monitor this status,
in a \emph{node status file}.  This file is periodically updated
while the DAG runs.
...
its location is given by \Arg{statusFileName} (which can be a full path).
...
\Bold{Important note:  as of version 8.1.6, the format of the node status
file has changed.}  The node status file is now a collection of ClassAds.
...
the node status file was completed and the time of the next update.
...
Possible DAG and node status values are:
