...
\Bold{Note that macro names cannot begin with the string "queue"
...
\begin{description}
\item[Special characters in VARS values]
\end{description}

It is now possible to have spaces and tabs within VARS values, as
explained below.  It is also possible to have double quotes and
backslashes within VARS values.  \Bold{Unfortunately, it is not
possible to have single quotes within VARS values.}  In order to
have spaces or tabs within a VARS value, you must use the "new-style"
argument format in your node job submit file (see
section~\ref{man-condor-submit-arguments}).  Double quotes must be
escaped differently depending on whether you use the new- or old-style
argument format (note that in both cases, double quotes require two
"levels" of escaping -- one for the parsing of the DAG file, and one for
passing the resulting value through \Condor{submit}).

Below is an example of how to pass special characters in VARS values.

The relevant parts of the DAG file are as follows (note that the
NodeA "second" value contains a tab):
\footnotesize
\begin{verbatim}
    ...
...
The (new-style) arguments line of the NodeA submit file is (note that
...
The (old-style) arguments line of the NodeB submit file is:
\footnotesize
\begin{verbatim}
    arguments    = $(first) $(second) $(third) $(misc)
\end{verbatim}
