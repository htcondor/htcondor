...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{DAGMan!DAG status in DAGMan's classad}
...
\Opt{DAG\_NodesTotal}:  The total number of nodes in the DAG, including
the FINAL node, if any.

\Opt{DAG\_NodesDone}:  The number of nodes that have finished successfully
(this means that the entire node has finished, not just the actual HTCondor
job or jobs).

\Opt{DAG\_NodesPrerun}:  The number of nodes for which a PRE script is
running or has been deferred because of a PRE script throttle setting.

\Opt{DAG\_NodesQueued}:  The number of nodes for which the actual HTCondor
job or jobs are queued (whether they are actually running, or are idle,
held, etc.).

\Opt{DAG\_NodesPostrun}:  The number of nodes for which a POST script is
running or has been deferred because of a POST script throttle setting.

\Opt{DAG\_NodesReady}:  The number of nodes that are ready to run but
which have not yet actually started running.

\Opt{DAG\_NodesFailed}:  The number of nodes that have failed (including
all retries, if any).

\Opt{DAG\_NodesUnready}:  The number of nodes that are not ready to run
(one or more of their parent nodes has not yet finished).

\Opt{DAG\_Status}:  The overall status of the DAG, with the same values
as the \Env{\$DAG\_STATUS} macro:
\begin{itemize}
\item 0: OK
\item 1: error; an error condition different than those listed here
\item 2: one or more nodes in the DAG have failed
...
\Opt{DAG\_InRecovery}:  1 if the DAG is in recovery mode; 0 otherwise.
