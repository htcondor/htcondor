...
The variable \Env{\$RETRY} evaluates to the "retry number" for the
node.  The first time a node is run, the value will be zero, and
it will be incremented for each retry.

For use as an argument to POST scripts only, the variable \Env{\$JOBID}
evaluates to the Condor ID of the node job (cluster and proc only).
(For example, 1234.0.)  For nodes with multiple jobs in the same
cluster, the proc value is the proc ID of the last job in the cluster.
