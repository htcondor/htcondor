...
DAGMan can help with the re-running of uncompleted
portions of a DAG, when one or more nodes result in failure (or when
a running DAG is removed with \Condor{rm}).
...
called a Rescue DAG.  A Rescue DAG is also produced if the
...
As of version 7.7.2, the Rescue DAG file is only a partial DAG file, 
as opposed to a full DAG file (unless the configuration variable
\Macro{DAGMAN\_WRITE\_PARTIAL\_RESCUE} is set to the non-default value
of \Expr{False}).  A partial Rescue DAG file contains only information
about which nodes are done, and the number of retries remaining for
nodes with retries.  It does not contain information such as the actual
DAG structure and the specification of the submit file for each node
job.  Partial Rescue DAGs are automatically parsed in combination with
the original DAG file, which contains information such as the DAG
structure.  This means that if you change something in the original DAG
file, such as changing the submit file for a node job, that change will
take effect when running a partial Rescue DAG.

If you remove a node from the original DAG, note that a DONE specification
in the Rescue DAG for a node that no longer exists is only a warning,
as opposed to an error, unless the \Macro{DAGMAN\_USE\_STRICT} configuration
variable is set to a value of 1 or higher.  Note that you can comment
out a DONE line in a rescue DAG to avoid a warning or error.

If you need to run a full Rescue DAG (either one left over from an older
version of DAGMan, or one produced by setting
\Macro{DAGMAN\_WRITE\_PARTIAL\_RESCUE} to \Expr{False}), you now need to
do that by directly submitting the Rescue DAG file, instead of the original
DAG file, for example:

\begin{verbatim}
  condor_submit_dag my.dag.rescue002
\end{verbatim}

If you just re-submit the original DAG file, \Condor{dagman} will try to
parse the Rescue DAG file in combination with the original DAG file, which
will result in failure if the Rescue DAG is a full DAG file.
...
number of retries (note that the
\Macro{DAGMAN\_RESET\_RETRIES\_UPON\_RESCUE} configuration variable
(see section~\ref{param:DAGManResetRetriesUponRescue}) controls whether
or not node retries are reset in a Rescue DAG).
...
causes \Condor{dagman} to output a rescue DAG file, even if the parsing of
a DAG input file fails.
...
a failed DAG.  (Note that a rescue DAG generated by the \Opt{-DumpRescue}
option is a full, as opposed to partial, DAG file, regardless of the
setting of the \Macro{DAGMAN\_WRITE\_PARTIAL\_RESCUE} configuration
variable.)
...
As of version 7.7.2, this naming scheme is no longer available.
