...
If the PRE script fails, then the job does not run but
the POST script does run; the exit value of the POST script determines
...
An exit value not equal to 0 indicates program failure, except as
indicated by the \Arg{PRE\_SKIP} command.
...
\hline \hline
node  & S & F & F       & F       & S & F & S & F & S & F & S & S & F & F  \\
\hline
\end{tabular}
