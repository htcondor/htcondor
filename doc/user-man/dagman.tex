...
The \Arg{SCRIPT} keyword is followed by either
...
% TEMP -- probably need to clarify this...
If the \Arg{DEFER} option is specified, and the script exits with the
exit code \Arg{status}, the script will be retried after \Arg{time}
seconds (or longer), rather than being considered failed.  While
deferred, a script does not count against the \Arg{maxpre} or
\Arg{maxpost} limit.
...
plus any jobs submitted to HTCondor or Stork,
...
\index{DAGMan input file!VARS keyword}
...
file, and uses the \Arg{VARS} entry to name files used by each job.
...
Note that the NodeA value for macro \Expr{second} contains a tab.
\footnotesize
\begin{verbatim}
...
The New Syntax \SubmitCmdNI{arguments} line of the submit description file
for NodeA is
\footnotesize
\begin{verbatim}
...
The Old Syntax \SubmitCmdNI{arguments} line of the submit description file
for NodeB is
\footnotesize
\begin{verbatim}
  arguments = $(first) $(second) $(third) $(misc)
...
% Note: this is an alternative to subsubsubsection, which we don't have.
\begin{description}
\item[Limitation: Splices and the RETRY of a Node]
\end{description}
...
% Note: this is an alternative to subsubsubsection, which we don't have.
\begin{description}
\item[Limitation: Splices and NOOP nodes]
\end{description}
\Todo

% Note: this is an alternative to subsubsubsection, which we don't have.
\begin{description}
\item[Limitation: Splices and DONE]
\end{description}
\Todo

% Note: this is an alternative to subsubsubsection, which we don't have.
\begin{description}
\item[Limitation: Splices and VARS]
\end{description}
\Todo

% Note: this is an alternative to subsubsubsection, which we don't have.
\begin{description}
\item[Limitation: Splices and PRIORITY]
\end{description}
\Todo
