...
Five variables (\Env{\$JOB}, \Env{\$RETRY},
\Env{\$MAX\_RETRIES}, 
\Env{\$DAG\_STATUS} and \Env{\$FAILED\_COUNT}) can be used within the
DAG input file as arguments passed to a PRE or POST script; three more
(\Env{\$JOBID}, \Env{\$RETURN}, and \Env{\$PRE\_SCRIPT\_RETURN}) can
be used as arguments to POST scripts only.
...
\Env{\$JOB} evaluates to the (case sensitive) string
defined for \Arg{JobName}.

\item \index{DAGMan!RETRY@\verb^$RETRY^ value}
\Env{\$RETRY} evaluates to an 
...
\Env{\$MAX\_RETRIES} evaluates to an integer value set 
...
\Env{\$JOBID} (for POST scripts only)
...
\Env{\$RETURN} (for POST scripts only) variable evaluates to
...
\Env{\$PRE\_SCRIPT\_RETURN} (for POST scripts only)
...
\item \Env{\$DAG\_STATUS} and \Env{\$FAILED\_COUNT} are documented in
...
If the node (or the PRE script of the node) specified by \Arg{JobName}
...
Note:  The implementation of this feature is somewhat inconsistent
with other aspects of DAGMan.  If a PRE script returns the
\Arg{AbortExitValue} value, the DAG is immediately aborted.  If the
actual HTCondor job for a node returns the \Arg{AbortExitValue} value,
the DAG is not aborted unless the node has no POST script.  If the
POST script returns the \Arg{AbortExitValue} value, the DAG is aborted.
...
Note that a DAG return value other than 0, 1, or 2 will cause the
actual \Condor{dagman} job to stay in the queue after it exits
...
\end{itemize}
\begin{verbatim}
      Vars F noderetry="$(RETRY)"
\end{verbatim}

Note that the special macro values can be inserted into a job's ClassAd,
as noted below.
