...
It may specify twelve types of items:
\begin{enumerate}
\item
...
\item
A splice to be included within the DAG.
\end{enumerate}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{dagman:SUBDAG}SUBDAG}

The \Arg{SUBDAG} keyword specifies a special case of a \Opt{JOB}
node, where the job is a DAG.
...
For more information, see section~\ref{sec:DAGsinDAGs}.
...
below).  Category names cannot contain white space.
...
For detailed information about splices, see 
...
with \Condor{rm}.
The file name of the rescue DAG depends on whether "old-style" or
"new-style" rescue DAG naming is used (see below).
...
in the Rescue DAG input file is the node -- if a node fails,
all parts of the node will be re-run even if some of them
worked the first time.  For example, if a node's PRE script
succeeds, but then the node job fails, when the rescue DAG is
run the PRE script for that node will be re-run.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:NestedBasics}Basics}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{dagman:NestedRecursion}\Condor{submit\_dag} recursion}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{dagman:NestedRescue}Nested DAGs and rescue DAGs}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{dagman:NestedFilePaths}File paths in nested DAGs}
...
\begin{verbatim}
  ...
  PARENT A CHILD X1
  # All final nodes in X1 must finish before the initial nodes in X2 can begin
  PARENT X1 CHILD X2
