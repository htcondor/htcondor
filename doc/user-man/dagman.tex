...
at the same time will be submitted.  The node priority is also passed
down to the individual node jobs, so the node priority also affects the
order in which jobs will be run once they are submitted (see
~\ref{sec:JobPriority} for more information about job priority).

The priority value is an integer (which can be negative).  A larger
numerical priority is better (will be submitted before a smaller numerical
value).  The default priority is 0.
Priorities are propagated to node jobs and sub-DAGs
...
not required, that some jobs run before others.
...
large numbers of "sibling" nodes.
...
\Bold{The "effective" priority for a node or sub-DAG (the priority with
which it is actually submitted) is the sum of the
...
value on the \Condor{submit\_dag} command line).  This algorithm for
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%TEMP -- is this the right place for this?
\subsubsection{\label{sec:dagman_include_command}INCLUDE}
...
the parent of another splice, all "terminal" nodes of the parent splice
are parents of all "initial" nodes of the child splice.  The CONNECT,
...
\Opt{CONNECT} \Arg{ParentSpliceName} \Arg{ChildSplice2Name}
...
%TEMP -- need to add diagrams here

%TEMP -- make a simple example

Restrictions:
%TEMP -- check these and make sure they are right
\begin{itemize}
\item Pin\_ins and pin\_outs must be numbered consecutively starting
at 1.
%TEMP -- should we call these parent and child splices???
\item The pin\_outs of the parent splice in a connect command must
match the pin\_ins of the child splice in the command.
\item All "terminal" nodes (nodes with no children) of a parent
splice must be connected to a pin\_out.
\item All "initial" nodes (nodes with no parents) of a child splice
must be connected to a pin\_in.
\end{itemize}
