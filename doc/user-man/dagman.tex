...
file as arguments passed to a PRE or POST script. These variables must be
standalone, separated from other words by whitespace. For example, a construction such as
\Env{job\_status=\$RETURN} will not cause substitution of the \Env{\$RETURN} value,
and the command line of the script will contain the token
``\Env{job\_status=\$RETURN}''.
