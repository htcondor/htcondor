...
\label{dagman:NOOP}
The optional \Arg{NOOP} keyword identifies a job that should not
actually be submitted to Condor when the node is "run".  This is
an optimization useful in cases such as debugging a complex DAG
structure, where some of the individual jobs are long-running.  In a
case such as this, you may want to initially run the DAG with
some jobs marked as NOOPs, and then remove the \Arg{NOOP} keywords
once the overall DAG has been finalized.  The PRE and POST scripts
(if they exist) for NOOP jobs \Bold{are} still run; if you want to avoid
running the PRE and POST scripts, you can just comment them out.
Even though a NOOP job is not submitted, the submit file specified
for the job must exist (the log file for the job is used, because
DAGMan generates dummy submit and terminated events for the job).
A NOOP \emph{job} is considered to always succeed, but the node can still
fail, since the PRE and POST scripts might fail.
...
itself in the event of a failure to complete the workflow.  The
DONE nodes are not executed when the rescue DAG is run, allowing
the workflow to pick up from the previous endpoint.  Users
should generally not use the \Arg{DONE} keyword -- the new \Arg{NOOP}
keyword is much more flexible in avoiding the actual execution of a
node.  Note that, for any node marked \Arg{DONE} in a DAG, all of
its parents must also be marked \Arg{DONE} -- otherwise a fatal error
will result.  Also, the \Arg{DONE} keyword applies to the entire node
-- a node marked as done will not have its PRE or POST script (if
they exist) run, and the actual Condor job will not be submitted.
...
\Opt{NOOP} and \Opt{DONE} for \Opt{DATA} nodes have the same effect
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Retrying Failed Nodes}
...
there is a failure in your work flow.  For example, if one of the
