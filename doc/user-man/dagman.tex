...
Priorities are propogated to children, to SUBDAGs, and to the Condor job itself,
via the \Attr{JobPrio} attribute in the job's ClassAd. The priority is defined to
be the maximum of the DAG PRIORITY directive for the job itself and the PRIORITYs
of all its parents. Here is an example to clarify:
...
In this example, node B is a child of nodes A and C. \textit{a priori}, the
priority of node B is 0, but gets assigned priority 60 because that is the
maximum of the priorities of its parents. Node D is a child of node C only:
since the declared priority of node D is at least as big as that of node C,
node D is assigned a priority of 100; all nodes in the D subdag will have
priority at least 100.  This priority is assigned by DAGman; there is not a way
to change the priority in the condor submit file for a job, as DAGman will
override any such \AdAttr{priority} directives in the submit file. This means
that for DAGs with a large number of edges (representing dependencies), the
priorities of child nodes far from the root nodes will tend to be homogeneous.
The priorities of a tree, or DAGs with a relatively small number of
dependencies, will tend to have varying priorities.
