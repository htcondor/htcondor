...
In order to have spaces or tabs within a value used as an argument,
...
the single quote must be escaped with another single quote (\Expr{''})
for \Condor{submit} parsing (see the NodeA \Expr{fourth} macro below).
...
The submit description file for NodeA uses New Syntax for the
...
The submit description file for NodeB uses Old Syntax for the
\SubmitCmdNI{arguments} command:
\footnotesize
\begin{verbatim}
  arguments = $(first) $(second) $(third) $(fourth) $(misc)
...
The submit description file for NodeC uses New Syntax for the
\SubmitCmdNI{arguments} command:
\footnotesize
\begin{verbatim}
  arguments = "$(args)"
