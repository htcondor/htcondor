...
the nodes marked as completed will not be reexecuted.
...
\subsection{Visualizing DAGs with dot}
\index{DAGMan!dot}
\index{dot}
\index{DAGMan!visualizing dags}

DAGMan can assist you in visualizing DAGs by creating \emph{dot}
files. Dot is a program that is part of the graphviz package,
available from \URL{http://www.research.att.com/sw/tools/graphviz},
and it is used to draw pictures of DAGs. 

You can instruct DAGMan to produce one or more dot files by adding a
line to your DAGMan input file. For example, to produce a single dot
...
This will create a file called dag.dot. You can create a visualization
of your DAG by giving this file to dot. For example, if you want to
create a Postscript file, you can execute:
...
\item \emph{UPDATE}  This will update the dot file every time a
significant update happens. 

\item \emph{DONT-UPDATE} This will only create a single dot file, when
the DAGMan begins executing. It is the default if you don't specify
UPDATE. 

\item \emph{OVERWRITE} This will overwrite the dot file each time it
is created. It is the default if you don't specify DONT-OVERWRITE.

\item \emph{DONT-OVERWRITE} Instead of using the name you specify,
DAGMan will create a file with the name followed by a period and a
number. For example, if you file is dag.dot, it will create files
``dag.dot.0'', ``dag.dot.1'', ``dag.dot.2'', and so on. This option is
the most useful when you combine it with the UPDATE option so that you
can visualize the history of the DAG after it has finished executing. 

\item \emph{INCLUDE include-file} If you want to include extra dot
commands in your dot file, this command will instruct DAGMan to
include your file in the dot files that it produces. This may be
useful if you don't want to edit the dot files that are created,
