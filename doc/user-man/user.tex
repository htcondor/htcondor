...
%
% .... or alternativly called the 'warm fuzzies' section
% <smirk>  
% 
%
...
clasifications.  Users submit their compute jobs to Condor, Condor puts
...
such a fasion that is to their advantage. Certainly a seller would rank
...
looking for the fast floating-point machine available, i.e. you want to
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Roadmap for running jobs with Condor}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\Condor{prio}. If desired, Condor can even inform you everytime your job
...
redirect console ouput (stdout and stderr) and keyboard (stdin) input
...
or the command-line. If your program opens a static filename everytime,
you will likely need to make a seperate subdirectory for each run to
...
can used when either the Standard Universe's additional services are not
desired or when the job cannot abide by the Standard Universe's
restrictions.

\subsubsection{Standard Universe}
...
sharing a filesystem via NFS (or AFS) or if the user has an account
...
deseriable services. However, all Standard Universe jobs must be
relinked with the Condor libraries. Although this is a simple process,
after doing so there are a few restrictions on what the program can do:
\begin{enumerate}
	\item On some platfoms, specifically HPUX, SunOS, and Digital Unix
...
be successfully relinked for submission into the Standard Universe.
...
\VersionNotice. To relink a program with the Condor libraries for
...
Note that even once your job is relinked, you can still run your program
...
therefore the relink step necessary for Standard Universe jobs could not
be performed. 
\begin{verbatim}
  ####################     
  #                       
...
\underline{except} for the ClassAd attribute names that appear in the
ClassAd expressions that you write!  ClassAds attribute and string
...
Condor has two independendant priority controls: \Term{job}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Interjob Dependencies: DAGman metascheduler}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\Todo
...
attribute of the job's classad.  This value of this attribute be specifed by
...
\end{verbatim}
where \Opt{machine-name} is the name of the machine you want to inspect.
This command will display a \condor{master} ClassAd; if the attribute ``RealUid'' equals
