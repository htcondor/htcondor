...
but the user must enable Condor's file transfering mechanism.
...
%%%%%%%%%%%% 
\subsubsection{Default behavior across the Condor Universes}
%%%%%%%%%%%%
...
\verb@input@,
\verb@output@, and
\verb@error@ commands are not supported.
This is not usually an impediment (shared file system or not), since
PVM jobs are set up to have the master direct the workers.
...
%%%%%%%%%%%% 
\subsubsection{Condor's File Transfer Mechanism}
%%%%%%%%%%%%
...
This is usually only useful if partial runs of your job are valuable
for some reason (for example, if your application does its own
checkpointing).
\item \verb@NEVER@: Condor's file transfer mechanism is disabled. 
\end{enumerate}

For vanilla, Java, and MPI universes, the defaults for
\verb@transfer_files@ are \verb@NEVER@ for UNIX, and \verb@ONEXIT@
for Windows.
