...
If \verb@when_to_transfer_output@ is specified in the submit
description file,
but \verb@should_transfer_files@ is not, Condor assumes a
...
If neither \verb@when_to_transfer_output@ or \verb@should_transfer_files@ 
...
system for jobs. 
Therefore, if 
neither \verb@when_to_transfer_output@ or \verb@should_transfer_files@
...
\verb@when_to_transfer_output@ and
...
Again, file names are delimited with a comma.
...
Using file transfer is slightly complicated by the
fact that you have to think about file paths from
two points of view: on the submit machine and on the
execute machine.

The files you specify in the \verb@transfer_input_files@ command
must be specified as they are accessed on the submit machine.
On the other hand, your program must access the files as they are
found on the execute machine.

More specifically, there are three ways to specify paths
for \verb@transfer_input_files@:
\begin{enumerate}
\item Relative to the submit directory, if \verb@initialdir@ is not
  specified.
\item Relative to the initial directory, if \verb@initialdir@ is specified.
\item Absolute.
\end{enumerate}

Each of these types of input file path will be shown in examples
below.

Before executing the program, Condor copies the input files to 
a temporary spool directory in which the program runs.  Therefore,
the program must access the input files \emph{without} paths.
Because all transferred files are put into a single directory,
if you have two input files with the same name in two different
directories, the files will collide when transferred.  If this
happens, the last one on the list overwrites the earlier one(s).

If your program creates output files, it must create them
in its current working directory.  Attempting to transfer
files back from another directory will fail, even if
your program manages to create the files.  It is
okay to create temporary files in a directory such as \File{/tmp}
(if you are sure it will exist on all possible execute machines)
as long as you do not try to transfer them back.
If you put a path in a file name specification in the
\verb@transfer_output_files@ command, the path will refer
to a path on the submit machine, not the execute machine.

Hopefully some examples will make this clear.

\begin{description}

\item[Example Background]

Assume we have a program called \verb@my_program@ that takes three command-line
arguments: two input file names and an output file name.  Further,
assume that we have a \verb@/scratch/test@ directory which is our current
working directory; within that directory
we have a \verb@logs@ subdirectory and a \verb@files@ subdirectory
that contains input files \verb@in1@ and \verb@in2@.  Also assume that
we have a \verb@logs2@ directory within the \verb@files@ directory.

So our directory tree looks like this:
\begin{verbatim}
...
Here is a simple example.  Note that the file names
for \verb@transfer_input_files@ contain the \verb@files@ directory,
since the paths are relative to the current working
directory.  Further note that the output file will
end up in the current working directory, not in the
\verb@files@ directory.
...
Here is the same thing, except with absolute paths to
the input files.
...
Here we are using \verb@initialdir@.  In this case, the output file
will end up in the \verb@files@ directory, not in the current working
directory.  Note that we also have to have a \verb@logs2@ directory
within the \verb@files@ directory, because the log file names are
also relative to the initial directory.  However, the 
executable is \emph{not} relative to the initial directory.
...
%--------------------------
\item[Example 4 -- Illustrates Error]

Now here is an example that will fail.  Because the file
names passed to the program have paths, the program will not be
able to create them on the execute machine -- there will
be no \verb@files@ directory available, since the file structure
is flattened when input files are transferred (all input
files are transferred to the single spool directory).
...
%--------------------------
\item[Example 5 -- Illustrates Error]

This example also fails, because my\_program tries
to access the files via absolute paths.
...
%--------------------------
\item[Example 6 -- Illustrates Error]

This example fails, because the program creates an
output file in a directory other than the current
working directory.
...
Assuming that the \verb@/tmp@ directory exists on the
execute machine, the program will actually run.  However,
Condor will fail when attempting to transfer back the
output file.
