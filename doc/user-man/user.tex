...
%%%%%%%%%%%% 
\subsection{Submitting Jobs Without a Shared File System} 
%%%%%%%%%%%%
...
%%%%%%%%%%%% 
\subsubsection{\label{sec:file-transfer}
Condor's File Transfer Mechanism}
%%%%%%%%%%%%
\index{File Transfer Mechanism}

To enable the file transfer feature, you must specify
\verb@transfer_files@ in the job's submit description file.
An example is:
...
The \verb@transfer_files@ command tells Condor when output files are
to be transferred back to the submit machine after the job
has executed on a remote machine.
It also enables transfering input files to your job (see below). 
There are three possible values for \verb@transfer_files@:

\begin{enumerate}

\item \verb@ONEXIT@: Condor transfers output files back to the submit
machine only when the job exits on its own.

\item \verb@ALWAYS@: Condor will always do the transfer, whether the
job completes on its own, was preempted by another job, or killed.
...
\item \verb@NEVER@: Condor's file transfer mechanism is disabled. 
\end{enumerate}

For vanilla, Java, and MPI universes, the defaults for
\verb@transfer_files@ are \verb@NEVER@ for UNIX, and \verb@ONEXIT@ for
Windows.
