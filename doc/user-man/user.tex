...
or AFS. 
Alternatively, Condor has a mechanism for transfering files on behalf
of the user.
...
into a Globus RSL and subsequently submitted to Globus via the GRAM
protocol.
...
However, the default configuration for Condor is that each machine is
in its own UID and file system domain, using the full hostname of the
machine as the name of the domains.
So, if a pool \emph{does} have access to a shared file system, the
pool administrator \emph{must} correctly configure Condor so that all
the machines mounting the same files have the same
\AdAttr{FileSystemDomain} setting.
Similarly, all machines that share common user information must be
configured to have the same \AdAttr{UidDomain}.

If a job is going to rely on a shared file system, Condor uses the
\AdAttr{requirements} expression to ensure it runs on a machine in the
...
and \AdAttr{FileSystemDomain} as the submitting machine.
This default is almost always correct.
However, in a pool spanning multiple \AdAttr{UidDomain} and/or
\AdAttr{FileSystemDomain}s, the user may have to specify a different
\AdAttr{requirements} expression to have their job run on the right
machines.
...
\AdAttr{FileSystemDomain} to be ``cs.wisc.edu'' for all the machines
that mounted the shared files, and to the full hostname for each
machine that did not, for example, ``jimi.cs.wisc.edu''.
...
settings correctly (the default is that each machine in a pool is in
...
\emph{only} run on the machine it was submitted from.
...
A shared file system is presumed for submitting jobs to Condor
under some circumstances.
Condor works well without a shared file system,
but the user must enable the Condor File Transfer mechanism.
In this case, Condor will transfer any files needed by your job into a
temporary working directory on the remote machine, execute your job,
and transfer back the output to your submitting machine.
To use this feature, the user must specify which files to transfer,
and when the output files should be copied back to your submit
machine.
This specification is done within the submit description file for the
job. 
The default behavior regarding file transfers varies across the
different Condor universes, and between UNIX and Windows machines.
...
For the standard universe, the existence of a shared file system
is not relevant.
Access to files (input and output) are handled through Condor's
remote system call mechanism.
The executable and checkpoint files are transfered automatically, if
needed. 
...
mechanism must be explicitly enabled (see above).
On Windows machines, Condor presumes no access to a shared file
system, enabling the file transfer mechanism by default.
However, Windows users still might need to specify which files to
...
Condor is only using the machine from where the job is submitted,
...
To enable the file transfer feature, you must specify at least two
settings in the job's submit description file:
...
The \verb@should_transfer_files@ setting determines if Condor should
transfer input files to the remote machine where your job executes,
and transfer the output back to your submit machine.
There are three possible values:

\begin{enumerate}

\item \verb@YES@: Condor will always transfer input and output files
for your job.

\item \verb@IF_NEEDED@: Condor will only transfer files if your job is
matched with a machine in a different \Attr{FileSystemDomain} than the
one your submit machine belongs to.
If the job is matched with a machine in your local
\Attr{FileSystemDomain}, Condor will not transfer files and will rely
on the shared file system.
See the section above on using Condor with a shared file system for
more details.
...
whether the job completes on its own, was preempted by another job, or
killed.
This is only useful if partial runs of your job are valuable for some
reason (for example, if your application does its own checkpointing).

\end{enumerate}

There is no default for \verb@when_to_transfer_output@.
...
\Note The combination of: \verb@should_transfer_files = IF_NEEDED@ and
\verb@when_to_transfer_output = ON_EXIT_OR_EVICT@ would produce
undefined file access semantics.
Therefore, this combination is prohibited by \Condor{submit}.

On UNIX, the file transfer mechanism is disabled by default.  If none
of the above settings are defined, Condor assumes
\verb@should_transfer_files = NO@

On Windows, Condor does not provide any way to use a shared file
system for your jobs. 
...
If file transfer is enabled, Condor will automatically transfer the
executable, the file defined as the standard input (if specified with
\verb@input@), and any jar files (for Java universe),  before the job
is run on a remote machine.
If the job requires any other input files, the user should utilize the
\verb@transfer_input_files@ command.
This list specifies any other files you want Condor to transfer to a
remote site to set up the execution environment for the job before it
is run.
All of these files are placed in the same temporary working directory
as the job's executable.
At this time, directories can not be transfered in this way.
For example:
...
temporary directory at the remote site are sent back.
Most of the time, this is the best option.
To restrict the files that are transfered, specify
the exact list with \verb@transfer_output_files@.
If you define this list and any of the files do not exist when the
...
Depending on what you specify for the \verb@should_transfer_files@
setting, it is important that the \Attr{requirements} expression for
your job contains the right logic to ensure that the job is matched
with a resource that meets your file transfer needs.
If you do not specify a \Attr{requirements} expression in your submit
file, or if the expression you specify does not refer to the
attributes listed below in any way, \Condor{submit} will add an
appropriate clause into the \Attr{requirements} expression for your
job.
\Condor{submit} will append these clauses with \verb@&&@, logical AND,
to ensure that these conditions are met for your job.
The default clauses corresponding to the different possible values of
\verb@should_transfer_files@ are listed below:

\begin{enumerate}

\item \verb@YES@: \verb@(HasFileTransfer)@
  If your job is always going to transfer files, the only requirement
  is that it is matched with a machine that has the capability to
  transfer files.
  This is simply a backward compatibility issue, since all versions
...
\item \verb@NO@:
  ...
  In addition, Condor will automatically add the
  \Attr{FileSystemDomain} attribute to your job ad, with whatever
  string is defined for the \Condor{schedd} that you are submitting
  your job to.
  If your job is not using the file transfer mechanism, Condor assumes
  you will need a shared file system, and therefore, a machine in the
  same \Attr{FileSystemDomain} as your submit machine.

\item \verb@IF_NEEDED@: 
\begin{verbatim}
  (HasFileTransfer || (TARGET.FileSystemDomain == MY.FileSystemDomain))
\end{verbatim}
  If Condor will optionally transfer files, it only has to require
  that the machine it is \emph{either} capable of transferring files
  \emph{or} in the same file system domain.

\end{enumerate}

To ensure that the job is matched to a machine with enough local disk
space to hold all the transfered files, Condor automatically adds an
attribute to your job add, \Attr{DiskUsage} which includes the total
size of your job's executable and all input files you have specified
to be transfered.
Then, Condor adds an additional clause in your \Attr{Requirements}
...
If you use \verb@should_transfer_files = IF_NEEDED@ and would prefer
your job ran in the local file system domain and did not transfer files
(but are still willing to allow the job to run remotely and transfer
files), you should define a \Attr{rank} for your job.  For example:

\begin{verbatim}
rank = (TARGET.FileSystemDomain == MY.FileSystemDomain)
\end{verbatim}

The \Attr{rank} expression is a floating point number, so if you have
other things you want to use to rank the possible machines this job
will run on, you should add them together, not use \verb@&&@.
For example:
...
However, given that \Attr{kflops} can vary widely among machines,
whether or not you were in the same file system domain probably
wouldn't effect where your job ran if you used this setting.  
If you are mostly interested in your job running in the same file
system domain if possible, and only care about kflops among the
machines in the file system domain, you should give a \emph{weight} to
the part of your rank that's matching the file system domains.
For example: 
...
settings in the submit file result in two corresponding string
attributes in your job classad: \Attr{ShouldTransferFiles} and
\Attr{WhenToTransferOutput}.
However, these settings are only valid when the job is matched with an
execute machine running Condor version 6.5.3 or greater.
...
If you examine your job with the \Opt{-long} option to \Condor{q}, and
you see \Attr{TransferFiles}, that attribute is only there for
backward compatibility, and is ignored if matched with a machine
running version 6.5.3 or greater.  
There were a number of problems with this old attribute, since it
wasn't flexible enough to handle the new \verb@IF_NEEDED@
functionality, and was confusing for users.
Therefore, \Attr{TransferFiles} is deprecated, and we will no longer
document its use.
If your submit file refers to \verb@transfer_files@, you should
consider switching it to use the settings described above.
...
\begin{itemize}
\item \Env{CONDOR\_SCRATCH\_DIR} \index{CONDOR\_SCRATCH\_DIR} - This variable gives the name of a directory
...
\item \Env{CONDOR\_VM} \index{CONDOR\_VM} - This variable gives the name of the virtual machine,
