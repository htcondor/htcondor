...
\subsubsection{\label{user-man-machad}Rank Expression Examples}
...
but the user must specify which files to transfer from the
machine where a job is submitted to the machine where the
job is executed.
The specification is done within the submit description file.
...
For both the vanilla universe and the mpi universe,
access to files through a shared file system is presumed as
a default.
If there is no shared file system, then
Condor must be explicitly told to transfer the job's input files.
Two commands accomplish this task.
\begin{verbatim}
  transfer_files = ONEXIT
  transfer_input_files = file1 file2 file3
\end{verbatim}
The \verb@transfer_files@ command tells Condor when files are to be
transferred.
When set to \verb@ONEXIT@, it transfers files from the execute machine
back to the submit machine when the job exits (completes).
Alternatively, it could be set to \verb@ALWAYS@, and Condor will do
the transfer when the job exits, or when it is preempted, or when
it is to be killed.
The \verb@transfer_input_files@ command tells Condor which files 
(other than the executable, and the files used in an \verb@input@
command) to transfer.

For the Java universe,
placing no additional commands into the submit description file
causes behavior that requires a shared file system.
When no shared file system exists, place
\begin{verbatim}
  transfer_files = ONEXIT
\end{verbatim}
or
...
With either of these commands, the executable and any jar files
(listed in a \verb@jar_files@ command) will be transferred.
