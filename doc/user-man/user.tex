...
\begin{verbatim}
  include : partition.sh |
\end{verbatim}
In this example, the bar character at the end of the line 
causes the script \File{partition.sh} to be invoked,
and the output of the script is incorporated into the submit description file.
Assume that this script partitions the problem space of a computation.
Perhaps this defines command line arguments that specify each partition's
boundaries.
The script outputs the following,
which is incorporated to direct the queuing of three jobs:
\begin{verbatim}
  arguments = -start 0 -stop 1200 -algorithm 1
  queue
  arguments = -start 1201 -stop 1800 -algorithm 2
  queue
  arguments = -start 1801 -stop 15000 -algorithm 1
  queue
\end{verbatim}
