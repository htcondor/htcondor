...
The files you specify in the \SubmitCmd{transfer\_input\_files} command
...
\begin{enumerate}
\item Relative to the submit directory, if \SubmitCmd{initialdir} is not
  specified.
\item Relative to the initial directory, if \SubmitCmd{initialdir} is specified.
\item Absolute.
\end{enumerate}
...
for \SubmitCmd{transfer\_input\_files} contain the \verb@files@ directory,
...
Here we are using \SubmitCmd{initialdir}.  In this case, the output file
