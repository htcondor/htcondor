...
If you are using a logfile for the job(which is recommended) the exit
...
\subsubsection{Cross Submitting}
...
You may do this for \Expr{standard} and \Expr{vanilla} universes, just
make sure you have the correct executable for the target architecture.
To see the architecture and operating systems of the machines
in your pool,
run \Condor{status}.

\subsubsection{True Heterogenous Submitting}

Now suppose you wish to submit either a \Expr{standard} or a
\Expr{vanilla} job to \emph{many} different architectures to gain full
use of a diverse architecture and operating system pool.
As in the above example, you are going to need to create or have available
executables that can work on each on the architectures in which you are
interested. Once this is done, you can write your requirements using
a special ``MachineAd Substitution Macro'' that will allow Condor to
automatically choose the correct executable for the machine where the
job was matched.  

\Note This special macro has a certain form and some
restrictions in its use.

The form of the macro that would appear in the submit description file is:
\begin{verbatim}
  $$(MachineAdAttribute)
\end{verbatim}

The restriction is that this special macro form may \emph{only} be used in
the \AdAttr{executable}. \AdAttr{environment}.  and \AdAttr{arguments}
attributes in the submit description file. Please be aware that this special
substitution macro is ignored in all other submit description attributes.

The \$\$() informs Condor to select the requested 
\AdAttr{MachineAdAttribute} from the machine on which the job had been 
selected to run and substitute it into the submit description attribute.

\Note The substitution will only occur \emph{after} a machine
has been selected on which the job may execute. This has implications when
using this special macro in your submit description file, e.g.,
\AdAttr{executable} attribute.

\Bold{Example 1}

This is an example where you want to use a lot of different types of
machine to render some images using a popular free rendering engine
called \Prog{povray} within the standard universe. 

The first thing you must do is create a set of executables that you wish to 
submit into your pool. Let's say your pool contains LINUX Intel,
Solaris26 Intel, and Irix 6.5 SGI machines. Once you've compiled
\Prog{povray}(linking with \Condor{compile}) on these machines you must 
place them all into the same
initial working directory so that they may be found by Condor. 
It is required that you name them by attributes that can be found in the
machine ClassAd; conveniently, we will choose \AdAttr{Arch} and \AdAttr{OpSys}.
...
Once this is done, you now have to worry about what kind of machines with
which you want your job to match. Your \AdAttr{requirements} expression
will let Condor know which machines should attempt to run your job.

\begin{verbatim}
  ####################
  #
  # Example of true heterogeneous submission
  #
  ####################
...
Suppose Condor matches your job with an \Bold{IRIX65 SGI} machine.
The resource asks the submitting machine for the executable that it
wishes to run, specifically the \Condor{shadow} that has been spawned
for this job.  The shadow process determines if the substitution macro
was used in either the \AdAttr{executable}, \AdAttr{environment}, or the
\AdAttr{arguments} attribute and if so, substitutes the corresponding
resource attributes into those specific job attributes and evaluates them.

When the shadow discovers that the \AdAttr{Executable} attribute really
resolves to \Prog{povray.IRIX65.SGI}, it chooses that executable and
sends it to the resource to be executed correctly.

%%%%%%%%%%%% 
\subsubsection{Common Errors with the Special Substitution Macro}
%%%%%%%%%%%% 
\index{running a job!on multiple architectures}
\index{running a job!on multiple operating systems}
\index{heterogeneous pool!submitting a job to}
\begin{description}
	\item[Use of a non-existent \AdAttr{MachineAdAttribute}.]
		If the specified \AdAttr{MachineAdAttribute} does not
		exist in the machine's ClassAd, then Condor will place
		your job on hold until the problem is resolved.

	\item[Not handling all combinations of \AdAttr{MachineAdAttribute}.]
		An example of this error would be, using the
		povray example, if you kept the submit description file
		the same, but removed one of the executables.  In this
		case, Condor will report back to you that one of its
		executables are missing when it happens to match your
		job against a resource that requires the removed binary.
\end{description}


%%%%%%%%%%%% 
\subsubsection{Submitting a Job to Run on Binary Compatible Operating Systems}
%%%%%%%%%%%%
...
Some binary executable jobs are able to run on multiple architectures
or operating systems that are For example, a SPARC binary may be able
to run on either ``SUN4x'' or ``SUN4u'' machines.

You can use the 
\AdAttr{requirements} command in
the submit description file to
specify the target architectures and operating systems.
For example, if your executable can run on any SPARC Solaris machine, you can
specify
\begin{verbatim}
