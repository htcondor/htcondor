...
\Opt{queue} \oOpt{<int expr>} \oOpt{<list of vars>} \Opt{in} \oOpt{slice} \Opt{<list of items>}

\Opt{queue} \oOpt{<int expr>} \oOpt{<list of vars>} \Opt{matching} \oOpt{files | dirs} \oOpt{slice} \Opt{<list of items with file globbing>}

\Opt{queue} \oOpt{<int expr>} \oOpt{<list of vars>} \Opt{from} \oOpt{slice} \Opt{<file name> | <list of items>}

All optional items have defaults.
\begin{itemize}
\item If \Expr{<int expr>} is not specified, it defaults to the value 1.
\item If \Expr{<list of vars>} is not specified, it defaults to the single
...
for each job queued.
That macro is then substituted into the \SubmitCmdNI{arguments} command.
