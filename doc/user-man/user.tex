...
okay to create temporary files in a directory such as \verb@/tmp@
(if you're sure it will exist on all possible submit machines)
as long as you don't try to transfer them back.
If you put a path in a file name specification in the
\verb@should_transfer_files@ command, the path will refer
...
Here's a simple example.  Note that the file names
...
Here's the same thing, except with absolute paths to
the input files.
...
Here we're using \verb@initialdir@.  In this case, the output file
...
Now here's an example that will fail.  Because the file
