...
has be superseded by the~\Expr{Parallel} universe.
...
The \Expr{Parallel} universe is for programs that require multiple machines
for one job.
...
Note that the comparison operators (<, >, <=, >=, ==) compare strings
case insensitively.  The special comparison operators =?= and =!= compare
...
If vanilla, Java, Parallel or MPI
