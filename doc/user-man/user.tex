...
\Url{http://www.globus.org}) through the Condor interface.
...
Standard universe jobs use Condor's \Term{remote system call}
...
The \Condor{shadow} on the machine where the job was
...
Transparent checkpoint and migration and remote system calls are highly
...
\Term{checkpoint file} which is approximately the size of the virtual
...
Alternatively, a Condor \Term{checkpoint server} can be set up to utilize
...
\index{Condor!re-linking for}
Re-linking a program with the Condor libraries (\File{condor\_rt0.o},
\File{libcondorsyscall.a} and \File{libcondorc++support.a}) is a
simple, one-step process with Condor \VersionNotice.
To re-link a program, run \Condor{compile}. 
See the command reference page for \Condor{compile} on
...
Example 2 queues 2 copies of program the program \Prog{mathematica}. The
...
A job that is linked with the Condor libraries with the \Condor{compile}
command
...
environment variables \Env{X509_CERT_DIR}, \Env{X509_USER_CERT},
\Env{X509_USER_KEY} can be used to override the default filenames and 
locations.
\Note{the AUTHENTICATION_METHOD configuration value list must contain 
...
can be overridden by setting the X509_* variables in your enviroment 
...
Globus X509 certificate, and the PATH to the \Prog{globusrun} program 
must be in your environment.
\Note{to allow a globus resource to join your Condor pool, your 
administrator must add the hostname(s) to the HOSTALLOW_WRITE and
HOSTALLOW_READ configuration values.}
