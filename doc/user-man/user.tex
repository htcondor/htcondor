...
\textu{not} an effective method of determining if Condor is running with
...
owner of the process, not the \emph{real} owner.  (See the \verbatim{getuid(2)} and
\verbatim{geteuid(2)} Unix man pages for details.)  In Unix, a process running
under the real uid of root may switch its effective uid.  (See the
\verbatim{seteuid(2)} man page.)  For security reasons, the daemons only set effective uid to
