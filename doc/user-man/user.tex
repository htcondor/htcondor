...
As a default, for jobs other than those submitted to the non-Condor-C grid universe,
any files that are modified or created by the job in the
temporary directory at the remote site (ignoring subdirectories) are transferred back
to the machine from which the job was submitted.
Most of the time, this is the best option.
To explicitly specify the files and directories that are transferred,
...
job exits, Condor considers this an error, and puts the job on hold.
...
For grid universe jobs other than Condor-C, files to be transferred 
...
working directory that have been modified or created, ignoring
subdirectories.
...
the job's temporary working directory on the execute machine.  When
the files and directories are copied back to the submit machine, they
...
accessible on all possible execute machines.  However,
Condor will not automatically clean up or transfer these files when the job
...
other than within the temporary directory that the 
