...
checkpointing feature of standard universe, but just specifying
\verb@ON_EXIT_OR_EVICT@ is not enough to make a job capable of
checkpointing.  The job must be designed to save and restore its state
using the files that are saved at eviction time.

The files that are transferred back at eviction time are not stored in
the location where the job's final output will be written when the job
finishes.  Condor manages these files automatically, so usually the only
reason for a user to worry about them is to make sure there is enough space
to store them.  The files are stored in a temporary directory within the
directory defined by the configuration variable \MacroNI{SPOOL}.  The
directory is named using the \Attr{ClusterId} and \Attr{ProcId} job
...
\begin{enumerate}
  ...
  \item any files created by the job in the remote scratch directory; this
only occurs for non-grid jobs and for Condor-C grid jobs
\end{enumerate}
...
\SubmitCmd{output} and \SubmitCmd{error} commands are placed into the
...
For \SubmitCmd{grid} universe jobs, files to be transferred 
