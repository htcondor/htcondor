...
%%% ClassAd attribute: HoldCode
\index{ClassAd job attribute!HoldCode}
\index{HoldCode!job ClassAd attribute}
\item[\AdAttr{HoldCode}:]    An integer value that represents the
...
2 & Globus middleware reported an error.  
  \Attr{HoldSubCode} is the GRAM error number. \\ \hline
...
6 & The \Condor{starter} failed to start the executable.
  \Attr{HoldSubCode} is the Unix  error number. \\ \hline
...
%%% ClassAd attribute: HoldSubCode
\index{ClassAd job attribute!HoldSubCode}
\index{HoldSubCode!job ClassAd attribute}
\item[\AdAttr{HoldSubCode}:]    An integer value that represents further
information to go along with the \Attr{HoldCode}, for
some values of \Attr{HoldCode}.
See \Attr{HoldCode} for the values.
