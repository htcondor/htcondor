...
\item[\AdAttr{NumJobStarts}:]  A count of the number of times the
...
\item[\AdAttr{NumJobReconnects}:]  A count of the number of times a
  job successfully reconnected after being disconnected (when the
  \Condor{shadow} and \Condor{starter} lose contact, e.g. because of
  transient network failures or a \Condor{shadow} or \Condor{schedd}
  restart).
  This is only defined for jobs that can reconnected (Vanilla and Java
  universes).
...
\item[\AdAttr{NumShadowExceptions}:]  A count of the number of
  times the \Condor{shadow} for a given job had a fatal error.

%%% ClassAd attribute: NumShadowStarts
\index{ClassAd job attribute!NumShadowStarts}
\index{NumShadowStarts!job ClassAd attribute}
\item[\AdAttr{NumShadowStarts}:]  A count of the number of
  times a \Condor{shadow} was started for a given job.
  This is not defined for Local and Scheduler universe jobs since
  those do not have a \Condor{shadow} associated with them.
