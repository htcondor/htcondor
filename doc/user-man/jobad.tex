...
17 & In the standard universe, the job and shadows versions aren't
  compatible. \\ \hline
...
\item[\AdAttr{NumShadowStarts}:]  An integer count of the number of
  ...
  This attribute is not defined for \SubmitCmd{local} and
...
\index{WindowsBuildNumber!job ClassAd attribute}
\item[\AdAttr{WindowsBuildNumber}:]   
An attribute utilized only for Windows jobs.
The value resolves to the build number of the current running Windows release.

%%% ClassAd attribute: WindowsMajorVersion
\index{ClassAd job attribute!WindowsMajorVersion}
\index{WindowsMajorVersion!job ClassAd attribute}
\item[\AdAttr{WindowsMajorVersion}:]   
An attribute utilized only for Windows jobs.
The value resolves to the major version number of the current running Windows
release. 

%%% ClassAd attribute: WindowsMinorVersion
\index{ClassAd job attribute!WindowsMinorVersion}
\index{WindowsMinorVersion!job ClassAd attribute}
\item[\AdAttr{WindowsMinorVersion}:]   
An attribute utilized only for Windows jobs.
The value resolves to the minor version number of the current running Windows
release.
