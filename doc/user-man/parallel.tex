...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{parallel-clusters-processes-subprocesses}

When 
a user submits a job to HTCondor with \Condor{submit}, HTCondor in parallel 
...
%\subsubsection{\label{sec:parallel-machine-count-vs-request-cpus}
%\Arg{machine\_count} vs.~\Arg{request\_cpus}}
...
the submit description file must have multiple \Expr{queue} commands (or have
an integer appended after white space). To create multiple subprocesses, the 
submit description file must have \SubmitCmd{machine\_count} have an integer
value greater than 1. Then within each subprocess, we can use commands like
\SubmitCmd{request\_cpus} or \SubmitCmd{request\_memory} to request that each 
...
\begin{description}
\item[queue \Arg{N}] Gives the job \Arg{N} processes with all the properties
listed above the \SubmitCmd{queue} command in the submit description file (with 
a default value of $N=1$). The command \SubmitCmd{machine\_count} must be given.
%How many processes with the previously listed 
%\Attr{machine\_count} and \Attr{request\_cpus} you want.
...
particular slot.
%\item[machine\_count] How many slots/machines/hosts you want associated with a given process.
...
particular, \SubmitCmd{request\_cpus} is useful for partitionable slots so as to
...
%\item[request\_cpus] How many CPUs each machine/host should have. Each 
%\item[request\_cpus] How many CPUs each machine/host should have available. Each 
%combination of machines and CPUs is a process with its own unique \Attr{ProcID}.
%When the 
%\item[request\_memory] How much RAM each slot should have
\end{description}

% Machine count <- how many machines/hosts you want. Each host can have multipel CPUs
% Machine count <- how many machines/hosts you want. Each host can have multiple CPUs
% Request CPUs <- how many CPUs you want on a given machine
...
With the ability to set \SubmitCmd{queue N}, \SubmitCmd{machine\_count}, and 
\SubmitCmd{request\_cpus}, the user needs to ensure that MPI is given the 
correct number of CPUs. To that end, we have a brief formula in 
equation~\ref{eq:sum-number-of-cpus}:

%Be sure to add them up correctly in your submit description file.
\begin{equation}
...
%combination of machines and CPUs is a process with its own unique \Attr{ProcID}.
%When the 
%\item[queue \Arg{N}] How many processes with the previously listed 
%\Attr{machine\_count} and \Attr{request\_cpus} you want.
%\end{description}
...
% Make sure you do the math for your call to mpirun. Make sure that the sum of 
% the requested CPUs adds up to the number of nodes you want.
% The queue command should match up to the machine count as well?
