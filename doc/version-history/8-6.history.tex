...
quota to some groups when \Macro{ACCEPT\_SURPLUS} is on and
\Macro{NEGOTAITOR\_ALLOW\_QUOTA\_OVERSUBSCRIPTION} is true (the default)
\Ticket{6514}
...
\Macro{MAX\_JOBS\_SUBMITTED}.  If this limit was ever reached by a
submit with more than one proc in the cluster, the limit would be
reduced by the difference until the schedd was restarted.
\Ticket{6460}

\item Fixed a bug when handling a VM universe job that specified a disk
image which existed but was in the wrong format; for some formats that
would the file being the wrong size.  Instead of offlining the VM universe,
HTCondor now recognizes this situation and puts the job on hold.
\Ticket{6505}

\item Fixed a bug on Windows that would often result in the job sandbox on the execute node
not being deleted when the \Condor{schedd} relenquished its claim on the slot before the
\Condor{starter} had exited.
\Ticket{6497}

\item Fixed a bug with the \Macro{RESERVED\_DISK} parameter that would not
accept an argument larger than 2 Gb.
\Ticket{6472}

\item Fixed a bug in the Java universe that prevented jar files with relative
paths to work when submitting from a Linux submit machine to a Windows
execute machine or vice versa.
\Ticket{6474}
...
\item Reduced preen connections to the schedd. Preen now connects only when
specifically needed, and automatically disconnects after 
\MacroNI{PREEN\_MAX\_SCHEDD\_CONNECTION\_TIME} seconds.
\Ticket{6490}

\item Fixed a bug that resulted in an incorrect value in the DONE column of
\Condor{q} output when multiple job clusters had the same batch name.
\Ticket{6469}

\item Removed unneeded \MacroNI{copy\_to\_spool} statement from default interactive
submit file.
\Ticket{6315}
...
\item \Condor{chirp} will now no longer abort when given a command that it cannot
successfully execute, such as fetching a file that does not exist.
\Ticket{6402}
