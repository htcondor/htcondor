...
% gittrac 1654
\item Fixed a bug with how the remote system call \Syscall{getrlimit}
functioned in the Standard Universe.  Under certain conditions with
32-bit and 64-bit Standard Universe binaries, \Syscall{getrlimit}
would erroneously report 2147483647 bytes as a limit when
\textbf{RLIM\_INFINITY} should have been the correct response.

% gittrac 1631
\item Fixed an misleading error message issued by \Condor{run} which stated the 
\verb@DAGMan job was aborted by the user@ when the job submitted
by \Condor{run} was aborted by the user.
It now states \verb@The job was aborted by the user@.
...
to \File{/dev/null}, and \Condor{submit} refused any setting of these files for
VM universe jobs.  
...
incorrectly transferred back to the submit machine 
...
when receiving updated matchmaking information for offline ads that do
not exist.
