...
% gittrac #921
\item Allow submit files to specify \SubmitCmd{IwdFlushNFSCache} expression to
control if Condor tries to flush the NFS cache for a job's initial working
...
% gittrac #909
\item Removed attempted limiting of swap space via 'ulimit -v' using the
VirtualMemory parameter in condor\_limits\_wrapper.sh
...
% gittrac #883
\item Removed a check for root ownership of script specified by
\MacroNI{VM\_SCRIPT}

% gittrac #884
\item Fixed a bug in writing the header of the \MacroNI{EVENT\_LOG}
...
%gittrac #801
\item Fixed a bug in Windows XP where using \MacroNI{USE\_VISIBLE\_DESKTOP}
...
% gittrac #920
\item Fixed a bug that allowed running VMs to be leaked if the \Condor{startd}
crashed.
...
% gittrac #933
\item Added \MacroNI{STARTD\_PER\_JOB\_HISTORY\_DIR} to allow ads of completed
jobs to be stored in a directory separate from the existing
\MacroNI{PER\_JOB\_HISTORY\_DIR}.
...
% gittrac #981
\item Fixed a bug that caused the \Condor{schedd} to segfault when spooling
more than 9 files.

% gittrac #1011
\item Fixed a bug that caused the \Condor{startd} to crash on Debian Stable.

% gittrac #1033
\item Fixed keyboard activity detection on Windows XP.

% gittrac #1068
\item Fixed a bug in \Condor{had} that caused it to be not start the controlled
daemon if CCB was enabled.
