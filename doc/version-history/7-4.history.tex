...
\item \MacroNI{TCP\_FORWARDING\_HOST} has existed in Condor since
  7.0.0, but was not documented.  This configuration variable
  specifies the host or IP address that should be used as the public
  address of this daemon.  This is useful if Condor on this host may
  be reached through a NAT or firewall by connecting to a different IP
  address that forwards connections to this host.  It is assumed that
  the port number on the \MacroNI{TCP\_FORWARDING\_HOST} that forwards
  to this host is the same port number assigned to Condor on this
  host.  This option could also be used when ssh port forwarding is
  being used.  In this case, the incoming addresses of connections to
  this daemon will appear as though they are coming from this host
  rather than from the real remote host, so any authorization settings
  that rely on host addresses should be considered accordingly.
...
% gittrac #1019
\item Fixed a problem introduced in version 7.4.0 that caused GSI
authentication between Condor processes to fail with using a
non-legacy format X509 proxy.

% gittrac #923
\item Improved handling of previously-submitted gt2 grid jobs on
release from hold when there is no globus-job-manager for the job running
on the remote resource.

% gittrac #453
\item Fix a problem with job leases for jobs that use a \condor{shadow}.
...
a DAG description file. There is now no limit to the number of splices
one may include into a DAG description file except, of course, for the
...
\item Fixed a bug that caused \MacroNI{ALLOW\_CONFIG} and
  ...
  \MacroNI{DENY} configuration parameters to incorrectly handle a
  ...
  a single '*' or the equivalent '*/*'.  These bugs have existed since
  before 6.8.

% gittrack #905
\item Fixed a bug introduced in Condor version 7.3.0 that can cause Condor daemons to
crash when reading malformed network addresses.
...
In 7.3.2 and 7.4.0, this same command produced an empty xml ClassAd.
...
more straightforward way to select attributes since 7.3.2 is to use the
\Opt{-attributes} option.
...
DAG with POST scripts, if the global event log (see
section~\ref{param:EventLog}) is turned on.
...
\item CCB now works with \MacroNI{TCP\_FORWARDING\_HOST}.  Previously,
  it the reversed connection was made to the private address rather than
  to \MacroNI{TCP\_FORWARDING\_HOST}.

% gittrac #852
\item Removed a bad optimization that caused some information about job
execution to be lost during job completion or removal if a history file
wasn't configured.

% gittrac #893
\item Condor now checks whether \MacroNI{GRIDFTP\_URL\_BASE} is set before
submitting cream grid jobs, as that parameter is required for cream jobs
to function properly. If the parameter isn't set, cream jobs are put on
hold with an appropriate message.

% gittrac #912
\item Fixed a bug in \MacroNI{cream\_gahp} which could cause crashes when
there are more than 500 cream jobs queued.

% gittrac #972
\item Improved recovery when Condor crashes during submission of a cream
grid job. Before, affected jobs would remain in \MacroNI{REGISTERED} state
on the cream server, but never run.

% gittrac #954
\item Improved the \MacroNI{HoldReason} message when cream grid jobs are
held by the \condor{gridmanager}.

% gittrac #895
\item When naming a resource for a cream grid job, properly recognize the
format used by the standard cream client UI:
\MacroNI{https://foo.edu:8443/cream-pbs-cream\_queue}.

% gittrac #795
% The memory leak is not worth documenting.
\item \MacroNI{SOAP\_SSL\_CA\_FILE} is now consulted in addition to
...
and a username or cluster id, they would act on all jobs matching the
