...
% gittrac #416
\item String literals within \verb@$$([])@ expressions failed to be evaluated
and resulted in the job going on hold.  This problem has existed since before
Condor 7.0.0.
...
\item Condor version 7.4.3 released on August 12, 2010.
...
\item
Prepare hooks are now invoked as the condor user, instead as the job
user.

\item
Improved the file extension detection on Windows.

\item
condor\_wait could occassionally get stuck in an infinite loop,
having missed the execution event of the job it is waiting for.
This is now fixed.

% gittrac 1413
\item
Fixed a bug in the startd cron that only occurred on Windows. Startd
cron scripts failed to run if a initial directory was left
unspecified.

% gittrac 1012
\item
Fixed a bug where a job would be incorrectly placed on hold, with 
a confusing error message about failing to start a your job XXX\.-1. This
would happen if copy\_to\_spool was true, the job contained \$\$ macros,
and preen ran after the job was submitted and before it started.

% gittrac 1427
\item
Added the jobs\_vertical\_history table to the list of tables that
quill periodically reindexes.

\item
Fixed bug in preen where it would delete startd history files.
...
% gittrac #1333
\item
Allowed the condor config file entries GSI\_DAEMON\_TRUSTED\_CA\_DIR and 
GSI\_DAEMON\_DIRECTORY to be set with environment variables, as the rest of 
condor config variables can be.
