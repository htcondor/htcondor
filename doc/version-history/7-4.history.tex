...
in the output of \Condor{status} -collector.
...
Fixed a bug that prevented parallel universe jobs from running on startds
with dynamic slots enabled.
...
configured for high availability with multiple \Condor{collector}s.

% commit f44a68fb351e528ea5b251dd2c3cf9767b0c1fba
\item Fixed a bug where multiple entries of output from 
\Condor{status} -negotiator
would be on a single line.  They are now listed one per line.

% #778
\item Fixed a bug where \Condor{submit} -dump would crash if multiple
jobs were queued in a single submit file.

% #742
\item Fixed a bug where a slot whose associated job disappeared
could remain Claimed/Idle until the claim lease expired.
The slot should now promptly return to Unclaimed/Idle.

% commit 0d5e3ad8fc85f0cd0dc58f73b503c76c0ad49bc4
\item Fixed a bug where a \Condor{startd} using dynamic slots could
