...
% gittrac #1238
\item Fixed a bug that caused Condor to miscount the number of CPUs and
hyper-threads on some modern machines.

% gittrac #1215
\item Fixed a bug that could corrupt the job queue log
if the \Condor{schedd}'s attempt to compact it fails.
...
\item Condor version 7.4.2 released on Month Date, 2010.
...
\item Fixed a bug which caused the \Condor{startd} to incorrectly
  count the number of procesors on some machies with ``hyper
  threading'' enabled.  This bug was introduced in
