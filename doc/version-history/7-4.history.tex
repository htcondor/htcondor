...
% gittrack #905
\item Fixed a bug introduced in 7.3.0 that can cause Condor daemons to
crash when reading malformed network addresses.

% gittrack #871
\item Fixed a problem introduced in 7.3.2 that changed the behavior of
...
% gittrack #907
\item Fixed a bug introduced in 7.3.2 that resulted in messages such
  as the following even in cases where no problem in communicating
  with the collector had been encountered:
...
% gittrack #859
\item Fixed a bug that has been in \Condor{startd} since before
  version 6.8.  If the startd ever failed to send signals to the
  \Condor{starter} process, it could fail to properly clean up the
  machine ClassAd, leaving attributes from
  \MacroNI{STARTD\_JOB\_ATTRS} in the ad but not making them visible
  in \Condor{status} queries.  One possible problem resulting from
  this could be matches being made by the negotiator that are then
  rejected by the startd.  Repeated messages such as the following
  would then result in the startd log:
...
These attributes were already present internally to the schedd, 
just not published.
...
% #795
\item \MacroNI{SOAP\_SSL\_CA\_DIR} is now consulted when authenticating
an https proxy for Amazon EC2 (when \MacroNI{AMAZON\_HTTP\_PROXY} is set).
...
\item The default configuration now uses
  ...
  Submission params have also been added, they include;
  \MacroNI{kvm\_disk}, \MacroNI{kvm\_transfer\_files}, and \MacroNI{kvm\_cd\_rom\_device}.

% #872
\item Removal of \MacroNI{XEN\_DEFAULT\_KERNEL} and \MacroNI{XEN\_DEFAULT\_INITRD}, as 
  well as \MacroNI{xen\_kernel=any}.

% #775, cf02764d9d0fdd2b36ef3629f862f856ee41a717, and more
\item Optimizations in core Condor systems should provide minor speed improvements throughout.

\item Some log messages were rewritten for clarity.

\item Some additional logging was added to aid debugging.
...
Also improved memory usage when VOMS extensions aren't used.
...
% #862
\item Fixed output of \Condor{status} when \Opt{-java} or \Opt{-vm} is
used.

\item Fixed a problem in the \Condor{schedd} daemon introduced in
  7.3.2.  For schedd's with lots of jobs having periodic release
  expressions, this could result in the \Condor{schedd} taking a long
  time while evaluating periodic expressions, causing it to become
  unresponsive to queries and other tasks.  With a job queue of 30,000
  jobs, a period of unresponsiveness of an hour was observed, whereas
  the evaluation of periodic expressions in this same environment
...
directory in \MacroNI{EXECUTE} might be left behind.
...
% 826 
\item Fixed a bug in the startd where vacating jobs would not 
respect the \MacroNI{job\_lease\_duration}.

% 823
\item Updated the max log size to the max OS file size.  

% 802
\item Updated the detection of HasVM in the startd to publish an 
update to the collector when \MacroNI{VM\_RECHECK\_INTERVAL} is 
specified.
