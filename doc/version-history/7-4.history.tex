...
\item Added example Mac OS X launchd configuration file for starting Condor.
...
% gittrac #1611 #1612
\item The vm universe now works properly on machines that have Condor's
Privilege Separation enabled.

% gittrac #1624
\item \Condor{submit} no longer pads a vm universe job's disk usage
estimation by 100MB.

% gittrac #1553
\item Fixed a bug with the \Macro{vm\_cdrom\_files} submit file
attribute that caused VMware vm universe jobs to fail if the virtual
machine alread had a CD-ROM image associated with it.

% gittrac #1508
\item Fixed permissions of ssh private key file that's created for 
Amazon EC2 jobs. Previously, the permissions were too lax.

% gittrac #1465
\item Configuration parameters \Macro{SOAP\_SSL\_CA\_DIR} and
\Macro{SOAP\_SSL\_CA\_FILE} are now properly used when authenticating
with Amazon EC2 servers.

% gittrac #1484
\item Fix a bug with the \Macro{<subsys>\_LOCK} configuration parameter
that could let daemons writing to the same daemon log overwrite each others
entries and cause daemons to exit when the log is rotated.

% gittrac #1557
\item Fixed a bug that caused nordugrid jobs to fail if the
\Macro{grid\_resource} attribute included a port as part of the server
hostname.
