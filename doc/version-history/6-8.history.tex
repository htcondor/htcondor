...
\item Fixed a bug that prevented dollar dollar expansions with a default 
argument that contained forward slashes, e.g., 
\begin{verbatim}
...
pop-up any of those annoying command windows.
...
with VARS lines specifying more than one macroname (this bug was
...
If \Attr{GSI\_DAEMON\_DIRECTORY} was set and \Attr{GRIDMAP} wasn't set, 
...
\item In unix, when \Condor{schedd} did not have enough memory to create
a process to handle file transfers, many repetitions of this error would
result in \Condor{schedd} running out of file descriptors.

\item File transfer failures for spooled jobs would sometimes result in
\Condor{schedd} child processes hanging around for 8 hours before finally
exiting.  Too many such processes could result in memory exhaustion.
