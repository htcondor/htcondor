...
\item The \Condor{schedd} no longer forks seperate processes to chown job
...
sites with very busy \Condor{schedd}s were encountering scaling problems.
...
\item The \Condor{master} now detects orphaned process from other
instantiations of Condor and hard kills them. An administrative email
is sent which describes which pids were killed along with their command
line. See entry on \Macro{KILL\_ORPHANS\_ON\_STARTUP} in 
section~\ref{param:KillOrphansOnStartup}.

\item Because by default the \Condor{startd} references the job
ad attribute \AdAttr{NumCkpts}, Condor's default configuration
will now round-up the value of NumCkpts in order to improve 
matchmaking performance.  See entry on \Macro{SCHEDD\_ROUND\_ATTR}
...
\item Fixed bug in the \Condor{master} daemon where it would only
...
\item Fixed a bug in the \Condor{schedd} that caused it to stop
negotiating for grid universe jobs if it decided it couldn't spawn
any new shadow processes.

% Gnats PR 751
\item Added the ProcessId class (which more uniquely identifies a
process than just a PID does) to the \Condor{dagman} ``abort duplicate
...
% Gnats PR 743
\item Fixed a bug in the \condor{starter} which caused it to look in the
wrong place for the job's executable if \Attr{TransferExecutable} was set
to \Expr{True} in the job ad.

\item \Condor{history} no longer crashes if \Attr{HISTORY} isn't defined
in the Condor configuration file.

\item Fixed an unintentional change to the value of \Opt{-Condorlog}
in a \condor{dagman} submit file: it is once again the log file of
the first node job.

\item Fixed a bug in \Condor{q} that would cause "condor\_q -hold" or
"condor\_q -run" to exit with an error on some platforms.

\item Fixed a bug under unix in which a misconfiguration of
\MacroNI{MAIL} would cause \Condor{master} to restart all of its child
daemons whenever it tried (and failed) to send email to the
administrator.
...
\Condor{startd} to abort.  Previously, it would continue running,
but some invalid values (e.g. 0) could cause it to stop sending
periodic ClassAd updates to the collector, even after being
reconfigured with a valid value.  Only a complete restart of
\Condor{startd} was sufficient to get it out of this state.
