...
\item When starting up for the first time, \Condor{schedd} ensures
that the initial job queue log file contains a sequence number,
because in the case where this wasn't being inserted, this was causing
Quill to crash.  In practice, this was only a problem under Windows,
because of the likelihood of the log compaction operation failing
during the schedd's initial startup.
