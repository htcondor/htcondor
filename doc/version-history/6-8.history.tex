...
received too many matches in one negotiation cycle (more than 1000 on linux).
...
\item A bug has been fixed in the \Condor{quill} daemon which would
...
of detecting potentially incorrect submit events (those with Condor IDs
not matching what is expected).

% PR 814
\item \Condor{dagman} now truncates existing node job user log files
to zero length, rather than deleting them.  This prevents breaking the
...
by the server (e.g. \Condor{schedd}) before the client
(e.g. \Condor{shadow}) was done using it.  The result of this problem
was communication failure the next time the client tried to connect to
the server.  In some cases, this caused jobs to be requeued (to be run
again), because the final update of to the job queue failed.
