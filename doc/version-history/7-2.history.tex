...
\item The JobRouter now uses a timeslice timer for periodic expression
  ...
  to 60 seconds, just like the \Condor{schedd}.
...
\item Fixed a bug on Windows that made it impossible to use
  \textbf{S5} (Shutdown) for hibernation.
...
\item Resolved bug where \SubmitCmd{vm\_macaddr} was improperly
  parsed, and thus ignored, by \Condor{submit} for jobs in the VM
  Universe.
