...
and \Condor{submit\_dag}.  (This is mainly for testing.)
...
\item An ill-formed network packet could crash a Condor daemon.  This wouldn't
be seen in normal Condor operation, but sometimes port-scanning software could
trigger such a crash.

\item \Condor{q} would sometimes exit with  zero when it could not connect to
a schedd.  Now it exits with an error code in this case.
...
\item Fixed a problem affecting \Condor{starter} when in privsep mode.
After the user job exited, an error was printed in the
\Condor{starter} log file complaining that it failed to `chown' the
...
\item Fixed the \Condor{transferer} and documentation to consistently
  use \MacroNI{MAX\_TRANSFERER\_LIFETIME} as the limit of the High
  Availability transferer lifetime.

% gittrac #285
\item Fixed a bug that caused \Condor{dagman} to crash if a splice
DAG has node categories (in other words, if a DAG with node categories
is spliced into another DAG).

\item Changed a number of splice-related \Condor{dagman} debug messages
to not be printed at the default verbosity (they are now mostly printed
at debug level 4).
