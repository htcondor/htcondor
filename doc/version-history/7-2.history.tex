...
\item Added \MacroNI{PREEMPTION\_REQUIREMENTS\_STABLE} and
  \MacroNI{PREEMPTION\_RANK\_STABLE} which tell Condor if all
  attributes in the \MacroNI{PREEMPTION\_REQUIREMENTS} and
  \MacroNI{PREEMPTION\_RANK} variables will not change within
...
\item Fixed the Collector so that ``hibernating'' machines never timeout.

\item Fixed some of the class ad values of a ``hibernating'' machine
  when gathered from the Collector.  For instance, all ``hibernating''
  machines are unclaimed and idle, have no load, the cpu is not busy, and
  the keyboard and console are set as if they had been idle for a long time.
