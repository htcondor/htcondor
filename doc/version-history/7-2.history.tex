...
\item Added support for SCHEDD\_CLUSTER\_{INITIAL|INCREMENT}\_VALUE knobs.
	The \Macro{SCHEDD\_CLUSTER\_INITIAL\_VALUE} config knob should be an
	integer that specifies the cluster number to use for the first submission
	to the schedd (defaults to 1).  The
	\Macro{SCHEDD\_CLUSTER\_INCREMENT\_VALUE} will be used to increment the
	cluster id upon successive submits (defaults to 1).
