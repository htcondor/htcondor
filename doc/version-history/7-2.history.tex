...
\item \Attr{JAVA\_MAXHEAP\_ARGUMENT} now defaults to \Opt{-Xmx1024m}.
The installation process of Condor will reset this to the undefined 
value in the local configuration file if the detected JVM is not from Sun.
...
% PR 959
\item Added the DAGMAN\_COPY\_TO\_SPOOL configuration macro to control
whether the condor\_dagman binary gets copied to the spool directory
when a DAG is submitted.  See \ref{param:DAGManCopyToSpool} for details.
...
\item Fixed a bug on Windows in \Condor{mail} which was causing it to become
unresponsive run when. Further, the application would start to increasingly 
consume more memory when run.

\item Fixed a bug on UNIX where \Condor{configure} would provide
incorrect defaults for the \Attr{JAVA\_MAXHEAP\_ARGUMENT} attribute in
the installed configuration files. The new current default for Sun Java
JVMs is \Opt{-Xmx1024m}.

\item Fixed a bug on UNIX where \Condor{configure} would imply that using
user ``root'' or UID 0 for \Opt{--owner} is a good thing--it is not,
and then complain that it could not find user ``root'' in the password
file in this circumstance.

\item Fixed a bug on UNIX where \Condor{configure} would emit errors
...
\item Fixed a bug where execute-side daemons that were started before the
\Condor{credd} would fail to match with Windows \SubmitCmd{run\_as\_owner}
jobs. This condition would persist until the execute-side daemons were
