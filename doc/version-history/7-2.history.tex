...
\item The new string configuration variable \Macro{TRANSFERER}
  ...
  transfer of the \MacroNI{STATE\_FILE}.  This is part of the high
  availability framework.
  Prior to Condor 7.2.2, this was hard coded to
  ``\MacroUNI{RELEASE\_DIR}/sbin/condor\_transferer''.  The use of
...
% gittrac #313
\item Fixed a bug that was causing the \Condor{replication} daemon
  (part of the high availability framework) to
  start the \Condor{transferer} client incorrectly; the end result is
  that the \Condor{transferer} was unable to authenticate via GSI
  using host-based certificates.

\item \AdAttr{RemoteWallClockTime} could get too big in some cases after
a restart of the schedd for jobs that were running at the time of the
restart.

% gittrac #231
\item Fixed a bug that was causing the \Condor{startd} to log an
  error message ``ioctl(SIOCETHTOOL/GWOL) failed: Operation not
  permitted (1)'' when started as a personal Condor on Linux.
