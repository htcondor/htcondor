...
% gittrac #504
\item Fixed a bug wherein the ``scripts as executables'' code
  incorrectly handles files with unknown file extensions. This meant
  that, for instance, unknown file extensions would be treated as an
  error, rather than a Windows executable, as they were originally.
...
% gittrac #510
\item Fixed a bug in the \MacroNI{USE\_VISIBLE\_DESKTOP} code that was
  causing the Windows created by the job be behave incorrectly.
