...
\item This release addresses the reported 7.2.0 problems with the
Windows Condor distribution.  Please the ``Bugs Fixed'' section bellow
for further details.
...
\item Added Standard Universe support for gfortran.

\item Added support for '*' to match all leases for many operations.

\item Added support for standard out and standard error to be greater
than 2 Gigabytes.
...
% PR 964
\item Added \Opt{-version} and \Opt{-help} flags to
\Condor{submit\_dag}.
...
\item Fixed a bug in \Condor{chirp} which was causing it to crash on
invocation.
...
application also increaseses its memory consumption.
...
\Opt{--owner} option is a good thing--it is not, and then complain
that it could not find user \Login{root} in the password file in this
circumstance.
...
processes after 20 seconds if SIGQUIT (``fast shutdown'') failed.  The
\Condor{master} will actually wait \MacroNI{SHUTDOWN\_FAST\_TIMEOUT}
...
the inability to specify a custom PATH setting for a job from its
submit file.

\item Changed \Condor{submit} \Opt{-debug} to warn when ignoring
environment variables (when \SubmitCmd{get\_env = true})

\item Fixed a long-standing memory leak in SOAP interface.  The result
was a few hundred bytes of memory would be leaked for each
connection. This could eventually cause the
...
crashing if the user specified a \MacroNI{NUM\_SLOTS\_TYPE\_N} without
specifying a \MacroNI{SLOT\_TYPE\_N}.

\item The sample configuration files now correctly set the default
universe to Vanilla Universe.  This default has been true since 7.2.0,
but was not reflected in the sample configuration files.

\item Fixed default \Attr{RequestMemory}: changed \Attr{ImageSize} to
\Attr{ceiling(ImageSize/1024.0)}. The \Attr{RequestMemory} units are
MiB, it is compared directly to a slot's Memory attribute which is
MiB. The \Attr{ImageSize} is in KiB. The old code was broken such that
a 22KiB \Attr{ImageSize} would use up 22MiB on a slot. Not so bad, but
if the \Attr{ImageSize} (calculate initially from executable size)
were say 20MiB it would mean a request for 20GiB of slot \Attr{Memory}
...
\item A FAQ entry for Windows was added describing how a problem with
the machines performance counters may cause the \Condor{procd} to
crash.
