...
\item The JobRouter now uses a timeslice timer for periodic expression
  evaluation similar to the \Condor{schedd}.  The evaluation interval
  is controlled by the \MacroNI{PERIODIC\_EXPR\_INTERVAL} and defaults
  to 60 seconds, just like the schedd.

\item Job Router now resets the source job if a failure occurs when
  updating the \Condor{schedd} for a periodic expression that
  evaluated to true.  The job's periodic expressions should be
  evaluated again sometime in the future with a successful update.
...
\item Added \MacroNI{JOB\_ROUTER\_ATTRS\_TO\_COPY} configuration
  ...
  to internally hardcoded attributes that are copied.
...
\item Fixed a bug where if any idle slot satisfied the
  ...
  sleep state irespective of any active slots.

\item Fixed a bug on Windows that made it impossible to use \Verb@S5@
  (Shutdown) for hibernation.
...
\item If \Condor{q} fails to connect to a \Condor{schedd} it now
  returns an error.
...
\item Resolved bug where \SubmitCmd{vm_macaddr} was improperly parsed,
  and thus ignored, by \Condor{submit} for jobs in the VM Universe.
