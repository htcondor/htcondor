...
\item Added \MacroNI{OFFLINE\_LOG} and \MacroNI{OFFLINE\_EXPIRE\_ADS\_AFTER}
  configuration options.  They control where off-line (or hibernating) machine
  ads are stored persistently, and the lifetime of each persistent ad,
  respectively.
...
\item Added a few definitions and a new sub-section to the Green 
  Computing documentation. The new section, as well as the definitions,
  pertain to the handling of hibernating machines.  They explain how
  to keep track of hibernating machines in the Collector.
