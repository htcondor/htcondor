...
high traffic machine, this leak would eventually render the
\Condor{schedd} unresponsive.
...
\item Fixed an erroneous log message that would report that \Macro{HOOK\_UPDATE\_JOB\_INFO} had run, but would print the \Macro{HOOK\_PREPARE\_JOB} path.  The correct hook would run, this was only a logging error.  The log message is only visible at the D\_FULLDEBUG level.
