...
when removing jobs that were in the process of being submitting
with \Condor{submit} with the spool or remote options.
\Ticket{4866}

\item Fixed a bug that can cause the \Condor{schedd} to start more jobs
than specified by \Macro{MAX\_JOBS\_RUNNING} and later kill the excess
running jobs.
\Ticket{4554}

\item Fixed a minor bug that would emit a ``grep not found'' warning
when using \Condor{ssh\_to\_job} to login to a MacOS execute node.
\Ticket{4789}
