...
\item Fixed grid-type gce to work with the current version of Google
Compute Engine (GCE).
\Ticket{4586}

\item Improve cleanup of Google Compute Engine (GCE) instances that
are terminated.
\Ticket{4832}
...
identifies by submit commands \SubmitCmdNI{ec2\_access\_key\_id}
...
\item Fixed a rare bug that can cause the \Condor{schedd} to write to an
