...
\item HTCondor version 8.2.7 not yet released.
%\item HTCondor version 8.2.7 released on Month Date, 2014.
...
\item It has come to our attention that memory usage by the \Prog{ec2\_gahp}
may grow without bound in certain situations, particularly when jobs use
\Expr{x509://} URLs.  The problem lies in a dependency of \Prog{ec2\_gahp},
libcurl.  We believe the libcurl developers fixed this problem in version
7.24.0, and we've verified that version 7.38.0 does not have this problem.  If
you see the \Prog{ec2\_gahp}'s memory usage becoming too large, consider
upgrading your installation of libcurl.  If your operating system vendor
does not provide a new enough version of libcurl, you may need to build a
more recent version, and set \Macro{EC2\_GAHP} to a wrapper script which
ensures that the \Prog{ec2\_gahp} uses that version.
