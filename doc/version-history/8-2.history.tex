...
\item Fixed a bug that only manifested on Linux 3.14 kernels and
later that would cause the collector to respond very slowly
to queries.
\Ticket{4489}
...
version given by the CONDOR\_VERSION command, causing it to think that
its file transfer peer is the same version as itself.
\Ticket{4473}
