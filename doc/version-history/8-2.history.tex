...
groups to be unset when the shadow process is reused to run another job.
This could result in the job being held with the hold reason
``Failed to initialize user log to <path>''.
\Ticket{4672}

\item Fixed a bug in \Condor{schedd} that caused remote \Condor{history}
commands to fail if \Macro{LIBEXEC} wasn't explicitly set in the
configuration file.
\Ticket{4678}

\item Fixed a bug in \Condor{ganglia} that caused it to incorrecly log
that gemtric was being used when \Condor{reconfig} was invoked.
\Ticket{4680}
...
then the affected jobs will be placed in the HELD status.
Previously, any failure to talk to the remote daemon would result in
the \Condor{gridmanager} considering it temporarily unavailable and
waiting for it to become available again.
\Ticket{4557}
