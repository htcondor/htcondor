...
\item Updated the post install script in the RPM packages to preserve
the \File{/etc/condor/condor\_config.local} file if it was modified since
...
a hold reason of ``Unspecified gridmanager error'' when runtime
information wasn't reported by the remote server.
\Ticket{4736}

\item Updated Windows builds of HTCondor to use the latest version of OpenSSL.
\Ticket{4733}

\item Fixed a bug in \Condor{urlfetch} that caused it to sometimes not fetch the URL when
it should have because the cache file did not exist.
\Ticket{4732}

\item Fixed a bug in error checking while performing job output file
transfer when \Attr{OutputDestination} is used.
\Ticket{4739}
