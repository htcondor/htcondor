...
\item Fixed a bug introduced in HTCondor 8.2.4 that caused the daemon log
file of the \Condor{schedd} to not rotate when
...
for Condor-C jobs (i.e. grid universe jobs with grid-type ``condor'') to
fail.
\Ticket{3379}

\item Fixed a bug in which \Condor{qsub} mishandled setting a disk space
request with a command line argument similar to \Opt{-l file=2048MB}.
\Ticket{4606}
