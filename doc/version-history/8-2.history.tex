...
\item On Windows only, doing a \Condor{rm} of a \Condor{dagman} job
does not work correctly -- the \Condor{dagman} is immediately killed
and does not write a rescue DAG or remove its node jobs.  (Note that
this bug has probably existed since \Condor{dagman} was first
