...
\item Fixed a bug that happened on newer Linux kernels where
with cgroups enabled, the OOM killer would kill the job when it
went over the memory allocation.  Now the starter catches the OOM
signal and puts the job on hold with an appropriate message.
\Ticket{4435}
...
\item Fixed problems where job rank may not always work as documented
	due to a bug in HTCondor's autocluster mechanism.
\Ticket{4403}

\item Update the HTCondor DRMAA library to 1.6.2. This version fixes a
couple minor bugs in the functions for querying how a job exited.
\Ticket{4413}

\item \Condor{submit} no longer fails if the value of
\SubmitCmd{x509userproxy} is a relative path and \SubmitCmd{initialdir}
is set to a directory that's not the current working directory of
\Condor{submit}.
\Ticket{4415}
...
\item Fixed a bug that resulted in \Condor{q} \Opt{-format} of an non existent
attribute printing out "undefined" instead of nothing when using the \Opt{\%s}
format specifier.
\Ticket{4418}

\item Fixed a bug in \Condor{shadow} that caused the user's supplemental
groups to be unset when trying to write to the user job log.
This could result in the job being held with the hold reason
``Failed to initialize user log to <path>''.
\Ticket{4437}

\item Fixed a bug in \Prog{cream\_gahp} that would corrupt memory when
using more than the default number of worker threads.
\Ticket{4416}

\item Fixed a bug that could cause the \Prog{cream\_gahp} to fail at
startup because it couldn't locate a Globus threading library.
\Ticket{4440}
