...
\item Fixed a bug preventing \Condor{chirp} from finding its configuration
file in the default location.
\Ticket{4625}

\item Fixed a bug where \Condor{router\_q} would query the wrong queue if the
job router were configured to route jobs away from the source.
\Ticket{4599}
