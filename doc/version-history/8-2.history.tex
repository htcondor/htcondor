...
\item Using \condor{compile} on programs which call posix\_memalign()
will now work.
\Ticket{4486}

\item EC2 grid universe jobs which use the X.509 authentication method will
no longer crash if \Env{USER} is not set.
\Ticket{4540}
...
\item Fixed: \condor{qsub} mishandled setting a memory request with "-l mem=2048MB" or similar.
\Ticket{4549}
