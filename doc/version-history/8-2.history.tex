...
\item The new configuration parameter \Macro{NETWORK\_HOSTNAME} allows
you to set what hostname HTCondor uses to identify the local machine.
If \Macro{NETWORK\_HOSTNAME} isn't set, then HTCondor will use the
gethostname() function to determine the machine's hostname.
This parameter is useful if the machine has multiple network interfaces
...
\item On Windows, items in the QUEUE\_SUPER\_USERS list are now checked in a case-insensitive way,
since usernames are case-insensitive on Windows.
\Ticket{4579}

\item Fixed a bug that could prevent the \Condor{schedd}'s job queue log from rotating
on Windows.
\Ticket{4548}

\item Fixed a bug that caused all HTCondor daemons to leak a small amount of memory on reconfig.
\Ticket{4582}
...
\item Fixed a bug that sometimes prevented adding a .txt extension to the log file name of HTCondor
deamon on Windows.
\Ticket{4571}
