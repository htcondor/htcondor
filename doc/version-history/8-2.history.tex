...
\item Fixed a bug where we were looking for \Macro{IsOwner} instead of
\Macro{IS\_OWNER} in the configuration.  Now, you can specify either,
and \Macro{IsOwner} takes precedence.
\Ticket{4949}
...
These features are conflicting ways to efficiently allocate a
partitionable slot to multiple job requests. Enabling both for the same
slot causes problems.
\Ticket{4950}

\item Fixed a bug that could cause a partitionable slot to remain in
Matched state forever when \MacroNI{CONSUMPTION\_POLICY} and
\MacroNI{NEGOTIATOR\_INFORM\_STARTD} are set to \Expr{True}.
\Ticket{4945}
