...
\item Fixed an inconsistency where \Macro{JOB\_RENICE\_INCREMENT},
if not set, defaulted to 10 in vanilla universe and 0 in standard 
universe.  Now defaults to 0 everywhere, which matches what
...
\item HTCondor version 8.2.4 not yet released.
%\item HTCondor version 8.2.4 released on Month Date, 2014.
...
\item Fixed a bug wherein \Condor{schedd} versions 8.2 could
not send jobs to, or claim \Condor{startd}s running 8.0 or older.
\Ticket{4687}
...
a log message of ``child failed because PID XXX is still in use by DaemonCore''.
\Ticket{4646}
...
attributes in bytes rather than Kibibytes on Windows.
\Ticket{4638}

\item Fixed a bug that resulted in the \Condor{kbdd} on Windows sometimes
exiting with error 0xC0000374 which indicates heap corruption.
\Ticket{4634}

\item Changed the \Condor{master} to work around a bug in the c-runtime
on Windows that resulted in the \Condor{master} restarting whenever the
system clock was changed to account for daylight savings time.
\Ticket{3572}
