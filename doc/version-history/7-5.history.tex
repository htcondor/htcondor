...
%gittrac #1310
\item The in version 7.5.2 introduced configuration variable 
 \MacroNI{NEW\_LOCKING} has been changed to
 \MacroNI{CREATE\_LOCKS\_ON\_LOCAL\_DISK} and now defaults to \Expr{True}.
...
% gittrac #1717
\item Condor no longer tries to invoke glexec if the job's X.509 proxy
is expired.
...
% gittrac #1720
\item Using hostnames for host-based authentication (i.e. in the
\Macro{ALLOW\_*} and \Macro{DENY\_*} configuration parameters)
doesn't work on Mac OS X 10.4. Later versions of the OS are not
affected. As a workaround, IP addresses can be used instead.
