...
% gittrac #1947
\item If an individual Daemon ClassAd Hook manager is not ``named'', the
  jobs under it will attempt to use incorrectly named parameters. For
  example, the following will not work (because the \Condor{startd}
  Daemon ClassAd Hook will be fail to lookup the ``test'' job's
...
The code will incorrectly use \Macro{STARTD\_TEST\_MODE} and
\Macro{STARTD\_TEST\_EXECUTABLE} instead.

If instead, the Daemon ClassAd Hook Manager had been named, the code
...
This \emph{old} (named ClassAd Hook Manager) behavior is, as of 7.5.6,
documented as unsupported and going away (primarily because it's
confusing), but still works.  It is believed that this same behavior
exists in all 7.x releases of Condor, but because the naming feature
is used, it's gone undetected.

This affects the \MacroNI{STARTD\_CRON} and \MacroNI{SCHEDD\_CRON}
ClassAd Hook Managers, and will be fixed in Condor Version 7.6.0.
