...
\item \Condor{dagman} now prints a warning if an unrecognized macro is
...
\item The new configuration variable
  ...
  previous releases of Condor.  This configuration setting currently
  ...
  override this configuration setting by using the
...
% gittrac #1869
\item Job attribute \AdAttr{JobLeaseDuration} is now set appropriately
...
% gittrac #741
\item Doing \Condor{hold} on a \Condor{dagman} job on Windows no longer
removes the node jobs of the DAG (the behavior on Windows now matches
the behavior on other platforms).

% gittrac #1490
\item Doing \Condor{hold} and then \Condor{remove} on a \Condor{dagman}
...
% gittrac #1910
\item If a cycle exists in the OtherJobRemoveRequirements expressions (see
~\pageref{man-condor-submit-OtherJobRemoveRequirements}) of a set of jobs
(for example, if two jobs have OtherJobRemoveRequirements expressions that
attempt to remove the other job) removing any of the jobs will cause the
...
\item In Unix, the following executables used to be located in both
  ...
  directory: condor, \Condor{checkpoint}, \Condor{reschedule}, and
  \Condor{vacate}.
...
% gittrac #1849
\item Fixed a memory leak in \Condor{schedd} introduced in 7.5.4.
