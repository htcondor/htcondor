...
\item For clarity, the \Condor{startd} and \Condor{schedd} ``cron''
...
\item Updated Condor's Chirp protocol with new commands. The Chirp C++ client
 and \Condor{chirp} command were updated to use the new commands. See section
...
\item The \Condor{startd}'s benchmarks are no longer hard coded into
  ...
  shipped with Condor 7.5.6: \Condor{mips} and \Condor{kflops} (in the
  ...
  via the new \Macro{BENCHMARK\_LIST} macro.
...
values as \Expr{True}/\Expr{False} instead of 1/0 in the
\File{dagman.out} file.

% gittrac #1790
\item When job attributes are modified via \Condor{qedit}, the changes
are now propagated to the \Condor{shadow} and \Condor{gridmanager}. This
...
\item The Daemon ClassAd Hooks no longer support the ``old''
  ...
  \Macro{STARTD\_CRON\_JOBS} and \Macro{HAWKEYE\_JOBS} macros.  In
  previous versions of Condor, the \Condor{startd} would issue a
  warning if this syntax was found, but, starting with 7.5.6, any use
  of these macros will be ignored.  Instead, the
  \Macro{STARTD\_CRON\_JOBLIST} syntax should be used.

% gittrac #434
\item Added the configuration variables \Macro{DAGMAN\_VERBOSITY},
\Macro{DAGMAN\_MAX\_PRE\_SCRIPTS}, \Macro{DAGMAN\_MAX\_POST\_SCRIPTS},
and \Macro{DAGMAN\_ALLOW\_LOG\_ERROR} (see
...
\item A bug has been fixed that caused SOAP transactions in the
  \Condor{schedd} daemon to result in log message of the form 
\begin{verbatim}
Timer <X> not found
\end{verbatim}
...
\item A bug has been fixed in \Condor{config\_val} program which
  caused it to try to contact the \Condor{collector} before printing
