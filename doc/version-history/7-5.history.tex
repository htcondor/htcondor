...
%\item Condor version 7.5.3 released on June 23, 2010.
...
be overriden by higher levels in the DAG splicing structure.
...
\item Daemon logs can now be rotated several times instead of only once 
  ...
  configuration variables \Macro{MAX\_NUM\_<SUBSYS>\_LOG} need to be set 
  to a value greater than 1. The file endings will be ISO timestamps, wheras
  the current oldest rotated file will still have the ending \File{.old}.
...
\item The new configuration variable \Macro{MAX\_NUM\_<SUBSYS>\_LOG} 
  will determine how often the daemon log of \Macro{SUBSYS} will rotate.
  The default value is 1 which leads to the old behavior of a single 
  rotation into a \emph{.old} file.
