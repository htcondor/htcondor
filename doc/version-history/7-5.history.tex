...
\item When \MacroNI{NOT\_RESPONDING\_WANT\_CORE} is \Expr{True}, the
...
which the child process gets hung while writing a core file.
...
% gittrac #1652
\item The configuration parameter \MacroNI{STARTD\_SENDS\_ALIVES} now only
needs to be set for the \Condor{schedd}. Also, the default value has
changed to \Expr{True}.

% gittrac #1593
\item The job attributes \SubmitCmd{amazon\_user\_data} and
...
with the value of \SubmitCmd{amazon\_user\_data} occurring first.

% gittrac #1653
\item The new configuration parameter \MacroNI{GRAM\_VERSION\_DETECTION}
can be used to disable Condor's attempts to distinguish between \Expr{gt2}
(i.e. GRAM2) and \Expr{gt5} (i.e. GRAM5) servers.
...
\item Fixed the output in the \Condor{master} daemon log file to be
  ...
  \Opt{-set} and \Macro{ENABLE\_PERSISTENT\_CONFIG} is false. The previous
...
% gittrac 1139
\item \Condor{master} now invalidates its ClassAd, as represented in the 
  \Condor{collector} daemon before it shuts down.
...
containing a space was used with the submit attributes
...
a GAHP server it's using writes a large amount of data to its stderr.

% gittrac #1653
% gittarc #1475
\item Fixed a bug that could cause the \Condor{gridmanager} to wrongly
conclude that a \Expr{gt2} (i.e. GRAM2) server was a \Expr{gt5} (i.e. GRAM5)
server. Such a conclusion can be disastrous, as Condor's mechanisms to
prevent overloading a \Expr{gt2} server are then disabled. New configuration
parameter \MacroNI{GRAM\_VERSION\_DETECTION} can be used to disable Condor's
