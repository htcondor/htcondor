...
delegations done between Condor daemons and tools.  The value is a boolean
...
between GRAM2 and GRAM5 servers (i.e. grid-types 'gt2' and 'gt5').
Users can submit jobs using a grid-type of 'gt2' or 'gt5' for either
type of server.
...
% gittrac #1100
\item Added new submit file attribute \Macro{cream\_attributes}. When
submitting a grid universe job destined for the CREAM grid system, this
attribute can be used to add addtional attribute/value pairs to the CREAM
job description that Condor creates.
...
% gittrac #1128
\item The submit file attribute \Macro{vm\_macaddr} can now be used to set
...
% gittrac #1173
\item \Macro{condor\_q -globus} is now much more efficient in its
communication with the \Condor{schedd}.
...
% gittrac #1196
\item Configuration parameter \Macro{LOCAL\_CONFIG\_DIR} is no longer
