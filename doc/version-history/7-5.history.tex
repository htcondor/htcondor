...
% gittrac #1897
\item This development series of Condor is no longer officially released for the 
  platforms AIX PPC, PPC 64 SLES 9, PPC MacOS 10.4, Solaris 5.9, Solaris 5.10, 
  IA64 RHAS 3, PS3 YDL 5.0, and x86 Debian 4.
...
\item New script \Condor{gather\_info} (in \File{bin/}) creates reports with 
  information out of a Condor pool about a specific job id. It also 
  gathers some understanding of the pool under which it runs.
...
\item \Condor{q} -better-analyze now identifies jobs that have not yet been 
  coonsidered by matchmaking, instead of characterizing them as not 
...
\item Added new parameter \Macro{CONDOR\_VIEW\_CLASSAD\_TYPES}, a
  string list, that specifies the types of ads that will be forwarded to
  a \Macro{CONDOR\_VIEW\_HOST}. 
  Behavior is defined in section~\ref{sec:Collector-Config-File-Entries}.

% gittrac  #677
\item Added a -local-name command line flag to
...
% gittrac #1892
\item Fixed a bug in the parallel universe introduced in 7.5.5
    where the schedd would crash under certain conditions when
...
%gittrac #677
\item Condor in the 7.5.0 to 7.5.5 version range failed to
  implement the ``local name'' functionality properly. This was a regression
  from the Condor 7.4 series. It is now fixed and the local name functionality
  has been restored.
