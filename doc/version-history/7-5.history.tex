...
\item Under Linux, if the \Condor{schedd} daemon's spool directory is
  on an ext3 filesystem, Condor can now scale to a larger number
  of spooled jobs.  Previously, Condor created two sub-directories
  within the spool directory for each spooled job and for each running
  job.  The ext3 filesystem only supports 31997 sub-directories.  This
  effectively limited the number of spooled jobs to less than 16
  thousand.  Now, Condor creates a hieararchy of sub-directories within
  the spool directory to increase the limit of number of spooled jobs
  in ext3 to 320 million, which is likely to be larger than other limits
