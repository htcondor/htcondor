...
\item Condor now uses the version of ClassAds that we provide as a stand-alone 
library (\URL{http://www.cs.wisc.edu/condor/classad/}). Previously, Condor 
used an older version of ClassAds that was heavily tied to the Condor 
development libraries. This change should be largely transparent in the 
current development series. In the next development series (7.7.x), we  
will start to use features of ClassAds that were unavailable in the 
older library. 
See section~\ref{sec:classad-differences} for details.
...
% gittrac #1138
\item The \Condor{q} command with option \Opt{-analyze} is now identical to
the option \Opt{-better-analyze}.
The \Opt{-better-analyze} option includes a superset of information,
and also includes more specific information on a job's \SubmitCmd{requirements}.
...
\item The new configuration parameter \MacroNI{TARGET\_MACHINE\_ATTRS}
specifies a list of machine ad attributes that should have 'Target.'
prepended to them in any job ad expressions.

\item The new configuration parameter \MacroNI{TARGET\_JOB\_ATTRS}
specifies a list of job ad attributes that should have 'Target.'
prepended to them in any machine ad expressions.
...
% gittrack #1141
\item The default value for \MacroNI{SEC\_DEFAULT\_SESSION\_DURATION}
  was effectively 3600 in 7.5.0.  This produced longer than desired
  cached sessions for short-lived tools such as \Condor{status}.  It
  also produced shorter than possibly desired cached sessions for
  long-lived daemons such as \Condor{startd}.  The default has been
  restored to what it was before 7.5.0, with the exception of
  \Condor{submit}, which has been changed from 1 hour to 60s.  For
...
% gittrack #767
\item As of 7.5.0, the following warning was logged in the
  \Condor{starter}'s log even when the deprecated configuration
