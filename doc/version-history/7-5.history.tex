...
%gittrac 1752
\item The \Condor{dagman}/\Condor{submit\_dag} command-line flag
\Opt{-DumpRescue} now works even if parsing of the DAG file fails (to
help in figuring out what went wrong).  In this case, \Condor{dagman}
produces a file called \Arg{dag\_file}.parse\_failed, which has the
new \Macro{NO\_RUN} keyword (this prevents DAGMan from running the
resulting DAG file, since it is likely to be syntactically correct,
but incomplete).
