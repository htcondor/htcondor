...
% gittrac #1689
\item Fixed a bug introduced in Condor 7.5.3. When file transfer failed
for a Cream grid universe job, Condor would write a hold event to the job
log, but not actually put the job on hold.

% gittrac #1694
\item Fixed a bug in the \Condor{gridmanager} that could cause it to crash
while handling grid-type cream jobs destined for different resources.
