...
% gittrac #1661
\item The default UNIX Sys-V init script has been competely reworked.
  In addition to new features (see New Features, below), this changes
  the following:
  \begin{itemize}
    \item The default location of the Condor configuration file is now
      ``/etc/condor/condor\_config''.  This can be overriden by
      editing the syconfig file or the init script itself.
    \item The default location of the Condor installation is now
      ``/usr/'', with binaries in ``/usr/bin'' and ``/usr/sbin''.
      This can be overriden by editing the syconfig file or the init
      script itself.
  \end{itemize}
...
\item The default UNIX Sys-V init script has been competely reworked.
  ...
  Major features / changes in the new script:
  \begin{itemize}
    \item Supports the use of a Linux-style ``sysconfig'' file
