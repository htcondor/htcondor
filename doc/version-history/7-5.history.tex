...
%gittrac #1479
\item \Condor{dagman} now prints the number of queued jobs in the DAG
that are on hold,
...
%gittrac #825
\item \Condor{dagman} now prints a note when the \Condor{submit\_dag}
and \Condor{dagman} versions differ, even if the difference is
permissible.

% gittrac #61
\item Added default values for \MacroNI{<SUBSYS>\_LOG} configurations. If a 
  \MacroNI{<SUBSYS>\_LOG} configuration variable is not set in 
  \File{condor\_config} or \File{condor\_config.local} it will default to 
  \MacroNI{\$(LOG)/<SUBSYS>LOG}.
...
\item Since 7.5.2, the following \Condor{job\_router} features were
  ...
  \Attr{JobShouldBeSandbox}, and \Attr{JobFailureTest}.
