...
%\item Condor version 7.5.6 released on Month Date, 2010.
...
% gittrack 1800
\item \MacroNI{NETWORK\_INTERFACE} and
  \MacroNI{PRIVATE\_NETWORK\_INTERFACE} may now specify a network
...
\item This version of Condor uses a different layout in the spool
  ...
  back to the older format, stop Condor and backup the spool directory
  ...
  \verb|$(SPOOL)/<#>/cluster<#>.ickpt.subproc<#>|.  Then edit
...
% gittrac 1706
\item Under Linux, if the \Condor{schedd} daemon's spool directory is
  ...
  job.  The ext3 file system only supports 31997 subdirectories.  This
...
%gittrac 793
\item The \Condor{shadow} daemon uses less memory than it has since 7.5.0.
Memory usage should now be similar to the 7.4 series.

%gittrac #1077
% Note: add link here when the actual manual section is updated.
...
% gittrac 1773
\item Since 7.5.2, \Condor{q} speed was not as high as earlier 7.5 and
  7.4 releases, especially when retrieving large numbers of jobs.
  Viewing 100k jobs took about four times longer.  This has been fixed
  in this release, and performance should be about the same as before
  7.5.2.

% gittrac #1737
\item A bug introduced in Condor version 7.5.4 prevented parallel jobs
with multiple \SubmitCmd{queue} statements in the submit description file
from working with \Condor{dagman}.  This is now fixed.
...
macro rather than the user's coredumpsize limit.

% gittrac #1777
\item Fixed a case introduced in 7.5.4 under Windows in which the following
spurious log message was produced:
...
% gittrac #1088
\item Since 7.4.1, Condor-C jobs submitted without file transfer enabled could
...
\item Fixed a memory leak caused by use of the ClassAd \Expr{eval()}
  function.  This problem was introduced in 7.5.2.
