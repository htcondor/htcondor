...
\item On previous versions of condor, the starter could not 
write the .machine.ad and .job.ad files to the execute directory
when PrivSep was enabled.  This has now been fixed, and these files
...
%gittrac #1641
\item Added new job status TRANSFERRING\_OUTPUT. This status is used
when transferring a job's output files after it has finished running.
Jobs with this status will have their \AdAttr{JobStatus} attribute set to 6.
The standard \Condor{q} display will show the job's status as `>'.
...
% gittrac #1849
\item Fixed a memory leak in \Condor{schedd} introduced in Condor version 7.5.4.
...
% gittrac #1828
\item \MacroNI{SCHEDD\_QUERY\_WORKERS} was effectively ignored when
  \Condor{q} authenticated itself to the schedd.  The query was always
  processed in the main schedd process rather than in a sub-process.
...
\item Fixed a problem affecting jobs that store their output in the
  ...
  transfer.  This problem was introduced in 7.5.4 when support for
  transfer of directories was added.

% gittrac #1749
\item Fixed a problem affecting the \Condor{master} daemon since
  7.5.3.  The \Condor{master} daemon would crash if it was instructed
  to shut down a daemon that was not currently running but which was
...
undercount the number of running local and scheduler universe jobs. In
Condor 7.5.4, this undercounting could cause the daemon to crash.

% gittrac #1695
\item Fixed a bug that could cause the \Condor{gridmanager} to crash if
a GAHP server didn't behave as expected on startup.

% gittrac #1747
\item Improved the hold reason in several failure cases for CREAM grid jobs.

% gittrac #1771
\item The KFLOPS reported by \Condor{status} \Opt{-run} \Opt{-tot} could erroneously be reported as negative.  This has been fixed.

% gittrac #1867
\item Since 7.5.4 in Condor-C, refreshing the proxy of the job in the
  remote queue did not work.  Therefore, if the original job proxy
