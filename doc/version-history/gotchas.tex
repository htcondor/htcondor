...
\item  Previously, Condor's RPM packaging installed into \File{/opt}. Now
	...
	into appropriate subdirectories in \File{/usr}.  See
...
\item  For those who compile Condor from the source code, rather than using
	packages of pre-built executables, Condor is now built using \Prog{cmake}
	on all platforms. This means the process for building Condor from source
	code has changed. See the Condor Wiki
	(\URL{http://wiki.condorproject.org}) for instructions on building Condor.
...
\item  Quill and Stork are no longer included in the binary packages
	...
	\Term{Contribution Modules}. Contribution Modules are optional packages
...
\item The layout in the \MacroNI{SPOOL} directory has changed. As a result,
...
\item Issues regarding detection of keyboard activity on Linux and
	...
	platforms, simply ensure that the subsystem \Expr{KBDD} is listed in the
	\MacroNI{DAEMON\_LIST} configuration variable definition on startup.
