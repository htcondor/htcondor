...
be world-writable, as long as it's not on a root-squashed NFS mount and is
...
time not downloadable as a binary package.
...
configured to use the Network Service Cache Daemon (nscd) checkpointing
...
\item The \Condor{procd} no longer truncates its log file on startup.
...
scheduler universe jobs (e.g. Condor cron jobs).
