...
\item Fixed a buffer overflow which could cause the \Condor{schedd} to crash.

\item Fixed a stack overflows in the \Condor{Negotiator}.

\item Fixed \Condor{submit} \Opt{-dump} such that it would function with the
standard universe.

\item Fixed a memory leak in the \Condor{startd} which happened during a
\Condor{reconfig}.
