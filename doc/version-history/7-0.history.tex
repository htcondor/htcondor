...
\item \Condor{configure} (and condor{install}) now detects missing
  ...
  line option ``--ignore-missing-libs'' will cause it to not exit
  after the messages have been printed, and to proceed with the
  installation.

\item Added a ``--force'' command line option to \Condor{configure}
  (and condor{install}) which will turn on ``--overwrite'' and
  ``--ignore-missing-libs''.
...
\item In Linux, the job queue database file may now exceed 2GB.  Previously,
the \Condor{schedd} would halt with an error when trying to write past
the 2GB mark.
