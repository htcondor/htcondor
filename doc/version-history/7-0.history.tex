...
\item \Macro{VALID\_SPOOL\_FILES} now automatically includes "SCHEDD.lock",
...
the time of job suspension/unsuspension and the corresponding entries
in the job userlog.

\item Fixed a problem in \Condor{q} \Opt{-better-analyze} handling
requirements expressions containing anything =!= UNDEFINED.

\item Configuration parameter \Macro{GRIDMANAGER\_GAHP\_CALL\_TIMEOUT}
is now recognized for nordugrid grid universe jobs.

\item Fixed a bug that could cause \Condor{schedd} to abort (and restart)
some time after a graceful restart when jobs to which the schedd reconnected
were preempted.

\item Fixed a bug causing failure to reconnect to jobs which use
\$\$([\textit{expression}]) in their ClassAds.  The jobs would go on
hold with a hold reason, ``Cannot expand \$\$([\textit{expression}]).''
...
\item A clipped port to FreeBSD 7.0 x86 and x86\_64 is available.
...
\item Fixed a bug that cause the Quill daemon to crash.
...
script expects to be run as root not only on initial start up but on restart
as well.
...
(e.g. \MacroNI{SEC\_DEFAULT\_NEGOTIATION=NEVER}).  This bug was introduced
in Condor 7.0.0.
