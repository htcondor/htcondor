...
of wildcards in authorization lists.
...
use of wildcards in any \MacroNI{DENY} list,
...
\item When the configuration paramter \Expr{SCHEDD\_NAME} ends in `@',
the \Condor{schedd} will no longer insert its hostname into the value.
This makes it possible to configure a fail-over \Condor{schedd} on a
backup machine.
...
\item Added configuration variable: \Expr{LOCK\_FILE\_UPDATE\_INTERVAL}.
...
on Windows if more than 2 Condor-C jobs were submitted at the same time.
...
to crash when starting parallel or MPI universe jobs.  In some cases,
...
\item The \Condor{procd} now periodically updates the timestamps on
...
to fail on startup on Windows 2000.

\item Fixed a problem where standard universe jobs would fail to start
when using PrivSep, if the \Macro{PROCD\_ADDRESS} setting were not
defined.
...
\item If attributes \Attr{StreamOut} or \Attr{StreamErr} are missing from
the job classads of grid universe jobs, the default value is now
\Attr{False}.
