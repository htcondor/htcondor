...
\item When \Macro{NEGOTIATOR\_CONSIDER\_PREEMPTION} is defined to be false,
this no longer results in machines in the Owner state being ignored
during matchmaking.  Previously, even if \MacroNI{START} was true,
machines in the Owner state were disregarded.

\item Setting \Attr{JobLeaseDuration} less than 15 minutes caused the
\Condor{schedd} to abort and restart the next time \Condor{reconfig} was
run.  The error message in the \Condor{schedd} log looked like this:
...
\Condor{schedd}, and \Condor{collector}.  This leak would probably
require many months of continuous operation before causing noticeable problems.

\item Fixed a \Condor{schedd} crash when doing a fast shutdown of running
local universe jobs or any job that had been reconnected to when the schedd
started up.
...
\Expr{SEC\_DEFAULT\_SESSION\_DURATION} from 8640000 seconds (100 days)
