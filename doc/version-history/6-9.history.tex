...
\item Implemented completely new version of Quill.  Quill can now
record information about all the daemons into a relational database.
See the quill chapter for more details.

\item Added VM universe, to allow running of jobs under the xen or
VMware virtual machines.

\item Improved speed of the \Condor{schedd} writing to its database
...
\item The \Condor{startd} now records historical statistics about the
  ...
  state/activity pairs, the \AdAttr{TotalTime...} attribute for that
...
\item A new parameter \Macro{CronWindow} has been added. It is an alias
to \Macro{DeferralWindow}.
...
\item Set default parameters for the \Condor{dbmsd}. The new settings
are \Macro{DBMSD}, which is set to ``\Code{DBMSD = \$(SBIN)/condor\_dbmsd}''
, \Macro{DBMSD\_ARGS}, which is set to ``\Code{DBMSD\_ARGS = -f}'', and
\Macro{DBMSD\_LOG}, which is set to ``\Code{DBMSD\_LOG = \$(LOG)/DbmsdLog}''
