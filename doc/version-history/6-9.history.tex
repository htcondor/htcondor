...
\item Fixed a bug introduced in 6.9.3 that caused failure to reconnect to jobs
that were left running when the \Condor{schedd} shut down.

% Gnat PR 652
\item Trailing commas in the configuration parameters \Attr{DAEMON\_LIST},
\Attr{DC\_DAEMON\_LIST}, and \Attr{MASTER\_HA\_LIST} are now ignored.
Before, they could cause the \Condor{master} to misbehave, including exiting
with an error.

\item when \Attr{LOG\_ON\_NFS\_IS\_ERROR} is set to false, do not emit
a warning about user logs files being on NFS.
