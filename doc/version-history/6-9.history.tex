...
\item As of version 6.9.3, the entire Condor system has undergone a
  ...
  Back when we choose this terminology, it made since, since each of
  these resource was like an independent machine in your pool, with
  its own state, classified ad, claims, and so on.
  ...
  created using tools like VMware or Xen -- entire operating systems
  running inside a given process (usually emulating the underlying
  hardware) on a host machine.
  So, to avoid confusion with these other kinds of virtual machines,
  the old  \Term{virtual machine} terminology has been changed, and we
  now use the term \Term{slot}.

  Numerous configuration settings, command-line arguments to Condor
  tools, classified ad attribute names, and so on, have all been
...
Condor installation to be more compatible with FreeBSD.
