...
\item Fixed a bug where \Condor{dagman}, if removed from the queue via
      \Condor{rm}, would fail to remove jobs it had just submitted, if
      those jobs' submit events had not yet appeared in the userlog.
...
machines (or vice versa) failes for machine\_count > 1.
