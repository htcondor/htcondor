...
\item Fixed a DAGMan bug where DAGMan failed to monitor Stork job logs.

\item Fixed a bug under Windows which could cause Condor to kill processes that
...
\item Fixed a segfaulting condition under Windows when the daemons or tools
	looked up information about processes running on the system.

\item Fixed a bug where the daemons could crash if \Macro{COLLECTOR\_HOST}
	or \Macro{NEGOTIATOR\_HOST} was defined to be something bogus.

\item Fixed a bug that would crash the \Condor{collector} when
  \Macro{COLLECTOR\_NAME} was too long.

\item Fixed a minor bug in file transfer.  Specifically, if your VANILLA
  job had \Expr{when\_to\_transfer\_output} set to ON\_EXIT\_OR\_EVICT, wrote
  more than one output file, and was actually evicted, the \Condor{shadow}
  would EXCEPT and your job would be rerun.

\item Fixed an error in \Condor{dagman}: if submit files for individual
  nodes referred to the same log file with different paths, \Condor{dagman}
  would read log events incorrectly and the DAG would fail.  \Condor{dagman}
  is now able to recognize that the different paths actually refer to
  the same log file.

\item Fixed a bug where the environment of jobs in any universe could be
	corrupted.

\item Fixed a bug which would result in the segfaulting of a daemon while it
	was writing to its logfile.

\item Fixed a bug in the checkpoint server related to signal handling which
	caused the daemon to wedge sometimes.
...
\item The default setting for \Macro{POOL\_HISTORY\_DIR} is no
  longer \Macro{SPOOL}.  Using the spool directory would result
  in history files being obliterated by \Condor{preen}.

\item The \Condor{startd} now properly cleans up execute directories on
  root-squashed NFS mounts.

\item The \Attr{image\_size} attribute of a job on Mac OS X is much closer
to the values that ps returns. Previously it would be highly inflated.

\item If a node submit file doesn't specify a log file, the warning message
	now gets printed out in the the DAGMan log file.
