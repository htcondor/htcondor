...
\item In version 6.6.7, we fixed a number of bugs related to the
  ...
  However, some sites were using \Opt{-format} in ways we didn't
...
\item Fixed an issue that would prevent \Condor{store\_cred} to succeed
...
\item Fixed a bug that would cause the RemoveReason attribute for a job 
  to be set incorrectly in some circumstances. (Specifically, when a job
  was not running and a \AdAttr{periodic\_remove} expression caused the job to be
  cancelled.)

\item Fixed \Condor{submit} so that for all possible submit file
  settings, both \verb@ThisStyle@ and \verb@this_style@ will work.
