...
\item Fixed a bug in \Condor{dagman} which could cause it to ASSERT()
improperly if it was removed from the queue while it had waiting PRE
scripts throttled.
...
\item On Windows, it was possible under certain circumstances for execute
directories to not be cleaned up properly. This has been fixed.

\item On Windows, certain Asian locales would cause the \Condor{starter}
to crash due to character translation problems. This has been fixed.

\item Condor will now properly report memory sizes that exceed 2 GB
on Windows.

\item On Windows, the \Condor{starter} would be unable to run jobs if
the \verb@LOG@ path had a period (.) in it. This has been fixed.
