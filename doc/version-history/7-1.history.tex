...
\item The StarterLog for local universe jobs now displays the job id
in each line in the file so that intermingled messages relevant to
different jobs running concurrently can be understood.
...
\item Fixed a bug in the View Collector that caused it to ``except''
  (crash) when it received a machine ad without a valid state.  It now
  logs this (D\_ALWAYS) and ignores the ad.
