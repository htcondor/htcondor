...
DAGs) to \Condor{dagman}.  See \pageref{dagman:SUBDAG} for details.
...
\item The \Condor{lease\_manager} daemon has been added.  This daemon
  ...
  persisted to disk by the \Condor{lease\_manager}.  The documentation
...
\item Condor can now be configured to keep a backup of the job queue
  log on a local filesystem in case \Condor{schedd} operations
...
\item The configuration parameter
  ...
  \Macro{FAILED}, the \Condor{schedd} will attempt to keep a backup
  ...
  to none \Macro{NONE}, no backups should be performed even in the
  event of failure.  If it is set to \Macro{ALL}, then at all
  transactions should be backed up.  (The \Macro{ALL} option will
