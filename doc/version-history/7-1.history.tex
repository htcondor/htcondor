...
\item The BIND\_ALL\_INTERFACES knob now defaults to true.

\item The PrepareJobHook can now modify the job ad before execution.
...
\item Added \MacroNI{SEC\_DEFAULT\_AUTHENTICATION\_TIMEOUT}, where
\emph{DEFAULT} may be replaced by the usual list of contexts for
security settings (e.g. \MacroNI{CLIENT}, \MacroNI{READ},
\MacroNI{WRITE}).  This specifies the number of seconds Condor should
allow for authentication of network connections to complete.  Previously,
GSI authentication was hard-coded to allow 5 minutes for authentication.
Now it uses the same default as all other methods: 20 seconds.

\item Added \MacroNI{STARTER\_UPDATE\_INTERVAL\_TIMESLICE}.  This
specifies the highest fraction of time the starter should spend
...
long time, the starter will monitor less frequently than specified by
...
\item Fixed bug in Java universe where each slot would be told to
potentially use all the memory on the machine.  Now, each java vm
gets the physical memory divided by the number of slots.

\item On Windows, slot users would sometimes show up in the Windows Welcome
  Screen.  This has now been resolved (but the slot users need to be manually
