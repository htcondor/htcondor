...
\item Minor improvements for Computing On Demand (COD) support: 
  Simplified the interface to the \Condor{cod} command-line tool
...
\item PVM: Added support for \Procedure{pvm\_export}.
...
\item Fixed a minor problem (introduced in version 6.5.2 while fixing
  ...
  \Macro{DAEMON\_LIST}, and define \Macro{HAWKEYE} to be the path to
  the \Emph{hawkeye\_master} program, the \Condor{master} will spawn
  everything correctly.

\item PVM: Fixed the \Attr{Environment} attribute in the submit description 
