...
%\item HTCondor version 8.1.5 released on Month Date, 2013.
...
\item Fixed bug in \Condor{on}, \Condor{off}, \Condor{restart},
\Condor{reconfig}, and \Condor{set\_shutdown}. When multiple machines
were named on the command line, these tools could report 
``Can't find address for master XXXX'' for some daemons even though
the daemons are being advertised properly in the \Condor{collector}.
\Ticket{4207}

\item Fixed a bug that can cause the \Condor{startd} to become unresponsive
when starting a job obtained via the Work Fetch Hook.
\Ticket{4210}
...
\item Add ganglia statistics for total job starts, total job preemptions
in a startd.  This allows ganglia to graph the total job preemptions
across all startds in a pool.
\Ticket{3965}
...
\item Several general mechanisms for specifying user-defined \Condor{startd} resources
have been enhanced so that GPUs can be easily defined and used with HTCondor. This includes
having HTCondor automatically set \Env{CUDA\_VISIBLE\_DEVICES} for GPU jobs.

\Macro{MACHINE\_RESOURCE\_<name>} and \Macro{MACHINE\_RESOURCE\_INVENTORY\_<name>}
now support the the definition user-defined resources as a list of resource
identifiers.  When HTCondor allocates a user-defined resource to a slot, if the resource
has identifiers, then HTCondor will assign specific instances of the resouce and
publish the assignment into the slot ClassAd using the new attribute \Attr{Assigned<name>}.
It will also publish the assignment into the job's environment as \Env{\_CONDOR\_Assigned<name>}
If the new configuration variable \Macro{ENVIRONMENT\_FOR\_Assigned<name>} is specified,
HTCondor will also publish the assigned resources into the job's environment using the
environment variables given in this configuration variable.
\Ticket{4141}
\Ticket{4148}
...
\item A new configuration constant \Macro{DETECTED\_PHYSICAL\_CPUS} contains
the number of physical (non-hyperthreaded) CPUs. An existing configuration
constant \Macro{DETECTED\_CORES} contains the number of hyperthreaded CPUs.
A new configuration constant \Macro{DETECTED\_CPUS} will match the value of
either \Macro{DETECTED\_CORES} or \Macro{DETECTED\_PHYSICAL\_CPUS} depending
on the state of \Macro{COUNT\_HYPERTHREAD\_CPUS}. These new \Macro{DETECTED\_}
configuration constants are meant to be referenced in other configuration values;
they cannot be overridden.
The \Macro{NUM\_CPUS} is now set by default to \Macro{DETECTED\_CPUS}.
\Ticket{4197}
...
\item the \Condor{startd} will no longer abort when it runs out of user-defined
resources like GPUs while assigning automatic resource shares to slots.
\Ticket{4176}
