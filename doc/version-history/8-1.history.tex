...
\item Added \Macro{EC2\_RESOURCE\_TIMEOUT}, which controls how long HTCondor
will wait for an unresponsive EC2 service before putting the corresponding
jobs on hold.
\Ticket{4113}
...
\item HTCondor now treats a failure to terminate an EC2 instance
because the instance no longer exists as a success.  This will allow EC2
jobs to exit the queue if the service purges termination records quickly.
\Ticket{4133}
