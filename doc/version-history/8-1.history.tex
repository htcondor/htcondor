...
\item The \Condor{negotiator} records maxdelta and round-robin information
in the log.
\Ticket{3871}
...
\item The temperature attributes returned by \Condor{gpu\_discovery} contained
values in Celsius, but ended in F to indicate Fahrenheit. The endings have been
changed to C to correctly indicate Celsius. For instance \Attr{CUDA<n>DieTempF}
has been changed to \Attr{CUDA<n>DieTempC}.
\Ticket{4294}

\item Fixed a bug that caused the \Condor{startd} to report the
following when a plugin couldn't be run:
\begin{verbatim}
