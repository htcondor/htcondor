...
\item The \Condor{config\_val} tool now supports the \Opt{dump} and \Opt{verbose}
options when querying configuration values remotely from daemons via the
\Opt{master}, \Opt{collector}, \Opt{startd}, \Opt{schedd}, and \Opt{negotiator}
options.
\Ticket{3894}
...
\item The \Condor{history} command line to has been enhanced to support
remote history via the \Opt{pool} and \Opt{name} options.
\Ticket{3897}
