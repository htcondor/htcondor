...
\item In the \Condor{startd}, changed the way cpus are allocated to
slots in configurations where there are more slots than cpus.
Cpus are now distributed equally between slots that aren't configured
to receive a specific number (via parameter \Macro{SLOT\_TYPE\_<N>}).
Before, these slots received 1 cpu each.
