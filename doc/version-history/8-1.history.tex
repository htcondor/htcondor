...
\item The python bindings can now accept a list of collector host in the
constructor of the Collector object.  This eases the use the bindings for
...
\item Fixed a bug that can cause the \Condor{schedd} to advertise a stale
address in the ScheddIpAddr attribute of its submitter ads, resulting in
other daemons being unable to contact it.
The problem occurs when using both the shared-port daemon and CCB, and
the \Macro{CCB\_ADDRESS} is chanaged.
\Ticket{4250}
