...
The dedicated scheduler cpu usage within the \Condor{schedd} is now
throttled, so that it cannot consume all of the cpu, starving the vanilla
scheduler.  This throttle can be adjusted by the new knob
\Macro{DEDICATED\_SCHEDULER\_DELAY\_FACTOR}.  This knob, which defaults to five,
sets the ratio of time spent not in the dedicated scheduler to the ratio
of time scheduling parallel jobs.  That is, with the default of five, 
a max of 20 percent of the schedulers time will go to scheduling
parallel jobs.
\Ticket{4048}
