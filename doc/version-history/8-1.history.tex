...
\item When terminating a job on a non-Amazon service,
check (at the next polling interval) if the job has terminated,
and if not, retry.  This means that HTCondor will not lose track of jobs
on services with unreliable termination.
\Ticket{3438}
