...
\item The \Macro{ALLOW\_*} and \Macro{DENY\_*} access lists may now include netgroups,
using the syntax +netgroup\_name.  \Macro{QUEUE\_SUPER\_USERS} may now include 
...
\item \Attr{Disk} and \Attr{TotalDisk} are now published into the machine ClassAd as
64-bit integers rather than being capped at the maximum value of a 32-bit integer.
\Ticket{1784}

\item In an effort to improve scalability under heavy load, a new tuning
	parameter \Macro{MAX\_REAPS\_PER\_CYCLE} was exposed (see
...
signal is now successfully sent to the job.  Before, a \Condor{rm}
