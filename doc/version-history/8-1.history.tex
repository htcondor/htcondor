...
\item In the Pegasus/\Condor{dagman} workflow metrics
(see ~\pageref{sec:DAGMetrics}),
\Expr{dagman\_id} and \Expr{parent\_dagman\_id} are now reported
simply as the HTCondor cluster ID of the \Condor{dagman} job.  This
eliminates any privacy concerns with reporting the schedd address
(which includes the submit machine's IP address).

\item The python bindings now can perform the equivalent of \texttt{condor\_ping}
as a part of the SecMan object.
\Ticket{3857}
