...
\item Fixed bug with \Condor{submit} -interactive when
...
\item Fixed the cURL file transfer plugin on Windows.
Previously cURL was not properly initializing Winsock, 
so all network operations would fail.
\Ticket{3979}

\item Fixed a bug that caused grid-type cream jobs to fail if the submit
description file contained the submit command \SubmitCmd{environment} or
