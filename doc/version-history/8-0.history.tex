...
\item Fixed a bug that could cause the negotiator to give out the same
	...
	wording ``INSANE: bestCached != bestSoFar''.
\Ticket{2245}

\item Added \AdAttr{ConcurrencyLimits} to the projection for claimed
resources in \Condor{negotiator}, so that concurrency limit accounting 
works properly across negotiation cycles when 
\Macro{NEGOTIATOR\_CONSIDER\_PREEMPTION} is \Expr{False}.
\Ticket{3815}

\item As of 7.9.6, when using CCB to connect to the \Condor{startd},
  the \Condor{negotiator} would sometimes crash (and be restarted)
...
\end{verbatim}
or to disable communication with the \Condor{startd} by setting
\begin{verbatim}
...
\item When invoking \Prog{glexec}, \Env{GLEXEC\_TARGET\_PROXY} is now
  set to \File{/dev/null}.  In some situations, it was previously set
...
\item Fixed a bug in \Condor{dagman} that could (in rare cases) cause
\Condor{dagman} to hang when using the
\OptArg{-maxidle}/\Macro{DAGMAN\_MAX\_JOBS\_IDLE} option.
\Ticket{3834}
