...
\item The default value of \Macro{ENABLE\_DEPRECATION\_WARNINGS} has been changed to false.
\Ticket{3848}
...
\item Workaround triggering a Linux kernel defect when using cgroups
and suspending jobs.
\Ticket{3487}

\item Fix missing converters in python bindings by providing
pyclassad shared library.
\Ticket{3780}
...
\item Code to publish Linux distribution attributes in machine ad was made more
	robust in the event that /etc/issue file was edited.
\Ticket{3854}
...
\item Fixed a bug that caused \Condor{submit} \Opt{-spool} to always fail
on Windows.
\Ticket{3791}
