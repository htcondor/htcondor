...
and, amount other problems, \Condor{startd} ads would appear
in the output of \Condor{status} with blank hostnames.
\Ticket{3956}
...
\item The configuration parameters \Macro{UPDATE\_COLLECTOR\_WITH\_TCP}
and \Macro{TCP\_UPDATE\_COLLECTORS} are now respected when forwarding
ads to a view collector (i.e. when using \Macro{CONDOR\_VIEW\_HOST}).
\Ticket{3986}
