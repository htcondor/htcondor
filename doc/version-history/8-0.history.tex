...
\item TEMPTEMP -- added checks for illegal VARS names -- add link
...
\item Fixed a crash in the \Condor{shadow} triggered when a disconnect
occurs just as the job terminates.
\Ticket{4127}

\item TEMPTEMP -- prios in rescue DAGs
\Ticket{4102}

\tiem TEMPTEMP -- dag node priority of 0 is the same whether set explicitly or implicitly?  (affects child either way)
