...
%\item HTCondor version 8.0.5 released on Month Date, 2013.
...
\Macro{RemoteGroupQuota} and \Macro{SubmitterGroupQuota} attributes
commonly used in the \Condor{negotiator} PREEMPTION\_REQUIREMENTS
setting.
\Ticket{4093}
...
\item -t (for -total) now works consistently in \Condor{status}.
\Ticket{4096}

\item Restored the omitted HTCondor Perl module.
\Ticket{4098}

\item In the RPM post-install script, check to make sure that the
SELinux unconfined\_execmem\_exec\_t type exists, before trying to
add it to the /usr/sbin/condor\_startd file context.
\Ticket{4034}

\item Fix a crash in both daemons and tools that happens when a
hostname in \Macro{COLLECTOR\_HOST} can't be resolved to an IP
address.
\Ticket{3946}
