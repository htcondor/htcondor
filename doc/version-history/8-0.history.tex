...
%\item HTCondor version 8.0.4 released on Month Date, 2013.
...
\item A spurious warning about undefined PREEMPT\_VANILLA expressions
has been removed.
\Ticket{3984}

\item You may now specify binary user-data files for EC2 jobs.
\Ticket{3924}
...
HTTP file transfers also now fail if the file does not exist (but the server
does).
\Ticket{3904}

\item Fixed a rare bug where the \Condor{schedd} would exit
with an ERROR message pertaining to select when very
runnign with a very heavy load and many \Condor{shadows}
are timing out.
\Ticket{3947}

\item If cgroups were enabled, \Condor{ssh\_to\_job} would
...
\item The configuration file for Windows erronously had two entries
for JAVA\_CLASSPATH\_SEPARATOR with the second entry specifying the
separator for UNIX platforms and overriding the first entry. This bug
has been fixed.
\Ticket{3957}
...
\item Fixed a bug that can cause ClassAd function quantize() to not
evaluate properly when its second argument is a list and ClassAd caching
is enabled.
\Ticket{3967}

\item The configuration setting \Expr{STARTD\_CRON\_AUTOPUBLISH=If\_Changed}
did not work correctly.  Updates were sent to the \Condor{collector}
