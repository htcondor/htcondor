...
grow arbitrarily in size with cgroup based tracking was in effect.
\Ticket{4343}
...
\item In some unusual error conditions, the \Condor{schedd} and
\Condor{shadow} could treat a failed file transfer as if it had
succeeded.
As a result, the \Condor{schedd} could try to run a job whose input
files weren't available, or a job could leave the queue as completed
