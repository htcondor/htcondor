...
%\item HTCondor version 7.8.7 released on Month Date, 2012.
...
\item Fixed a problem where a ssh\_to\_job or interactive job session would be terminated prematurely if the execute machine was configured to track process trees via a dedicated login (aka \Macro{DEDICATED\_EXECUTE\_ACCOUNT\_REGEXP} is being used).  
\Ticket{3232}
...
\item The configuration setting \MacroNI{ENABLE\_ADDRESS\_REWRITING}
was not correctly applied to the \Condor{schedd} address when
claiming a slot.  The behavior was always as though
\MacroNI{ENABLE\_ADDRESS\_REWRITING} was false.  This could result
...
is submitted with an X.509 proxy that doesn't have VOMS attributes.
\Ticket{3356}
