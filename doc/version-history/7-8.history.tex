...
\item Fixed a bug that caused \Condor{schedd} to, by default, share the job queue.
\Ticket{3196}

\item Fixed a bug that could cause \Condor{q} to not print all jobs when
it thought it was querying an old \Condor{schedd}.
\Ticket{3206}
...
was disabled or \Macro{SEC\_ENABLE\_MATCH\_PASSWORD\_AUTHENTICATION} was
set to \Expr{True}.
\Ticket{3208}

\item The log message that is generated when the \MacroNI{EXECUTE} directory
