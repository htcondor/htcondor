...
%\item Condor version 7.8.4 released on Month Date, 2012.
...
\item Fixed the \Condor{schedd} where it would crash when a submit file
had a malformed dollar dollar expression.

\item Fixed a case where a daemon could crash and leave behind a log
file owned by root.  This root-owned file would then cause
subsequent attempts to restart the daemon to fail.
\Ticket{2894}

\item Macros that are prefixes of ``\MacroNI{DOLLAR}'' were not expanded
properly.
\Ticket{3217}
...
%\item Condor version 7.8.3 released on Month Date, 2012.
