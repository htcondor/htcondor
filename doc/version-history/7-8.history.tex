...
\item Condor version 7.8.7 not yet released.
%\item Condor version 7.8.7 released on Month Date, 2012.
...
\item Jobs that are submitted with the ``-spool'' option to \Condor{submit}
will no longer be left indefinitely in hold state.  Instead the jobs will be
removed if spooling fails, and will also periodically remove jobs that have
been in hold state due to file transfer for at least twelve hours.
\Ticket{3200}
