...
\Macro{LSF\_GAHP} and \Macro{SGE\_GAHP}. The older parameters are
...
the init script to hang if condor wasn't running
\Ticket{2872}

\item Fixed a bug that caused Parallel Universe jobs using
Parallel Scheduling Groups to occasionally stay idle even when
...
\item Changed \Condor{ckpt\_probe} to link statically, which should make the
