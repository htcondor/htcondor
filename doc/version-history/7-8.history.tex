...
\Macro{GLEXEC\_RETRY\_DELAY} and a maximum of 100 times that value.
\Ticket{2415}
...
\item (This bug was fixed in version 7.8.4)
\Macro{ULOG\_JOB\_RECONNECT\_FAILED} events were not considered by the userlog
code. This has been fixed.
\Ticket{3215}

\item Fixed a bug that caused Cream grid jobs to become held with a
HoldReason of ``CREAM error: Transfer failed: globus\_ftp\_client: 
an invalid value for url was used'' when the jobs did not have any
input or output files to transfer.
\Ticket{3415}

\item Fixed a bug that could cause condor daemons to abort on \Condor{reconfig}
when the configuration variable \Macro{STATISTICS\_WINDOW\_SECONDS} was reduced.
\Ticket{3443}
...
daemon to become unreachable via CCB until restarted.
\Ticket{3476}

\item If \Condor{shared\_port} was using a dynamic port, and the
master was using the shared port, then if \Condor{shared\_port} died, all
subsequent attempts to restart it on a different port failed.
\Ticket{3478}

\item The \Prog{nordugrid\_gahp} no longer exits if the job ad contains
