...
\item Although not user-visible, there were multiple updates to remove places
in the code where potential buffer overrruns could occur, thus removing
potential attacks.  None were known to be exploitable.

\item Although not user-visible, there were updates to the code to improve
error checking of system calls, removing some potential security threats.  None
were known to be exploitable.

\item Although not user-visible, some code that was no longer used was removed.
...
\item Filesystem (FS) authentication was improved to check the unix permissions
