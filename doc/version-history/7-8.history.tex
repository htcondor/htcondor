...
\item Condor now contains a tool, accountant\_log\_fixer, that can fix the
...
This causes \Condor{dagman} jobs to fail to restart. This bug exists
...
need such intervention when upgrading from condor version 7.8.5.  To restart
a \Condor{dagman} job, the simplest solution is to do ``\Condor{submit}
$\langle$DAGFile$\rangle$.condor.sub'', where the original \Condor{dagman} was
submitted with ``\Condor{submit\_dagman} $\langle$DAGFile$\rangle$''.
\Ticket{3207}
