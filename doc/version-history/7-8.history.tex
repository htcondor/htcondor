...
logs is again available as a shared library on linux and mac.
...
\Macro{ClassAdXMLParser} in the ClassAds library:
  \begin{itemize}
  \item Several methods named \Macro{ParseClassAd} were declared, but never
    implemented. \Ticket{3049}
  ...
  \item The parser could go into an infinite loop or leak memory when
    reading a mal-formed ClassAd XML document. \Ticket{3045}
  \end{itemize}

\item Fixed a bug that prevented the -f command line option of condor_history
from being recognised. The -f option was being interpreted as -forward. At least
four letters are now required for the -forward option (-forw) to prevent ambiguity.
\Ticket{3044}
...
\item The default value for \Macro{GROUP\_SORT\_EXPR} was changed so that
the none group would always negotiate last when using hierarchical group quotas.
The default value for \Macro{NEGOTIATOR\_ALLOW\_QUOTA\_OVERSUBSCRIPTION} was changed
to true.  These changes were made to make negotiation behave more like it did 
