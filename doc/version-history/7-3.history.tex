...
\item Note that this release is incompatible when communicating with
...
\item Optimized removal of large numbers of jobs.  Previously, removal
of 10s of thousands of jobs would cause the \Condor{schedd} to consume
a lot of cpu time for several minutes.

\item Reduced memory usage in \Condor{shadow}.  Since there is one
\Condor{shadow} process per running job, this helps increase the
number of running jobs a submit machine can handle.  Under Linux 2.6,
we found that running 10,000 jobs from a single submit machine
requires about 10GB of system RAM.  We also found in this case that to
run more than 10,000 simultaneous jobs requires a 64-bit submit
machine.  Under 32-bit Linux, kernel memory becomes exhausted,
regardless of how much additional RAM the system has.

\item Reduced memory usage of \Condor{collector} when using
\MacroNI{UPDATE\_COLLECTOR\_WITH\_TCP}.
