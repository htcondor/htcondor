...
% gittrac #12
\item Removed unconditional newline printed at the bottom of \Condor{status}
\Opt{-format}. Code which parses the output of this command should be careful
...
\item \Condor{fetchlog} may now fetch history files from a \Condor{schedd}.

\item The history file kept by the \Condor{schedd} may now be rotated daily or
monthly.
...
\item \Condor{dagman} now automatically generates a default user log
...
the \Condor{collector}. Various daemons that register themselves with 
generic ads can have tools which use the \Opt{-subsystem} option manipulate
their ads properly.
...
\Attr{EnteredCurrentStatus} to be reset when the job is routed instead
of copied over verbatim from the original job.

% from 7.5.2
% gittrac #581
\item Fixed a bug with dynamic slots where it was possible to lose the 
\Attr{SLOTx\_STARTD\_ATTRS} machine classad attribute in the slots.
...
\item Fixed a crash in \Condor{q} \Opt{-analyze} when there is no 
\Condor{schedd} daemon ad file.

\item Fixed a \Condor{procd} crash when the environment of a monitored process
exceeded 1MB in /proc.

% gittrac #708
\item Fixed a bug which could cause the \Condor{credd} to crash on windows if
a credential is requested which is not in the password store.

\item Fixed a bug that caused the \Condor{master} to fail to re-execute itself
when it noticed a new binary which happened when defining a value for the
\Macro{MASTER\_LOCK} configuration file parameter.
...
% gittrac #695
\item Fixed race condition which could cause a crash in the \Condor{collector}
and \Condor{schedd} on shutdown.

% gittrac #690
\item Fixed a bug where \Condor{master} would sometimes die and produce
...
% gittrac #580
\item Fixed bug where \Macro{MAX\_JOB\_QUEUE\_LOG\_ROTATIONS} and
\Macro{GRIDMANAGER\_SELECTION\_EXPR} config settings would not work
...
\item Fixed shutdown race condition in \Condor{master} with
...
%gittrac #575
\item Fixed a significant memory leak in the \Opt{gahp\_server}. This
