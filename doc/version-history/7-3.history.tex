...
\item Added \Condor{ssh\_to\_job} for interactive debugging of running
jobs.  See page~\pageref{man-condor-ssh-to-job} for details.
...
In other words, instead of parsing all submit files at startup, and 
immediately opening the specified log files, \Condor{dagman} now parses
the node job submit files just before each job is submitted, and has each
log file open only when relevant jobs are in the queue or executing POST
scripts.
...
% gittrac #551
\item When the VM universe is enabled but Condor's test of the VM software
fails, Condor will periodically retry the VM software test until it
succeeds.
...
% gittrac %627
\item The \Condor{gridmanager} now advertises grid ads properly when there
are multiple collectors.
