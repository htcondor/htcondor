...
%gittrac #654
\item The \SubmitCmd{nordugrid\_gahp} now limits the number of connections
...
\item Added the option \Opt{-attributes} to \Condor{q} to limit the
attributes which are displayed when using \Opt{-xml} or \Opt{-long}.
...
\item Condor's power management capabilities are now implemented as a
  ...
  external program (specified by \Macro{HIBERNATION\_PLUGIN}) to
  perform the detection of available low power states and the
  switching to these low power states.

\item A new Condor daemon \Condor{rooster} has been added to wake up
hibernating machines when their \Macro{UNHIBERNATE} expression
becomes true.  The configuration options for \Condor{rooster}
...
\item Changes to the value of \MacroNI{CERTIFICATE\_MAPFILE} or the contents
of the file to which it refers no longer require a full restart of Condor.
Instead, a reconfig will cause the changes to be utilized.
...
%gittrac #621
\item The new configuration macro \Macro{DELEGATE\_FULL\_JOB\_GSI\_CREDENTIALS}
...
\Condor{rooster}).  The default value is \Code{LastMatchTime=!=UNDEFINED}.
The \Attr{LastMatchTime} attribute is a special attribute that is created
in the offline startd ad when the a job would have been matched to it
if it had been online.

%gittrac #383
\item The new configuration \Macro{HIBERNATION\_PLUGIN} specifies the
  path to the ``plug in'' which the \Condor{startd} will use to detect
  the low power state capabilities of a machine, and to switch the
  machine to a low power state.

%gittrac #383
\item The new configuration \Macro{HIBERNATION\_PLUGIN\_ARGS}
...
%gittrac #383
\item The new configuration \Macro{HIBERNATION\_OVERRIDE\_WOL} can be
