...
\item HTCondor version 8.7.9 was released on FIXME.
...
inneffective.  This has been a problem since v8.6, but we expect to fix
this problem in our next stable release (before our next developer release).
\Ticket{6697}

\item Policies implemented by the startd may not function as desired while
the startd is shutting down.  Specifically, if the \Macro{PREEMPT}
...
ineffective.  This has been a problem since v8.6, but we expect to fix
this problem in our next stable release (before our next developer release).
\Ticket{6701}
...
\item Added a new tool, \Condor{now}, which tries to run the specified
job \emph{now}.  You specify two jobs that you own from the same schedd:
the now-job and the job to preempt.  The latter is immediately preempted
(without retirement); after the preempted job is done vacating, if the
schedd still has the claim to the preempted job's slot, it will immediately
start the  now-job there.  The now-job must be idle and the preempt-job
must be running.  If you're a queue super-user, the jobs must still have
the same owner, but it doesn't have to be you.
\Ticket{6659}
