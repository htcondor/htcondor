...
\Expr{pypi\_staging} to build the framework for Python wheels. This option and target
...
\item Changed the default value of configuration parameter \MacroNI{IS\_OWNER} to \Expr{False}.
The previous default value is now set as part of the \texttt{use POLICY : Desktop}
...
\item Fixed a bug with concurrency limits where condor\_qedit'ing a 
job in the queue, and changing any of the concurrency limits of that
job had no effect.
\Ticket{6448}
...
\item The job log should no longer contain empty submit warnings.
\Ticket{6465}

\item HTCondor now reports all submit warnings, not just the first one.
\Ticket{6446}

\item Fixed a bug that could result in an incorrect total reported by \condor{rm} when the
\Opt{-tot} option is used.
\Ticket{6450}
