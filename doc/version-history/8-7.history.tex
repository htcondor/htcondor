...
%\item HTCondor version 8.7.10 released on Month Date, 2018.
...
\item For grid-type boinc jobs, the following job ad attributes can be
...
\item Fixed a bug that occured when starting Docker Universe jobs that
would cause the start to crash and the jobs to cycle between \Expr{running}
and \Expr{idle} status.
\Ticket{6725}
...
result in noticable performance reduction of the \Condor{schedd}.  Removing this inefficiency
