...
job \emph{now}.  You specify two jobs that you own (or which have the
same owner, if you are a queue super-user) from the same schedd: the now-job
and the preempt-job.  The former is immediately preempted (without retirement);
after the preempt-job is done vacating, if the schedd still holds the claim
to the preempt-job's slot, it will immediately start the now-job.
\Ticket{6659}
