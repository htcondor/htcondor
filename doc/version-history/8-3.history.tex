...
\item The \Condor{collector} now never forks to handle queries of certain type
of ads.  These types are those that a typical condor pool has very few of, 
even in a large pool, and include, the collector, negotiator, and scheduler 
ads.  As a result, query throughput of the collector should be much faster.
\Ticket{4821}
...
\item The configuration parameter \Macro{GRACEFULLY\_REMOVE\_JOBS} now
controls how a running job is killed when a job policy expression
causes it to be held or removed.
Previously, it was consulted only when the user ran \Condor{hold} or
\Condor{rm}.
\Ticket{4679}
