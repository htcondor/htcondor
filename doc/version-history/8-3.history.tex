...
\item Configuration setting \Macro{ENCRYPT\_EXECUTE\_DIRECTORY} 
	is now honored on Linux (assuming ecryptfs-utils package is installed
	and the Linux kernel is version 2.6.29 or newer), and also
	\SubmitCmd{encrypt\_execute\_directory} can now be specified
	by the user with \Condor{submit} on a per-job basis.
\Ticket{4856}
