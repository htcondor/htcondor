...
\item Improved support for mixed mode operation.  HTCondor daemons will now
advertise both an IPv4 and an IPv6 address if both are enabled.  HTCondor
tools and daemons will pick an address based on which protocol(s) it has
enabled.  This allows IPv4-only clients to function with mixed mode
pools.  Older clients and daemons will use IPv4 exclusively.
\Ticket{4934}
...
\item \Condor{q} now has a \Opt{totals} option, and the \Opt{dag} option has been
changed to show all of the jobs in the DAG when a DAG ID argument is provided.
\Ticket{5032}
\Ticket{5052}
