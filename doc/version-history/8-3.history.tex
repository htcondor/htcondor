...
\item Fixed a startd bug introduced in v8.3.2 where startd slots can get stuck
	forever in Preempting/Killing state when they are claimed by a v8.3.1 or
	older schedd.
\Ticket{4807}

\item Fixed an issue introduced in v8.3.0 that could cause the \Condor{schedd}
      to become unresponsive when a client was using the V3 query protocol.
\Ticket{4805}
