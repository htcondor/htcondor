...
\item \Condor{dagman} is now \emph{required} to run in workflow/default
node log mode (previously workflow node log mode was the default, but
DAGMan could be told to not use the workflow node log file).  (In
other words, the \Opt{-dont\_use\_default\_node\_log} command-line
flag to \Condor{submit\_dag} is disabled and the
\Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} configuration macro can no
longer be set to \Expr{False}).
\Ticket{3994}
