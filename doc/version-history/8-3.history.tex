...
the shared port ID to `self'.  For example, a shared port daemon listening
on 1.2.3.4 port 9618 could be addressed as
``<1.2.3.4:9618?noUDP=true\&sock=self>''.  This may be useful for certain
tools, e.g., \Condor{config\_val}.
\Ticket{5209}

\item The \Condor{schedd} now advertises \Attr{NumJobStartsDelayed} and
\Attr{NumPendingClaims}.
\Ticket{5158}
...
\item Fixed a bug causing slow shut-downs on Windows.
\Ticket{5209}
...
\item Fixed a bug that would cause the \Condor{schedd} to crash when using the parallel universe, partitionable slots and dollar dollar expansion.
\Ticket{5201}
