...
\item \Condor{q} will now abbreviate the address field in the header for each \Condor{schedd}. It also
no longer incorrectly shows "Submitter:" in the header when it should be showing "Schedd:"
\Ticket{5071}

\item \Condor{q} \Opt{-io} now shows transfer information for Vanilla universe jobs as well as Standard universe jobs.
...
\item Fixed a bug that would cause events to be lost when \Macro{EVENT\_LOG\_LOCKING}
or \Macro{ENABLE\_USERLOG\_LOCKING} were set to false on Windows.
\Ticket{5115}
