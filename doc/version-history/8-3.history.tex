...
\item When the \Condor{collector} queried for its own ClassAd, it will now return the most current values
for Collector statistics in that ClassAd.  Also new attributes have been added to the Collector's ClassAd
to show overall number of lost updates and the loss ratio as well as the largest number of unique Startd
and Submitter ClassAds the Collector has seen.
\Ticket{4888}

\item Augmented the debug() ClassAd function to print an appropriate
error message for undefined ClassAd functions.
\Ticket{4885}
...
\item Split the condor-externals RPM into condor-externals and
condor-external-libs so that the both the 32-bit and 64-bit external
libraries could be installed when running the 32-bit static shadow
on a 64-bit system.
\Ticket{5021}

\item Added an obsolete clause to the HTCondor RPM to prevent the ClassAd
library packaged in EPEL begin selected over the ClassAd library that we
provide.
\Ticket{5035}
