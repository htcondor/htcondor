...
reconnect to the \Condor{startd}s of previously running jobs on startup.
The report is written to the location specified by the new configuration
parameter \Macro{SCHEDD\_RESTART\_REPORT}.
Once all reconnect attempts are complete, a copy of the report is also
emailed to HTCondor administrator account.
\Ticket{4783}

\item Added new ClassAd functions envV1ToV2() and mergeEnvironment(),
useful for manipulating the environment variable lists in job ads.
\Ticket{4574}
...
\item Fixed a bug that caused messages like the following to be printed
in the daemon log of the \Condor{schedd}:
\begin{verbatim}
