...
\item Fixed bug whereby tuning configuration variable \Macro{MAX\_ACCEPTS\_PER\_CYCLE} was ignored when \Condor{shared\_port} is used, potentially causing significant delays when issuing new commands to heavily loaded HTCondor schedds.
\Ticket{5146}
...
\item Added an option to audit connections made through the
\Condor{shared\_port} daemon's socket directory, allowing the
administrator to check for abuse.
\Ticket{4803}
