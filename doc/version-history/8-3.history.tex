...
%\item HTCondor version 8.3.8 released on Month Date, 2015.

\end{itemize}

%TEMPTEMP -- add DAGMan sleep change in here...
...
starts up.  This script tunes several Linux kernel parameters to the values
we suggest for improved scalability.  See \Macro{ENABLE\_KERNEL\_TUNING},
\Macro{KERNEL\_TUNING\_LOG}, and \Macro{LINUX\_KERNEL\_TUNING\_SCRIPT}.
\Ticket{5126}

\item The configuration variable \Macro{SLOTS\_CONNECTED\_TO\_KEYBOARD} will now
default to NUM\_CPUS rather than to 1.  Because of this change, when keyboard
activity is detected all slots will have their \Attr{KeyboardIdle}
attribute updated.
\Ticket{5225}
...
can be expected to reduce memory useage of the \Condor{collector} and \Condor{schedd}.
\Ticket{5222}
...
\item In submit files, setting \Macro{TRANSFER\_INPUT\_FILES} to "" will indicate
that no input files should be transferred rather than producing a syntax error.
\Ticket{5159}

\item The \Condor{kbdd} will now ignore small mouse movements that occur after a long
period of inactivity on both keyboard and mouse.  The value of small is defined by the new
configuration variable \Macro{KBDD\_BUMP\_CHECK\_SIZE}, and the period of inactivity
that triggers this behavior is defined by \Macro{KBDD\_BUMP\_CHECK\_AFTER\_IDLE}.
\Ticket{5210}

\item The \Condor{user\_prio} tool has been enhanced.  It now has two forms of \Opt{-long} output -
The traditional form, enabled by the \Opt{-legacy} option, and a new form that has a separate
ClassAd for each user and group.  This new form allows \Condor{user\_prio} to support
additional new arguments: \Opt{-constraint} and \Opt{-autoformat}.
\Ticket{5127}

\item \Condor{q} \Opt{-better-analyze} has been enhanced to understand that sub-expressions referring
...
\item Fixed a bug in \Condor{who} that sometimes prevented it from producing any output unless the
\Opt{-allpids} argument was passed.  This bug was introduced in HTCondor 8.3.6.
\Ticket{5227}
...
\item Fixed problems with the handling of \Macro{SLOT\_TYPE\_n\_STARTD\_ATTRS}
(where n is a slot type number). Also when a \Macro{SLOT\_TYPE\_n\_value} is explictly set
to nothing, this now overrides \Macro{value} for that slot type rather than being ignored.
\Ticket{5132}

\item Fixed a bug that caused the ClassAd attributes \Attr{DetectedCpus} and \Attr{DetectedMemory}
to be inserted into all ClassAds.  These attributes are now inserted only into daemon ClassAds.
\Ticket{5171}

\item Fixed a bug that caused \Condor{q} to interpret a username that starts with a number as a job id.
\Ticket{5172}

\item Fixed bugs in the \Macro{REAL()} and \Macro{CHOICE()} configuration functions
...
on Linux platforms to prevent crashes when using \Macro{CLASS\_AD\_USER\_LIBS}
or \Macro{CLASSAD\_USER\_PYTHON\_MODULES} to load ClassAd plug-in functions.
\Ticket{5184}
