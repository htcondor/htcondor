...
%\item HTCondor version 8.3.7 released on Month Date, 2015.
...
on Linux, filenames are no longer encrypted by default.  This behavior can
be enabled by setting \Macro{ENCRYPT\_EXECUTE\_DIRECTORY\_FILENAMES} to
\Expr{True}.
\Ticket{5108}
...
\item \Condor{dagman} now allows at most two submit attempts for a FINAL
node job if the DAG has been \Condor{rm}'ed.
\Ticket{5089}

\item A job ad's \Attr{ConcurrencyLimits} attribute can now be a ClassAd
expression that references attributes of the machine ads that the job
may be matched to.
The expression must evaluate to a string that has the required format.
\Ticket{5006}
...
could not benefit from the matchlist caching option of the
\Condor{negotiator}, which could result in it taking more time to match
...
\item Fixed a bug when running parallel universe jobs on a pool
with partitionable slots, where too many slots would get 
claimed for the job.
\Ticket{5110}
...
\item Fixed a bug in \Condor{submit} where using submit attribute
...
\item Fixed an incorrect Win32 default value for the
DAGMAN\_CONDOR\_RM\_EXE configuration macro.  This caused \Condor{dagman}
(on Windows) to not correctly remove running node jobs if the
ABORT-DAG-ON feature was triggered.
\Ticket{5114}
