...
\item An EC2 grid universe job now advertises its access key ID in its job ad.
\Ticket{4903}
...
\item A new DAGMan feature allows a single retry of a PRE or POST script 
after a specified delay, 
where the retry attempt is
...
\item Fixed a bug with \Condor{chirp} where generic events logged
to the user log with the ulog command where written in the timezone
