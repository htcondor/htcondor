...
\item Added a new configuration variable \Macro{CURB\_MATCHMAKING} which
can be used to configure the schedd to cease requesting more machine resources
from the central manager during overload situations.
\Ticket{4905}

\item Some HTCondor-specific environment variables that are set in the
environment of a batch job, such as \_CONDOR\_JOB\_AD, are now also set for
ssh\_to\_job and interactive job sessions.
\Ticket{4943}

\item Condor{q} Opt{-constraint} will now show a summary line.
\Ticket{4992}
...
\item Fixed a bug in the python bindings where multi-threaded python
programs would deadlock when a Schedd.query() would return more than
one result.
Also fixed a bug in Schedd.xquery() where the constraint argument did not
default to "true" as documented, but had to be explicitly defined.
\Ticket{5000}
