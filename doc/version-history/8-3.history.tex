...
\item The python bindings now work with pyton's pickle module. \Ticket{5099}
...
\item When the \Condor{startd} starts up and shuts down, it removes any left over files and directories in the execute directory.  Previous, it would remove all sub-directories, including the special "lost+found" directory, which the file system check program may need when recovering files.  Now it never removes that directory.
\Ticket{4577}
...
\item The default values of the configuration variables listed in
Table~\ref{table:new-knob-defaults} have changed,
...
\MacroNI{NEGOTIATOR\_INFORM\_STARTD} & \Expr{True} & \Expr{False}  \\ 
\MacroNI{MAX\_JOBS\_PER\_SUBMISSION} & largest positive integer & 20000  \\ 
\MacroNI{MAX\_JOBS\_PER\_OWNER} & largest positive integer & 100000  \\ 
\hline
\end{tabular}
\caption{\label{table:new-knob-defaults}Changes to configuration variable defaults in HTCondor 8.3.7}
\end{table}
\end{center}
\Ticket{4988}
...
\item The default value for configuration parameter
\Macro{UPDATE\_COLLECTOR\_WITH\_TCP} has changed to \Expr{True}.
The new configuration parameter \Macro{UPDATE\_VIEW\_COLLECTOR\_WITH\_TCP}
controls whether UDP or TCP is used to forward update to the
\Condor{collector}s specified via \Macro{CONDOR\_VIEW\_HOST}.
