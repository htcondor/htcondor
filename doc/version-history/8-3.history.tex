...
forces the addition of a list of job attributes when 
considering autoclustering.
The new configuration variable \Macro{REMOVE\_SIGNIFICANT\_ATTRIBUTES}
prevents the addition of a list of job attributes when 
considering autoclustering.
\Ticket{4797}
...
\item The \Macro{DAGMAN\_USE\_OLD\_DAG\_READER} configuration macro
is no longer supported.  Setting it to \Expr{True} will result in
a warning, and will have no effect on how the DAG file is read.
See section~\ref{param:DAGmanUseOldDagReader} for details.
\Ticket{4186}
