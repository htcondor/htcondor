...
\item On newer Linux platforms (such as RHEL7), execute hosts can now
prevent jobs from acquiring additional OS-level privileges by setting
\MacroNI{JOB\_NO\_NEW\_PRIVS} to \Expr{True}.  For example, this would
prevent jobs from gaining root access by invoking \Prog{sudo}.
\Ticket{4443}
