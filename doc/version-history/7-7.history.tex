...
\item With Condor 7.7.2, if you had multiple Condor installations, it was 
possible for some installations to use the wrong version of the ClassAds 
library. This should no longer be an issue. Addtionally, the binaries 
now use RUNPATH instead of RPATH, allowing you to use the
LD\_LIBRARY\_PATH environment variable to set where they look for their
shared libraries.
\Ticket{2539}
...
\item A new submit file command \SubmitCmd{want\_graceful\_removal}
...
state apply in this case.  This new submit file command replaces a
different mechanism that was added in 7.5.2 to achieve some of the
same effects.  Starting in 7.5.2, for vanilla jobs under Linux, if the
job set \SubmitCmd{remove\_kill\_sig} or \SubmitCmd{kill\_sig}, the
hard-kill signal that Condor would normally send to end the job was
replaced with the signal specified by the user.  This is no longer the
case.  The soft-kill signal may still be customized using
...
the \Condor{startd} (i.e. not grid, local, or scheduler universe).
...
\item A new configuration variable \MacroNI{MachineMaxVacateTime} is
now used to express the maximum time in seconds that the machine is
willing to wait for a job to gracefully shut down.  The default is
600 seconds (10 minutes).  The boolean \MacroNI{KILL} expression was
previously used to terminate the graceful shutdown of jobs.  It
should normally be set to \Expr{False} now.  If desired, it may be
...
\item \Condor{master} now successfully advertises itself in a personal
condor installation when the collector is configured to use port 0
and to operate through a shared port.
\Ticket{2555}

\item Since 7.7.1, \MacroNI{WANT\_HOLD} did not work unless
\MacroNI{WANT\_HOLD\_SUBCODE} was set to something non-zero.
\Ticket{2565}
...
\File{/usr/lib/condor/} or \File{/urs/lib64/condor/} in the native package
releases.
\Ticket{2389}
\Ticket{2390}
