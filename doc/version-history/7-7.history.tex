...
%by either adding the \Opt{-no\_postrun} option to \Condor{submit\_dag},
%or by setting the new configuration variable 
%\Macro{DAGMAN\_POST\_IGNORE\_PRE} to \Expr{False}.
%\Ticket{2057}
...
% gittrac #2366
\item The default value of \Attr{OpSys} on Windows machines has been changed to
"Windows", and a new attribute \Attr{OpSysVer} has been added that contains the version
number of the operating system.  This behavior is controlled by a new parameter
\Macro{ENABLE\_VERSIONED\_OPSYS} which defaults to false on Windows and true on
other platforms.  A new attribute \Attr{OpSys_And_Ver} will always contain the
versioned operating system.  Note that this change could cause problems with mixed pools
because version 7.7.2 \Condor{submit} may add \Attr{OpSys}="Windows" but machines running
versions prior to 7.7.2 will be publising a versioned \Attr{OpSys} value unless there is
an override in the config file.
\Ticket{2366}
...
\item
The new submit file option \Attr{+KeepClaimIdle}, which takes an integer
number of seconds as an argument,  has been added to the submit
file. If this is set, when the job exits, if there
are no other jobs immediately ready to run for this user, the schedd,
instead of relinquishing the claim back to the negotiator, will keep
the claim for that many seconds.  This is useful if another job will
be arriving soon, which can happen with linear DAGs.  The startd slot
