...
\item On Linux system, the starter can now optionally measure the
PSS (Proportional Set Size) of each Condor job, if the config file
option USE\_PSS is set to true.  Previously, it was unconditionally
on, which can cause performance problems in the procd when running
