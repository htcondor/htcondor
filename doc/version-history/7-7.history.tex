...
\item On Linux systems which support bind type of filesystem mount (generally,
RHEL 5 systems and newer), the administrator can configure the condor\_startd 
to provide per-job filesystem mounts.  One use might be to provide each job
it's own view of /tmp and /var/tmp, which are private to that Condor job,
and cleared when the job exits.  This is implemented with the
MOUNT\_UNDER\_SCRATCH parameter, which describes which directories to
bind mount.
\Ticket{2015}
...
\item Condor will now send email if submit notification is set to error and
...
\item By default, if a node has child nodes, then \Condor{dagman} uses the
KeepClaimIdle attribute, introduced in Condor version 7.7.1, to hold onto a
claim.  This is a slight optimization, as it avoids waiting for a negotiation
cycle.  The hold time defaults to 20 seconds.  This is controlled by the
\Macro{DAGMAN\_HOLD\_CLAIM\_TIME} configuration variable.
\Ticket{2673}

\item Improved output of \Condor{q} \Opt{-dag} to show the dag structure as a tree,
with children indented below their parents.
\Ticket{1281}

\item Fixed the output of \Condor{q} \Opt{-dag} so that children of a
non-existent DAGman node would not be mistakenly shown as belonging to another
DAGman. This can happen, for example, when a \Condor{dagman} process dies while
its children are still running.
\Ticket{2463}

% Note: this is just kind of a stub so there's something in the version
% history when I merge this branch to the master -- I need to fully
% document this new feature ASAP.  wenger 2011-12-22
\item Added the new FINAL node feature to \Condor{dagman} -- this allows
the user to declare a FINAL node in the DAG, which is always run at the
end of the workflow, whether it ended successfully or not.
\Ticket{1482}
...
specifies the amount of time in seconds that \Condor{schedd} will hold a claim
idle for a DAGman job, using the \AdAttr{KeepClaimIdle} attribute in the JobAd.

\item \AdAttr{ResidentSetSize} and \AdAttr{ProportionalSetSizeKb} now
report the maximum observed memory usage.  Previously, they reported
the most recently observed memory usage.  This change makes these
attributes similar to \AdAttr{ImageSize}, which also reports the
maximum observed value.  Previously, \AdAttr{ResidentSetSize} was
...
\item \AdAttr{ResidentSetSize} is now rounded by default using the new
...
\item \Condor{submit} would allow ec2\_secret\_access\_key and
  ec2\_access\_key\_id to be directories.
\Ticket{2619}
...
and the \Condor{starter}.  This problem was introduced in 7.7.2.
\Ticket{2591}

\item Fixed a bug in the \Condor{gridmanager} that could cause crashes
and prevent the \Attr{x509UserProxyEmail} from being set properly for
...
\item Condor version 7.7.4 has all of the features and fixes of 7.7.3,
with one major addition.  This is the ability to run in a pure IPv6 network,
which is disabled by default.  To \emph{enable} IPv6 code, 
set the configuration variable
\begin{verbatim}
  ENABLE_IPV6 = True
\end{verbatim}
Note that running in a mixed IPv6 and IPv4 environment has not been tested,
