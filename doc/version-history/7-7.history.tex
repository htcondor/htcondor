...
\item The condor\_hdfs daemon has been moved to the contrib section, and is 
...
\item Support for the gt4 grid-type (i.e. Web Services GRAM) in the grid
...
\item The submit file attribute \SubmitCmd{globus\_xml} is no longer
recognized. The following configuration parameters are no longer
recognized:
\begin{itemize}
  \item \Macro{GRIDFTP\_SERVER}
  \item \Macro{GRIDFTP\_SERVER\_WRAPPER}
  \item \Macro{GRIDFTP\_URL\_BASE}
  \item \Macro{GT4\_GAHP}
  \item \Macro{GT4\_LOCATION}
  \item \Macro{GT42\_GAHP}
  \item \Macro{GT42\_LOCATION}
