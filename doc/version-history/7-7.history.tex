...
\item On Linux systems, the starter now has an ability to run a job under
a chroot'ed directory.  If the parameter NAMED\_CHROOT is set to a list
of directories an execute machine, and the job has a attribute
RequestedChroot, and the RequestedChroot matches an entry in the NAMED\_CHROOT
list, then the starter calls chroot with that directory as an argument.  Note
it is up to the administrator to provide a full environment for the job to run in.
\Ticket{2698}
...
\item Added the \OptArg{-expand} option to condor\_config\_val.  If \OptArg{-dump}
and \OptArg{-expand} are given to the command, all config file macros are
expanded before they are printed out.
\Ticket{2867}

\item The \OptArg{-sort} option for \Condor{status} has been generalized to
