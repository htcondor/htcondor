...
\item New format tags \%v and \%V have been added for use by \Condor{status} -format.
These tags request that the value of the expression or attribute be printed using
a format appropriate to its type.
...
when it notices that the \Condor{master}
...
\Macro{MEMORY\_USAGE\_METRIC\_VM} specify the value that the \Condor{starter} will
...
nonnegative integer.  It was formerly limited to values between 0 and 60,
inclusive.
\Ticket{2864}
