...
\item Added a new tool called \Condor{gather\_info} which collects data about
a Condor installation and, if desired, a specific job. This information
is useful to Condor developers to help debug a problem in a pool or with a job.

\item \Condor{userprio} supports new flags: The -grouporder flag displays submitter 
...
\item \Condor{collector} now avoids performance problems that happened
previously when clients initiated communication with the collector but
then delayed sending input.
\Ticket{2506}

\item When using versions of glexec that create a copy of the proxy for use
...
\item A new configuration variable \Macro{NEGOTIATOR\_SLOT\_CONSTRAINT} 
...
\item A new configuration variable \Macro{NEGOTIATOR\_SLOT\_POOLSIZE\_CONSTRAINT} 
replaces \Macro{GROUP\_DYNAMIC\_MACH\_CONSTRAINT}, which has been retained as a
...
was made obsolete by \Macro{NEGOTIATOR\_SLOT\_CONSTRAINT} so it has been removed.
\Ticket{2277}

\item The configuration variables \Macro{IGNORE\_NFS\_LOCK\_ERRORS}
  and \Macro{BIND\_ALL\_INTERFACES} no longer support, undocumented,
  'Y' or 'y' to mean True.
...
\item Added a regression test for partitionable slot functionality.
\Ticket{2514}

% gittrac 2500
\item When the \Condor{procd}'s named command pipe is removed, or whose
inode has been changed while the daemon is running, the \Condor{procd}
will exit instead of previously continuing to execute in a useless mode
of operation.

\item Added logic to the \Condor{negotiator} daemon's Accountant
...
\item \MacroNI{NETWORK\_INTERFACE} did not have any effect.  This bug
was introduced in 7.7.0.
\Ticket{2513}

\item Glexec now also works when condor is running as root.
\Ticket{2503}
...
\item Fixed \Condor{preen} to honor the remove flag even for lock files, and
to remove lock files from the correct directory depth.
\Ticket{2495}
\Ticket{2496}
\Ticket{2497}

\item There is command line support to both suspend and continue jobs. The new tools \Condor{suspend} and \Condor{continue} will suspend and continue running jobs.
\Ticket{2368}
...
environment specified for the job and environment inherited from \Prog{glexec},
the environment always takes precedence when running the job.
Previously, the environment inherited from \Prog{glexec} took precedence.
\Ticket{2096}
...
when set to \Expr{True}, will prevent the \Condor{starter} from 
...
\item Fixed a bug introduced in 7.7.1 in the standard universe where
the \Syscall{getdirentries} call failed during remote I/O situations.
\Ticket{2467}
...
\item Offline ads with neither \Attr{MyType} nor \Attr{TargetType} 
...
\item Fixed a bug that could cause the \Condor{schedd} to crash after
...
\item In 7.7.2, the Condor daemons on Linux rely on shared libraries.  A bug in 7.7.1 and all previous versions of condor, prevents a 7.7.1 master from 
starting and 7.7.2 or later daemons.  This also means that a 7.7.1 master cannot upgrade itself to 7.7.2.  If a 7.7.1 \Condor{master} binary is replaced with a 7.7.2 \Condor{master} binary, Condor will shut off, and need to be restarted
by hand.
