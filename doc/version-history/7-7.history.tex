...
\item The dedicated scheduler used to attempt to change the
  \Attr{Scheduler} attribute on all parallel job procs in a durable
  fashion, resulting in an fsync() for each proc. This has been
  changed to nondurable, improving scalability by reducing the 
	number of fsync() calls without impacting correctness. \Ticket{2367}
...
\item \Macro{COLLECTOR\_ADDRESS\_FILE} is now set in the example configuration, similar to \Macro{MASTER\_ADDRESS\_FILE}.  This configuration is required when  \Macro{COLLECTOR\_HOST} has the port set to 0 (``select any available port'')  In other environments it should have no visible impact.
...
\item On \Condor{schedd} startup, with parallel universe jobs, the job
  ...
  dating back to Apr 2001. The expectations are now resolved. 
	\Ticket{2367}
...
% This is unlikely to make it into 7.7.1
% Comment it out for now and plan to move it to 7.7.2
% gittrac #2057
