...
Changed behavior of DAGman, so that by default POST scripts will be run
regardless of the return value from the PRE script in the same node. This
behavior can be returned to the old behavior of not running the POST script by
either adding the \Opt{-no\_postrun} option to \Condor{submit\_dag} or setting
the new configuration variable \Macro{DAGMAN\_POST\_IGNORE\_PRE} to False.
\end{itemize}
