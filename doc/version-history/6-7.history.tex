...
\item Added a feature to DAGMan that will allow VARS names to include
...
\item Added an \Expr{INCLUDE} config file entry that will be the location 
	of the header files shipped with Condor that are to be used with
	libcondorapi.a. 
	You may find out what this value is via \Condor{config\_val}.
...
\item When supplying a -L<condor/installation/include> linker flag to
	find the libcondorapi.a library via the -l linker flag to
	various compilers, under linux there would be errors emitted
	that indicated an undefined symbols problem linking with
	libc. 
	Condor ships a version of glibc in the same location as the
	libcondorapi.a file, and there was a libc mismatch during linking
	of program which only used libcondorapi.a.  
	This has been fixed.
