...
state and trying to run again.  This currently does not apply to
...
failure to transfer files, and failure to open input or output files.
In all such cases, a specific hold reason is specified in the job
ClassAd, along with a numeric hold code and subcode.  If you with to
...
\item Fixed a big introduced in 6.7.19, which could cause the startd
...
\Stork{rm\_cred}, respectively. This is beacuse the Windows
...
\item For Condor's auto-clustering of jobs for matchmaking, numerical
job attributes can now be rounded to improving clustering. Add the
following to your configuration file:
\Attr{SCHEDD\_ROUND\_ATTR\_<foo>} = X, where \Attr{<foo>} is the attribute
to round and X is the number of decimal places to round. Rounding always
rounds UP. So to round a value up to the nearest 100, set X = 2.
The actual, non-rounded value will also be stored in the job ad as an
attribute named \Attr{<foo>\_RAW}.
...
parameter \Macro{POOL\_PASSWORD\_FILE}. This file will be owned by the
...
\item Added configuration parameter \Macro{SYSAPI\_GET\_LOADAVG}, which
can be used to disable the calculation of system load average on
execute machines. The default value is true (same behavior as previous
versions of Condor). If set to false, load average is not computed and
...
\item When auto-clustering jobs for matchmaking, ensure that the job's
\Attr{Requirements} expression is always considered significant.

\item On Windows, when Condor was running as a normal user (i.e. not as
local system), sending a soft-kill (i.e. WM\_CLOSE message) to a user
job would always fail. This is now fixed.

\item Fixed a couple bugs that could cause the \Condor{master} to crash
on a shut-down or restart.
