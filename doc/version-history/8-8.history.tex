...
\item The Job Router no longer sets an incorrect \AttrNI{User} job
...
installed as the default runtime.  Also fixed a bug that would sometimes cause
...
\item Python binding API method Schedd.submit() and submitMany() now edits job \AttrNI{Requirements}
	expression to consider the job ad's \AttrNI{RequestCPUs} and \AttrNI{RequestGPUs} attributes.
\Ticket{6918}
