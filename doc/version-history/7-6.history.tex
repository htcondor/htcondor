...
\item In 32-bit Condor, \MacroNI{ImageSize} of processes larger than 4GB
was reported as 4GB.  This limit has been raised to 4095GB.

\item Fixed a bug where a completed Xen or KVM VM universe job's modified
disk image files wouldn't be transferred back to the submit machine.
\Ticket{2530}

\item Fixed a bug where a \Condor{starter} configured to use job hooks 
could fail to run a job but not wait for the job exit hook to complete before
exiting.  The \Condor{starter} will wait
\Macro{<Keyword>\_HOOK\_JOB\_EXIT\_TIMEOUT} seconds before continuing with
the shutdown if the job exit hook has not completed.
\Ticket{2543}

\item In Condor 7.5.4, an improvement was made to avoid reliance on
  \MacroNI{NEGOTIATOR\_HOST} matching a reverse DNS lookup of the
  negotiator.  However, this improvement was not made to the dedicated
  scheduler, so matchmaking of parallel jobs was still subject to the
  problems associated with the old algorithm.  Now, the dedicated
  scheduler benefits from the same improved algorithm as the
  non-dedicated scheduler.  \Ticket{2540}
  
\item Occasionally there have been problem with Windows 
  \MacroNI{LOGON\_NETWORK} authentication leading to users 
  being locked out from their account.
  A new configuration variable, \MacroNI{SKIP\_WINDOWS\_LOGON\_NETWORK}, 
  was introduced which - if set to true - allows to skip this logon
  mechanism entirely. This only affects users using \Condor{credd}. 
  \Ticket{2549}  

\item Condor now correctly groups CREAM jobs based on how CREAM servers 
authorize and map them, this is, based on X509 proxy subject name and 
first VOMS attribute. Previously, all VOMS attributes were considered.
This could cause unexpected behavior due to aliasing of CREAM leases
