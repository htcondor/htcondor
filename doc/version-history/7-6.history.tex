...
\item Updated \Condor{userprio} callbacks to handle submitter names of arbitrary length.
\Ticket{2445}

\item Provided an informative error message when \Condor{userprio} cannot locate the \Condor{negotiator}.
\Ticket{2478}

\item Removed spurious boolean flag reset in \Condor{q} that resulted in order dependency between ``-long'' and ``-format''.
\Ticket{2498}
...
\item Fixed a memory leak in the \Condor{negotiator} caused by failure to
...
\item The \Condor{schedd} failed to start up when any job ClassAd
value contained ASCII character 255.
\Ticket{2450}

\item \Condor{preen} failed to honor -remove and would always remove
  lock files.
\Ticket{2497}

\item \Condor{preen} expected old format for local lock file paths, it
  now understands the proper cormat.
\Ticket{2496}

\item \Condor{preen} would EXCEPT when processing multiple subdirs for
  local locks.
\Ticket{2495}
