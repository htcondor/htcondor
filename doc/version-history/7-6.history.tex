...
\item The FS authentication method now works when \File{/tmp} is
...
\item Fixed a bug that caused failure to start jobs when using privsep
and supplemental group process tracking.  Prior to 7.6.4, this problem
only happened when \MacroNI{USE\_CLONE\_TO\_CREATE\_PROCESSES} was set
to false.  In 7.6.4, the problem happened regardless of the setting.
\Ticket{2658}
...
\item Fixed a bug where \Macro{transfer\_input\_files} did not work for directories on Windows.
\Ticket{2387}

\item Fixed a bug that could cause failure to clean up job processes
when using glexec after a restart of \Condor{master}.
\Ticket{2614}
...
idle jobs or send ads to the \Condor{collector}.
\Ticket{2647}
