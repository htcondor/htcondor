...
\item Previously, in \Condor{hadoop} if \MacroNI{HDFS\_DENY} was
undefined, \MacroNI{HOSTDENY\_WRITE} and \MacroNI{HOSTDENY\_READ}
were used to build the deny list.  Now \MacroNI{DENY\_WRITE} and
\MacroNI{DENY\_READ} are also used.
\Ticket{2414}
...
% gittrac #2096
\item Jobs run via glexec always had their \Env{PATH} environment
variable cleared.  Now, if \Env{PATH} was specified for the job, (in
any of the ways that job environment may be specified), this setting
is used.

% gittrac #2413
\item Fixed an infinite loop that could happen in Condor daemons
shortly after receipt of a new connection.  This problem was
introduced in 7.5.6.

\item Fixed a problem in \Condor{hadoop} that caused it to exit shortly
after starting up if \MacroNI{HDFS\_DENY}, \MacroNI{HOSTDENY\_WRITE}, and
\MacroNI{HOSTDENY\_READ} were undefined.
\Ticket{2414}
