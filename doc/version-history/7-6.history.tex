...
% gittrac #2016
\item Prior to Condor 7.5.0, commenting out \MacroNI{PREEN} in the
  default configuration file disabled \Condor{preen}.  Starting in
  7.5.0, there is an internal default value for \MacroNI{PREEN}, so if
  it is not set in any configuration file, \Condor{preen} will still
  run.  To disable it, \MacroNI{PREEN} can be explicitly set to
  nothing.
...
\item The configuration veriable \Macro{JAVA\_MAXHEAP\_ARGUMENT} has been removed. 
  This means that Java universe jobs will now run with the JVM's default maximum heap
  setting unless specified othwerwise by the user via \Macro{java\_vm\_args} in the submit file. 
...
\Condor{dagman}) macro.  (Now only '\$(...' is considered a macro.)
