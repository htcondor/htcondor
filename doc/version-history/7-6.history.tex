...
\item Fixed a bug in \Condor{softkill} that would cause it to fail
to kill the target process when run by a personal Condor inside a
System Condor slot account on Windows 7 and Vista.
\Ticket{2677}

\item \MacroNI{HDFS} was not listed in the default
\MacroNI{DC\_DAEMON\_LIST}, so \Condor{hdfs} exited
shortly after being started, and the HDFS service
did not run.
\Ticket{849}
