...
\item Removed restriction on having macros in submit files for \Condor{dagman}
\Ticket{2428}

\item \Condor{dagman} now uses, by default, watches only its default node
...
with whatever log is specified by the user.  DAGman only writes POST Script
TERMINATE events to its default log, not to the user log.  DAGman specifies
that the default user log is in plain text format, not XML format.
\Ticket{2807}
