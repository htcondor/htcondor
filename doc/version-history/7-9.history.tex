...
\item When delegating X509 proxies, HTCondor can now adjust the amount of
allowable clock skew between the two parties using the configuration variable
\Macro{GSI\_DELEGATION\_CLOCK\_SKEW\_ALLOWABLE} which is specified in seconds.
...
\item The administrator can now set \Macro{CheckpointPlatform} to change the
string used by the standard universe to determine if a job checkpointed on
one machine can be started on another.
\Ticket{3544}

%% Yes, SSSE 3 has three Ss.
\item Machine ClassAds now include \AdAttr{has\_sse4\_1},
\AdAttr{has\_sse4\_2}, and \AdAttr{has\_ssse3}, set if the corresponding
instructions are available on that machine.
\Ticket{3544}
