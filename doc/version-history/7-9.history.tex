...
\item A timer reset was added to the end of the negotiation cycle to reduce
drift in the negotiation cycle delay.
\Ticket{3332}

\item The default for the undocumented configuration parameter
  \Macro{X\_RUNS\_HERE} was inverted from TRUE to FALSE with the
  release of 7.7.3. Its default has been reset to TRUE. When FALSE,
  the \Condor{master} will not start the KBDD.

\item The batch\_gahp is no longer dependent on perl module XML::Simple
...
%\item HTCondor version 7.9.2 released on Month Date, 2012.
...
configuration knob, \Macro{JOB\_DEFAULT\_NOTIFICATION}, so that admins can
...
\Attr{X509UserProxySubject} to the wrong value when the job's X509
...
In some instances, modifying \Env{LD\_LIBRARY\_PATH} can cause the
...
partionable resources in the job log.
\Ticket{3318}
