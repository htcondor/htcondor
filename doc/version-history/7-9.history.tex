...
\item Fixed a bug that caused the schedd to crash when condor\_rm was
used with the \Opt{-f} option on parallel jobs.
\Ticket{3561}

\item Fixed a bug that could cause HTCondor-C jobs to fail when they are
part of a DAG. The jobs would be held with a hold reason of
``Failed to initialize user log to /dev/null or <DAG log path>''.
\Ticket{3474}

\item Fixed a bad file descriptor leak in \Condor{ft-gahp}.
\Ticket{3559}
...
\item Running \Condor{rm} with the \Opt{-f} option on a parallel job can cause
