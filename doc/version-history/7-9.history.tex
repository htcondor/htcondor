...
\item The \Condor{negotiator} now sanity-checks accountant log file once only
on startup.  Iteration through the accountant ClassAd log structure is faster.
\Ticket{3011}
...
\Condor{dagman} in the local universt to not write to the default node log
file, when \Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} is true (the default), and a
user log is also defined. This is fixed. \Ticket{3111}
\end{itemize}
