...
\item Added code to \Condor{dagman} so that VARS assignments in top-level DAGs are applied to splices
\Ticket{1780}

\item Condor now uses libraries from Globus 5.2.1.
\Ticket{2838}

\item When authenticating Condor daemons with GSI, Condor now checks
that the server name in the certificate matches the hostname that
the client is connecting to.  For backward compatibility, any daemon
certificates matching \MacroNI{GSI\_DAEMON\_NAME} are allowed,
whether or not they satisfy the hostname check.  Therefore, if
\MacroNI{GSI\_DAEMON\_NAME} is empty (the default), only
certificates passing the hostname check are authenticated.  If
\Expr{GSI\_DAEMON\_NAME=*} then no certificates will be rejected
due to hostname checks.  Rather than using
\MacroNI{GSI\_DAEMON\_NAME}, we recommend using the new
configuration settings \MacroNI{GSI\_SKIP\_HOST\_CHECK} and/or
...
(and new job ClassAd attribute \Attr{BatchQueue}) specified which job
