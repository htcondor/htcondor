...
\item Fixed a bug that caused fetching user priorities from the negotiator
to take a long time. User priorities requested during the negotiation cycle
will not necessarily have to wait until the end of the cycle to be returned.
\Ticket{3535}

\item Fixed a bug that would cause parallel universe jobs to fail when 
\Expr{USE\_NFS=True}. This may cause a potential issue with 
\Condor{chirp} \Opt{lookup}, though testing seems to show that this is not an issue.
\Ticket{3390}

\item Fixed a bug introduced in 7.9.4 that sometimes causeed
\AdAttr{ExitCode} and \AdAttr{ExitBySignal} not to be set in the job
ad when the job terminated.  These attributes were correctly
reported in the user log but were not propagated to the job ad and
