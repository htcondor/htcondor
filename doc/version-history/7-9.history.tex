...
\item When submitting jobs to a remote batch system (e.g. BOSCO), file
transfer no longer requires a network connection from the remote machine
...
\Condor{gridmanager} now ensures that no unsual characters are used in
the name of the job sandbox directory it creates.
\Ticket{3294}

\item When a GAHP server fails to start, the \condor{gridmanager} now
puts the affects jobs on hold.
\Ticket{3301}

\item Set \Env{GLOBUS\_LOCATION} in the environment of \Prog{batch\_gahp},
