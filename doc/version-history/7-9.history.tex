...
%\item HTCondor version 7.9.5 released on Month Date, 2013.
...
\item Allow the number of bits in a delegated proxy to be specified
by the receiving side using \Macro{GSI\_DELEGATION\_KEYBITS}
\Ticket{3503}

\item When there are multiple users
waiting to transfer files in the limits set by
\Macro{MAX\_CONCURRENT\_UPLOADS} and/or
\Macro{MAX\_CONCURRENT\_DOWNLOADS}, the scheduler now gives the users
...
\Condor{submit}, the job owner will be set by the \Condor{schedd} based
on how you were authenticated. This will make it easier to submit jobs
to a remote \Condor{schedd} where your credentials may map to a different
account name.
\Ticket{3370}

\item \Condor{dagman} now reports the DAG status (total number of nodes,
nodes queued, nodes finished, etc.) to its classad.  See
section~\ref{sec:DAGStatusClassad} for more information.
\Ticket{1782}
...
\item When using file transfer concurrency limits, additional I/O
  usage statistics are now published in the ClassAd of the
  ...
  users and individually for recently active users.  The ClassAd
  attributes are described below.  Each of these may be prefixed by a
  username (e.g. \AdAttr{username\_FileTransferUploadBytes}).  In this
  case, it represents activity by the specified user.  The published
  username is actually the file transfer queue name, as defined by
  \Macro{TRANSFER\_QUEUE\_USER\_EXPR}.  This expression defaults to
  the name of the job owner.  The attributes that are rates have a
  suffix that specifies the timespan of the exponential moving
  average.  By default the timespans that are published are 1m, 5m,
  1h, and 1d.  This can be changed by configuring
  \Macro{TRANSFER\_IO\_REPORT\_TIMESPANS}.  These attributes are only
  reported once a full timespan has accumulated.
...
\item[\AdAttr{FileTransferUploadBytesPerSecond\_<timespan>}]
  Exponential moving average over the specified timespan of the rate
...
\item[\AdAttr{FileTransferFileReadLoad\_<timespan>}] Exponential
  moving average over the specified timespan of the rate at which
...
\item[\AdAttr{FileTransferNetReadLoad\_<timespan>}] Exponential moving
  average over the specified timespan of the rate at which submit-side
...
\item The EC2 GAHP now correctly parses XML namespaces (as returned, for
instance, by some installations of Eucalyptus).  A related bug, which could
cause HTCondor to put the job on hold, incorrectly believing that the cloud
service had purged it, was also fixed.
\Ticket{3492}

\item The \Condor{negotiator} now checks to see if \Macro{MAX\_TIME\_PER\_SUBMITTER}
has been exceeded while negotiating with single \Condor{schedd}. This configuration
variable used to only be effective if a submitter used multiple schedds.
\Ticket{3504}
...
can cause a crash when transferring files for grid-type batch jobs going
to a remote cluster.
\Ticket{3529}
