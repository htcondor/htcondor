...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{\label{sec:New-7-9-7}Version 7.9.7}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item HTCondor version 7.9.7 not yet released.
%\item HTCondor version 7.9.7 released on Month Date, 2013.
...
\item For errors which have no response body, EC2 jobs now report the 
HTTP code instead.
\Ticket{3541}
...
\item The maximum size of a daemon log file can now be specified in seconds or bytes.
Only bytes was allowed previously.
\Ticket{3560}
...
\item When submitting a grid-type batch job, the value of
\SubmitCmd{request\_memory} is now propagated to the batch system
submission request.
\Ticket{3398}

\item The set of Python bindings introduced in HTCondor v7.9.4 is now
...
\AdAttr{Owner\_<username>\_}.  In 7.9.5, they had a prefix of
...
\item Fixed a bug in ASSIGN\_CPU\_AFFINITY when running on systems
with partitionable slots.
\Ticket{3597}

\item Fixed a bug in the condor\_starter where filesystems mounted by
the automounter would not be seen by the job.
\Ticket{3601}

\item Fixed the condor\_updates\_stats script so that it works with
...
\item Do not leak file descriptors in the \Prog{batch\_gahp} when reading
a job's X.509 proxy.
\Ticket{3558}
...
\item Fixed a bug with the dedicated scheduler reconnect mode, where
if the schedd crashed with a parallel universe job running after the 
job's first job lease duration interval, parallel universe jobs would 
not successfully reconnect, and they
would restart from scratch.
\Ticket{2291}
