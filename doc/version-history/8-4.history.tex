...
\item Added new parameter SCHEDD\_USE\_SLOT\_WEIGHT, which may be needed
if SLOT\_WEIGHT is not the default of Cpus, and refers to expressions
in the job ad.
\Ticket{5996}

\item Fixed bug in the Hold if Cpus Exceeded configuration template metaknob.
\Ticket{5933}

\item Fixed a bug in the LIMIT\_JOB\_RUNTIMES configuration template metaknob
...
\item Fixed a bug that allowed a user to bypass the \Macro{MAX\_JOBS\_PER\_OWNER}
...
in order to change \Macro{REMOVE\_SIGNIFICANT\_ATTRIBUTES} to remove an attribute that
...
\item When transferring a job's sandbox, the permissions on subdirectories
...
\item Compiled benchmarking programs as a Position Indenpendent Executable.
Position Indepentdent Executables are a requirement for entry into Debian 9.
\Ticket{5994}

\item Fixed a denial of service attack when using the \Condor{credd} on the
Windows platform.
\Ticket{5984}
