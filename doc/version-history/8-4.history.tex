...
\item HTCondor version 8.4.5 not yet released.
%\item HTCondor version 8.4.5 released on Month Date, 2016.
...
\item Fixed a bug where HTCondor would unconditionally retry non-success1ful
...
\item Fixed a bug that would cause schedds flocking to remote
...
the config setting \Macro{PER\_JOB\_HISTORY\_DIR}) may briefly
...
\item Tuned a bug fix from the 8.5.3 release that was designed to allow for
more than 100 dynamic slots to be a bit more generous in allocating Disk
so that the slots would less prone to fail to match subsequent jobs.
\Ticket{5535}
...
\item Fixed a bug in \Condor{off} \Opt{-peaceful} that could result in sending the
off command to machines that had never received the peaceful command that should have proceeded it.
\Ticket{5504}
