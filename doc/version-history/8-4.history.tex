...
\item Fixed a bug whereby the \Condor{collector} process would exit with an error several times per hour if config knob \Macro{NO\_DNS} is set to True.
\Ticket{5762}
...
item Fixed a bug in the Windows build that was assigning the same Windows version
info to all binaries built in a single directory. This resulted in crash boxes
for most of the HTCondor tools being reported as a crash of \Condor{gpu\_discovery}
\Ticket{5795}
