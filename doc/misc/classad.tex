...
\begin{description}
  ...
  \item[\Code{Boolean regexp(String pattern, String target \Lbr\ , String options \Rbr)}]
    ...
    Returns \Expr{TRUE} when \Expr{target} matches 
    the regular expression given by \Expr{pattern}.
...
% early. Stupid solution, but if it works. . .
  ...
  \item[\Code{String regexps}] \Code{(String pattern, String target, 
   String substitute \Lbr\ , String options \Rbr) } 
    ...
    given by string \Expr{pattern}
    to scan through the string \Expr{target}.
    When \Expr{target} matches the regular expression
    given by \Expr{pattern},
...
% LaTeX can't get the right stuff on a single line, so break the item[]
% early. Stupid solution, but if it works. . .
  \index{ClassAd functions!replace()}
  \item[\Code{String replace}] \Code{(String pattern, String target, 
   String substitute \Lbr\ , String options \Rbr) } 
    Uses the regular expression 
    given by string \Expr{pattern}
    to scan through the string \Expr{target}.
    Returns a modified version of \Expr{target},
    where the first substring that matches \Expr{pattern}
    is replaced by the string \Expr{substitute}, with
    backslash expansion performed.
    Equivalent to \Code{regexps()} with the \Expr{f} option.
    If any argument is not a string, returns \Expr{ERROR}.

% LaTeX can't get the right stuff on a single line, so break the item[]
% early. Stupid solution, but if it works. . .
  \index{ClassAd functions!replaceall()}
  \item[\Code{String replaceall}] \Code{(String pattern, String target, 
   String substitute \Lbr\ , String options \Rbr) } 
    Uses the regular expression 
    given by string \Expr{pattern}
    to scan through the string \Expr{target}.
    Returns a modified version of \Expr{target},
    where every substring that matches \Expr{pattern}
    is replaced by the string \Expr{substitute}, with
    backslash expansion performed.
    Equivalent to \Code{regexps()} with the \Expr{fg} options.
    If any argument is not a string, returns \Expr{ERROR}.
