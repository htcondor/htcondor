...
\begin{description}
  ...
  \item[\Code{Boolean stringList\_regexpMember(String pattern, String list \Lbr\ , String delimiter, String options \Rbr})]
    Returns \Expr{TRUE} if the string \Expr{pattern} is a regular expression
    that matches an item in the string \Expr{list}.
    The optional \Expr{delimiter} string says how to delimit,
    while string \Expr{options} modifies how the match is performed.
