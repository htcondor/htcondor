...
if an attribute name appears without the prefix \Expr{TARGET.} or \Expr{MY.},
...
As Condor now uses New ClassAds internally,
to refer to an attribute in the target ad,
its name \emph{must} be preceded with \Expr{TARGET.} in the expression.
...
adding \Expr{TARGET.} to attribute names that normally occur
in job ClassAds.
To extend this injection of \Expr{TARGET.} as needed for additional attributes,
the attributes are specified with the configuration variables
\Macro{TARGET\_MACHINE\_ATTRS} and \Macro{TARGET\_JOB\_ATTRS}.
