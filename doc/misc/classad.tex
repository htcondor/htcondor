...
The evaluation of expressions is also slightly different.
In New ClassAds,
when evaluating an expression in the context of two ClassAds,
for example when matching a job ad with a machine ad,
if an attribute name appears without a
the prefix \Expr{Target.} or \Expr{My.},
it is only looked up in the local ClassAd.
In Old ClassAds,
if the attribute is not in the local ClassAd,
it will then be looked up in the target ClassAd.
In New ClassAds, if to refer to an attribute in the target ad,
its name must be preceded with \Expr{Target.} in the expression.

Condor contains a routine that scans job ClassAd expressions,
adding \Expr{Target.} to attribute names that normally occur
in machine ClassAds,
 and vice versa. 
If a job or machine ClassAds contain additional attributes
that this routine is to look for,
use the \Macro{TARGET\_MACHINE\_ATTRS}
and \Macro{TARGET\_JOB\_ATTRS} configuration variables.
