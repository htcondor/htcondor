...
\begin{description}
  ...
  \item[\Code{String formatTime(\Lbr\ Integer time \Rbr\ \Lbr\ , String format \Rbr)}]
		...
		percent signs (\%) followed by a single letter. To have a percent sign in
		your output, you must use a double percent sign (\%\%).  If
		\Expr{format} is not specified, it will default to ``\%c''.
		...
		\begin{description}
			...
			\item[\%c] local date and time representation
			\item[\%d] day of the month (01-31)
			\item[\%H] hour in the 24-hour clock (0-23)
			\item[\%I] hour in the 12-hour clock (01-12)
			\item[\%j] day of the year (001-366)
			\item[\%m] month (01-12)
			\item[\%M] minute (00-59)
			\item[\%p] local equivalent of AM or PM
			\item[\%S] second (00-59)
			\item[\%U] week number of the year (Sunday as first day of week) (00-53)
			\item[\%w] weekday (0-6, Sunday is 0)
			\item[\%W] week number of the year (Monday as first day of week) (00-53)
			\item[\%x] local date representation
			\item[\%X] local time representation
			\item[\%y] year without century (00-99)
			\item[\%Y] year with century
			\item[\%Z] time zone name, if any
		\end{description}
