...
\item[\AdAttr{FileTransferUploadBytes}] Total number of bytes uploaded
  ...
  the username (e.g. \AdAttr{username\_FileTransferUploadBytes}).  The
  published username is actually the file transfer queue name, as
  defined by \Macro{TRANSFER\_QUEUE\_USER\_EXPR}.
...
\item[\AdAttr{FileTransferUploadBytesPerSecond\_<timespan>}]
  ...
  The timespans that are published are configured by
  \Macro{TRANSFER\_IO\_REPORT\_TIMESPANS}, which defaults to
  1m, 5m, 1h, and 1d.  When less than one full timespan has
