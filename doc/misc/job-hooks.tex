...
\item[Userid hook runs as]
  The \Macro{<Keyword>\_HOOK\_FETCH\_WORK} runs with the same
privileges as the startd.  When condor was started as root,
this is usually the "condor" user, or the user specified in
the CONDOR\_IDS parameter.
...
\item[Userid hook runs as]
  The \Macro{<Keyword>\_HOOK\_REPLY\_FETCH} runs with the same
privileges as the startd.  When condor was started as root,
this is usually the "condor" user, or the user specified in
the CONDOR\_IDS parameter.
...
\item[Userid hook runs as]
  The \Macro{<Keyword>\_HOOK\_EVICT\_CLAIM} runs with the same
privileges as the startd.  When condor was started as root,
this is usually the "condor" user, or the user specified in
the CONDOR\_IDS parameter.
...
\item[Userid hook runs as]
  The \Macro{<Keyword>\_HOOK\_PREPARE\_JOB} runs with the same
...
\item[Userid hook runs as]
  The \Macro{<Keyword>\_HOOK\_UPDATE\_JOB\_INFO} runs with the same
...
\item[Userid hook runs as]
  The \Macro{<Keyword>\_HOOK\_JOB\_EXIT} runs with the same
privileges as the job itself.
