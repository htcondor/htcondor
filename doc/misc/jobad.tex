...
3 & The \MacroNI{PERIODIC\_HOLD} expression evaluated to \Expr{True}. Or, \MacroNI{ON\_EXIT\_HOLD} was true & User Specified\\ \hline
...
\index{ClassAd job attribute!IOWait}
\index{IOWait!job ClassAd attribute}
\item[\AdAttr{IOWait}:]  I/O wait time of the job recorded by
the cgroup controller in seconds.
...
\index{ClassAd job attribute!JobCurrentFinishTransferInputDate}
\index{JobCurrentFinishTransferInputDate!job ClassAd attribute}
\label{JobCurrentFinishTransferInptDate-job-attribute}
\item[\AdAttr{JobCurrentFinishTransferInputDate}:]  Time at which the job most
recently finished transferring its input sandbox.  Measured in the
number of seconds since the epoch (00:00:00 UTC, Jan 1, 1970)

%%% ClassAd attribute: JobCurrentFinishTransferOutputDate
\index{ClassAd job attribute!JobCurrentFinishTransferOutputDate}
\index{JobCurrentFinishTransferOutputDate!job ClassAd attribute}
\label{JobCurrentFinishTransferOutputDate-job-attribute}
\item[\AdAttr{JobCurrentFinishTransferOutputDate}:]  Time at which the job most
recently finished transferring its output sandbox.  Measured in the
number of seconds since the epoch (00:00:00 UTC, Jan 1, 1970)
...
\index{ClassAd job attribute!JobCurrentStartTransferInputDate}
\index{JobCurrentStartTransferInputDate!job ClassAd attribute}
\label{JobCurrentStartTransferInputDate-job-attribute}
\item[\AdAttr{JobCurrentStartTransferInputDate}:]  Time at which the job most
recently began transferring its input sandbox.  Measured in the
number of seconds since the epoch (00:00:00 UTC, Jan 1, 1970)
...
\index{ClassAd job attribute!JobDisconnectedDate}
\index{JobDisconnectedDate!job ClassAd attribute}
\label{JobDisconnectedDate-job-attribute}
\item[\AdAttr{JobDisconnectedDate}:]  Time at which the \condor{shadow}
and \Condor{starter} became disconnected.
Set to \Expr{Undefined} when a successful reconnect occurs.
...
0 is used for comparison purposes.
...
\index{ClassAd job attribute!Requirements}
\index{Requirements!job ClassAd attribute}
\item[\AdAttr{Requirements}:] A classad expression evaluated by the
\Condor{negotiator}, \Condor{schedd}, and \{Condor{startd} in the context of 
slot ad.  If true, this job is eligible to run on that slot.  If the job 
requirements does not mention the (startd) attribute \Macro{OPSYS}, the 
schedd will append a clause to Requirements forcing the job to match the 
same \Macro{OPSYS} as the submit machine.  The schedd appends a simliar 
clause to match the \Macro{ARCH}.  The schedd parameter
 \Macro{APPEND\_REQUIREMENTS}, will, if set, append that value to every 
...
\index{ClassAd job attribute!TransferInFinished}
\index{TransferInFinished!job ClassAd attribute}
\item[\AdAttr{TransferInFinished}]:
When the job finished the most recent recent transfer of its input sandbox,
measured in seconds from the epoch.
(00:00:00 UTC Jan 1, 1970).

%%% ClassAd attribute: TransferInQueued
\index{ClassAd job attribute!TransferInQueued}
\index{TransferInQueued!job ClassAd attribute}
\item[\AdAttr{TransferInQueued}]:
If the job's most recent transfer of its input sandbox was queued, this
attribute says when, measured in seconds from the epoch
(00:00:00 UTC Jan 1, 1970).

%%% ClassAd attribute: TransferInStarted
\index{ClassAd job attribute!TransferInStarted}
\index{TransferInStarted!job ClassAd attribute}
\item[\AdAttr{TransferInStarted}]:
When the job actually started to transfer files, the most recent time it
transferred its input sandbox, measured in seconds from the epoch.  This
will be \emph{later than} \AdAttr{TransferInQueued} (if set).
(00:00:00 UTC Jan 1, 1970).
...
\index{ClassAd job attribute!TransferOutFinished}
\index{TransferOutFinished!job ClassAd attribute}
\item[\AdAttr{TransferOutFinished}]:
When the job finished the most recent recent transfer of its output sandbox,
measured in seconds from the epoch.
(00:00:00 UTC Jan 1, 1970).

%%% ClassAd attribute: TransferOutQueued
\index{ClassAd job attribute!TransferOutQueued}
\index{TransferOutQueued!job ClassAd attribute}
\item[\AdAttr{TransferOutQueued}]:
If the job's most recent transfer of its output sandbox was queued, this
attribute says when, measured in seconds from the epoch
(00:00:00 UTC Jan 1, 1970).

%%% ClassAd attribute: TransferOutStarted
\index{ClassAd job attribute!TransferOutStarted}
\index{TransferOutStarted!job ClassAd attribute}
\item[\AdAttr{TransferOutStarted}]:
When the job actually started to transfer files, the most recent time it
transferred its output sandbox, measured in seconds from the epoch.  This
will be \emph{later than} \AdAttr{TransferOutQueued} (if set).
(00:00:00 UTC Jan 1, 1970).
