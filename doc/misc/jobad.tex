...
Note: events are written to this log file in addition to any log file
specified in the job's submit file.
...
(the auxiliary log).  (All events not specified in the
\Attr{DAGManNodesMask} string are not written to the auxiliary event log.)
The value of this attribute is determined
by DAGMan, and is passed to the job via the \Condor{submit} command line.
