...
\item[\AdAttr{PreserveRelativeExecutable}:]  If this attribute is set
to \Expr{True} then the \Condor{starter} will not prepend \AdAttr{Iwd}
to \AdAttr{Cmd}, when \AdAttr{Cmd} is a relative pathname
and \AdAttr{TransferExecutable} is \Expr{False}.  The default value
for \AdAttr{PreserveRelativeExecutable} is \Expr{False}.  This
attribute is of primary interest for users of \Macro{USER\_JOB\_WRAPPER}
for the purpose of allowing an executable to be resolved by the user's
path in the job wrapper.
