...
Defines the maximum amount (per hour) you're willing to pay to run this job.
...
Used for grid type ec2 jobs;
reports the reason for most recent EC2-level state transition.  Can be used
to determine (among other things) if a Spot Request was terminated because
the spot price rose.
...
%%% ClassAd attribute: Group
\index{ClassAd job attribute!Group}
\index{Group!job ClassAd attribute}
\item[\AdAttr{Group}:] The accounting group name, as set in the
\Condor{submit} file via the group command.  This attribute
...
%%% ClassAd attribute: GroupUser
\index{ClassAd job attribute!GroupUser}
\index{GroupUser!job ClassAd attribute}
\item[\AdAttr{GroupUser}:] The user name associated with the
...
The total size in megabytes of input files to be transferred for the
job.  Files transferred via file transfer plugins are not included.
