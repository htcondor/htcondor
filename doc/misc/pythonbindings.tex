...
which always have the \texttt{type} property.  Events
which are not of type \texttt{JobEventType.NONE} always have the
\texttt{cluster} and \texttt{proc} properties, and may have other properties,
...
\texttt{events( deadline )}

Return self (an iterator), after setting the deadline.  If the
\texttt{deadline} argument is \texttt{None}, there is no deadline and the
iterator will block forever.  Otherwise, the deadline is the current time
plus the \texttt{deadline} argument, and the iterator will block waiting
for new events up until the deadline.
