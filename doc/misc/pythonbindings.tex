...
\texttt{events( stop_after )}

Return an iterator (self), which will stop waiting for new events after
\texttt{stop_after} seconds have passed.  The iterator may return an
arbitrary number of events, including zero, before \texttt{stop_after} seconds
have passed.  If the \texttt{stop_after} argument is \texttt{None},
