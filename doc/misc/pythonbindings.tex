...
\\ \hline
\texttt{drainJobs( (int)drain_type, (bool)resume_on_completion, (expr)check ) }

Begin draining jobs from the startd.
Returns a draining request_id.

Parameter \texttt{drain_type} type of drain to perform, from the DrainTypes enum either Fast, Graceful or Quick.
Parameter \texttt{resume_on_completion} is true if the startd should start accepting jobs again once
...
\\ \hline
\texttt{cancelDrainJobs( (int)request_id ) }

Cancel a draining request.

Parameter \texttt{request_id} If specified, cancels only the drain command that returned the given request_id
\\ \hline
\end{tabular}
\end{flushleft}
