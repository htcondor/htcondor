...
Given a python function \texttt{function}, register it as a ClassAd function.
This allows you to invoke the python function from within a ClassAd evaluation
context.  The optional argument, \texttt{name}, allows you to give the function
an alternate name within the ClassAd library.
...
Returns a python list of external references found in \texttt{expr}.  An
``external reference" is any attribute in the expression which is not
found in the \texttt{ClassAd}.

\\ \hline
\texttt{internalRefs( expr )}

Returns a python list of internal references found in \texttt{expr}.  An
``internal reference" is any attribute in the expression which is
found in the \texttt{ClassAd}.
\\ \hline
