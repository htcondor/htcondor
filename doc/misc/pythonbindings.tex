...
Parameter \texttt{opts} specifies any additional query options.  Non-default options are
\texttt{QueryOpts.AutoCluster}, which returns autoclusters in the schedd, not jobs,
and \texttt{QueryOpts.GroupBy}, which returns aggregates, not jobs.
\texttt{QueryOpts.GroupBy} which must be used with an attr\_list.
When the \texttt{QueryOpts.GroupBy} option is supplied, the returned ads will be similar to those
returned by the \texttt{QueryOpts.AutoCluster}, but calculated on the fly using the
provided attr\_list as the list of attributes to aggregate on.  When either of these options
are used, each returned ad will have a JobCount attribute that indicates how many jobs have
that set of values for the set of attributes returned, and a JobIds attribute that has the smallest
and largest job id for that aggregate.
...
Note this method is deprecated and may be removed in future HTCondor releases. 
To submit jobs, please use the  Submit Class (documented below) instead of 
this method.
...
\textbf{The \texttt{SubmitResult} class:}
\begin{flushleft}
\begin{longtable}{|p{16cm}|} \hline
...
\end{longtable}
\end{flushleft}
\pagebreak% This forces the next paragraph, heading, and table onto the same
% page.  There's a different way to do that, but this is one is nice and easy.
\subsubsection{JobEventLog}
\label{Python-JobEventLog}
The \PyBind{JobEventLog} reads HTCondor job event (user) logs.  See the example
in section \ref{Python-Example}.  Its iterators return \PyBind{JobEvent}s,
...
be accessed via the subscript operator.  \PyBindNI{JobEvent} objects
...
Closes any open underlying file.  Subsequent iterations on this JobEventLog
will immediately terminate (will never return another JobEvent).
...
The following is an example of using the \PyBind{JobEventLog} class:
...
Print the ClassAd in JSON format. 
