...
at \AdAttr{TotalMachineDrainingBadput}.  Therefore, it does not treat
checkpointed evictions differently from non-checkpointed ones.  When
\Condor{startd} restarts, its counters start over from 0, so the
...
by looking at \AdAttr{TotalMachineDrainingUnclaimedTime}.  When
\Condor{startd} restarts, its counters start over from 0, so the
average is only over the time since the daemons have been alive.
...
\index{ClassAd Defrag attribute!MyCurrentTime}
\item[\AdAttr{MyType}:]  This is set to ``Defrag''.
