...
%%
%% TODO! 
%% Flesh out this list and provide useful explainations
%% 
...
\begin{description}
 ...
 \item[\Attr{Cmd}] (string, executable path)

 \item[\Attr{Iwd}] (string, initial working directory)
...
\begin{description}
 ...
 \item[\Attr{In}] (string, path to stdin)

 \item[\Attr{Out}] (string, path to stdout)

 \item[\Attr{Err}] (string, path to stderr)

 \item[\Attr{Env}] (string, environment)

 \item[\Attr{Args}] (string, arguments)

 \item[\Attr{JobUniverse}] (integer, {5 = vanilla, 10 = java} are the
 allowed values, vanilla is the default if not specified)

 \item[\Attr{JarFiles}] (string, JAR files for java universe)

 \item[\Attr{KillSig}] (int, signal to send for graceful shutdown,
 SIGTERM by default)

 \item[\Attr{StarterUserLog}] (string, path to UserLog written by
 \Condor{starter}) 

 \item[\Attr{StarterUserLogUseXML}] (bool, should UserLog use XML or
 old Condor syntax)

 \item[\Attr{ClusterId}] (int, job cluster id, can also specify to
 \Condor{cod} with \Opt{-cluster})

 \item[\Attr{ProcId}] (int, job process id, can also specify to
 \Condor{cod} with \Opt{-proc})

\end{description}

\Note If any path attribute (Cmd, In, Out, Err, StarterUserLog) is not
...
\Note You should not define \Attr{Owner} in this case, since Condor
