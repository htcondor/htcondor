...
feature is always enabled on the \Condor{startd} but is limited to the 
...
use the \SubmitCmd{load\_profile} command in the job's submit file as
follows:
\begin{verbatim}
load_profile = true
\end{verbatim}

\Note{This feature is currently not compatible with \SubmitCmd{run\_as\_owner} 
and will be ignored if the two are combined.}
