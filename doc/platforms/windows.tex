...
\Condor{schedd}.  This approach works in environments where
the user's password is only needed on the submit machine, which
is the case unless the \SubmitCmd{run\_as\_owner} capability described
...
When the \SubmitCmd{run\_as\_owner} feature is needed, it is necessary
to configure a centralized \Condor{credd} daemon to manage the secure
password store.  This makes each user's password available, via an encrypted
connection to the \Condor{credd}, to any execute machine that may
need it. The following section contains instructions for setting up
a \Condor{credd} to manage passwords.
...
password storage and retreival. It is first necessary to select
a machine on which to run the \Condor{credd}. Often, the machine
acting as the pool's central manager is a good choice. An important
restriction, however, is that the \Condor{credd} host must be a
machine running Windows.

All configuration settings necessary to enable the \Condor{credd} are
contained in the example \File{etc$\backslash$condor\_config.local.credd}
...
the \Macro{LOCAL\_CREDD} setting customized to point to the machine
hosting the \Condor{credd} and the \Macro{ALLOW\_CONFIG} setting
...
\File{etc$\backslash$condor\_config.local.credd} example file.)
\begin{verbatim}
...
authentication method to faciliate secure communication between execute
machines and the \Condor{credd}. In order for \verb@PASSWORD@ 
...
stored on the \Condor{credd} host, then the other machines in the pool.

To store the pool password on a given machine, run
\Code{condor\_store\_cred -c add} when logged in with the administrative
...
\Code{condor\_store\_cred -c -n exec01.cs.wisc.edu add}.

Once the pool password is distributed, execute
\Code{condor\_reconfig -all} from the central manager. This will cause
each execute machine to test its ability to authenticate with the
\Condor{credd}. To see whether this test worked for each machine in the
pool, run the command
\Code{condor\_status -f "\%s$\backslash$t" Name -f "\%s$\backslash$n" ifThenElse(isUndefined(LocalCredd),$\backslash$"UNDEF$\backslash$",LocalCredd)}.
...
can simply set the \SubmitCmd{run\_as\_owner} macro in the job's
submit file to True.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:windows-scripts-as-executables}Using Windows \
Scripts as Job Executables}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Historically, Condor has had limited support for scripting jobs on
Windows.  Typically, jobs on Windows used executables or batch files.
This limitation has now been lifted: any script recognized by Windows
can be used in place of a job's \SubmitCmd{executable} attribute.
Condor can determine how to interpret the script using the file's
extension.  If there is no file extension, the file will be treated as
it has been in the past, as a Windows executable.

This feature requires little to no configuration, depending on the
type of scripts being used.  For instance, Perl scripts, using
\textit{ActivePerl} require no administrative intervention. On the
other hand, \textit{Windows Scripting Host} scripts require a small
amount of configuration to work correctly.

In Microsoft nomenclature, \textit{verbs} are actions that can be
taken upon a given a file. These are most familiar to users in the
form of the words \textbf{Open}, \textbf{Print...}, \textbf{Edit},
etc. which are displayed in the context menu when a file is
right-clicked on. The commands for each of these verbs is stored in
the registry under the \Code{HKEY\_CLASSES\_ROOT} hive. In general,
they take the form:

\begin{verbatim}
HKCR\<FileType>\Shell\<OpenVerb>\Command
\end{verbatim}

Where \textit{FileType} is the name of a file type, \textit{OpenVerb}
is the verb and \textit{Command} is the method by which the file is
interpreted. In the Condor configuration file we allow for the
selection of any one verb to act as the file type's interpreter. As
mentioned above, Condor uses the file's extension to make the
distinction between file types.  This extension is used in the
configuration variable to change the verb Condor uses to interpret the
file type. For instance, the following:
...
Tells Condor to use the \textbf{Open2} verb when interpreting scripts
with the given file extensions.  The example tells Condor to use the
command-line version of the \textit{Windows Scripting Host}, rather
than the default windowed version, whose command is defined in the
\textbf{Open} verb. This is done to allow the standard input, output
and error to be redirected, if necessary|which is not possible when
using the Windows interpreter.

Some script interpreters which require access to the user's registry,
like the \textit{Windows Scripting Host} scripts, will fail if they
are not given registry access:

\begin{verbatim}
CScript Error: Loading your settings failed. (Access is denied.)
\end{verbatim}

To remedy this error, and possibly many other file type errors, add
the \SubmitCmd{load\_profile} attribute to the job's submit script:

\begin{verbatim}
load_profile = True
\end{verbatim}

After this, there should be no registry access errors.  This technique
should also work on many other interpreters; however, it may also be
the case that it is not required at all. For instance,
\textit{ActivePerl} does not, by default, require access to the user's
registry hive (unless the script explicitly reads or modifies the
user's registry settings) so the above attribute can be omitted.
