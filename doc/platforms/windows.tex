...
a machine on which to run the \Condor{credd}. 
Often, the machine
...
contained in the example \verb|etc\condor_config.local.credd|
...
on all Condor nodes. 
A copy of these settings is available in the comments contained in the
...
authenticated communication to work, a \Term{pool password} must be chosen
...
\begin{verbatim}
  condor_store_cred -c -n exec01.cs.wisc.edu add
\end{verbatim}
