...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{\label{sec:vmware-symlink-bug}My VMware jobs are failing and being put on hold.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Strange behavior has been noted when Condor tries to run a VMware
job using a path to a VMX file that contains a symlink. An example
of an error message you might see in such a job's user log would
be:
\begin{verbatim}
...
\begin{itemize}
\item If you are using file transfer
(\SubmitCmd{vmware\_should\_transfer\_files = true}), then ensure that
the \Macro{EXECUTE} settings on all execute machines do not contain
symlink path components.
\item If you are using a shared file system, ensure that the
\SubmitCmd{vmware\_dir} setting in your submit file does not use
symlink path name components.
\end{itemize}
...
Running Condor on an affected Xen kernel oftens results in failures
