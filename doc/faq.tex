...
supposed to represent the total load average on the system caused by
any running Condor job(s).
Unfortunately, it's basically impossible to get an accurate number for
this, without support from the operating systems which is not
available.
So, Condor does the best it can, and it mostly works in most cases.
However, there are a number of ways things can go wrong.
...
So, the default suspend policy now shipped with Condor uses the
...
explaination is presented:

When a job has periodic load behavior, and its load is changing over
time, the system load changes over time.  
...
It seems like Condor should be able to avoid this problem, but for a
host of reasons there is no space to describe here, it can't.  
It turns out that there's no good way (without help which doesn't
exist from the operating systems Condor runs on) to get this right.
The only way we could try to compute these numbers more accurately
...
A simliar sampling error can occur when Condor is starting a many
...
What is needed is a way for Condor to check to see if the load of the
machine has been high over a certain period of time.
There is a startd attribute, \AdAttr{CpuBusyTime} that can be used for
this purpose.
This macro returns the time \MacroUNI{CpuBusy} (defined in the default
config file) has been true, or 0 if \MacroUNI{CpuBusy} is false.
