...
ImageSize is in kilobytes).  \Macro{ImageSize} is not a perfect measure
of the memory requirements of a job.  It over-counts memory that is shared
between processes.  It may appear quite large if the job mmaps a large file.
It does not account for memory that the job uses indirectly in the operating
system's filesystem cache.
...
memory requirements of your program, in megabytes. This example says that
your program requires 1 gigabyte of memory. If you underestimate the
...
jobs when they get a big ImageSize, you will need to change those
policies.
