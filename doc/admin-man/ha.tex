...
The \Condor{had} and \Condor{replication} daemons cannot use the
\Condor{shared\_port} daemon.
They require their own separate ports, specified in the configuration
file (see example below).
Other daemons on the machine, including the \Condor{negotiator}, can
use \Condor{shared\_port} when the high availability daemons are in use.
