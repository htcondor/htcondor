...
The \Condor{had} and \Condor{replication} daemons use the
\Condor{shared\_port} daemon by default.  If you want to use more than one
\Condor{had} or \Condor{replication} daemon with the \Condor{shared\_port}
daemon under the same master, you must configure those additional daemons
to use nondefault socket names.  (Set the \texttt{-sock} option in
\MacroNI{<NAME>\_ARGS}.)  Because the \Condor{had} daemon must know the
\Condor{replication} daemon's address a priori, you will also need to set
\MacroNI{<NAME>.REPLICATION\_SOCKET\_NAME} appropriately.
...
The configuration for machines that will \emph{not} be central managers
...
%high availability.
%\item Configuration variables \MacroNI{ALLOW\_NEGOTIATOR} and
%\MacroNI{ALLOW\_ADMINISTRATOR} contain all machines
