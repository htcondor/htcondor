...
\Note The Condor high availability mechanisms discussed below
...
\Condor{collector}s and \Condor{negotiator}s, and doesn't query
redundant collectors to get the current \Condor{negotiator} (as it
does when communicating with it's local pool).  As a result, if the
default \Condor{negotiator} fails, the \Condor{schedd} doesn't learn
about it, and, thus, talk to the new \Condor{negotiator}.

\item When the \Condor{negotiator} is unable to communicate with a
\Condor{collector}, it ``moves'' to the next in the list.
Unfortunately, it doesn't go back to the start of the list.  When
combined with the above, a backup \Condor{negotiator} will never get
