...
\item Use \Prog{perl} to run the \Prog{create\_ca} script, providing the fully-qualified pathname of the install directory (e.g., perl create\_ca /usr/local/condor/ca).
...
run: condor_cert <certificate directory to create> 
	[suggested directory: $HOME/.globus]
\end{verbatim}

\item Upon success, mail the certificate request (<cert dir>/usercert\_request.pem) to your condor admin account (at cs.wisc.edu, it's "condorca")

\item If approved, the admin will send you a signed certificate, which you must save as <cert dir>/usercert.pem

\item Authenticated submissions require a variable "cert\_dir" to be specified
in the submit file, which is set to the full path of their certificate
directory.  Under the current configuration, the new schedd will allow
remote submission.  Here is a sample submission file:
\begin{verbatim}
   cert_dir = /home/yourname/.condorcerts
