...
\Condor{collector}, with the exception of the collector forwarding
updates to any collectors specified in \MacroNI{CONDOR\_VIEW\_HOST},
where UDP is used.
...
\item[\Macro{UPDATE\_VIEW\_COLLECTOR\_WITH\_TCP}]
  ...
  forward updates to collector(s) specified by \MacroNI{CONDOR\_VIEW\_HOST},
...
\item[\Macro{TCP\_UPDATE\_COLLECTORS}]
  ...
  \MacroNI{UPDATE\_COLLECTOR\_WITH\_TCP} is set to \Expr{False}.
...
Each HTCondor daemon that sends updates to the \Condor{collecotr} will have
1 socket open to it.
