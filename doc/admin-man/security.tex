...
% Note: the "process information on MacOS X" case is kinda a bug.
% An existing interface was made root only because it gave away too
% much information.  However Apple is working on a new, safe and
% available to all users interface.  At which point this example
% won't be true and a new example will be needed. The old example
% used to be "load information on IRIX", but we no longer support
% IRIX.
   In addition, some system information cannot be obtained without
   \Login{root} access on some platforms (such as process information on MacOS X).  As a
...
\item[\Condor{kbdd}] On platforms that need the \Condor{kbdd} (Digital
    Unix) the \Condor{kbdd} must run as \Login{root}.  If it is
...
It is important to note that the pool password, as a widely
distributed secret, is only as safe as the ``weakest link'' in terms
of the machines on which it is stored. If a malicious user were to
discover the pool password, they could act as a trusted component of
Condor. Attack possibilities include things as bad as telling Condor
to execute arbitrary code on behalf of any user on any execute
machine. When considering password authentication, it is important to
ensure that all machines that will store the pool password (and all
users that have \Login{root} or \Login{Administrator} access on those
