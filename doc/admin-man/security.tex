...
the \Condor{negotiator} daemon.  Security negotiation happens when one
Condor daemon or tool sends a command to another Condor daemon.  Negotiation
between users for machines, on the other hand, is what the \Condor{negotiator}
manages as part of its matchmaking process.
...
(This example illustrates the point, but there is actually more to the
story.  Some commands, such as submitting jobs \emph{always} require
authentication, no matter what the policy says, because the identity
of the user needs to be known in order to carry out the operation.
...
map file do not specify a domain for the user (i.e. \verb|user@domain|),
...
# authorize based on usernames produced by the map file
...
to use for mapping GSI names to usernames within condor.  For example,
...
Under Windows (or Unix if you wish), storing the pool password is done
via the \Opt{-c} option to \Condor{store\_cred}. Running
\begin{verbatim}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:Security-Password-Example} Example Security Configuration Using Pool Password}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:Security-Password-Example} Example Using Pool Password for Startd Advertisement}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
involves a single shared secret.  This does not scale well as you add
remote users who configure their schedds to flock to your pool.
However, you can still use the pool password for authenticating portions
of your pool, while other parts (such as remote schedds) are authenticated
by some other means.

In this example, just the startds in the pool are required to have the
pool password when they advertise themselves to the collector.
...
to use the GSI grid mapfile (configured with \Macro{GRIDMAP}
...
In addition, the following are used to control authorizationi of
...
with the \Macro{SLOTx\_USER} configuration variables,
...
\item{
On Windows, \MacroNI{SLOTx\_USER} will only work if the credential
