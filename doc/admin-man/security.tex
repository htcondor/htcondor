...
The Condor system makes a lot of network connections between different
daemons.  
If each one of these was to be authenticated and new keys
were generated for each connection, Condor would not be able to scale
well.  
Therefore, Condor uses the concept of \emph{sessions} to cache
relevant security information for future use and greatly speed up the
establishment of secure communications between the different Condor
daemons.

The first time a connection is made from one daemon to another, a
session is established.  
This session has a lifetime, after which it expires and can no longer
be used but a new session must be created
But while a valid session exists, it can be re-used as many times as
needed, thereby saving expense on future connections. 
There is a \emph{session key} that both sides know, and this key proves the
identity of the entity on either end of the connection because it was
exchanged securely if you use a "strong" authentication method such as
Kerberos or GSI. 
Other authentication methods like \MacroNI{NTSSPI},
\MacroNI{FS\_REMOTE},  \MacroNI{CLAIMTOBE}, and
\MacroNI{ANONYMOUS} do not support secure key exchange, so someone
listening on the wire may be able to impersonate the client or server
if you are using sessions. 

In order for sessions to be secure, you must have encryption or
integrity enabled. 
...
If encryption or integrity is not used, then it is possible for an
...
\MacroNI{SEC\_DEFAULT\_NEGOTIATION}. This takes parameters similar to
...
of Condor.  
Condor tools like \Condor{q}, \Condor{status}, etc. create a
session that expires after one minute. 
(Theoretically they should not create a session at all because the
session cannot be reused between program invocations, but this is
difficult to do in the general case.)
This allows a very small window for any possible attack, and helps
keep the memory footprint of running daemons down because they are not
keeping track of all of the sessions.
You can manually tune the session durations yourself using macros in
