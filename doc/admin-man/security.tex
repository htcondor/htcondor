...
\item[Users] Because authorization or capability in an operating system is
...
\Login{root}, while other common operations are to be owned by a
separate user.  This is usually the \Login{condor} user, but any user
can be chosen.
...
checks. 
For example, the job submission command (\Condor{submit}) can be restricted
...
categorized into groups called \Term{access levels},
based on the type of operation performed.
The user of a specific commands must be authorized at
an access level.
For example,
the \Condor{status} command requires the \DCPerm{READ} access level.
...
Condor tools such as \Condor{submit} and \Condor{config\_val} are client tools.
...
How can Condor ensure that commands are really being sent by trustworthy users?
What if jobs are operating on sensitive data that should not be seen by outsiders?
How can a job know its data has not been tampered with?
These issues
can be addressed with Condor's \emph{authentication},
\emph{encryption}, and \emph{integrity} features. But before we
discuss any of these features, we'll look into the process by which
Condor decides which (if any) of these features will be used for a
...
be used when a connection is established is called \emph{security
negotiation}. Negotiation's primary purpose is to determine what
\emph{features} (authentication, encryption, and integrity) will be
enabled for a connection. In addition, since Condor supports multiple
technologies for authentication and encryption, negotiation will also
decide which technologies are compatible between the client and
daemon sides of the connection.
...
%to decide upon the security settings. The daemon replies to the client
%with a set of reconciled policies that controls the communication,
%including authentication, encryption, and integrity algorithms.  For
%instance, if the client insists upon using GSI authentication, while
%the daemon considers it optional, then GSI authentication will be
%used.
...
the \Condor{negotiator}.

The security policy used during negotiation is determined by a set of
configuration macros. These macros belong to two classes: those that
affect negotiation of security features, and those that affect the
negotiation of methods within a specific feature. The following
sections discuss these settings in detail.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:security-negotiation-features}
               Negotiation of Security Features}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The macros that determine what features will be used during
client-daemon communication follow the pattern:
\begin{verbatim}
    SEC_<context>_<feature>
\end{verbatim}

Ignoring the \verb@<context>@ component for now, the \verb@<feature>@
component determines which security feature's policy is being
set. Continuing Frida's example, she wants to avoid using
authentication if possible and sets:
\begin{verbatim}
    SEC_DEFAULT_AUTHENTICATION = OPTIONAL
\end{verbatim}
The \Condor{schedd} that she would like to submit to, however, is
...
The security feature policy macros can be set to any of the following
values:
\begin{verbatim}
    REQUIRED
    PREFERRED
...
Table~\ref{table:Security-Negotiation} shows how security negotiation
resolves various client-daemon combinations of security feature policy
settings. It is clear from this table that Frida's (client) setting of
\verb@OPTIONAL@ along with the \Condor{schedd}'s (daemon) setting of
\verb@REQUIRED@ result in authentication being enabled for their
communications.

\begin{table}[tb]
\centering
\label{table:Security-Negotiation}
\begin{tabular}{|c|c|c|c|c|c|}
  ...
  \hline
\end{tabular}
\caption{This shows how negotiation determines whether a given security
         feature will be used. ``Yes'' means the feature will be
	 used during communication, while ``No'' means it will not. ``Fail''
         means that the policy settings are incompatible and the communication
	 cannot continue.}
\end{table}

Security feature policy macros exist for the following values of
\verb@<feature>@:
\begin{verbatim}
    AUTHENTICATION
    ENCRYPTION
...
are possible unless verb@NEGOTIATION@ is used. In addition, neither
\verb@ENCRYPTION@ or \verb@INTEGRITY@ may be used on a connection
unless \verb@AUTHENTICATION@ is enabled (since authentication is
needed to provide a secure key exchange).

%It may seems strange that \verb@NEGOTIATION@ is part of the above
%list. How can Condor components negotiate regarding whether or not
%they'll negotiate? The answer is that the protocol for determining
%whether negotiation will be used is much less flexible. Table X is
%annotated to show cases where the \verb@NEGOTIATION@ policy fails to
...
The full list of possible values for the \verb@<context>@ component is:
\begin{verbatim}
    CLIENT
    READ
    WRITE
...
Setting \verb@SEC_CLIENT_<feature>@ will determine the policy for all
...
actions (\DCPerm{ADMINISTRATOR}=level commands) require authentication
but not be so strict regarding inquiries on pool status
(\DCPerm{READ}-level commands). Such a policy could include the
settings:
\begin{verbatim}
...
Finally, the \verb@DEFAULT@ value for \verb@<context>@ will be checked
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{\label{sec:security-negotiation-methods}
...
Condor can support a variety of \emph{methods}, or technologies, for
...
The \verb@<context>@ part has the same meaning as it does for the
...
(\ref{sec:Security-Authentication}) and encryption
(\ref{sec:Security-Encryption}) for all possible names.
