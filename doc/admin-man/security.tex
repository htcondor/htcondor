...
while other users are allowed administrative priveleges over Condor itself.
...
The \verb@DEFAULT@ value for \verb@<context>@ is utilized
by Condor when none of the specific policy settings
(\verb@CLIENT@, \verb@READ@, \verb@WRITE@, etc.)
are present.
...
For the daemon side, there are seven macros to configure authentication:
\index{SEC\_DEFAULT\_AUTHENTICATION macro@\texttt{SEC\_DEFAULT\_AUTHENTICATION} macro}
\index{configuration macro!\texttt{SEC\_DEFAULT\_AUTHENTICATION}}
