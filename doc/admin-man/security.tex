...
Security in Condor is a broad issue, with many aspects to
consider. Because Condor's main purpose is to allow users to run
arbitary code on large numbers of computers, it is important to try to
limit who can access a Condor pool and what privileges they have when
using the pool. This secdtion covers these topics in depth. 

It is important to understand what kinds of attacks on your resourcess
Condor can prevent and which ones it cannot prevent. If a user knows
of a security exploit that can elevate their privilege to the root or
administrator account, Condor will not prevent them from being able to
do so. Condor does not run jobs in sandboxes (standard universe jobs
are a partial exception to this), so it cannot prevent user jobs from
malicious actions such as launching distributed denial of service
attacks. Condor assumes that you can trust your users. Condor can
prevent unauthorized access to your Condor pool to help you ensure
that only trusted users have access to your pool. In addition, Condor
can provide encryption and integrity checking to ensure that data
(Condor's and users' data) has not been examined to tampered with. 
...
\item[Users] What user is used to run the Condor daemons? What user is
used to run jobs? The short answer to the first question is that we
prefer that the daemons are run as root, but that there is another
user they can switch to for most operations. This is usually the
``condor'' user, but any user can be chosen. This topic is discussed
...
and executables. Note that the encryption is uesd for network
