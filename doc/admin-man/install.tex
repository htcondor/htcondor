...
Our packaging provides a Yum repository, and installation and configuration
in one easy step.
It is currently available for Linux systems only.
As of Condor version 7.5.1, native packaging is utilized,
such that items are placed in FHS locations.

Yum repositories are named to distinguish stable releases from
development releases and by RHEL version number. 
There are currently RPMs in four directories:
\begin{itemize}
 \item \File{/p/condor/public/html/yum/stable/rhel5/}
 \item \File{/p/condor/public/html/yum/stable/rhel4/}
 \item \File{/p/condor/public/html/yum/development/rhel5/}
 \item \File{/p/condor/public/html/yum/development/rhel4/}
\end{itemize}

Add or remove the package, as appropriate.
Then update the repository index, utilizing the correct directory.
For example,
\begin{verbatim}
  createrepo /p/condor/public/html/yum/stable/rhel5/
\end{verbatim}

Before a first installation, 
create a \Login{condor} user and group as appropriate to the installation.
The Condor package will automatically add a \Login{condor} user/group
if it does not exist already.
Sites with a targeted security policy should add the \Login{condor} 
user/group manually before installation.
...
As needed, edit the Condor configuration files to customize.
...
and the ordered search algorithm utilized by Condor for finding
its configuration includes this directory.

Installation is then complete.  Start Condor daemons:
\begin{verbatim}
