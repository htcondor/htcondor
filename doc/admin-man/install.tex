...
Unix site.  Condor's installation will set up a default configuration which
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{installation!downloading}
\index{Unix installation!downloading}
\index{downloading}
The first step to installing Condor is to download it from the Condor
web site, at \Url{http://www.cs.wisc.edu/condor}.  The downloads are
available from the downlaods page, which is at
\Url{http://www.cs.wisc.edu/condor/downloads/}.

The Condor files are currently available from two sites, the main
site at the University of Wisconsin--Madison, Madison, Wisconsin, USA,
as well as at the Istituto Nazionale di Fisica Nucleare Sezione di
Bologna, Bologna, Italy.  Please choose the site nearest you.

Please also be sure to keep track of where you download the binary into.
...
\begin{description}
\item[\File{DOC}] file containing directions for where to find the 
		  Condor documentation
\item[\File{INSTALL}] these installation directions
\item[\File{LICENSE.TXT}] by installing Condor, you agree to the contents of
		  the LICENSE.TXT file
\item[\File{README}] general info
\item[\File{condor\_install}] Perl script to install and configure Condor
...
centralized information repository for the Condor pool,
and it is also the
...
time, or on one that will be rebooted quickly if it
does crash.
Also
consider
network traffic and your network layout when choosing your central
manager.
All the daemons send updates (by
...
decisions.
You can install Condor under any user,
however
there are both serious security and performance consequences.
Please see section~\ref{sec:Non-Root} on page~\pageref{sec:Non-Root}
in the manual for the details and ramifications of
on running Condor as a Unix user other
than root.
...
create a Unix user named condor on all machines in your pool.  The Condor
...
\item[\File{spool}] The spool directory holds the job queue and history
...
to AFS.  We're working on a solution, it is just not ready for
\VersionNotice{}.
So, what this means is that you are probably not going to want
