...
\begin{description}
\item[\File{DOC}] directions on where to find HTCondor documentation
...
\item[\File{bin}] directory which contains the distribution HTCondor
		  user programs.
\item[\File{sbin}] directory which contains the distribution HTCondor
		  system programs.
...
\item[\File{man}] directory which contains the distribution HTCondor
		  manual pages.
\item[\File{src}] directory which contains the distribution HTCondor
...
Here are an ordered set of steps that get HTCondor running using the RPM.
\begin{enumerate}
