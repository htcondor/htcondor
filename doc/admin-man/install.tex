...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Will you have a 'condor' user and will it's home
directory be shared? }
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The global config file is shared by all machines in the pool.
...
You can also have config files specific to each platform in your pool.
See
...
Next, Condor tries to load the local config file(s).
The only way to specify the local config file(s) is in the global config
file, with the \Macro{LOCAL\_CONFIG\_FILE} macro.  If that macro isn't
set, no local config file is used.  This macro can be a list of files
or a single file.
...
The local root config file(s) are found with the
\Macro{LOCAL\_ROOT\_CONFIG\_FILE} macro.  If that isn't set, no local
root config file is used.  This
...
located on a shared filesystem, if possible.
...
\item[STEP 5: Filesystem and UID domains.]
     ...
     \Term{vanilla} jobs (ones that aren't relinked with the Condor
...
Once you have completed STEP 10, you're done.  \Condor{install} prints
