...
The easiest way to install Condor is to use the one or both of the
scripts scripts provided to help you: \Condor{install} and
\Condor{init}.  You should run these scripts as the user that you are
going to run the Condor daemons as.  First,run \Condor{install} on any
machine that will be a fileserver for shared files used by Condor,
such as the release directory, and possibly the condor user's home
directory.
...
used files that aren't created by \Condor{install}.  In the most simple
case, where nearly all of Condor is instaled on a shared file system,
even though Condor{install} will create nearly all the files and
directories you need, you will still need to use \Condor{init} to
