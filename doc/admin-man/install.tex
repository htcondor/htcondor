...
the 'local directory' (specified by the \Macro{LOCAL\_DIR} parameter in the
config file).

If you have a 'condor' user with a local home directory on each
machine, the \Macro{LOCAL\_DIR} could just be user condor's home directory
('\Macro{LOCAL\_DIR} = \MacroU{TILDE}' in the config file).  If this user's home
...
directory ('\Macro{LOCAL\_DIR} = \MacroU{TILDE}/hosts/\MacroU{HOSTNAME}' for example).  If
...
a shared location (such as \File{/usr/local/condor/logs/\$(HOSTNAME)}) so that
...
parameter in the config file (such as \File{/var/lock/condor}).
...
doesn't point to a real file, \File{/etc/condor/condor\_config} is checked
next.  If that doesn't exist, and there is a 'condor' account on the
machine, \File{\Tilde condor/condor\_config} is tried.  If none of those locations
...
\File{/etc/condor/condor\_config.local} is searched.  If that doesn't exist,
\File{\Tilde condor/condor\_config.local} is tried.  If none of those locations
...
\File{/etc/condor/condor\_config.local.root} and finally
\File{\Tilde condor/condor\_config.local.root}.  If none of these files are found,
...
\begin{itemize}
     ...
     \File{/usr/local/condor/bin}) which you have Condor users add to their
     PATH environment variable, or copy those files directly into a
     well known place already in user's PATHs (such as
     \File{/usr/local/bin}).  With the above examples, you could also leave
     the binaries in \File{/usr/local/condor/bin} and put in soft links from
     \File{/usr/local/bin} to point to each program.
...
the web at: \Url{http://www.cs.wisc.edu/condor/manual}.
...
is that you are probably not going to want to have the \Macro{LOCAL\_DIR} for
Condor on AFS.  However, you can (and probably should) have the Condor
\Macro{RELEASE\_DIR} on AFS, so that you can share one copy of those files
...
The easiest way to install Condor is to run the \Condor{install} script.
...
\Condor{install} on your file server and it will setup everything you
need for all your machines.  If not, you will need to run
\Condor{install} on each machine to setup Condor.
...
file server, you only have to run \Condor{install} once.  If they are
separate machines, run \Condor{install} on your file server first, then
...
\begin{enumerate}
\item  Read through the \Release{etc/condor\_config} file.  There are
...
\item   Condor can monitor the activity of your mouse and keyboard,
    ...
    \Macro{CONSOLE\_DEVICES} entry in the \condor{startd} section of the config
    file.  On most platforms, we provide reasonable defaults.  For
    example, the default device for the mouse on Linux is 'mouse',
    since most Linux installations have a soft link from '\File{/dev/mouse}'
    that points to the right device (such as tty00 if you have a
    serial mouse, psaux if you have a PS/2 bus mouse, etc).  If you
    don't have a \File{/dev/mouse} link, you should either create one (you'll
    be glad you did), or change the \Macro{CONSOLE\_DEVICES} entry in Condor's
    config file.  This entry is just a comma seperated list, so you
    can have any devices in \File{/dev} count as 'console devices' and
    activity will be reported in the \condor{startd}'s classad as
...
\item   (HPUX 10 and Solaris machines only) Set the ARCH parameter in the
    \condor{config} file.  On HPUX 10 and Solaris machines, there are
    ...
    condor\_config file in \Release{etc/condor\_config}.  In the
...
(\textbf{NOTE}: On Alphas and IRIX machines, there will also be a
	'\Condor{kbdd}' -- see section~\ref{sec:kbdd} of the manual for details.)
...
\item  (Optional) Do a full install for the \Condor{compile} script.
    ...
    complete the full install, you will be able to use \condor{compile}
...
\item  Try building and submitting some test jobs.  See \File{examples/README}
    for details.
...
\Condor{master} daemon) as user root.  If you must start Condor as some
