...
\item[Up to two IP addresses will be advertised.]
  ...
  used to contact the collector as its public address, since this is the most likely to be
  accessible to other processes that query the same collector.
  The \Macro{NETWORK\_INTERFACE} setting can be used to specify the
  public IP address Condor should advertise, and
...
should consider if using the Condor Connection Broker, CCB is
right for them.
