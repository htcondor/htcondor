...
An upper limit on the number of slots allocated for a group
...
policy is that no more than twenty jobs are every running
concurrently from the physicists, and only ten from the chemists.
These machines are otherwise identical, and it doesn't matter
which machines run which group's jobs, just that the proportions
are correct.

This could be implemented by configuring the twenty slots owned by the physics group
to prefer or require (using the \MacroNI{RANK} or \MacroNI{START} configuration macros, respectively)
physics jobs, and the rest to prefer chemistry jobs. However, this locks
policy to specific machines, which can be difficult to manage, especially
when the size of the pool changes.

To implement this policy, without resorting to \MacroNI{RANK}, 
set in the config file on the central manager the following:
...
If the sum of the child quotas exceeds the parent, then the child quotas are scaled down
in proportion to their relative sizes.  For example, in this case, if a power failure removed
half of the original nodes, leaving fifteen slots, physics would be scaled back to a quota
of ten, and chemistry to five.  This scaling can be disabled by setting the negotiator
configuration knob \MacroNI{NEGOTIATOR\_ALLOW\_QUOTA\_OVERSUBSCRIPTION} to true.  If the sub
of the child quotas is less than the parent, the child quotas are left intact, they are not
scaled up.  That is, if somehow the number of slots doubled from thirty to sixty, 
physics would be limited to 20 slots, and chemistry 10.

Note that the group names must be pre-defined.  They do not have to begin with "group\_", but that is 
the convention, which helps to avoid naming conflicts.  Specifying absolute
values for the size of the quota is called using static quotas.

Each job must then define which group it belongs to.  Currently this is opt-in, and
the system trusts each user to put the correct group in the submit file.  Note this
is per job, not per user.  A given user may be a member of many groups.
Jobs identify which group they are in by setting the \SubmitCmd{accounting\_group}
and \SubmitCmd{accounting\_group\_user} commands in the submit description file, 
as specified in section~\ref{sec:group-accounting}.

Example submit file snippet:

\begin{verbatim}
accounting_group = group_physics
accounting_group_user = MyUserName
\end{verbatim}

The size of each group may also be expressed as a proportion.  This is sometimes callled
dynamic group quotas, because the size of the quota is dynamically recalculated every
negotiation cycle, based on the total available size of the pool.  Continuing this same example, assume
that one-third of the pool should be allocated to chemistry, and two-thirds to physics.  We want to
maintain this ratio even as the size of the pool changes, perhaps because of machine failures, arrival
of new machines, or other reasons.  The submit files remain the same, and the configuration on the
central manager becomes:
...
Note that the values of the quotas must be less than 1.0, indicating percentage.  As with
static slots, if the sum of the children exceeds one, they are scaled down proportionally
so that their sum does equal 1.0.  If their sum is less than one, they are not changed.

There may be a hierarchy of quota groups.  For example, if the physics group
consists of high-energy (hep) and low-energy (lep) sub-groups, and the high-energy sub-group 
owns fifteen of the twenty physics slots, and the low-energy group owns the
rest. This can be implemented with the the following central manager configuration:
...
This hierarchy may be more useful when dynamic quotas are used, the above example with
dynamic quotas would have the following configuration:
...
These percentages in child groups are expressed in terms of their parents' quota.
That is, the high-energy physics sub-group is allocated 75\% of the 66\% that physics gets
of the entire pool, however many that might be.  If there are 30 machines in the pool, that
would be the same 15 machines as specified above.

High-energy physics users can indicate which group their jobs should go it with the
following submit file additions.  This is the same when using either static
or dynamic quotas.
...
The root of this tree is implicitly created by the system,
and not accessible by configuration.  It is called the \AdStr{<none>}
group.  Any job that is \emph{not} part of any group is considered part of the
this \AdStr{<none>} group.

In all the examples above, the hierarchy is merely a notational convenience.  All of the
examples could be implement with a flat structure, though it might be more confusing for
the administrator.  Surplus is the concept that creates a true hierarchy.

If a given group or sub-group accepts surplus, then it is allowed to exceed the configured
quota, by using "leftover" quota unused by other groups.  Surplus is disabled for all groups
by default.  The administrator may enable surplus either for all groups by setting
\Macro{GROUP\_ACCEPT\_SURPLUS} to true.  Or, surplus may be enabled for individual groups
by setting \Macro{GROUP\_ACCEPT\_SURPLUS\_groupname} to true.  Consider the following example:
...
\begin{verbatim}
  ...
  GROUP_ACCEPT_SURPLUS_group_physics = true
  GROUP_ACCEPT_SURPLUS_group_physics.lep = true
  GROUP_ACCEPT_SURPLUS_group_physics.hep = true
\end{verbatim}

This configuration is the same as above for the chemistry users.  However, \Macro{GROUP\_ACCEPT\_SURPLUS}
is set to false globally, and false for the physics parent group, but true for the children
group\_physics.lep and group\_physics.lep.  This setting means that group\_physics.lep and group\_physics.hep
are allowed to exceed their quota of 15 and 5, respectively, but their sum cannot exceed 20, for that is their
parent's quota. If the group\_physics had \Macro{ACCEPT\_SURPLUS} set to true, then either group\_physics.lep
and group\_physics.hep would not be limited at all.  

Surplus slots are distributed "bottom-up" in the quota tree. That is, any leaf nodes of this tree with 
excess quota will share it with any peers which accept surplus. Any subsequent excess will then be 
passed up to the parent node and over to all of it's children, recursively.
Any node that does not accept surplus implements a hard cap on the number of slots that the sum of it's children use.

After the \Condor{negotiator} calculates the quota assigned to each of these group, possibly adding in surplus, it then 
negotiates with the schedds in the system to try to match jobs to each group.  It does this one group at a time.  By
default, it goes in "starvation group order", that is, the group whose current usage is furthest from its quota goes first,
then the next, and so on.  The "<none>" group implicitly at the root of the tree goes last.  This sorting can be overridden
with the \Macro{GROUP\_SORT\_EXPR} setting.  The negotiator evalutes this classad expression of each group ad, then sorts
the group by the floating point result, and negotiaties in that order.  The attributes available are documented
in section~\ref{sec:Priorities-in-Negotiation-and-Preemption}

One possible group quota policy is strict priority.  For example, a site prefers physics users to match
as many slots as they can, and only when all the physics jobs are running, and there are idle slots, are chemistry jobs
allowed to run.  The default "starvation group order" can be used to implement this.  By setting
the configuration knob \MacroNI{NEGOTIATOR\_ALLOW\_QUOTA\_OVERSUBSCRIPTION} to true, and setting the physics quota to
a number so large that it can't ever be met, say, one million, the physics group will always be the "most starving" group,
will always negotiate first, and will always be unable to meet the quota.  Only when all the physics jobs are running
...
The \Condor{userprio} command can show the current quotas in effect, and the current usage by group.

For example:
\begin{verbatim}
$ condor_userprio -quota
...
This shows that there are two groups, each with 60 jobs in the queue.  group\_physics.hep has
a quota of 15 machines, and group\_physics.lep has 5 machines.  Other options to \Condor{userprio},
such as -most will also show the number of resources in use.
