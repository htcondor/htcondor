...
\end{verbatim}
In this case, the unused, surplus quota of 10 machines is assigned to
the higher level with in the hierarchy.  
...
which is less than the parent group, which has a dynamic quota of .6.
...
\end{verbatim}
In this case, the sum of dynamic quotas for the subgroups is .7, 
which is greater than the parent group, at .5 .
A warning is written to the log for the \Condor{negotiator} daemon,
and each of the subgroups will have their dynamic group quota scaled
by (.5)/(.7) = .7143 for this example.
If the pool has 100 slots, 
then the \Expr{project1} subgroup is assigned a dynamic quota that
equals (100)(.7143)(.2) which is 14.286 machines. 
The \Expr{project2} subgroup is assigned a dynamic quota that
equals (100)(.7143)(.3) which is 21.429 machines. 
The \Expr{project3} subgroup is assigned a dynamic quota that
equals (100)(.7143)(.2) which is 14.286 machines. 
The quota for each of \Expr{project1} and \Expr{project3} results 
in the truncated amount of 14 machines,
with the 0.286 fractional portion of the calculated quota assigned to
the parent group, \Expr{group\_stat}. 
The quota for \Expr{project2} results in the truncated amount of 21 machines,
with the 0.429 fractional portion of the calculated quota assigned to
the parent group, \Expr{group\_stat}. 
...
%The user name that is used for accounting and prioritization purposes
%is still the group user as specified by \AdAttr{AccountingGroup} in
%the job ClassAd.
...
%more than ten machines because \MacroNI{GROUP\_AUTOREGROUP\_chemistry}
%is true.
