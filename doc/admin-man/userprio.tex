...
In addition, Condor may preempt a machine claim and reallocate it when
conditions change.
...
The following attributes may be used within the definition
of \MacroNI{PREEMPTION\_REQUIREMENTS} and \MacroNI{PREEMPTION\_RANK}.
In these attributes, those with names that begin with the
string \Attr{Submitter} refer to characteristics about the candidate
job's user; those with names that begin with the string \Attr{Remote}
