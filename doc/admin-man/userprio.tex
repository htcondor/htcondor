...
These is then called dynamic group quotas, 
...
the example is recast with
one-third of the pool allocated to chemistry and two-thirds to physics.
...
The fraction of a subgroup's quota are expressed with respect to
their parent group's quota.
That is, 
...
Surplus is disabled for all groups
...
with the \Macro{GROUP\_SORT\_EXPR} setting.  The negotiator evaluates this classad expression of each group ad, then sorts
the group by the floating point result, and negotiates in that order.  The attributes available are documented
