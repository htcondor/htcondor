...
problem of starvation of large jobs due to fragmentation of resources.
The problem is that the resources of machines over time may become
partitioned into slots suitable for running small jobs.  If a
sufficient number of these slots don't happen to become idle at the
same time on a machine, a large job will not be able to claim that
machine, even if it has a better priority than the small jobs.
...
are defragmented.  The \Condor{defrag} daemon provides an automated
policy for doing that.  To use it, \MacroNI{DEFRAG} must be added to
\MacroNI{DAEMON\_LIST} and the defragmentation policy must be
configured.  Typically, only one instance of \Condor{defrag} would be
run per pool.  It is a light-weight daemon that should not require a
lot of system resources.

Example configuration that enables \Condor{defrag}:
...
The above example policy tells \Condor{defrag} to initiate draining of
jobs from 1 machine per hour but to avoid initiating new draining if
there are 20 completely defragmented machines or 10 machines in a
draining state.  A full description of each configuration option
used by \Condor{defrag} may be found in section~\ref{sec:Config-defrag}.

By default, when a machine is drained, existing jobs are gracefully
evicted.  This means each job will be allowed to use the remaining
time promised to it by \MacroNI{MaxJobRetirementTime}.  If it has not
finished when the retirement time runs out, the job will be killed
with a soft kill signal, so it has an opportunity to save a checkpoint
(if the job supports checkpointing).  No new jobs will be allowed to
start while the machine is draining.  To reduce unused time on the
...
There is a tradeoff between reduced starvation and throughput.
...
Some of the machine's resources may go unused during draining if some
jobs finish before others.  If non-checkpointable jobs are killed
because they run past the end of their retirement time during
draining, this also adds to the cost of draining.
...
spent by jobs that were killed due to draining.  These are advertised
respectively in the \AdAttr{TotalMachineDrainingUnclaimedTime} and
\AdAttr{TotalMachineDrainingBadput} attributes.  \Condor{defrag}
averages these values across the pool and advertises the result in its
daemon ad in the \AdAttr{AvgDrainingBadput} and
\AdAttr{AvgDrainingUnclaimed} attributes.  Details of all attributes
in the \Condor{defrag} daemon ad are described in
section~\ref{sec:Defrag-Attributes}.
