...
environment. \Prog{glexec} takes an X.509 proxy and a command to run as
inputs and maps the proxy to a local identity (i.e., a UNIX UID) which
it then uses to execute the command. Like the \Condor{root\_switchboard}
command, which provides similar functionality for Condor's PrivSep mode
(see \ref{sec:PrivSep}), \Prog{glexec} must be installed as a root-owned
...
\Prog{glexec} is also used when performing maintenance actions like
...
Configuration for \Prog{glexec} support is straightforward. The boolean
\Macro{GLEXEC\_JOB} setting must be set \Expr{True} on execute machines
where \Prog{glexec} is to be used. Condor also must be given the full
path to the \Prog{glexec} binary using the \Macro{GLEXEC} setting. Note
that Condor must be started as a non-root user when \Prog{glexec} is used.
(When Condor runs as root, it can perform actions as other UIDs arbitrarily
and \Prog{glexec}'s services are not needed.) Finally, for a job to execute
properly in \Prog{GLEXEC\_JOB} mode, it must be submitted with a proxy
specified via \SubmitCmd{x509userproxy} in its submit file, since a
proxy is needed as input to \Prog{glexec}.

Earlier versions of Condor employed a different form of \Prog{glexec}
support, where the \Condor{starter} ran under the same UID as the job.
This feature was enabled using the \Macro{GLEXEC\_STARTER} setting. It
is now deprecated in favor of \MacroNI{GLEXEC\_JOB} mode.
