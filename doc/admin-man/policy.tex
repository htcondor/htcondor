...
\item[\Macro{CLAIM\_WORKLIFE}]
  ...
  negotiator gives a schedd a claim to a slot, the schedd will keep
  running jobs on that slot as long as it has more jobs with matching
  requirements, and \Macro{CLAIM\_WORKLIFE} has not expired, and it is
  ...
  preempted, the claim is closed.  When \Expr{CLAIM\_WORKLIFE} is -1,
...
using \MacroNI{MAXJOBRETIREMENTTIME}.  This can be done with
...
Unlike \MacroNI{MAXJOBRETIREMENTTIME}, the above policy does not prevent
