...
The \Macro{START} expression is explained.
It describes the conditions that must be met for a machine to
start a job.
The \Macro{RANK} expression for a machine is described.
...
\item[F] The machine moves from Matched to Owner if either the
  \Expr{START} expression locally evaluates to FALSE, or if the 
...
\item[J] The resource will move from Preempting to Owner if the
  ...
  was used, or if the \Expr{START} expression locally evaluates to
...
\item[K] The resource will move from Backfill to Owner for the
  following reasons:
  \begin{itemize}
  \item The \Expr{EVICT\_BACKFILL} expression evaluates to TRUE
  \item The \Condor{startd} receives a \Condor{vacate} command
...
\Expr{START\_BACKFILL} expression evaluates to TRUE, the machine will
...
\item[\Macro{PERIODIC\_CHECKPOINT}] If the machine is in the
...
The policy utilizes two slots,
one (called slot1 in the example) that only runs jobs identified
as high priority jobs.
The second slot (called slot2 in the example) is 
set to run jobs according to the usual policy and to suspend them
when slot1 is claimed.  A policy for a machine with more than one physical
CPU may be adapted from this example.  Instead of having 2 slots,
you would have 2 times the number of physical CPUs.  Half of the slots
would be for high priority jobs and the other half would be for suspendable
jobs.

\footnotesize
\begin{verbatim}
# Lie to Condor, to achieve 2 slots with only a single CPU
NUM_CPUS = 2

# slot1 is the high-prio slot, while slot2 is the background slot...
...
# Only start jobs on slot2 if there is no job on slot1, and if the
# machine is otherwise idle...  NOTE: the "Busy" activity is only in
# the Claimed state, and only when there is an active job, so that is
# good enough for our needs...
SLOT2_START = ( (slot1_Activity != "Busy") && \ 
                (KeyboardIdle > $(StartIdleTime)) && \
	        ($(CPUIdle) || (State != "Unclaimed" && State != "Owner")) )

# Only suspend jobs on slot2.  Suspend if there is keyboard activity or
# if a job starts on slot1...
SUSPEND = (SlotID == 2) && \
