...
job ClassAd attribute that is also utilized in the configuration.
...
identifies itself as suspendable or nonsuspendable.
  ...
  then the policy set for preemption determines which will be running.
  \item If both the currently running job and the potentially running job 
...
A job that identifies itself as nonsuspendable goes back into the
job queue.
\end{itemize}
...
START = \
  IsHighPrioSlot && IsHighPrioJob || \
  IsHighPrioSlot!=True && IsHighPrioJob=!=True && $(MyHighPrioSlotIsIdle)
...
must submit the job so that this attribute defined.
The following line appears in the job's submit description file:
\begin{verbatim}
+IsHighPrioJob = True
\end{verbatim}
