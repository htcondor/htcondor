...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:Is-Valid-Checkpoint-Platform}
The \Expr{IS\_VALID\_CHECKPOINT\_PLATFORM} expression}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{expression!\Expr{IS\_VALID\_CHECKPOINT\_PLATFORM}}

When a standard universe jobs checkpoints, there are certain information
embedded in the checkpoint which limits the machines able to resume
the checkpoint. 
Usually, these are obvious things like architecture or operating system,
but they also can be very subtle, like the difference between normal,
bigmem, and hugemem kernels for the Linux platform.
...
it must also behave correctly when evaluating in the context of non
standard universe jobs.

The current default policy for this expression in prose is this:

\Bold{ Allow all non standard universe jobs to run normally.  
Only allow standard universe jobs to run on machines with the new checkpointing
identification system. 
Run the job if it is a standard universe job, but had not ever checkpointed 
before. 
If the standard universe job had checkpointed before, then make sure the 
checkpoint platforms between the job and the machine match.}

The default boolean expression for the above policy (which may be
over ridden in the Condor configuration files) is:
...
The reason why \Expr{IS\_VALID\_CHECKPOINT\_PLATFORM}
is a seperate policy expression is because the complexity of
\Expr{IS\_VALID\_CHECKPOINT\_PLATFORM} can be very high and while this
functionality is conceptually seperate from the normal \Expr{START}
