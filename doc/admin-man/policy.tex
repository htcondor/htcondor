...
\item[N] The machine switches from the Owner to the Drained state
  whenever draining of the machine is initiated (e.g. by \Condor{drain}
  or \Condor{defrag}).
...
\item[P] The transition from Unclaimed to Drained happens
  if draining of the machine is initiated (e.g. by \Condor{drain}
  or \Condor{defrag}).
...
The Drained state is used when the machine is being drained (e.g. by
\Condor{drain} or \Condor{defrag}) and the slot has finished running
