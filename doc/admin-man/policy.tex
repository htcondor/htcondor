...
Transformations can be used to guarantee the presence of required job attributes,
to set defaults for job attributes the user does not supply,
...
based on the owner of the job while letting the job owner indicate a preference.

There can be multiple job transforms. Each transform can have a Reqirements expression
to indicate which jobs it should transform and which it should ignore. Tranforms
without a Requirements expression apply to all jobs.
Job transforms are applied in order.  The set of transforms and their order are
configured using the Configuration variable \Macro{JOB\_TRANSFORM\_NAMES}.

For each entry in this list there must be a corresponding \Macro{JOB\_TRANSFORM\_<name>}
configuration variable that specifies the transform rules. Transforms use the same
syntax as \Condor{job\_router} transforms; although unlike the \Condor{job\_router} there
is no default transform, and all matching transforms are applied - not just the first one.

The following example shows a set of two transforms. One that automatically assign an accounting group
to jobs based on the submitting user. And one that shows one possible way to transform Vanilla jobs to
Docker jobs.
...
The AssignGroup transform above assumes that a mapfile that can an map owner to one or more accounting
groups has been configuration via \Macro{SCHEDD\_CLASSAD\_USER\_MAP\_NAMES}, and given the name "Groups"

The SL6ToDocker transform above is most likely incomplete, as it assumes some custom attributes
\Attr{WantSL6} and \Attr{WantDocker} and \Attr{HasDocker} that your pool may or may not use.
