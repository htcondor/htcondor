...
\item[Claimed/Busy] If the \Expr{CONTINUE} expression evaluates to
  ...
  Claimed/Busy state (transition \Bold{15}).
...
seconds) that the startd is willing to wait for the job to finish on
its own.  The clock starts when the job is started and is paused
during any suspension, so the job may count on at least the specified
amount of runtime without getting killed by the startd.  The job may
...
nice\_user jobs have a default retirement time of 0, so they will
normally not wait in retirement.)
