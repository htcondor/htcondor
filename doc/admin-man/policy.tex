...
\item[\Macro{CPU\_Idle}] A boolean expression that evaluates to TRUE
    when the CPU is idle.

\item[\Macro{CPUBusy} or \Macro{CPU\_Busy}] Boolean expressions that evaluate
    to TRUE when the CPU is busy.
    When \Macro{CPUBusy} does not appear in the configuration files,
    \Macro{CPU\_Busy} is used in its place.

\item[\Macro{MachineBusy}] The CPU or the Keyboard is busy.

\item[\Macro{CPUIsBusy}] A boolean value set to the same value as either
    \Macro{CPUBusy} or \Macro{CPU\_Busy}.

\item[\Macro{CPUBusyTime}] The value 0 if \Macro{CPUBusy} (or 
    \Macro{CPU\_Busy}) is False; the time in seconds since
    \Macro{CPUBusy} (or \Macro{CPU\_Busy}) became True.
