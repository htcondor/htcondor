...
A final discussission details how the \Condor{startd} works.
Included are
...
expression on your machines to refererence the \Attr{Owner} attribute and
...
If you don't specify separate vanilla versions, the normal versions
...
If \Expr{WANT\_SUSPEND} is true, the startd will evaluate the
...
There is only a \Macro{PREEMPT} expression which describes the
...
\Macro{WANT\_VACATE} expression controls whether or not the job should
be vacated with a checkpoint or directly killed.  
The \Macro{KILL} expression only determines when the machine goes from
Preempting/Vacating to Preempting/Killing.  

Before, the \Macro{KILL} expression had to handle three distinct cases
(the transitions from Claimed/Busy, Claimed/Suspended and
Preepting/Vacating) and the \Macro{VACATE} expression had to handle
two (the transitions from Claimed/Busy and Claimed/Suspended).
Now, \Macro{PREEMPT} has to handle the same two cases as the previous
\Macro{VACATE} expression, but the \Macro{KILL} expression only
handles one.
In fact, very complicated policies can now be specified using all of
the default expressions, and only tuning the \Macro{WANT\_VACATE} and
\Macro{WANT\_SUSPEND} expressions.
In previous versions, if you made heavy use of the \Macro{WANT\_*}
expressions, the \Macro{KILL} expression could become incredibly
complicated. 
