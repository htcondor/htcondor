...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Machine ClassAd Attributes}
...
expression to False, not its \Expr{START} expression.  When the
\Expr{START} expression \Term{locally evaluates} to true, the machine
...
Since \Expr{WANT\_SUSPEND} isn't always true, our \Expr{PREEMPT}
...
So, if \Expr{WANT\_SUSPEND} is true, we want to to look at how long
we've been suspened to decide if we need to preempt.
However, if \Expr{WANT\_SUSPEND} is false, we want to preempt if the
...
Finally, \Expr{KILL} is still very easy, since it only determines when
...
We don't even have to define seperate Vanilla versions of any of
