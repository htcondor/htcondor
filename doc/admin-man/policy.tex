...
Setting \Macro{PREEMPT} to False disables preemption from activity
on the machine, for example because of console activity from
the machine owner.
\Macro{PREEMPTION\_REQUIREMENTS} is set to False to disable preemption
by users with better priority in the pool. 
Technically, that is part
of the \Condor{negotiator} policy, not the \Condor{startd} daemon policy.
Setting the
\Condor{startd}'s \Macro{RANK} expression to 0 removes the possibility of
\Condor{startd} preemption,
which is when the \Condor{negotiator} finds a job that the
\Condor{startd} likes better than the current job it is running.
For this purpose,
setting the rank to any constant or to nothing at all would
work just as well.

You should be aware of one disadvantage of this policy:
a low priority
...
continuous stream of short jobs.  The \Condor{negotiator} will not
be able to
