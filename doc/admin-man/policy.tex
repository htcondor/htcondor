...
If \MacroNI{WANT\_SUSPEND} is not TRUE (e.g. FALSE or UNDEFINED), then the startd will
...
seconds) that the \Condor{startd} will wait for the job to finish naturally
(without any kill signals from the \Condor{startd}).  The clock starts when the
...
\item[\MacroNI{MaxVacateTime}] Amount of time a job may be
