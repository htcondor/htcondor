...
negotiator, referred to as partitionable slot preemption,
or pslot preemption.
Normally, when the negotiator searches for a match for a job ad, it
looks at each slot ad individually.
With pslot preemption, the negotiator looks at a partitional slot and
all of its dynamic slots as a group.
If the partitionable slot does not have sufficient resources (memory,
cpu, disk) to be matched with the candidate job, then the negotiator
looks at all of the related dynamic slots that the candidate job can
preempt (following the normal preemption rules described elsewhere).
It adds the resources of each dynamic slot and adds them to those
of the partionable slot, one dynamic slot at a time.
Once the total resources are sufficient to enable a match, the
negotiator sends the match information to the schedd.
When the schedd claims the partitionable slot, the dynamic slots are
preempted so that their resources are retruned to the partitionable slot
for use by the new job.

To use this new matchmaking algorithm, set the following configuration
variable for all machines in your pool:
\begin{verbatim}
  ALLOW_PSLOT_PREEMPTION = True
\end{verbatim}

When the negotiator examines the resources of dynamic slots,
it sorts the slots by their \Attr{CurrentRank} attribute, where slots
with lower values are considered first.
...
eligible for preemption, regardless of how
\MacroNI{NEGOTIATOR\_CONSIDER\_EARLY\_PREEMPTION} is set.

When pslot preemption is enabled, the negotiator will not preempt
dynamic slots directly. It will preempt them only as part of a match
to a partitionable slot.

When multiple partitionable slots match a candidate job and the various
job rank expressions are evaluated to sort the matching slots, the ad of
the partitionable slot is used for evaluation.
This may cause unexpected results with some expressions, as attributes
like \Attr{RemoteOwner} won't be present in a partitionable slot that
