...
\item[\Macro{LINUX\_HIBERNATION\_METHOD}]
  ...
  behavior is to try:
  \begin{itemize}
  \item Detect and use the pm-util command line tools \verb@"pm-util"@ 
  \item Detect and use the \File{/sys/power} virtual files \verb@"/sys"@ 
  \item Detect and use the \File{/proc/ACPI} virtual files \verb@"/proc"@ 
  \end{itemize}
...
\item[\Macro{SEC\_TCP\_SESSION\_DEADLINE}]
  The total length of time in seconds until giving up
