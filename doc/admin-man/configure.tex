...
\item[\Macro{NETWORK\_INTERFACE}]
  The meaning of this configuration setting depends on the setting of
  \MacroNI{BIND\_ALL\_INTERFACES}.  If \MacroNI{BIND\_ALL\_INTERFACES}
  is true (the default), then this setting simply controls what IP
  address will be advertised as the public address of the daemon.
  If \MacroNI{BIND\_ALL\_INTERFACES} is false, then this setting
  specifies which IP address to bind network sockets to.
  If \MacroNI{BIND\_ALL\_INTERFACES} is false and \Macro{NETWORK\_INTERFACE}
  is unset, Condor chooses a network interface automatically.  It tries
  to choose a public interface if one is available.  If it can't decide
  which of two interfaces to choose, it will pick the first one.
