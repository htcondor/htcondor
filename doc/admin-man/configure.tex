...
\item[\Macro{SHADOW\_CHECKPROXY\_INTERVAL}]
  ...
  updated or should be refreshed.  The default is 600 (10 minutes).
  This setting should be small compared to the refresh interval
  required to keep delegated credentials from expiring (configured via
  \Macro{DELEGATE\_JOB\_GSI\_CREDENTIALS\_REFRESH} and
  \Macro{DELEGATE\_JOB\_GSI\_CREDENTIALS\_INTERVAL}).  If this setting
  is too small, proxy updates could happen very frequently, potentially
...
\item[\Macro{DELEGATE\_JOB\_GSI\_CREDENTIALS\_LIFETIME}]
 ...
 which delegated proxies should be valid.  The default is one day.  A
 value of 0 indicates that the delegated proxy should be valid for as
 long as allowed by the credential used to create the proxy.  The job
 may override this configuration setting by using the
 \SubmitCmd{delegated\_job\_GSI\_credentials\_lifetime} submit file
 command.  This configuration setting currently only applies to
 ...
 this setting were 0.  This setting has no effect if
 \Macro{DELEGATE\_JOB\_GSI\_CREDENTIALS} is false.

\label{param:DelegateJobGSICredentialsRefresh}
\item[\Macro{DELEGATE\_JOB\_GSI\_CREDENTIALS\_REFRESH}]
 A floating point number between 0 and 1 that indicates when delegated
 credentials with limited lifetime should be renewed.  The renewal is
 attempted periodically at or near the specified fraction of the lifetime of
 the delegated credential.  The default is 0.25.  This setting has no
 effect if \Macro{DELEGATE\_JOB\_GSI\_CREDENTIALS} is false or
 \Macro{DELEGATE\_JOB\_GSI\_CREDENTIALS\_LIFETIME} is 0.  For non-grid
 jobs, the precise timing of the proxy refresh depends on
 \Macro{SHADOW\_CHECKPROXY\_INTERVAL}.  To ensure that the delegated
 proxy remains valid, the interval for checking the proxy should be
