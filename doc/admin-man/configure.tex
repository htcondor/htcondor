...
\label{param:Run}
\item[\Macro{RUN}]
  Used by the condor init script to specify the directory where the Condor 
  master should write its process ID
  (PID) file. 
...
\item[\Macro{USE\_NFS}]
  ...
  read/write files directly on the executing machine without redirecting I/O back to the submitting
  ...
  This macro similarly changes the semantics of chirp file i/o
  when running in the vanilla, java and parallel universe.  If this parameter
  is set in those universes, chirp will not send i/o requests over the
  network as requested, but perform them directly to the locally mounted
  file system.  Other than chirp file access, this macro is unused
  outside of standard universe.
...
\item[\Macro{NEGOTIATOR\_READ\_CONFIG\_BEFORE\_CYCLE}]
  A boolean configuration parameter that defaults to \Expr{False}.
  When \Expr{True}, the \Condor{negotiator} will re-read the configuration
  prior to beginning each negotiation cycle.  Note, this operation will update
  configured behaviors such as concurrency limits, but not data structures
  constructed during a full reconfiguration such as the group quota hierarchy.
  A full reconfiguration, e.g. \Condor{reconfig}, remains the best way to 
  guarantee that all negotiator configuration is completely updated.
...
\item[\Macro{DAGMAN\_SUBMIT\_DELAY}]
  ...
  of values is any nonnegative integer.  If defined with a value less
