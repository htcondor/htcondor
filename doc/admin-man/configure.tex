...
\item[\Macro{UPDATE\_OFFSET}]
  Determines the delay, in seconds, that the \Condor{startd} should wait
  before sending its initial update (and the first update after a
  ``reconfig'') to the \Condor{collector}.  All other updates are
  sent after this initial update, as determined by
  \MacroUNI{UPDATE\_INTERVAL}.  Thus, the first update will be sent after
  \MacroUNI{UPDATE\_OFFSET} seconds, the second at
  \MacroUNI{UPDATE\_OFFSET} + \MacroUNI{UPDATE\_INTERVAL}, etc.
  This is useful when used in conjunction
  with \$RANDOM\_INTEGER() for large pools to spread out the updates
  sent by a large number of \Condor{startd}s.  This macro is defined
  in terms of seconds and defaults to zero.  For example:
\begin{verbatim}
startd.UPDATE_INTERVAL = 300
startd.UPDATE_OFFSET   = $RANDOM_INTEGER(0,300)
\end{verbatim}
  This would cause the initial update to be a random number of seconds
  (between 0 and 300), with all further updates occurring at fixed 300
...
\item[\Macro{JOB\_ROUTER\_DEFAULTS}]
  ...
  for all routes in the \Condor{job\_router} daemon's routing table.
  The meaning of the
...
\item[\Macro{JOB\_ROUTER\_ENTRIES}]
  ...
  each surrounded by square brackets and separated from each other by spaces.
  Each entry in the routing table describes a site that jobs may be
  routed to.

  \Condor{reconfig} will cause the \Condor{job\_router} daemon
  to rebuild the routing table.
  If this changes the name of a route,
  then the count of currently routed jobs on that route
  will not be accurate until the existing jobs finish.  Another option
  to achieve dynamic routing is to read the routing table from an
  external source using the \MacroNI{JOB\_ROUTER\_ENTRIES\_FILE} or
  the \MacroNI{JOB\_ROUTER\_ENTRIES\_CMD} configuration variable.


\label{param:JobRouterEntriesFile}
\item[\Macro{JOB\_ROUTER\_ENTRIES\_FILE}]
  Specifies a file that contains the ClassAds describing the
  routing table.  The format of this file is the same as
  \MacroNI{JOB\_ROUTER\_ENTRIES}, except that it is interpreted directly
...
\item[\Macro{JOB\_ROUTER\_ENTRIES\_CMD}]
  ...
  happens every \MacroUNI{JOB\_ROUTER\_ENTRIES\_REFRESH} seconds.
  To enter spaces in any arguments or in the command name itself,
  surround the right hand side of the definition with double quotes and use
...
\item[\Macro{JOB\_ROUTER\_ENTRIES\_REFRESH}]
  ...
  The default value is 0, meaning updates will only be read
  whenever \Condor{reconfig} is run or when the \Condor{job\_router} restarts.
...
\item[\Macro{MAX\_JOB\_MIRROR\_UPDATE\_LAG}]
  An integer value representing the maximum number of
  seconds the \Condor{job\_router} daemon should wait
  before giving up when waiting to see
  routed jobs appear in the \Condor{schedd}'s job queue log file.
  The default value is 600.
  It is unlikely that this value will need to be adjusted.
...
\item[\Macro{JOB\_ROUTER\_NAME}]
  Names multiple instances of the \Condor{job\_router} daemon for the same
