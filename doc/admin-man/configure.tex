...
\item[\Macro{AUTHENTICATION\_METHODS}]\label{param:AuthenticationMethods}
  ...
  \begin{itemize}
		...
		\item GSS Use Generic Security Subsystem, which is implemented in
			Condor with X.509 certificates.  
...
\item[\Macro{SUBSYS\_ARGS}] \label{param:SubsysArgs} This setting
  ...
  For example, you would set \Macro{STARTD\_ARGS} to specify any extra
...
\item[\Macro{PREEN}] \label{param:Preen} In addition to the daemons
  ...
  \Condor{master} finds the preen binary.
  If you don't want preen to run at all, just comment out this
  setting.

\item[\Macro{PREEN\_ARGS}] \label{param:PreenArgs} This setting
  controls how \Condor{preen} behaves by allowing you to specify
  command line arguments.
  This works just like \Macro{SUBSYS\_ARGS} explained above.
  The only difference is that you must specify this setting for
  \Condor{preen} if you want it to do anything.
  It's only because of command line arguments that \Condor{preen}
  takes any action at all.
  ``-m'' means you want email about files \Condor{preen} finds that it
  thinks it should remove.
  ``-r'' means you want \Condor{preen} to actually remove these files.
...
\label{param:MaxVirtualMachineTypes}
  ...
  Note: this is the maximum number of different \Bold{types} not of
...
\label{param:VirtualMachineTypeN}
  ...
  The ``<N>'' can be any integer from 1 to the value of
  \Macro{MAX\_VIRTUAL\_MACHINE\_TYPES}, such as
  \Macro{VIRTUAL\_MACHINE\_TYPE\_1}. 
...
\label{param:NumVirtualMachinesTypeN}
  This setting controls how many of a given virtual machine type
  (defined above) are actually reported to Condor.
  There is no default.

\item[\Macro{NUM\_VIRUAL\_MACHINES}]
\label{param:NumVirtualMachines}
  If you your SMP machine is being evenly divided, and the virtual
  machine type settings described above are not being used, this
  setting controls how many virtual machines will be reported.  
...
\item[\Macro{APPEND\_RANK\_STANDARD}] \label{param:AppendRankVanilla}
