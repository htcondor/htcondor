...
\MacroUNI{macro\_name}, while pre-defined expressions are listed as
...
\item[\Expr{CurrentTime}] \label{param:CurrentTime}
\index{CurrentTime expression@\texttt{CurrentTime} expression}
\index{configuration expression!\texttt{CurrentTime}}
  An expression
...
\item[\Macro{AUTHENTICATION\_METHODS}]\label{param:AuthenticationMethods}
  ...
  \begin{itemize}
		...
		\item GSS Use Generic Security Services, which is implemented in
...
\item[\Macro{GLOBUSRUN}] \label{param:Globusrun}
  This macro holds the full path to the \Exec{globusrun} program which is
