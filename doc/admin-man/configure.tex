...
\item[\Macro{SYSTEM\_JOB\_MACHINE\_ATTRS}]
  ...
  machine attributes that should be recorded in the job ad.  The
  ...
  file command.  The value recorded in the job ad is the evaluation of
  the machine attribute in the context of the job ad when the \Condor{schedd}
  daemon initiates the startup of the job.  If the evaluation results in
  an undefined or error result, the value recorded in the job ad will be
...
\item[\Macro{SYSTEM\_JOB\_MACHINE\_ATTRS\_HISTORY\_LENGTH}]
  This macro specifies the number of run attempts to store in
  the job ad when recording the values of machine attributes listed
