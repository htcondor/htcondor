...
\label{param:Docker}
\item[\Macro{DOCKER}]
  The path and executable name of the Docker CLI for execute machines
  that are set up to run the Docker CLI as a user other than root.
  The default value is \File{/usr/bin/docker}.
  Parsing of this configuration variable's value looks for \Expr{sudo}
  to the left of the path and executable name.
  If present, then the Docker CLI is run as root,
