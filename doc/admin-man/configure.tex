...
\item[\MacroU{DETECTED\_MEMORY}]
  Amount of detected physical memory (RAM) in megabytes.

\label{param:DetectedCores}
\item[\MacroU{DETECTED\_CORES}]
  Number of detected CPU cores.  This includes hyperthreaded cores if there
  are any.
...
\item[\Macro{STARTER\_ALLOW\_RUNAS\_OWNER}]
  ...
  account (true) or whether it will run as \MacroNI{SLOT<N>\_USER} or
...
% This should be gotten rid of, that functionality is DEAD.
%\label{param:CkptServerReplicationLevel}
%\item[\Macro{CKPT\_SERVER\_REPLICATION\_LEVEL}]
...
% This is currently not used in the codebase, it is part of the NETMAN feature.
%\label{param:ManageBandwidth}
%\item[\Macro{MANAGE\_BANDWIDTH}]

% This is currently not used in the codebase, it is part of the NETMAN feature.
%\label{param:NetworkHorizon}
%\item[\Macro{NETWORK\_HORIZON}]

% This may be part of the REPLICATION feature, which is DEAD in the ckpt server.
%\label{param:CkptServerHosts}
%\item[\Macro{CKPT\_SERVER\_HOSTS}]
...
\item[\Macro{STARTD\_CRON\_<ModuleName>\_PERIOD}]
  ...
  In \Expr{Wait For Exit} mode, the value has a different meaning; in
  this case 
...
\item[\Macro{STARTD\_CRON\_<ModuleName>\_MODE}]
  ...
  which runs in the \Expr{Wait For Exit} mode.
  ...
  In general, \Expr{Wait For Exit} mode is for modules that produce
...
\item[\Macro{DELEGATE\_FULL\_JOB\_GSI\_CREDENTIALS}]
  A boolean value that controls whether a full or limited GSI X509 proxy
  is delegated for grid-type gt2 grid universe jobs. The default value is
  \Expr{False}.
...
These macros affect how Condor deals with \Condor{ssh\_to\_job} (see
section~\ref{man-condor-ssh-to-job}), a tool that allows users to
interactively debug jobs.  With these configuration variables, the
admin can control who can use the tool and how the ssh programs are
invoked.

Details of how the ssh session works: A secure connection between
\Condor{ssh\_to\_job} and the starter is first authenticated and
authorized using Condor protocols.  The starter generates an ssh key
pair and sends it securely to \Condor{ssh\_to\_job}.  Then the starter
spawns sshd in inetd mode.  \Condor{ssh\_to\_job} then acts as a proxy
for the ssh client to communicate with sshd, using the existing
connection that was authorized by Condor.  \emph{At no point is sshd
listening on the network for new connections or running with anything
other than the privileges of the user identity running the job.}  If
CCB is being used to enable connectivity to the execute node from
outside of a firewall or private network, \Condor{ssh\_to\_job} is
able to make use of CCB in order to form the ssh connection.

\Condor{ssh\_to\_job} is intended to work out-of-the-box with OpenSSH
as installed in typical environments.  It does not work under windows.
If the ssh programs are installed in non-standard locations, paths to
these programs will need to be customized in the condor configuration.
Versions of ssh other than OpenSSH may work, but they would likely
require additional configuration of command-line arguments, changes to
the sshd configuration template file, and possibly modification of the
script used by the starter to set up sshd
(\verb|$(LIBEXEC)/condor_ssh_to_job_sshd_setup|).

\begin{description}
\label{param:EnableSSHToJob}
\item[\Macro{ENABLE\_SSH\_TO\_JOB}]
This configuration variable is read by the \Condor{starter} (the
process that manages a running job on the execute machine).  When true
(the default), the owner of the job or a queue super user on the
schedd where the job was submitted will be allowed to connect to the
job via ssh.  This setting may be an expression that refers to
attributes of the job and machine ads.  The job ad may be referenced by
TARGET and the machine ad may be referenced by MY.  Setting this expression
to false prevents \Condor{ssh\_to\_job} from starting an ssh session.

\label{param:ScheddEnableSSHToJob}
\item[\Macro{SCHEDD\_ENABLE\_SSH\_TO\_JOB}]
This configuration variable is read by the \Condor{schedd}.  When true
(the default), the owner of the job or a queue super user will be
allowed to connect to the job via ssh if the execute machine also
allows \Condor{ssh\_to\_job} access (see
\Macro{ENABLE\_SSH\_TO\_JOB}).  This setting may be an expression that
refers to attributes of the job (but not the machine).  Setting this
expression to false prevents \Condor{ssh\_to\_job} from starting an
ssh session for all jobs managed by the schedd.

\label{param:SSHToJobSSHCmd}
\item[\Macro{SSH\_TO\_JOB\_SSH\_CMD}]

This setting is ready by the \Condor{ssh\_to\_job} tool.  It specifies
the command and arguments to use when invoking the ssh client.  The
entire command plus arguments string should be enclosed in double
quotes.  Individual arguments may be quoted with single quotes, using
the same syntax as for arguments in a \Condor{submit} file.  The following
...
\item[\%k] known hosts file

\item[\%u] remote user

\item[\%x] proxy command suitable for use with the OpenSSH ProxyCommand option
...
Alternate ssh clients may be configured as well (for use with
\verb|condor_ssh_to_job -ssh <alt>|.  For example, to configure sftp
instead of ssh, use the configuration variable
\Macro{SSH\_TO\_JOB\_SFTP\_CMD}.  The default arguments are the same
as the default for ssh, except that if the command is scp, \%h is
omitted.


\label{param:SSHToJobSSHD}
\item[\Macro{SSH\_TO\_JOB\_SSHD}]
This setting is read by the \Condor{starter}, the process that manages
a running job on the execute machine.  It specifies the path to
\verb|sshd|, the ssh daemon.

\label{param:SSHToJobSSHDArgs}
\item[\Macro{SSH\_TO\_JOB\_SSHD\_ARGS}]
This setting is read by the \Condor{starter}, the process that manages
a running job on the execute machine.  It specifies the command-line
arguments to be passed to \verb|sshd| to handle an incoming ssh
connection on its stdin/stdout streams (inetd mode).  The entire
arguments string should be enclosed in double quotes.  Individual
arguments may be quoted with single quotes, using the same syntax as
for arguments in a \Condor{submit} file.  Within the arguments, the
characters \%f are replaced by the path to the sshd configuration file
and \%\% are replaced by \%.  The default is \verb|"-i -e -f %f"|.

\label{param:SSHToJobSSHDConfigTemplate}
\item[\Macro{SSH\_TO\_JOB\_SSHD\_CONFIG\_TEMPLATE}]
This setting is read by the \Condor{starter}, the process that manages
a running job on the execute machine.  It specifies the path to an
sshd configuration template.  The template is turned into an sshd
configuration file by replacing macros within the template that
specify such things as the paths to key files.  The macro replacement
is done by the script \verb|$(LIBEXEC)/condor_ssh_to_job_sshd_setup|.

\label{param:SSHToJobSSHKeygen}
\item[\Macro{SSH\_TO\_JOB\_SSH\_KEYGEN}]
This setting is read by the \Condor{starter}, the process that manages
a running job on the execute machine.  It specifies the path to
\verb|ssh_keygen|, a program used to create ssh keys.

\label{param:SSHToJobSSHKeygenArgs}
\item[\Macro{SSH\_TO\_JOB\_SSH\_KEYGEN\_ARGS}]
This setting is read by the \Condor{starter}, the process that manages
a running job on the execute machine.  It specifies the command-line
arguments to be passed to \verb|ssh_keygen| to generate an ssh key.
The entire arguments string should be enclosed in double quotes.
Individual arguments may be quoted with single quotes, using the same
syntax as for arguments in a \Condor{submit} file.  Within the
arguments, the characters \%f are replaced by the path to the key
file to be generated and \%\% are replaced by \%.  The default is
\verb|"-N '' -C '' -q -f %f -t rsa"|.  If the user specifies additional
arguments with \verb|condor_ssh_to_job -keygen-options|, those
arguments are placed after the areguments specified in
\Macro{SSH\_TO\_JOB\_SSH\_KEYGEN\_ARGS}.
