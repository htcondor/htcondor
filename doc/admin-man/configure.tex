...
On Linux systems which provide "bind" mounts and per-process tree
mount tables, such as Red Hat Enterprise Linux 5, the administrator
can optionally configure Condor to provide new filesystem bind mounts
for each job.  This is useful, for example, to give each jobs it's own
/tmp or /var/tmp filesystem mapping, independent of any other job.
This is controlled with the MOUNT\_UNDER\_SCRATCH parameter.  If set,
this is a comma separated list of directories to provide new mounts for.
If it is set to /tmp, Condor will make a directory under the execute
side sandbox called tmp, and bind mount that directory to /tmp.
...
\item[\Macro{DAGMAN\_HOLD\_CLAIM\_TIME}]
  An integer defining the number of seconds \Condor{dagman} will hold onto a
  claim after a job is finished, using the \Attr{KeepClaimIdle} attribute.  The
  default value is 20. Setting this value to 0 causes \Condor{dagman} not to set
  the attribute.
