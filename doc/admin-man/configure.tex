...
\item[\Macro{ADD\_SIGNIFICANT\_ATTRIBUTES}]
  A comma and/or space separated list of job ClassAd attributes that
  will always be added to the list of attributes for determining
  the sets of jobs considered as a unit (an auto cluster) in negotiation,
  when auto clustering is enabled.
...
\item[\Macro{REMOVE\_SIGNIFICANT\_ATTRIBUTES}]
  A comma and/or space separated list of job ClassAd attributes that
  are banned from being added to the list of attributes for determining
  the sets of jobs considered as a unit (an auto cluster) in negotiation,
  when auto clustering is enabled.
...
\item[\Macro{DAGMAN\_USE\_OLD\_DAG\_READER}]
  As of HTCondor version 8.3.3, the value is always the default 
  value of \Expr{False}.  
  Attempts to set it to \Expr{True} results in a warning, and will
  have no effect on how the DAG file is read. 
