...
\item[\Macro{CONCURRENCY\_LIMIT\_DEFAULT}]
  ...
  all resources that are not explicitly named by defining the
...
\item[\Macro{<NAME>\_LIMIT}]
  An integer value that describes the number of a resource available for
  concurrent job execution when requested for a job under concurrency limits
...
glexec is invoked multiple times during each attempt to run a job.  This
configuration setting only disables putting the job on hold for the
