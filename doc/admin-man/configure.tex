...
On Linux systems which provide bind mounts and per-process tree mount tables,
such as Red Hat Enterprise Linux 5, the administrator
can optionally configure Condor to provide new file system bind mounts
for each job.  This is useful, for example, to give each jobs its own
\File{/tmp} or \File{/var/tmp} file system mapping, 
independent of any other job.
If set,
this variable specifies a comma separated list of directories 
to provide new mounts for.
If it is set to \File{/tmp}, 
Condor will make a directory under the execute-side sandbox called \File{tmp},
and bind mount that directory to \File{/tmp}.
