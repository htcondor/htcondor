...
\item[\Macro{TRANSFER\_IO\_REPORT\_INTERVAL}]
  ...
  smallest timespan that is configured in
...
\item[\Macro{TRANSFER\_IO\_REPORT\_TIMESPANS}]
  A string that specifies a list of timespans over which I/O
  ...
  seconds over which the named timespan should extend.  The default is
  \verb|1m:60 5m:300 1h:3600 1d:86400|.  To provide sufficient
  resolution, the smallest reported timespan should be large compared
  to the sampling interval, which is configured by
  \Macro{TRANSFER\_IO\_REPORT\_INTERVAL}.  See
  page~\pageref{sec:FT-Scheduler-ClassAd-Attributes} for a description
  of the published attributes.

\label{param:IndividualTransferIoReport}
\item[\Macro{INDIVIDUAL\_TRANSFER\_IO\_REPORT}]
  A boolean value that when true causes I/O statistics to be published
  for individual transfer queues.  This setting is true by default.
  There is one transfer queue per user in the default configuration of
  \Macro{TRANSFER\_QUEUE\_USER\_EXPR}.  Only the recently active
  queues will be published.  See
  page~\pageref{sec:FT-Scheduler-ClassAd-Attributes} for a description
  of the published attributes.
