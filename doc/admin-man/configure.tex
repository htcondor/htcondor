...
\item[\Macro{ENCRYPT\_EXECUTE\_DIRECTORY\_FILENAMES}]
	When using an encrypted execute directory, the contents of the files
	will always be encrypted.  On Windows, the filenames are not encrypted,
	and this setting has no effect.  On Linux, the filenames may or may not
	be encrypted, but there is some overhead and restrictions on doing so
	(see ecryptfs documentation).  As a result, by default the filenames
	are not encrypted on Linux, although the administrator may choose to
	enable that behavior by setting this configuration variable to
	\Expr{True}.
