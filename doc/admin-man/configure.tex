...
\item[\Macro{GRIDMANAGER\_SELECTION\_EXPR}]
	By default, the schedd will start a new gridmanager process for each
	discreet user that submits a grid universe job (i.e. for each discreet
	value of job attribute \emph{Owner} across all grid universe job
	classads).  For additional isolation and/or scalability of grid job
	management, additional gridmanager processes can be spawned to share the
	load; to do so, set this configuration parameter to equal a classad
	expression.  The result of the evaluation of this expression in the
	...
	same gridmanager. For instance, to spawn a seperate gridmanager process to
	manage each unique remote site, put the following into the config file:
\begin{verbatim}
...
\item[\Macro{GRID\_MONITOR\_HEARTBEAT\_TIMEOUT}]
  If this many seconds pass without hearing from a working \Prog{grid\_monitor}, it is
...
\item[\Macro{GRID\_MONITOR\_RETRY\_DURATION}]
  When Condor-G attempts to start the \Prog{grid\_monitor} at a particular
  site, it will wait this many seconds to start hearing from the
  \Prog{grid\_monitor}. Defaults to 900 (15 minutes).  If this duration
  passes without success, the \Prog{grid\_monitor} will be disabled for the
  site in question for a period of time (controlled by
...
\item[\Macro{DAGMAN\_USER\_LOG\_SCAN\_INTERVAL}]
  Time in integer seconds that \Condor{dagman} waits between checking
  job log files for status updates. Setting this lower increases the CPU
