...
\item[\Macro{USE\_SHARED\_PORT}] This is a boolean value that
  ...
  For more information about using a shared port, see
  page~\pageref{sec:Config-shared-port}.
...
\item[\Macro{SLOT<N>\_JOB\_HOOK\_KEYWORD}]
  ...
  identification number. For example, on slot 1, th variable name will be
...
\item[\Macro{MAX\_JOBS\_RUNNING}]
  An integer value representing a limit on the number of processes
  ...
  The actual number of \Condor{shadow} daemons may be reduced,
...
\item[\Macro{SOAP\_SSL\_CA\_DIR}]
  ...
  CA will be authenticated.
  When SOAP over SSL is enabled, this parameter or
  \Macro{SOAP\_SSL\_CA\_FILE} must be set.
  This parameter is also used when communicating with an Amazon EC2 server
  (possibly through a proxy), though it's not required in that case.
  There is no default value.
...
\index{configuration!condor\_shared\_port configuration variables}
 ...
 support it (currently Linux and Mac OS X).  It is responsible for
 creating a TCP listener port shared by all of the Condor daemons for
 which \Macro{USE\_SHARED\_PORT} is true (except for the
 \Condor{collector}, which always uses its own command port, and the
 \Condor{schedd}, which creates a command port for local use by the
 shadows).
...
Shared ports currently do not work with SOAP.  Standard universe is also
not supported.
...
\item[\Macro{DAEMON\_SOCKET\_DIR}] This specifies the directory where
 Condor daemons will create named sockets so that incomming
 connections can be forwarded to them by \Condor{shared\_port}.  If
 this directory does not exist, it will be created.  Write access to
 this directory grants permission to receive connections through the
 shared port.  By default, the directory is created to be owned by
 Condor and is made to be only writable by Condor.  One possible
 reason to broaden access to this directory is if execute nodes are
 accessed via CCB and the submit node is behind a firewall with one
 open port for \Condor{shared\_port}.  In this case, commands that
 interact with the execute node such as \Condor{ssh\_to\_job} will not
 be able to operate unless run by a user with write access to
 ...
 circumvent it in this way.)  The default \MacroNI{DAEMON\_SOCKET\_DIR} is
 \verb|$(LOCK)/daemon_sock|.  This directory must be on a local filesystem
 that supports named sockets.
