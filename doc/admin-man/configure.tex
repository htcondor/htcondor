...
\item[\Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG}]
  ...
  \Bold{Note:  As of version 8.1.4, it is no longer recommended to set this
  macro to \Expr{False}, and doing so will produce a warning.  This
  option will probably be disabled entirely in a future version.  To
  run a DAG with \Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} set to \Expr{False},
  you will also have to set \Macro{DAGMAN\_USE\_STRICT} to 0.}
