...
\item[\Macro{KILL\_ORPHANS\_ON\_STARTUP}]
  \label{param:KillOrphansOnStartup} This boolean variable,
  when \Expr{True},
  causes the \Condor{master} to try to locate and hard
  kill orphaned processes from other instantiations of Condor. An
  administrative e-mail is sent detailing the PIDs (along with their
  command lines) which have been killed.
  The default value is \Expr{True}.
  Implemented only on Linux platforms.
