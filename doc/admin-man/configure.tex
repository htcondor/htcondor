...
\item[\Macro{DAGMAN\_DEFAULT\_NODE\_LOG}]
  ...
  the \Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} specification:
  %TEMPTEMP -- change File to Macro here (needs something special for @ to work)
  \begin{itemize}
  \item \File{@(DAG\_DIR)}: The directory in which the primary DAG file
  resides.
  \item \File{@(DAG\_FILE)}: The name of the primarly DAG file (not
  including any directory information).
  \item \File{@(CLUSTER)}: The cluster of this DAGMan process.
  \end{itemize}
  ...
  \File{@(DAG\_DIR)/@(DAG\_FILE).nodes.log}.
