...
\item[\Macro{GROUP\_DYNAMIC\_MACH\_CONSTRAINT}]
  This optional expression specifies which machine ads should be counted
  when computing the size of the pool for purposes of group quota
  allocation.  The default is to count all machine ads.  When extra slots
  exist for special purposes (e.g. suspension slots or file transfer slots),
  this expression can be used to inform the negotiator that only ``normal''
  slots should be counted when computing how big each group's share of
  the pool should be.
