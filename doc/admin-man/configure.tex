...
machines to drain.  The default is \Expr{PartitionableSlot}.  A machine
(i.e. \Condor{startd}) is matched if \emph{any} of its slots match
this expression.  Machines are automatically excluded if they are
already draining or if they match
...
default is \Expr{Cpus == TotalCpus}.  A machine is matched if
\emph{any} slot on the machine matches this expression.  Each
\Condor{startd} is considered to be one machine.  Whole machines are
excluded when selecting machines to drain.  They are also counted
