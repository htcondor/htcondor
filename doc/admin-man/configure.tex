...
\item[\Macro{MASTER\_SHUTDOWN\_$<$Name$>$}]
  ...
  \Note This program will be run with root (UNIX) or administrator
  (Windows) privileges.  The adminstrator must insure that this cannot
...
\item[\Macro{STARTD\_SENDS\_ALIVES}]
  ...
  these updates are sent via TCP.
  This may be useful if the \Condor{startd} daemon is on a private
  network or behind a firewall.
  This variable must be set to the same value for all \Condor{startd} and
  \Condor{schedd} daemons in the pool.
