...
\item[\Macro{DAGMAN\_DEFAULT\_NODE\_LOG}]
  ...
  the \MacroNI{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} specification (
  ...
  If not defined, \MacroNI{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} defaults
  to \Expr{@(DAG\_DIR)/@(DAG\_FILE).nodes.log}.

  A few more notes:
  \begin{itemize}
  \item The expression \Expr{\$(LOG)} will not have the expected effect
  if used in the \MacroNI{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} specification,
  ...
  \item The \Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} specification must
  ensure that the default node log files are named uniquely for each DAG.
  Therefore, the \Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} value should
