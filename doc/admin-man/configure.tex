...
\item[\Macro{STATISTICS\_WINDOW\_SECONDS}]
  ...
  collecting windowed daemon statistics (by convention, windowed statistics
  typically are of the form 'RecentXxx').  Any data contributing to a
  windowed statistic that is older than this number of seconds is dropped
  from the statistic.  For example if \Macro{STATISTICS\_WINDOW\_SECONDS} = 300, then
  any jobs submitted more than 300 seconds ago are not counted in the windowed
  statistic \Attr{RecentJobsSubmitted}.  Defaults to 1200.

  Configuration variables of the form \Macro{STATISTICS\_WINDOW\_SECONDS\_<collection>}
  override the global setting for a particular statistic collection.  Collection names
  currently implemented are 'DC' (or 'DAEMONCORE') and 'SCHEDD' (or 'SCHEDULER')

\label{param:StatisticsWindowQuantum}
\item[\Macro{STATISTICS\_WINDOW\_QUANTUM}]
  An integer value that controls the time quantization, in seconds, for the data structures 
  that maintain windowed statistics.  Defaults to 240.   Smaller values yield higher
  time resolution and reduce 'sawtoothing' of windowed statistic values, at the expense of
  requiring larger data structures for maintenance.

  Configuration variables of the form \Macro{STATISTICS\_WINDOW\_QUANTUM\_<collection>}
...
\item[\Macro{DAEMON\_SHUTDOWN}]
  ...
  One possible use case is for HTCondor glide-in, to have the
...
\item[\Macro{USE\_PROCESS\_GROUPS}]
  ...
  HTCondor is run under another job execution system (e.g. HTCondor Glidein).
...
configure HTCondor on SMP machines.
...
\item[\Macro{JOB\_DEFAULT\_NOTIFICATION}]
  The default notification for jobs. This value defaults to NEVER, so that
  HTCondor will not send email about events for jobs. Must be one of the values
  NEVER, ERROR, ALWAYS, or COMPLETE. As stated in the \Condor{submit} man page:
  ``If defined by ALWAYS, the owner will be notified whenever the job produces a
  checkpoint, as well as when the job completes. If defined by COMPLETE, the
  owner will be notified when the job terminates. If defined by ERROR, the owner
  will only be notified if the job terminates abnormally, or if the job is placed
  on hold because of a failure, and not by user request. If defined by NEVER, the
  owner will not receive e-mail, regardless to what happens to the job.''
...
\item[\Macro{CONDOR\_DEVELOPERS}]
  ...
  the HTCondor Team developers at the University of Wisconsin-Madison.
  The HTCondor Team uses
  ...
  If you do not wish this information to be sent to the HTCondor Team,
...
\item[\Macro{CONDOR\_DEVELOPERS\_COLLECTOR}]
  ...
  updates help the HTCondor Team see how HTCondor is being used around the world.
...
\item[\Macro{DEFAULT\_PRIO\_FACTOR}]
  Sets the priority factor for local users, as described in
...
\label{param:GroupSeparator}
\item[\Macro{GROUP\_SEPARATOR}]
  A string value that contains the separator character used between
accounting groups and the user name.  Defaults to \Expr{``.''} for
backward compatibility.  In this example, the separator character is set
to \Expr{``:''}, to allow the use of \Expr{``.''} in user names:
\begin{verbatim}
GROUP_SEPARATOR = ":"
\end{verbatim}
...
\item[\Macro{DAGMAN\_SUPPRESS\_NOTIFICATION}]
	...
	email notification when certain events occur.  If set to true, then all jobs
	submitted by \Condor{dagman} will have ``notification = never'' set. This does 
    not affect the notification of the \Condor{dagman} job itself. A user could set
	this flag to true, then would only receive one notification when the \Condor{dagman}
	job completes, rather than thousands of notifications from the jobs submitted by
	\Condor{dagman}. This flag defaults to true, meaning that no job notification will
	be sent for a job submitted by \Condor{dagman}.
...
If no port is specified, a port will be dynamically chosen; it may be
 different each time HTCondor is started.
\end{description}
