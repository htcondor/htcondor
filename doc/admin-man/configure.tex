...
\Condor{master} daemon's \MacroNI{DAEMON\_LIST}.
It is responsible for publishing information about HTCondor daemons to the
\TM{Ganglia} monitoring system.  In the typical case, a pool runs a single
instance of \Condor{gangliad}.  A default set of metrics are provided.
Additional metrics may be defined.

\begin{description}

\label{param:GangliaInterval}
\item[\Macro{GANGLIA\_INTERVAL}]
  The integer number of seconds between publication of metrics to Ganglia.
  The default value is 300.

\label{param:GangliaVerbosity}
\item[\Macro{GANGLIA\_VERBOSITY}]
  An integer that specifies the maximum verbosity level of metrics to be published
  to Ganglia.  Basic metrics have a verbosity level of 0, which is the default.
  Additional metrics can be enabled by increasing the verbosity to 1.  In the default
  configuration, there are no metrics with verbosity levels higher than 1.
...
\label{param:GangliaMetricsConfigDir}
\item[\Macro{GANGLIA\_METRICS\_CONFIG\_DIR}]
  Path to the directory containing files which define Ganglia metrics in terms of HTCondor
  ClassAd attributes to be published.  Each metric definition file contains a list of ClassAds.
  The attributes of each ad define the properties of a metric.  The value of each attribute may
  be a ClassAd expression that references properties of the daemon ad that is being monitored.
  The recognized metric attributes are the following:

  \begin{description}

  \item[name] The name of this metric.  Each metric must have a unique name for each machine.

  \item[value] A ClassAd expression that produces the value of this metric.  The default value is the
               value of the attribute with the same name as this metric.

  \item[desc] A description of the metric.  This is displayed when the user holds the mouse over the graph.

  \item[verbosity] The integer verbosity level of this metric.

  \item[daemon] The type of daemon ClassAd this metric monitors.  Possible values are
                \verb|"SCHEDD"|, \verb|"NEGOTIATOR"|, \verb|"ANY"|, and \verb|"STARTD"|.
                In addition, \verb|"STARTD_SLOT1"| may be specified to monitor the startd ad
                for slot 1 but not other slots.

  \item[title] The graph title for this metric.  The default is the metric name.

  \item[group] A string specifying the metric group.  The default is determined by the daemon type.

  \item[units] A string describing the units of this metric.

  \item[derivative] A boolean value that specifies if Ganglia should graph the derivative of this metric.
                    Ganglia versions prior to 3.4 do not support this.

  \item[type] The type of metric.  Possible values are \verb|"double"| and \verb|"string"|.
              The default is \verb|"double"|.

  \item[regex] This string value specifies a regular expression that matches attributes to
               be monitored by this metric.  This is useful for dynamic attributes that cannot
               be enumerated in advance, because their names depend on dynamic information such
               as the users who are currently running jobs.  When this is specified, one metric
               per matching attribute is created.  The default metric name is the name of the
               matched attribute.  If the value of this attribute needs to be modified before
               publication, it can be referred to as an attribute named ``regex'' in the \verb|value|
               expression.  Regular expression groups defined by parentheses may be substitued
               into the values of string attributes such as the name and description.  This is done
               by putting macros in the string values.  ``\Bs\Bs{}1'' is replaced by the first group,
               ''\Bs\Bs{}2'' by the second group, and so on.

  \item[aggregate] This string value specifies an aggregation function to apply instead of publishing
                   individual metrics for each daemon ad.  Possible values are \verb|"sum"|,
                   \verb|"avg"|, \verb|"max"|, and \verb|"min"|.

  \item[aggregate\_group] When an aggregate function has been
    specified, this string value specifies which aggregation group the
    current daemon ad belongs to.  The default is the metric name.
    Example use of aggregate grouping: if one wished to publish the
    sum of some attribute across different types of slot ads, one
    could make the metric name an expression that is unique to each
    type.  The default aggregate\_group would be set accordingly.

  \item[machine] The name of the host associated with this metric.  The default is taken from the
                 daemon ad.  If the daemon name is of the form \verb|extra@machine.name|, this
                 may be to distinguish the daemon from others of the same type running on the
                 same machine.  To avoid the metrics from these daemons overwriting each other,
                 the default machine name is set to \verb|extra.machine.name|.
  \item[ip] The IP of the host associated with this metric.  The default is determined from the
            daemon ad.

  \item[requirements] If specified, this may be used to restrict which daemon ads are monitored
                      by this metric.  By default, all daemon ads of the specified type are monitored.
