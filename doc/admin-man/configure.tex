...
references such a submit file and \Macro{DAGMAN\_LOG\_ON\_NFS\_IS\_ERROR}
is \Expr{True}, the DAG will abort during the initialization process. 
If \Macro{DAGMAN\_LOG\_ON\_NFS\_IS\_ERROR} is \Expr{False}, a warning
will be issued but the DAG will still be submitted.  It is \emph{strongly}
recommended that \Macro{DAGMAN\_LOG\_ON\_NFS\_IS\_ERROR}
remain set to the default value, because running a DAG with node job
log files on NFS will often cause errors.
If undefined, \Macro{DAGMAN\_LOG\_ON\_NFS\_IS\_ERROR} defaults to
\Expr{True}.
...
the lock file does exist, and \Macro{DAGMAN\_ABORT\_DUPLICATES} is
...
test is not foolproof: it's possible that, if \Condor{dagman} crashes,
the same PID gets re-used by another process before \Condor{dagman}
gets re-run on that DAG.  This should be quite rare, however.
If undefined, \Macro{DAGMAN\_ABORT\_DUPLICATES} defaults to
\Expr{False}.
