...
causes daemons the older daemons to fail when they attempt to
parse this syntax.
...
\item[\Macro{SOCKET\_LISTEN\_BACKLOG}]
  ...
  It sets the backlog setting of the listen() network call when a daemon
  creates a socket for incoming connections.
  It limits the number of new incoming network connections the OS will
  accept for a daemon that the daemon hasn't serviced yet.
