...
\item[\Macro{JOB\_ROUTER\_RELEASE\_ON\_HOLD}]
  True or False.  Defaults to True.  Controls how the \Condor{job\_router}
  will handle the routed job going on hold.  If True the \Condor{job\_router}
  will leave the source job in the same state the \Condor{job\_router}
  found when it was claimed.  If False the \Condor{job\_router} will not
  attempt to reset the source job to a pre-claimed state upon yielding
  control of the job.
