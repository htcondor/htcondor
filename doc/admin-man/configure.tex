...
the \Condor{procd} results in improved scalability becuase only one
...
\item[\Macro{VMWARE\_NETWORKING\_TYPE}]
  ...
  A string that defaults to the value \Expr{nat},
...
\item[\Macro{XEN\_CONTROLLER}]
  ...
  to be defined, nat and bridge networking must be configured
  within Xen's \File{xend-config.sxp} file,
  and networking parameters must be specified with
  the \MacroNI{XEN\_VIF\_PARAMETER} configuration variable;
  while, \Prog{virsh} uses the \MacroNI{XEN\_BRIDGE\_SCRIPT} 
  configuration variable to set up a bridged network,
  and does not need \MacroNI{XEN\_IMAGE\_IO\_TYPE} or
  \MacroNI{XEN\_VIF\_PARAMETER} configuration variables.
