...
\item[\Macro{NETWORK\_INTERFACE}]
  ...
  This setting may be an IP address (e.g. 123.123.123.123) or the name
  of a network device (e.g. eth0).  The `*' character may be used as a
  wildcard.  For example, 123.123.* would match a network interface
  with IP address 123.123.123.123 or 123.123.100.100.  The default
  value is `*', which matches all network interfaces.

  The effect of this setting depends on the value of
  \MacroNI{BIND\_ALL\_INTERFACES}.  There are two cases:

  \MacroNI{BIND\_ALL\_INTERFACES} is \Expr{True} (the default): In
  this case, \MacroNI{NETWORK\_INTERFACE} controls what IP address
  will be advertised as the public address of the daemon.  If multiple
  network interfaces match the setting and
  ...
  is used to communicate with the collector.  If
  \MacroNI{ENABLE\_ADDRESS\_REWRITING} is \Expr{False}, the IP address
  ...
  order of preference.  If it is desired to advertise an IP address
  that is not associated with any local network interface (for
  example, when TCP forwarding is being used),
  \Macro{TCP\_FORWARDING\_HOST} should be used instead of
  \MacroNI{NETWORK\_INTERFACE}.

  \MacroNI{BIND\_ALL\_INTERFACES} is \Expr{False}: In this case
  \MacroNI{NETWORK\_INTERFACE} specifies which IP address Condor
  should use for all (incoming and outgoing) communication.  If more
  than one IP address matches the setting, the IP address that is
...
\item[\Macro{PRIVATE\_NETWORK\_INTERFACE}]
  ...
  If another Condor daemon or tool is configured with the same
  \MacroNI{PRIVATE\_NETWORK\_NAME}, it will attempt to contact this
  daemon using the IP address specified here.  The syntax for specifying
  an IP address is identical to \MacroNI{NETWORK\_INTERFACE}.
...
\item[\Macro{STARTD\_SENDS\_ALIVES}]
  ...
  between the \Condor{schedd} and \Condor{startd} during a claim.
  Having the \Condor{startd} send the messages (the default) is usually
  the better option. If the messages need to be sent by the \Condor{schedd},
  this parameter can be set to \Expr{False}.
  If both daemons are Condor 7.5.4 or later, this parameter only needs
  to be set for the \Condor{schedd}. Otherwise, it must be set for both
