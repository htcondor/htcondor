...
\item[\Macro{OFFLINE\_LOG}]
  Deprecated by \MacroNI{PERSISTENT\_AD\_LOG}.
...
persistently stores absent ads.  See section~\ref{sec:Absent-Ads} on 
...
\item[\Macro{ABSENT\_REQUIREMENTS}]
  Evaluated by the collector when a machine ad would otherwise expire.  If 
  true, the ad instead becomes absent.

\label{param:AbsentExpireAdsAfter}
\item[\Macro{ABSENT\_EXPIRE\_ADS\_AFTER}]
  The integer number of seconds after which the collector forgets about an
  absent ad.  If 0, the collector will never forget.  Defaults to 30 days.
...
\item[\Macro{PERSISTENT\_AD\_LOG}]
  ...
  Deprecates \MacroNI{OFFLINE\_LOG}.
