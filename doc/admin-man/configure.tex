...
\item[\Macro{MAX\_TRANSFER\_INPUT\_MB}]
  ...
  files transferred via file transfer plugins.  The expression may
  refer to attributes of the job.  The special value -1 indicates no
  limit.  The default is -1.  The job may override the system setting
  ...
  with hold code 32.  If the job passes this initial test, but the
...
\item[\Macro{MAX\_TRANSFER\_OUTPUT\_MB}]
  ...
  with hold code 33.  The output will be transferred up to the point
