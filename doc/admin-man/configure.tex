...
Evaluation works in an unexpected way
...
\emph{not} the (expected) value of \Expr{mydir/B}.
...
\item[\Macro{DAGMAN\_DEFAULT\_NODE\_LOG}]
  ...
  of a DAG.
  Note that even if \Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} is set to
  ...
  If more than one DAG input file is specified, the name of the first is
  used to name the default log file.
...
\item[\Macro{DAGMAN\_USE\_OLD\_DAG\_READER}]
  A boolean value defining whether \Condor{submit\_dag} uses old
  (pre-8.0.6) code to read DAG files to find the per-DAG configuration
  file (if any), and sub-DAG files (if the \Opt{-do\_recurse} option is used).
  Note that \MacroNI{DAGMAN\_USE\_OLD\_DAG\_READER} is a special case
  among \Condor{dagman}-related configuration variables, in that setting
  it in a per-DAG configuration file will not have the desired effect.
  Therefore, if you want to set \MacroNI{DAGMAN\_USE\_OLD\_DAG\_READER}
  to a non-default value, you should do it in a machine or pool
  configuration file, or by setting the environment variable
  \MacroNI{\_CONDOR\_DAGMAN\_USE\_OLD\_DAG\_READER}.
  The default value if not defined is \Expr{False}.  We anticipate
  that this configuration variable should not have to be changed
  from the default setting -- the only reason for doing so is if a
  bug is found in the new DAG file-reading code.
