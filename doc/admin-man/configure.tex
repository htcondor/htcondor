...
\item[\Macro{MASTER\_NAME}] \label{param:MasterName}
  ...
  \Condor{master} should be defined to uniquely identify the seperate
  daemons. 

  If the \MacroNI{MASTER\_NAME} contains more than a hostname, it must
  ...
  sign with the fully qualified hostname of the local machine.
  If the string doesn not include an \verb$@$ sign, Condor will append
  one, followed by the hostname.
  The \verb$identifying-string$ portion described above can contain any
  alphanumeric ASCII characters or punctuation marks except \verb$@$
  (which is used to delimit the name from the hostname).
  However, we recommend that the string does not contain the \verb$:$
  ...
  host, the username that each \Condor{master} is executing as is the
  usual convention for that to use as the \verb$identifying-string$.
  This is easily done with \verb@MASTER_NAME = $(USERNAME)@ in the
  configuration file.

  If the \MacroNI{MASTER\_NAME} setting is used, and the
  \Condor{master} is configured to spawn a \Condor{schedd}, the name
  ...
  Since Condor makes the assumption that there is usually only one
  instance of the \Condor{startd} running on a machine, the
  \MacroNI{MASTER\_NAME} does not automatically get propagated to the
  \Condor{startd}.
  ...
  the \MacroNI{STARTD\_NAME} should be set to uniquely identify them
