...
\item[\Macro{PREEMPT}]
  A boolean expression that, when \Expr{True},
  causes Condor to stop a currently running job.  This expression
  is not evaluated if \MacroNI{WANT\_SUSPEND} is \Expr{True}.
...
\item[\Macro{WANT\_HOLD}]
  ...
  and \MacroNI{WANT\_SUSPEND} is \Expr{False} and \MacroNI{WANT\_VACATE}
  is \Expr{True},
