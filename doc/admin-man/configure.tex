...
\label{param:CollectorOfflineLog}
\item[\Macro{OFFLINE\_LOG}]
  The full path to a log file that will store the machine ClassAd 
  for every hibernating machine.  It is kept in a log so that if
  the Collector crashes, knowledge of the hibernating machines is
  not lost.

\label{param:CollectorOfflineExpireAds}
\item[\Macro{OFFLINE\_EXPIRE\_ADS\_AFTER}]
  An integer number of seconds that determines the lifetime of a
  machine ClassAd that represents a hibernating machine.  This
