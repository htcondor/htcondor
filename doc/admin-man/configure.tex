...
\item[\Macro{SHARED\_PORT\_PORT}]
  ...
  set this variable to \Expr{0}.  This causes the \Condor{shared\_port}
  to use a randomly selected port (in the range
  \MacroNI{LOWPORT} - \MacroNI{HIGHPORT};
  see section \ref{sec:Ports-Firewalls}).
...
\item[\Macro{CLASSAD\_USER\_PYTHON\_MODULES}]
  A comma separated list of python modules to load to be used during ClassAd
  evaluation.  If module \Expr{foo} is in this list, then function \Expr{bar}
  can be invoked in ClassAds via the expression \Expr{python\_invoke("foo", "bar", ...)}.
  Any further arguments are converted from ClassAd expressions to python; the
  functions return value is converted back to ClassAds.
  The python modules are loaded at configuration time, so any module-level statements
  are executed.  Module writers can invoke \Expr{classad.register} at the module-level
  in order to use python functions directly.

  Functions executed by ClassAds should be non-blocking and have no side-effects;
...
\item[\Macro{CONDOR\_Q\_USE\_V3\_PROTOCOL}]
  ...
  The default value is \Expr{False},
  as many \Condor{q} requests coupled with a busy \Condor{schedd}
  can cause an exceptionally slow response to the \Condor{q} requests.
...
If no port is specified, port 9618 will be used.  To specify that HTCondor
dynamically chose a port, use port number 0.  In order to decrease the
