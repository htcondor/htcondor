...
\item[\Macro{CLASSAD\_LOG\_STRICT\_PARSING}]
  When this configuration variable is set to \Expr{True}, ClassAd log files
  will be read using strict syntax checking for ClassAd expressions.  Classad
  log files include the job queue log and the accountant log.  When set to 
  \Expr{False}, ClassAd log files are read without strict expression syntax
  checking, which allows some legacy ClassAd log data to be read in a backward
  compatible manner.  Defaults to \Expr{True}.  This configuration variable
  may be deprecated in future releases, eventually requiring all ClassAd log
  files to pass strict ClassAd syntax checking. 
...
\index{SUBSYS\_DEBUG macro@\texttt{<SUBSYS>\_DEBUG} macro}
  ...
  \label{dflag:level}
  \item[\Dflag{LEVEL}]
    \index{SUBSYS\_DEBUG macro levels@\texttt{<SUBSYS>\_DEBUG} macro levels!D\_LEVEL@\texttt{D\_LEVEL}}
    This flag is different from the other flags, because it is
    used to change the formatting of all log messages that are printed,
    as opposed to specifying what kinds of messages should be printed.
    If \Dflag{LEVEL} is set, Condor will include the debugging level flags 
...
which contain whitespace.
  ...
  If the wrapper encounters an error, and is unable to run the user
  job, it is important that the wrapper indicate this to the Condor system
  so that Condor does not assign the exit code of the wrapper to the
  user.  To do this, the wrapper should write a useful error message
  to the file named in the environment variable \Env{\_CONDOR\_WRAPPER\_ERROR\_FILE},
  and exit with a non-zero value.
  If this file is created by the wrapper, Condor assumes that the wrapper
  has failed, and will mark the job as idle in the queue, and try to 
  re-run it.  The starter will also copy the contents of this file to
  the starter log, so the administrator can debug the problem.
...
\item[\Macro{USE\_PROCD}]
  ...
  always be used. By default, the \Condor{master} will start a
  \Condor{procd} that all other daemons that need process family tracking will use.
...
\item[\Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG}]
  A boolean value defining whether \Condor{dagman} watches for events in the
  default node log--that is, the value of \Macro{DAGMAN\_DEFAULT\_NODE\_LOG}.
  The boolean value defaults to true.  If set to true, then \Condor{dagman}
  writes POST script terminate events to the default log, and not to the user log
  specified in the submit file; \Condor{dagman} will also only read events from
  the default log file.  This must be set to false if a \Condor{dagman} from
  version 7.9.0 or later is submitting jobs to a \Condor{schedd} that is older
  than version 7.9.0.
...
\item[\Macro{GSI\_DAEMON\_NAME}]
  ...
  The \verb|*| character may be used as a wildcard character.
  When \MacroNI{GSI\_DAEMON\_NAME} is
  defined, only certificates matching
  \MacroNI{GSI\_DAEMON\_NAME} pass the authentication step, and no
  check is performed to require that the hostname of the daemon
  matches the hostname in the daemon's certificate.  When
  \MacroNI{GSI\_DAEMON\_NAME} is not defined, 
  the hostname of the daemon and certificate must match unless
  exempted by \MacroNI{GSI\_SKIP\_HOST\_CHECK} and/or
  \MacroNI{GSI\_SKIP\_HOST\_CHECK\_CERT\_REGEX}.

\label{param:GsiSkipHostCheck}
\item[\Macro{GSI\_SKIP\_HOST\_CHECK}]
This is a boolean setting that controls whether a check is performed during
...
expression are not required to have a matching daemon hostname and
certificate hostname.  The default is an empty regular expression,
