...
\item[\Macro{START\_LOCAL\_UNIVERSE}] \label{param:StartLocalUniverse}
  \Condor{schedd} uses this macro to determine whether to start
  a local universe job. During each poll interval, \Condor{schedd}
  evaluates this macro for each idle local universe job that it
  has. For a given job, if the \Macro{START\_LOCAL\_UNIVERSE} 
  macro evaluates to true, then the job's \Macro{Requirements} expression
  ...
  \begin{verbatim}
    MAX_LOCAL_JOBS = 10
    START_LOCAL_UNIVERSE = TotalLocalJobsRunning < $(MAX_LOCAL_JOBS)
  \end{verbatim}
  
  If \Macro{START\_LOCAL\_UNIVERSE} is not defined, the default setting 
  is \Expr{True}.
  
\item[\Macro{START\_SCHEDULER\_UNIVERSE}] \label{param:StartSchedulerUniverse}
  \Condor{schedd} uses this macro to determine whether to start
  a scheduler universe job. During each poll interval, \Condor{schedd}
  evaluates this macro for each idle scheduler universe job that it
  has. For a given job, if the \Macro{START\_SCHEDULER\_UNIVERSE} 
  macro evaluates to true, then the job's \Macro{Requirements} expression
  is evaluated. If both conditions are met, then the job is allowed
  to begin execution. 
  ...
  \begin{verbatim}
    ...
    START_SCHEDULER_UNIVERSE = TotalSchedulerJobsRunning < $(MAX_SCHEDULER_JOBS)
  \end{verbatim}
  
  If \Macro{START\_SCHEDULER\_UNIVERSE} is not defined, the default setting 
  is \Expr{True}.
...
\item[\Macro{MASTER\_<name>\_CONTROLLER}]
  ...
  \Condor{had}.   To configure the \Condor{negotiator}
  to be controlled by \Condor{had}:
