...
\item[\Macro{SHARED\_PORT\_PORT}]
  ...
  If \MacroNI{COLLECTOR\_USES\_SHARED\_PORT} is \Expr{True}, the default value, 
  and the \Condor{master} will launch a \Condor{collector},
  then the \Condor{shared\_port} daemon will ignore this value 
  and use the TCP port assigned to the \Condor{collector} 
  via the \MacroNI{COLLECTOR\_HOST} macro.

  The default value is \MacroUNI{COLLECTOR\_PORT}, which defaults to 9618.  
  The special value of \Expr{0} causes the \Condor{shared\_port} 
...
Normally, HTCondor uses the gethostname() function to query the OS about
the hostname of the local machine. This parameter can be used to override
the result of that query.
Among other things, the hostname is used in the to identify daemons in
an HTCondor pool, via the Machine and Name attributes of daemon ads.
This parameter can be used when a machine has multiple network interfaces
with different hostnames, and you want HTCondor to use a hostname that's
not the primary one.
...
\item[\Macro{DAEMON\_SOCKET\_DIR}] This specifies the directory where
  ...
  so using a path that's longer than 90 characters may cause failures.
  ...
  On Linux, daemons use abstract named sockets instead of normal
  named sockets.
  Abstract sockets are not not tied to a file in the filesystem.
  ...
  named socket in the directory indicated by \Macro{DAEMON\_SOCKET\_DIR}.
  The latter allows command-line tools like \Condor{ssh\_to\_job} to
  use \Condor{shared\_port} as described above.

  The default value of \Macro{DAEMON\_SOCKET\_DIR} is \Expr{auto}.
  This means to use the directory \Expr{\$(LOCK)/daemon\_sock}.
  On unix platorms other than Linux, if that path is too long
  (90 characters maximum), then instead the \Condor{master} will create a
  directory under \Expr{/tmp} with a name that looks like
  \Expr{/tmp/condor\_shared\_port\_XXXXXX} (where the X's are replaced
  with random characters).
  The \Condor{master} then tells the other daemons the exact name of
  the directory it created and they use it.

  If a different value is set for \Macro{DAEMON\_SOCKET\_DIR}, then that
  directory is used, regardless of the length of its pathname.
