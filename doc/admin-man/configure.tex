...
\index{SUBSYS\_DEBUG macro@\texttt{<SUBSYS>\_DEBUG} macro}
  ...
  \item[\Dflag{LEVEL}]
    ...
    that were in effect for each line of output.  This may be used to filter 
    log output by the level or tag it was output under, e.g. identifying all
    logging output at level D\_SECURITY, or D\_ACCOUNTANT, etc.
...
\item[\Macro{NEGOTIATOR\_ALLOW\_QUOTA\_OVERSUBSCRIPTION}]
  \fbox{This definition is not complete.}
  A boolean value that when the default value of \Expr{True} gives
  behavior of resource allocation when considering groups to be
