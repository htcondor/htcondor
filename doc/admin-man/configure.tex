...
\item[\Macro{USE\_SHARED\_PORT}] This is a boolean value that
  ...
  always have its own network port.  All other daemons that are configured
  to use a fixed port (e.g. via \verb|<SUBSYS>\_ARGS = -p <portnum>|) also
  will not use a shared port.  For more information about using
...
Shared ports do not work with SOAP.  To use a shared port for all
 daemons except for the SOAP-enabled daemon, simply set
 \Expr{USE\_SHARED\_PORT=True} and specify a fixed port for each
 SOAP-enabled daemon, using \verb|<SUBSYS>\_ARGS = -p <portnum>|.
...
Write access to this directory grants permission to receive
 ...
 directory must be on a local filesystem that supports named sockets.
