...
\item[\Macro{EMAIL\_SIGNATURE}]
  Every email sent by Condor includes a short signature line appended
  to the body.  By default, this signature includes the URL to the
  global condor project website.  If \MacroNI{EMAIL\_SIGNATURE} is set,
  the value of this parameter is used instead.  Note that the value
  can only be one line in length.  This could be used to direct users
...
\item[\Macro{STARTD\_HISTORY}]
	If this parameter is set to a file name, then the startd will
	maintain a job history file in the same way that the schedd
	does.  It will be rotated in the same way the schedd history
	file is, and the same parameters that apply to schedd history
	rotation apply to the startd history as well.
