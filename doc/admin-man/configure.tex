...
\item[\Macro{DISCARD\_SESSION\_KEYRING\_ON\_STARTUP}]
	...
	it was invoked with a new keyring named ``htcondor''.  
	Various Linux system services, such as OpenAFS and eCryptFS, use
	the kernel session keyring to hold passwords and authentication tokens.
	By replacing the keyring on startup, the \condor{master} ensures
...
\item[\Macro{ECRYPTFS\_ADD\_PASSPHRASE}]
	The path to the ecryptfs-add-passphrase commnand-line utility. If the path
	is not fully-qualified, then safe system path subdirectories (such as /bin,
	/usr/bin) will be searched.  Defaults to ``ecryptfs-add-passphrase''.  Used
	on Linux when setting \SubmitCmd{encrypt\_execute\_directory} is
	\Expr{True}.
