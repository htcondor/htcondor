...
\item[\Macro{NOT\_RESPONDING\_TIMEOUT}] \label{param:NotRespondingTimeout}
  When a Condor daemon's parent process is another Condor daemon, it will
  periodically send a short message to that parent saying that it's alive
  and well. If the parent doesn't hear from the child for a while, it will
  assume the child is hung, kill it, and restart it. This parameter
  ...
  ``alive'' messages at an interval of one third of this value.

\item[\MacroB{<SUBSYS>\_NOT\_RESPONDING\_TIMEOUT}] \label{param:SubsysNotRespondingTimeout}
  Identical to \Macro{NOT\_RESPONDING\_TIMEOUT}, but controls the timeout
  for a specific type of daemon. For example,
  \Macro{SCHEDD\_NOT\_RESPONDING\_TIMEOUT} controls how long the
  \Condor{schedd}'s parent daemon will wait without receiving an ``alive''
