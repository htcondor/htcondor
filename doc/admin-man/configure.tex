...
These are used primarily for Hawkeye, but can have other uses as
well.  The cron mechanism is used to run executables (called
``modules'' directly from the startd.  These output of these modules
is incorporated into the machine ClassAd generated by the startd.

\begin{description}

\item[\Macro{STARTD\_CRON\_NAME}]
\label{param:StartdCronName}
  Defines the logical name of the ``cron'' used by the startd.  While
  not required, it makes the remainder of the ``cron'' definitions
  much more readable and maintainable.  For example:
\begin{verbatim}
        STARTD_CRON_NAME = Hawkeye
\end{verbatim}
  This example allows us to refer to the ``job list'' by HAWKEYE\_JOBS
  rather than the normal STARTD\_CRON\_JOBS.  It also affects the
  acccess to the individual job settings (such as setting the job's
  environment, arguments, etc.

\item[\Macro{STARTD\_CRONJOBS}]
\label{param:StartdCronJobs}
  The list of the startd's ``cron'' jobs.  In Hawkeye, this is usually
  named HAWKEYE\_JOBS.

  This is a tab or newline separated list of jobs to run, of the form:
\begin{verbatim}
...
\end{verbatim}
  ...
  \begin{itemize}
    ...
    \item Name This is the logical ``name'' of the module.  This name
    must be unique (no two modules may have the same name).

    \item Prefix The ``prefix'' specifies a string which is prepended
    to all attributes that this module outputs for it's ClassAd.  For
    example, if a module's prefix is ``xyz\_'', and an individual
    attribute is named ``abc'', the resulting attribute would be
    ``xyz\_abc''.

    \item Executable This is used to specify the full path to the
    ...
    \item Period The period specifies the period at which the module
    should be run.  For non-continuous modules (see below), this
    specifies the period at which the module is started.  The
    modifiers ``s'', ``m'', and ``h'' for seconds, minutes, and hours
    (respectively) are recognized.  Thus 5m means run every five
    minutes.  If no modifier is specified, seconds is assumed.  For
    continuous mode, this specifies how long after the process dies
    before it's restarted.  A value of 0 is valid only in continuous
    mode.

   \item Options Several options are available.  

	continuous is used to specify a module which runs in continous
	mode (as described above).  In continous mode, the startd will
	attempt to keep the module running continuously; this is
	normally used for modules that don't normally exit.  If the
	``period'' is non-zero, the startd waits period seconds after
	the module exits before restarting it.

	nokill If, when it's time to run the module the next time (in
	non-continuous mode), the job is still running, this will
	cause the startd to not kill the job, and to let it continue
	running, instead.  By default, or with a "kill" option, the
	job will be killed and the next run will start when the
	current run dies.
	
  \end{itemize}

Startd cron modules are most frequently implemented as scripts,
altough there is no reason that they need to be scripts.  All scripts
need to output one or more "attribute / value" pairs, one line at a
time.
...
For Hawkeye, modules that produce an array of like values should (by convention)
...
by Hawkeye convention), a FIELDS attribute should be produced:
\begin{verbatim}
  list_1_a = value1a
  list_1_b = "value1b"
  list_1_c = value1c
...
The 'top' and 'df' Hawkeye modules produce these both, and are good
(although complex) examples.  It may be easier to look at their
outputs than the code that actually produces it.
\begin{verbatim}
  list_1_a = value1a
  list_1_b = "value1b"
  list_1_c = value1c
...
if the prefix is ``mymod\_'', the above will translate to the following
...
\item[\Macro{STARTD\_CRON\_module\_ARGS}]
\label{param:StartdCronModuleArgs}
  The command line arguments to pass to module ``module''.  In
  Hawkeye, this is usually named HAWKEYE\_module\_ARGS.

\item[\Macro{STARTD\_CRON\_module\_ENV}]
\label{param:StartdCronModuleEnv}
  The environment string to pass to module ``module''.  In Hawkeye,
  this is usually named HAWKEYE\_module\_ENV.  The syntax is the same
  as that of ~\ref{param:DaemonNameEnvironment} as
  \Macro{DAEMONNAME\_ENVIRONMENT}

\item[\Macro{STARTD\_CRON\_module\_CWD}]
\label{param:StartdCronModuleCwd}
  The working directory in which to start module ``module''.  In
  Hawkeye, this is usually named HAWKEYE\_module\_CWD.
