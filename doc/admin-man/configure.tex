...
\item[\Macro{TRANSFER\_QUEUE\_USER\_EXPR}]
  ...
  for round-robin scheduling in the file transfer queue.  Round-robin
  scheduling is used when there are multiple jobs waiting to transfer
  files within the limits set by \Macro{MAX\_CONCURRENT\_UPLOADS}
  and/or \Macro{MAX\_CONCURRENT\_DOWNLOADS}.  When choosing a new job
  to allow to transfer, the first job belonging to the transfer queue
  user who has least recently been given an opportunity to transfer
  will be selected.  By default, a transfer queue user is identified
  as the job owner.  A different user name may be specified by
  configuring \MacroNI{TRANSFER\_QUEUE\_USER\_EXPR} to a string
  expression that is evaluated in the context of the job ad.  For
  example, if this expression were set to a name that is the same for
  all jobs, file transfers would be scheduled in first-in-first-out
  order rather than round-robin by job owner.
...
\label{param:MemoryLimit} 
\item[\Macro{MEMORY\_LIMIT}]
...
\item[\Macro{DAGMAN\_MAX\_JOBS\_IDLE}]
  ...
  no limit on the maximum number of idle jobs. Note that nothing special is 
  done to the submit file: if a submit file has a ``queue 5000'' statement
  and \Macro{DAGMAN\_MAX\_JOBS\_IDLE} is set to 250, this submit file will
  result in 5000 jobs being submitted to the schedd, not 250; in this case, no
