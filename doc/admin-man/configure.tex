...
Note that by default, the configuration variable \MacroNI{DC\_DAEMON\_LIST}
is not defined in the configuration file.
If not defined, then define it by using the value
included as a comment in the default configuration,
together with the above redefinition.
Alternatively, modify the original definition by adding
the additional daemon's local name
(\Attr{XYZZY} in this example) to the list.
...
\item[\Macro{MASTER\_SHUTDOWN\_$<$Name$>$}]
  ...
  ``exe cl()'' call, or the similar Win32 ``\_exe cl()'' call) instead of
