...
\item[\Macro{RESERVED\_SWAP}] \label{param:ReservedSwap} Determines
  ...
  of \Macro{RESERVED\_SWAP} is 5 magabytes.
...
% Default value check end here: NRL 11 Dec 2003
...
\item[\Macro{HA\_name\_LOCK\_URL}]
  \label{param:HAnameLockURL} This macro controls the ``High
  Availability'' lock URL for a specific daemon ``name'' and
  overrides the system wide lock URL specified by
  \Macro{HA\_LOCK\_URL}.  If not defined for each daemon,
  \Macro{HA\_name\_LOCK\_URL} is ignored, and the value of
...
\item[\Macro{HA\_name\_LOCK\_HOLD\_TIME}]
  \label{param:HAnameLockHoldTime} This macro controls the ``High
  Availability'' lock hold time for a specific daemon ``name'' and
  overrides the system wide poll period specified by
  \Macro{HA\_LOCK\_HOLD\_TIME}.  If not defined for each daemon,
  \Macro{HA\_name\_LOCK\_HOLD\_TIME} is ignored, and the value of
...
\item[\Macro{HA\_POLL\_PERIOD}] \label{param:HALockPollPeriod} This
  ...
  released by the owning \Condor{master}.  If the \Condor{master} owns
  any locks, it will renew the lock during these polls, as well.
  \Macro{HA\_POLL\_PERIOD} defaults to 300 seconds (five minutes).

\item[\Macro{HA\_name\_POLL\_PERIOD}]
  \label{param:HALockPollNamePeriod} This macro controls the ``High
  Availability'' pool period for a specific daemon ``name'' and
  overrides the system wide poll period specified by
  \Macro{HA\_POLL\_PERIOD}.  If not defined for each daemon,
  \Macro{HA\_name\_POLL\_PERIOD} is ignored, and the value of
