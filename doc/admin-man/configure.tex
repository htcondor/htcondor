...
\item[\Macro{LOCK\_DEBUG\_LOG\_TO\_APPEND}]
  This setting controls whether a daemon's debug lock is used when
  appending to the log.  When \Expr{false} (the default), the debug
  lock is only used when rotating the log file.  This is more
  efficient, especially when many processes share the same log file.
  When \Expr{true}, the debug lock is used when writing to the log as
  well as when rotating the log file.  This setting is ignored under
  Windows, and the behavior is as though this setting were true.
  Under unix, the default value of \Expr{false} is appropriate when
  logging to filesystems that support the POSIX semantics of
  \Expr{O\_APPEND}.  On non-POSIX-compliant filesystems, it is
  ...
  filesystems will still avoid interleaved messages without requiring
  Condor to use a lock.  Our tests of AFS and NFS, for example, have
...
\item[\Macro{ENABLE\_ADDRESS\_REWRITING}]
  ...
  \MacroNI{NETWORK\_INTERFACE} is defined or
