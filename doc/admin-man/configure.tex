...
\item[\Macro{SCHEDD\_ASSUME\_NEGOTIATOR\_GONE}]
  ...
  specified, is 20 x \MacroUNI{NEGOTIATOR\_INTERVAL}.  If
...
\item[\Macro{SKIP\_WINDOWS\_LOGON\_NETWORK}]
  A boolean value that defaults to \Expr{False}
  When \Expr{True}, Windows authentication is longer
  made with method \MacroNI{LOGON\_NETWORK} first, but
  only with method \MacroNI{LOGON\_INTERACTIVE}. This
  can be useful if many authentication failures are 
  noticed, potentially leading to users getting locked
  out.
...
\item[\Macro{<Keyword>\_HOOK\_JOB\_EXIT\_TIMEOUT}]
  For the fetch work hooks,
  the number of seconds the \Condor{starter} will wait for the JOB\_EXIT
  hook to exit before continuing with job cleanup.  Defaults to 30 seconds.
  \MacroNI{<Keyword>} is is the hook keyword defined to distinguish
  between sets of hooks.
