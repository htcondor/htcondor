...
\item[\Macro{CKPT\_SERVER\_CHECK\_PARENT\_INTERVAL}]
  ...
  of the checkpoint server (i.e. the \Condor{master}) has died.  If the
  parent has died, the checkpoint server shuts itself down.  The default
  is 120 seconds.  A setting of 0 disables this check.
...
\item[\Macro{CKPT\_SERVER\_INTERVAL}]
	This is the maximum number of seconds the checkpoint server
	waits for activity on network sockets before performing other
	tasks. The default is 300 seconds.

\label{param:CkptServerClassadFile}
\item[\Macro{CKPT\_SERVER\_CLASSAD\_FILE}]
	This is a string that represents a file in the file system to which
	...
	which means do not keep a checkpoint server classad file.

\label{param:CkptServerCleanInterval}
\item[\Macro{CKPT\_SERVER\_CLEAN\_INTERVAL}]
	This is the number of seconds that must pass until the classad log file
	as described by the \Macro{CKPT\_SERVER\_CLASSAD\_FILE} variable gets
	truncated. The default is 86400 seconds--one day.

\label{param:CkptServerRemoveStaleCkptInterval}
\item[\Macro{CKPT\_SERVER\_REMOVE\_STALE\_CKPT\_INTERVAL}]
	This is the number of seconds between attempts to discover and remove
	stale checkpoint files. It defaults to 86400 seconds--one day.

\label{param:CkptServerSocketBufsize}
\item[\Macro{CKPT\_SERVER\_SOCKET\_BUFSIZE}]
	This is the number of bytes that represents the size of the TCP
	send/recv buffer on the socket file descriptor related to moving
	the checkpoint file to and from the checkpoint server. The default
	is 0, meaning to let the operating system decide the size.

\label{param:CkptServerMaxProcesses}
\item[\Macro{CKPT\_SERVER\_MAX\_PROCESSES}]
	This is the maximum number of child processes that could be working on
	behalf of the checkpoint server. This includes store processes and
	restore processes. The default is 50.

\label{param:CkptServerMaxStoreProcesses}
\item[\Macro{CKPT\_SERVER\_MAX\_STORE\_PROCESSES}]
	This is the maximum number of child process strictly devoted
	to the storage of checkpoints. The default is the value of
	\Macro{CKPT\_SERVER\_MAX\_PROCESSES}.

\label{param:CkptServerMaxRestoreProcesses}
\item[\Macro{CKPT\_SERVER\_MAX\_RESTORE\_PROCESSES}]
	This is the maximum number of child process strictly devoted
	to the restoring of checkpoints. The default is the value of
	\Macro{CKPT\_SERVER\_MAX\_PROCESSES}.

\label{param:CkptServerStaleCkptAgeCutoff}
\item[\Macro{CKPT\_SERVER\_STALE\_CKPT\_AGE\_CUTOFF}]
	This is the number of seconds that states if a checkpoint file hasn't
	been accessed in N number of seconds, it is considered stale. The
	default value is 5184000 seconds--sixty days.
