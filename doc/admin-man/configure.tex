...
\item[\Macro{GANGLIA\_VERBOSITY}] An integer that specifies the
...
\label{param:GangliaRequirements}
\item[\Macro{GANGLIA\_REQUIREMENTS}] An optional ClassAd expression
...
\label{param:GangliaPerExecuteNodeMetrics}
\item[\Macro{GANGLIA\_PER\_EXECUTE\_NODE\_METRICS}] A boolean value
...
\end{verbatim}
  ...
  additional examples: \File{\$(GANGLIA\_METRICS\_CONFIG\_DIR)/00\_default\_metrics}.
  ...
  \item[Machine] The name of the host associated with this metric.
    ...
    default machine name is set to the daemon name in this case.  For
    aggregate metrics, the default value of \Attr{Machine} will be the
    name of the collector host.
