...
\item[\Macro{ENABLE\_KERNEL\_TUNING}]
  A boolean value that defaults to \Expr{True}.  When \Expr{True},
  but only on Linux,
  the \Condor{Master} daemon will run the kernel tuning script once
  (as root) when it starts up.

\item[\Macro{KERNEL\_TUNING\_LOG}]
  A string value that defaults to \Expr{\$(LOG)/KernelTuningLog}.  If
  the kernel tuning script runs, its output will be logged to this file.

\item[\Macro{LINUX\_KERNEL\_TUNING\_SCRIPT}]
  A string value that defaults to \Expr{\$(LIBEXEC)/linux\_kernel\_tuning}.
  This is the script that the \Condor{master} will run to tune the
  kernel.
...
\item[\Macro{JOB\_IS\_FINISHED\_COUNT}]
  An integer value representing the number of jobs the the \Condor{schedd}
