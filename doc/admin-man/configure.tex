...
\item[\Macro{MAX\_JOBS\_PER\_OWNER}]
  ...
  permitted to have in a \Condor{schedd} daemon's queue.  The whole
  submission fails if the total number of jobs with the same owner
  would exceed this limit.
  The default value for this variable is the largest positive
  integer value.

  This configuration variable may be useful if \MacroNI{MAX\_JOBS\_SUBMITTED}
  is set, to ensure that no user can dominate the queue.
...
\item[\Macro{MAX\_JOBS\_PER\_Submission}]
  ...
  whole submission fails if the number of jobs in it would exceed this limit.
  The default value for this variable is the largest positive
  integer value.

  This configuration setting may be useful for catching user error,
