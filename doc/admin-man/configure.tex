%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Configuration Macros}\label{sec:Configuring-HTCondor-Macros}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\index{HTCondor!configuration-macros}
\index{configuration: macros}

The section contains a list of the individual configuration macros
...
\item[\Macro{HISTORY\_HELPER\_MAX\_CONCURRENCY}]
  ...
  queries allowed at a time; defaults to 50.  When this maximum is exceeded,
...
\item[\Macro{ENABLE\_IPV4}]
  ...
  HTCondor will not use IPv4.  If \Expr{auto}, which is the default,
  HTCondor will use IPv4 if it can find an interface with an IPv4 address,
  and that address is (a) public or private, or (b) no interface's
  IPv6 address is public or private.  If HTCondor finds more than one address
  of each protocol, only the most public address is considered for that
  protocol.
...
\item[\Macro{ENABLE\_IPV6}]
  ...
  HTCondor will not use IPv6.  If \Expr{auto}, which is the default,
  HTCondor will use IPv6 if it can find an interface with an IPv6 address,
  and that address is (a) public or private, or (b) no interface's
  IPv4 address is public or private.  If HTCondor finds more than one address
  of each protocol, only the most public address is considered for that
  protocol.
...
\index{SUBSYS\_LOG macro@\texttt{<SUBSYS>\_LOG} macro}
  ...
  The special value \Expr{SYSLOG} causes the daemon to log via the syslog
  facility on Linux.
...
\label{param:LogToSyslog}
\item[\Macro{LOG\_TO\_SYSLOG}]
  A boolean value that is \Expr{False} by default.
  When \Expr{True}, all daemon logs are routed to the syslog facility on
  Linux.
...
\index{SUBSYS\_DEBUG macro@\texttt{<SUBSYS>\_DEBUG} macro}
  ...
  \item[\Dflag{SECURITY}]
    ...
    section~\ref{sec:Security-Model} for more information about
...
\label{param:Allow}
\item[\Macro{ALLOW\Dots}]
  All macros that begin with either \Macro{ALLOW} or
  \Macro{DENY} are settings for HTCondor's security.
  See section~\ref{sec:Security-Authorization} on Setting up
  security in HTCondor for details on these
...
\item[\Macro{SETTABLE\_ATTRS\_<PERMISSION-LEVEL>}]
  ...
  command.
  Section~\ref{sec:Security-Authorization} on Setting up
  Security in HTCondor for details on these
  macros and how to configure them.  
  In particular, section~\ref{sec:Security-Authorization}
  on page~\pageref{sec:Security-Authorization} contains details specific to
  these macros.
...
\item[\Macro{CORE\_FILE\_NAME}]
  ...
  Defaults to \Expr{core.\$(SUBSYSTEM).WIN32}.
...
overriding the value returned by \Procedure{gethostname}.
...
It should be set to a fully-qualified host name that will resolve to
an IP address of the local machine.
...
\label{param:TrustLocalUidDomain}
\item[\Macro{TRUST\_LOCAL\_UID\_DOMAIN}]
  This parameter works like \MacroNI{TRUST\_UID\_DOMAIN}, but is only
  applied when the \Condor{starter} and \Condor{shadow} are on the same
  machine.
  If this paramater is set to \Expr{True}, then the \Condor{shadow}'s
  \MacroNI{UID\_DOMAIN} doesn't have to be a substring its hostname.
  If this paramater is set to \Expr{False}, then \MacroNI{UID\_DOMAIN}
...
\item[\Macro{IS\_OWNER}]
  ...
  the \Expr{Owner} state.
  While in the \Expr{Owner} state, the machine ad will not be matched to
  any jobs.
  The default value is \Expr{False} (never enter \Expr{Owner} state).
  Job ClassAd attributes should not be used in defining \MacroNI{IS\_OWNER},
...
\item[\Macro{MOUNT\_UNDER\_SCRATCH}]
  ...
  to a string that contains a comma separated list of directories. 
...
\end{verbatim}
  ...
  The default value is \Expr{''/tmp,/var/tmp''}.
  ...
  (See~\ref{PreCmd} for information on PreCmd.)

  Also note that, if \MacroNI{MOUNT\_UNDER\_SCRATCH} is defined, it
  must either be a ClassAd string (with double-quotes) or an expression
...
\item[\Macro{DOCKER}]
  ...
  See the Docker universe manual section for more details 
	(~\ref{sec:docker-install})
  An example of the configuration for running the Docker CLI:
\begin{verbatim}
  DOCKER = /usr/bin/docker
\end{verbatim}

\label{param:DockerVolumes}
\item[\Macro{DOCKER\_VOLUMES}]
  A list of directories on the host execute machine to be volume mounted
  within the container.  See the Docker Universe section for full details.
 	(~\ref{sec:docker-install})
...
\label{param:DockerDropAllCapabilities}
\item[\Macro{DOCKER\_DROP\_ALL\_CAPABILITIES}]
  A class ad expression, which defaults to true.  Evaluated in the context
  of the job ad and the machine ad, when true, runs the docker container
  with the command line option --drop-all-capabilities.  Admins should be
  very careful with this setting, and only allow trusted users to run with
  full linux capabilities within the container.

\label{param:DockerExtraArguments}
\item[\Macro{DOCKER\_EXTRA\_ARGUMENTS}]
...
\item[\Macro{START\_SCHEDULER\_UNIVERSE}]
  A boolean value that defaults to \Expr{TotalSchedulerJobsRunning < 500}.
  The \Condor{schedd} uses this macro to determine whether to start
...
\item[\Macro{CURB\_MATCHMAKING}]
  ...
  \Condor{negotiator}.  Defaults to \Expr{RecentDaemonCoreDutyCycle > 0.98}.
...
\item[\Macro{MAX\_CONCURRENT\_DOWNLOADS}]
  ...
  submitted from the same \Condor{schedd}.  The default is 100.  A
...
\item[\Macro{FLOCK\_COLLECTOR\_HOSTS}]
  ...
  requests.  \MacroUNI{ALLOW\_NEGOTIATOR\_SCHEDD} (see
  section~\ref{param:Allow}) must also be configured to allow
...
\item[\Macro{SUBMIT\_REQUIREMENT\_<Name>}]
  ...
  which is the \Expr{SCHEDD.} or
  \Expr{MY.} name space and the job ClassAd, 
  which is the \Expr{JOB.} or
  \Expr{TARGET.} name space.
...
\label{param:AlternateJobSpool}
\item[\Macro{ALTERNATE\_JOB\_SPOOL}]
  A ClassAd expression evaluated in the context of the job ad.
  If the result is a string, the value is used an an alternate spool
  directory under which the job's files will be stored.
  This alternate directory must already exist and have the same
  file ownership and permissions as the main \MacroNI{SPOOL} directory.
  Care must be taken that the value won't change during the lifetime
  of each job.
...
\item[\Macro{CHIRP\_DELAYED\_UPDATE\_PREFIX}]
  This is a string-valued and case-insensitive parameter with the default
  value of \AdStr{Chirp*}.  The
  string is a list separated by spaces and/or commas.  Each attribute passed
  to the either of the \Condor{chirp} commands \Opt{set\_job\_attr\_delayed} or
  \Opt{get\_job\_attr\_delayed} must match against at least one element in
  the list.  An attribute which does not match any list element fails.  A
  list element may contain a wildcard character (\AdStr{Chirp*}), which marks
  where any number of characters matches.  Thus, the default is to allow
  reads from and writes to only attributes which start with \AdStr{Chirp}.

  Because this parameter must be set to the
...
\label{param:SingularityExtraArguments}
\item[\Macro{SINGULARITY\_EXTRA\_ARGUMENTS}]
...
\label{param:JobDefaultLeaseDuration}
\item[\Macro{JOB\_DEFAULT\_LEASE\_DURATION}]
  The default value for the \SubmitCmd{job\_lease\_duration} submit command
  when the submit file does not specify a value. The default
  value is 2400, which is 40 minutes.
...
\item[\Macro{SUBMIT\_SKIP\_FILECHECKS}]
  ...
  For standard universe, the setting is ignored and file checks are
  always performed.
  The default value is \Expr{True}.
...
\label{param:SubmitDefaultShouldTransferFiles}
\item[\Macro{SUBMIT\_DEFAULT\_SHOULD\_TRANSFER\_FILES}]
  Provides a default value for the submit command
  \SubmitCmd{should\_transfer\_files} if the submit file does
  not supply a value and when the value is not forced by some
  other command in the submit file, such as the universe.
  Valid values are YES, TRUE, ALWAYS, NO, FALSE, NEVER and IF\_NEEDED.
  If the value is not one of these, then IF\_NEEDED will be used.
...
\label{param:CredMinTimeLeft}
\item[\Macro{CRED\_MIN\_TIME\_LEFT}]
  When a job uses an X509 user proxy, condor\_submit will refuse to
submit a job whose x509 expiration time is less than this many seconds
in the future.  The default is to only refuse jobs whose expiration
...
\label{param:NegotiatorName}
\item[\Macro{NEGOTIATOR\_NAME}]
  Used to give an alternative value to the \Attr{Name} attribute
  in the \Condor{negotiator}'s ClassAd and the \Attr{NegotiatorName}
  attribute of its accounting ClassAds.
  This configuration macro is useful in the situation
  where there are two \Condor{negotiator} daemons running on one machine,
  and both report to the same \Condor{collector}.
  Different names will distinguish the two daemons.
...
\label{param:NegotiatorJobConstraint}
\item[\Macro{NEGOTIATOR\_JOB\_CONSTRAINT}]
  An expression which constrains which job ClassAds are considered for
  matchmaking by the \Condor{negotiator}.
  This parameter is read by the \Condor{negotiator} and sent to the
  \Condor{schedd} for evaluation.
  \Condor{schedd}s older than version 8.7.7 will ignore this expression
  and so will continue to send all jobs to the \Condor{negotiator}.
...
\label{param:NegotiatorDepthFirst}
\item[\Macro{NEGOTIATOR\_DEPTH\_FIRST}]
  A boolean value which defaults to false.  When partitionable slots
are enabled, and this parameter is true, the negotiator tries to pack
as many jobs as possible on each machine before moving on to the next
machine.
...
\item[\Macro{NEGOTIATOR\_RESOURCE\_REQUEST\_LIST\_SIZE}]
  ...
  The default value is 200.  If \MacroNI{USE\_RESOURCE\_REQUEST\_COUNTS}
...
\item[\Macro{CREDD\_POLLING\_TIMEOUT}]
  An integer value representing the number of seconds that the \Condor{credd},
  \Condor{starter}, and \Condor{schedd} daemons will
  wait for valid credentials to be produced by a credential montior (CREDMON) service. 
  The default value is 20.
...
\item[\Macro{CRED\_SUPER\_USERS}]
  ...
  \Condor{store\_cred} command.
  When not on this list, users can only store their own credentials.
  Entries in this list can contain a single '*' wildcard character,
...
\label{param:GahpSSLCADir}
\item[\Macro{GAHP\_SSL\_CADIR}]
  The path to a directory that may contain the
  certificates (each in its own file) for multiple trusted CAs
  to be used by GAHP servers when authenticating with remote services.

\label{param:GahpSSLCAFile}
\item[\Macro{GAHP\_SSL\_CAFILE}]
...
\label{param:AzureGAHP}
\item[\Macro{AZURE\_GAHP}]
  The complete path and file name of the Azure GAHP executable.
  The default value is \File{\MacroUNI{SBIN}/AzureGAHPServer.py} on
  Windows and \File{\MacroUNI{SBIN}/AzureGAHPServer} on other platforms.
...
\label{param:SecUseFamilySession}
\item[\Macro{SEC\_USE\_FAMILY\_SESSION}]
  The ``family'' session is a special security session that's shared
  between an HTCondor daemon and all of its descendant daemons.
  It allows a family of daemons to communicate securely without
  an expensive authentication negotiation on each network connection.
  It bypasses the security authorization settings.
  The default value is \Expr{True}.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_SLOTS} 
  ...
  Only the slots specified in this comma-separated list may incorporate
  the output of the job specified by \Expr{<JobName>}.  If the list is
  not specified, any slot may.  Whether or not a specific slot actually
  incorporates the output depends on the output;
  see \ref{sec:daemon-classad-hooks}.
...
\label{param:StartdCronJobMetrics}
\item[\Macro{STARTD\_CRON\_<JobName>\_METRICS}]
  A space or comma -separated list.  Each element in the list is a metric
  type, either \Expr{SUM} or \Expr{PEAK}; a colon; and a metric name.

  An attribute preceded by \Expr{SUM} is a metric which accumulates over
  time.  The canonical example is seconds of CPU usage.

  An attribute preceded by \Expr{PEAK} is a metric which instead records
  the largest value reported over the period of use.  The canonical example
  is megabytes of memory usage.

  A job with \MacroNI{STARTD\_CRON\_<JobName>\_METRICS} set is a custom
  machine resource monitor (CMRM),  and its output is handled differently
  than a normal job's.  A CMRM should output one ad per custom machine
  resource instance and use \Attr{SlotMergeConstraint}s
  (see \ref{sec:daemon-classad-hooks}) to specify the instance to which
  it applies.

  The ad corresponding to each custom machine resource instance should have
  an attribute for each metric named in the configuration.  For SUM metrics,
  the attribute should be \Attr{Uptime<MetricName>Seconds}; for PEAK metrics,
  the attribute should be \Attr{Uptime<MetricName>PeakUsage}.

  Each value should be the value of the metric since the last time the
  job reported.  The reported value may therefore go up or down; HTCondor
  will record either the the sum or the peak value, as appropriate, for
  % both the duration of the job running in a slot assigned resources of the
         the duration of the job running in a slot assigned resources of the
    corresponding type.
  % corresponding type and the recent past, as defined by FIXME.

  For example, if your custom resources are SQUIDs, and you detected four
  of them, your monitor might output the following:
...
\end{verbatim}
  ...
  The names ('SQUIDsReport0') may be anything, but must be consistent from
  report to report and the ClassAd for each report must have a distinct
  name.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{sec:Config-Windows}Configuration File Entries Only for Windows Platforms}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\label{param:DefragDrainingStartExpr}
\item[\Macro{DEFRAG\_DRAINING\_START\_EXPR}] A ClassAd expression that
  replaces the machine's \Macro{START} expression while it's draining.
  Slots which accepted a job after the machine begain draining set the
  machine ad attribute \Attr{AcceptedWhileDraining} to \Expr{true}.
  When the last job which was not accepted while draining exits, all
  other jobs are immediately evicted with a \Attr{MaxJobRetirementTime}
  of 0; job vacate times are still respected.  While the jobs which were
  accepted while draining are vacating, the \MacroNI{START} expression is
  \Expr{false}.  Using \Expr{\$(START)} in this expression is usually
  a mistake: it will be replaced by the defrag daemon's \MacroNI{START}
  expression, \emph{not} the value of the target machine's \MacroNI{START}
  expression (and especially not the value of its \MacroNI{START} expression
  at the time draining begins).
...
\label{param:GangliadDefaultCluster}
...
\label{param:GangliadDefaultMachine}
