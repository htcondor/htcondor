...
\item[\Macro{STRICT\_CLASSAD\_EVALUATION}]
  A boolean value that controls how ClassAd expressions are evaluated. If
  set to True, then normal new ClassAd evaluation semantics are used.
  This means that attribute references without a \texttt{MY.} or
  \texttt{TARGET.} prefix are only looked up in the local ad.
  If set to False, old ClassAd evaluation semantics are used. The default
  is False. See section~\ref{sec:classad-differences}  on
  page~\pageref{sec:classad-differences} for details.
