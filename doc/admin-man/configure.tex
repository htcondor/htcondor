...
\item[\Macro{LOG}] \label{param:Log} Used to specify the
  directory where each Condor daemon writes its log files, and the
  operating system writes core files.
  The names
...
\item[\Macro{CREATE\_CORE\_FILES}] \label{param:CreateCoreFiles}
  Under Unix operating systems, this parameter
  defines whether or not Condor daemons are to
  create a core file if something really bad happens.  It is
  used to set
  the resource limit for the size of a core file.
  If this parameter is TRUE, the limit is increased to
  the maximum.  If it is set to FALSE, the limit is set at 0
  (which means that no core files are created).  Unix
  core files have the name syntax \texttt{core.}\textit{nnnnn}, where
  \textit{nnnnn} indicates the process id of the originating Condor daemon.

  Under Windows operating systems, the resulting "core" file is actually a
  debugging diagnostic file, composed primarily of a stack backtrace.  Windows
  "core" files have the name syntax
  \texttt{core.}\textit{SUBSYS}.\texttt{WIN32},
  where
  \textit{SUBSYS} indicates the originating Condor daemon, such as
  \texttt{SCHEDD}, \texttt{STARTD}, etc.
  ...
  to whatever behavior you want Condor to enforce.
  For all operating systems,
  the default value for this parameter is TRUE, and core files are produced in
  the directory defined by the
  \Macro{LOG}, macro, 
  section~\ref{sec:Condor-wide-Config-File-Entries}.
  Core files can be quite large.  Set this parameter to FALSE if your disk
  resources are tight, and you do not need the resulting debugging information.
% PKK
% Described in default config file: YES
% Defined in the default config file: NO
% Default definition in config file: bogus
