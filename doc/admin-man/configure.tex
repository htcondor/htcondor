...
\item[\Macro{MASTER\_SHUTDOWN\_$<$Name$>$}]
  ...
  The adminstrator must ensure that this cannot
...
The following macros describe the \Prog{Daemon ClassAd Hook}
capabilities of Condor.  The \Prog{Daemon ClassAd Hook} mechanism is
used to run executables (called jobs) directly from the
\Condor{startd} and \Condor{schedd} daemon.
The output from modules is incorporated into the machine ClassAd
generated by the respective daemon.  These capabilities are used in
Hawkeye, but can be used in other situations as well.

These configuration macros are divided into two sets.
The two sets occurred as the functionality and usage of
Condor's \Prog{Daemon ClassAd Hook} capabilities evolved.
...
\item[\Macro{STARTD\_CRON\_JOBLIST}]
  ...
  list of job names to run.  Each of these is the logical name of the
  job.  This name must be unique (no two jobs may have the same name).
...
\label{param:BenchmarkJobList}
\item[\Macro{BENCHMARK\_JOBLIST}]
  Similar to \Macro{STARTD\_CRON\_JOBLIST}, this configuration
  variable is defined by a white space separated
  list of benchmark job names to run.
  Each of these is the logical name of the job.  This name
  must be unique (no two benchmark jobs may have the same name).
...
\item[\Macro{STARTD\_CRON\_<JobName>\_PREFIX}]
  ...
  \MacroNI{STARTD\_CRON\_<JobName>\_PREFIX} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_PREFIX}.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_EXECUTABLE}]
  ...
  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration macro name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_EXECUTABLE} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_EXECUTABLE}.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_PERIOD}]
  ...
  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration macro name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_PERIOD} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_PERIOD}.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_MODE}]
  ...
  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration macro name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_MODE} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_MODE}.
  ...
  specified, the job will be run again after a reconfigure.

  The \Expr{OnDemand} mode is used only by the benchmark mechanism.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_RECONFIG}]
  ...
  \MacroNI{STARTD\_CRON\_<JobName>\_RECONFIG} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_RECONFIG}.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_RECONFIG\_RERUN}]
  ...
  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration variable name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_RECONFIG\_RERUN} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_RECONFIG\_RERUN}.

\label{param:StartdCronJobJobLoad}
\item[\Macro{STARTD\_CRON\_<JobName>\_JOB\_LOAD}]
  A floating point value that specifies the ``load'' that a specified
  load induces on the system.  This job load can be used to limit the
  jobs that run concurrently.

  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration variable name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_JOB\_LOAD} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_JOB\_LOAD}.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_KILL}]
  ...
  \Expr{STARTD\_CRON\_<JobName>\_MODE = Periodic}.
  The default value is \Expr{False}.

  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration macro name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_KILL} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_KILL}.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_ARGS}]
  ...
  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration macro name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_ARGS} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_ARGS}.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_ENV}]
  ...
  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration macro name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_ENV} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_ENV}.
...
\item[\Macro{STARTD\_CRON\_<JobName>\_CWD}]
  ...
  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration macro name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_CWD} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_CWD}.

\label{param:StartdCronJobOptions}
\item[\Macro{STARTD\_CRON\_<JobName>\_OPTIONS}]
  ...
  If \MacroNI{STARTD\_CRON\_NAME}
  is defined, then this configuration macro name is changed from
  \MacroNI{STARTD\_CRON\_<JobName>\_OPTIONS} to
  \MacroNI{\MacroUNI{STARTD\_CRON\_NAME}\_<JobName>\_OPTIONS}.
