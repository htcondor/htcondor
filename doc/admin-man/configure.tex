...
\item[\Macro{COLLECTOR\_PORT}]
  The default port used to contact the \Condor{collector}.  This parameter is
  referenced if no port is given and there is no other means to find the
  \Condor{collector} port.

  This also controls the default port the \Condor{collector} listens on if
  no port is specified.

\label{param:SharedPortPort}
\item[\Macro{SHARED\_PORT\_PORT}]
  The default TCP port used by the \Condor{shared\_port}.  If \Macro{COLLECTOR\_USES\_SHARED\_PORT}
  is \Expr{True} (the default value) and the \Condor{master} will launch a \Condor{collector},
  then the \Condor{shared\_port} daemon will ignore this value and use the TCP port
  assigned to the collector via the \Macro{COLLECTOR\_HOST} macro.

  The default value is \Expr{\$(COLLECTOR\_PORT)}, currently 9618.  The special value
  of \Expr{0} causes the \Condor{shared\_port} to use a randomly selected port.  \emph{Note}
  that this causes all HTCondor hosts to use TCP port 9618 by default, differing from previous
  behavior (previously, only the collector host used a fixed port).  To restore the prior
  behavior, set \Macro{SHARED\_PORT\_PORT} to \Expr{0}.
...
\item[\Macro{USE\_SHARED\_PORT}] A boolean value that
  ...
  effect.  The default is \Expr{True}.
...
\item[\Macro{DAGMAN\_DEFAULT\_NODE\_LOG}]
  ...
  Note that the special variable names listed below are not HTCondor
  configuration macros.
  Therefore, this configuration macro uses a special syntax 
  in which \Bold{\Expr{@}} instead of \Bold{\Expr{\$}} indicates
  a evaluation of these special variables.  Note that normal
  HTCondor configuration macros can also be used with the normal
  \Bold{\Expr{\$}} syntax (see the examples below).

  Special variables to be used only in this configuration macro:
  \begin{itemize}
  ...
  \item \Expr{@(OWNER)}: The username of the user who submitted the
  DAG.
  \item \Expr{@(NODE\_NAME)}: For sub-DAGs, the node name of the sub-DAG
  in the upper level DAG; for top-level DAGs, "undef".
  \end{itemize}
...
\item[\Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG}]
  \Bold{As of version 8.3.1, the value of this macro cannot be changed
  from the default (\Expr{True}).}  (The non-workflow/default node log
  feature has been disabled to
