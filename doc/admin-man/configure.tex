...
\item[\Macro{TCP\_FORWARDING\_HOST}]
  ...
  public address of this daemon.  This is useful if Condor on this
  host may be reached through a NAT or firewall by connecting to a
  different IP address that forwards connections to this host.  It
  ...
  from this host rather than from the real remote host, so any
...
the collector must be set in the coniguration file, it is necessary to
...
When the collector address is a shared port, TCP updates will be
 ...
 descriptors.  Typical unix systems have a default of only 1024 file
 descriptors per process.  Each daemon that publishes itself to the
 collector requires a file descriptor.  When using CCB, each registered
 CCB connection also requires a file descriptor.  When the condor is
 started as root, the number of file descriptors can be adjusted as in
 the following configuration:

\begin{verbatim}
COLLECTOR.MAX_FILE_DESCRIPTORS = 16000
\end{verbatim}

SOAP commands connot be sent over a shared port.  However, a daemon
