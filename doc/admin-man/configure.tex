...
\label{param:ShadowLazyQueueUpdate}
  ...
  time, it only effects the \AdAttr{NumJobStarts} counter) or if it
  should wait and only update the job queue on the next periodic
  update.
...
\item[\Macro{DEFAULT\_RANK}] \label{param:DefaultRank}
  ...
  \end{verbatim}
  which states that the job prefers machines where there is
  more main memory than the image size of the job.
...
representing the interval between job status checks to see
...
to the \Condor{vm-gahp} to be completed.
...
representing the amount of time Condor waits after a
virtual machine error, and before rechecking.
If the recheck fails, Condor disables the vm universe.
...
\item[\Macro{XEN\_VT\_VMC}]
\label{param:XenVTVMC}
...
These macros affect the configuration of Condor where Condor is
