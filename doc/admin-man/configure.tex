...
\item[\Macro{RESERVED\_SWAP}]
  ...
  of \MacroNI{RESERVED\_SWAP} is 0, which disables this check.
...
\item[\Macro{MAX\_JOBS\_RUNNING}]
  ...
  However, limiting the number of running scheduler and local universe
  ...
  This default value is an expression that depends on the total amount
...
\item[\Macro{SHADOW\_SIZE\_ESTIMATE}]
  This macro sets the estimated private virtual memory size of each
  \Condor{shadow} process.  It is specified in kilobytes.
...
\item[\Macro{NEGOTIATOR\_INTERVAL}]
  ...
  in seconds and defaults to 60 (1 minute).
