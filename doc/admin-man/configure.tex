...
\item[\Macro{MACHINE\_RESOURCE\_<name>}]
  ...
  instances will not be identifed.  If a list of identifiers is specified
...
\label{param:OfflineMachineResourceResourcename}
\item[\Macro{OFFLINE\_MACHINE\_RESOURCE\_<name>}]
  A list of resource identifiers for the customized local machine resource
  that are currently offline and should not be allocated to a slot.
  The identifiers specified here must match those specified in the corresponding
  \Macro{MACHINE\_RESOURCE\_<name>} or \Macro{MACHINE\_RESOURCE\_INVENTORY\_<name>}
  or they will be ignored.
  Use this to have resources that are detected and reported to exist by HTCondor
  but not assigned to slots. A restart of the \Condor{startd} is required
...
\item[\Macro{GANGLIA\_GSTAT\_COMMAND}]
  ...
  not know the list of monitored hosts, change the appropriate
  host name and port from this default string:
\footnotesize
\begin{verbatim}
