...
\item[\Macro{<SUBSYS>\_LOG\_KEEP\_OPEN}]\
  A boolean value that controls keeping the log file open between writes.
  When true, the daemon will not constantly open and close the log file
  between writes.  Instead the daemon will hold the log file open until the log
  needs to be rotated. When false, the daemon reverts to the old behavior
  of opening and closing the log file between writes.  When the
  \MacroUNI{<SUBSYS>\_LOCK} macro is defined, setting
  \MacroUNI{<SUBSYS>\_LOG\_KEEP\_OPEN} has  no effect as the daemon
  will unconditionally revert back to the open/close between writes behavior.
  On Windows the value defaults to true for all daemons. On Linux the value
  defaults to true for all daemons except the shadow due to a
...
\item[\Macro{STARTD\_PUBLISH\_DOTNET}]
  ...
  When set to true the startd will advertise all installed versions of the .NET framework
  in the DotNetVersions attribute in the startd classad.  The default value is true.
  Set to false to turn off .NET version advertising.
  
\label{param:DotNetVersions}
\item[\Macro{DOT\_NET\_VERSIONS}]
  A string expression that administrators can use to override the way .NET versions
  are advertised.  If the administrator wishes to advertise .NET installations but wishes
  to do so in a format different than what the startd publishes in its ads, setting a
  string in this expression will result in the startd publishing the string when
  \Macro{STARTD\_PUBLISH\_DOTNET} is true.  This value is empty by default.
\end{description}
...
\item[\Macro{SYSTEM\_PERIODIC\_HOLD\_REASON}]
  ...
  due to \MacroNI{SYSTEM\_PERIODIC\_HOLD} becoming true.  If it
  evaluates to a non-empty string, this value is used to set the
  job attribute \AdAttr{HoldReason}.  Otherwise, a default
  description is used.
...
\item[\Macro{SYSTEM\_PERIODIC\_HOLD\_SUBCODE}]
  ...
  attribute \AdAttr{HoldReasonSubCode}.  Otherwise, a default of 0 is
  used.  The attribute \AdAttr{HoldReasonCode} is set to 26, which
  indicates that the job went on hold due to a system job policy
  expression.
...
\item[\Macro{GROUP\_DYNAMIC\_MACH\_CONSTRAINT}]
  ...
  for purposes of group quota allocation.
...
\item[\Macro{AMAZON\_EC2\_URL}]
  The URL Condor should use when contacting the Amazon EC2 (SOAP) service.
...
\item[\Macro{AMAZON\_HTTP\_PROXY}]
  The http proxy that Condor should use when contacting the Amazon EC2 (SOAP)
...
\item[\Macro{DAGMAN\_USE\_STRICT}]
  ...
  in turning warnings into fatal errors, as follows:
  \begin{itemize}
  ...
  \end{itemize}
  Using a strictness value greater than 0 may help you find problems with
  your DAG that may otherwise escape notice.
...
\item[\Macro{SOAP\_SSL\_CA\_DIR}]
  ...
  This variable is also used when communicating with an Amazon EC2 (SOAP) server
