...
\item[\Macro{QUEUE\_SUPER\_USER\_MAY\_IMPERSONATE}]
  A regular expression that matches the usernames (i.e. job owner
  ...
  user who has had a job in the queue during the life of the schedd.
  For proper functioning of \Condor{shadow}, \Condor{gridmanager}, and
  \Condor{job\_router}, this expression, if set, must match the owner
  names of all jobs that these daemons will manage.  Note that a
  regular expression that matches only part of the username is still
  considered a match.  If acceptance of partial matches is not
  desired, the regular expression should begin with \verb|^| and end
  with \verb|$|.
