...
\item[\Macro{HIBERNATE}]
  ...
  \item[] "S3",   "3", "RAM", "MEM", "SUSPEND": Sleep (standby)
...
\item[\Macro{UNHIBERNATE}]
  ...
  woken up.
  The default value is \Expr{LastMatchTime =!= UNDEFINED}.
  ...
  when a job would have been matched to the machine if it had been on line.
  For multi-slot machines, the offline ClassAd for slot1 will also contain
  the attributes \Attr{slot<X>\_LastMatchTime},
...
\item[\Macro{HIBERNATE\_PLUGIN}]
  A string which specifies the path to the hibernation ``plug in''
  that the \Condor{startd} should use in the detection of low power
  states and switching to the low power states.  The default value is
  The default value is \File{\$(LIBEXEC)/power\_state}.  A default
  executable in that location which meets these specifications is
  shipped with Condor. The executable will be invoked in either one of
  two modes:
  \begin{enumerate}
  \item ``power\_state ad'':  In this mode, the program is expected to
    output a ClassAd to it's standard output stream.  The
    \Condor{startd} will use this ClassAd to determine what low power
  \item ``power\_state set $<$mode$>$'': In this mode, the program is
    expected switch to the specified low power mode.
  \end{enumerate}
  The ClassAd that the program prints to standard output in ``ad''
  mode can specify the following attributes:
  \begin{enumerate}
  \item HibernationSupportedStates - {\it Required} This is a comma
    separated list of low power modes that are available.  The
    recognized mode strings are the same as those in the table for
    \MacroNI{HIBERNATE}.
  \item HibernationMethod - {\it Optional} This is a text string which
    describes the mechanism used by the program.  The Linux program
    will produce ``NONE'', ``/sys'', ``/proc'', or ``pm-utils''.
  \item HibernationRawMask - {\it Optional} This is an integer which
    represents the bit mask of the modes detected.
  \end{enumerate}
  In ``set'' mode, the program will be passed one of the following
  strings representing a low power state: ``S1'', ``S2'', ``S3'',
  ``S4'', ``S5''.
...
\item[\Macro{LINUX\_HIBERNATION\_METHOD}]
  ...
  This is used by the default hibernation plugin executable that is
...
machine that can read from the pool's \Condor{collector} daemon.
