...
Both the naive and the malicious Condor user are prevented
from subverting the system through its configuration.
The order in which files are parsed is
\begin{enumerate}
...
% info here about _condor_ environment variables, and where
% they fit into the ordered evaluation
...
\item[\Macro{DAEMON\_SHUTDOWN}] \label{param:DaemonShutdown}
  ...
  this expression.
  If it evalutes to TRUE, the daemon will gracefully shut itself down
  ...
  \Note This functionality does not work in conjuction with Condor's
...
\item[\Macro{SECONDARY\_COLLECTOR\_LIST}]
  ...
  Just use the \Macro{COLLECTOR\_HOST} setting, which can define a
  list of collectors.
...
\item[\Macro{SSL\_SERVER\_CAFILE}]
\label{param:SSLServerCAFile}  The path and file name of
the file containing the single trusted CA's certificate
for the server side of a communication authenticating 
with SSL.

\item[\Macro{SSL\_SERVER\_CADIR}]
\label{param:SSLServerCADir}  
The path to a directory that may contain the 
certificates for multiple trusted CAs 
for the server side of a communication authenticating 
with SSL.
...
\item[\Macro{SSL\_SERVER\_CERTFILE}]
\label{param:SSLServerCertfile}  
The path and file name of
the file containing the public certificate
for the server side of a communication authenticating
with SSL.

\item[\Macro{SSL\_SERVER\_KEYFILE}]
\label{param:SSLServerKeyfile}
The path and file name of
the file containing the private key
for the server side of a communication authenticating
with SSL.

\item[\Macro{SSL\_CLIENT\_CAFILE}]
\label{param:SSLClientCAFile} 
The path and file name of
the file containing the single trusted CA's certificate
for the client side of a communication authenticating 
with SSL.

\item[\Macro{SSL\_CLIENT\_CADIR}]
\label{param:SSLClientCADir} 
The path to a directory that may contain the 
certificates for multiple trusted CAs 
for the client side of a communication authenticating 
with SSL.
...
\item[\Macro{SSL\_CLIENT\_CERTFILE}]
\label{param:SSLClientCertfile}
The path and file name of
the file containing the public certificate
for the client side of a communication authenticating
with SSL.

\item[\Macro{SSL\_CLIENT\_KEYFILE}]
\label{param:SSLClientKeyfile}
The path and file name of
the file containing the private key
for the client side of a communication authenticating
with SSL.
