...
An \verb@else@ key word and configuration statements are not required,
...
followed by 
\begin{itemize}
  \item the \verb@defined@ keyword followed by the name of a configuration 
    variable.
    If the variable is defined, the configuration statement(s) are
    incorporated into the configuration.
    If the variable is \emph{not} defined, the configuration statement(s) are
    not incorporated into the configuration.
    As an example,
\begin{verbatim}
...
\end{verbatim}
    ...
    \emph{not} yet defined within the configuration.
  \item the \verb@version@ keyword, representing the version number of
    of the daemon currently reading the configuration, 
...
\end{verbatim}
    ...
    version of the daemon reading this configuration is 8.1.6.
  \item \verb@True@ or \verb@yes@ or the value 1.
    The configuration statement(s) are incorporated into the configuration.
  \item \verb@False@ or \verb@no@ or the value 0
    The configuration statement(s) are \emph{not} incorporated into the 
    configuration.
  \item \verb@$(<configuration variable>)@ may be used where the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\index{configration!function macros}
...
\item[\Macro{DOCKER}]
  ...
  This would be the case when the default value of \File{/usr/bin/docker}
  is used.
  The second way places the string \Expr{sudo} to the left of the 
  path and executable name.
  When the parser finds this form of the value,
  the Docker CLI is invoked and run as root by the \Condor{starter},
  and it is presumed that the user that the \Condor{starter} daemon is
...
\item[\Macro{SUBMIT\_REQUIREMENT\_<Name>}]
  A boolean expression evaluated by the \Condor{schedd} daemon
  against a job ClassAd,
  that when \Expr{False} causes the \Condor{schedd} to reject 
...
\item[\Macro{SCHEDD\_RESTART\_REPORT}]
  ...
  When the \Condor{schedd} starts, it will write a report to this file
  about its attempts to reconnect to the \Condor{startd}s for all jobs
...
\item[\Macro{CGROUP\_MEMORY\_LIMIT\_POLICY}]
  A string with possible values of \Expr{hard}, the default value
  \Expr{soft} and \Expr{none}.
...
\item[\Macro{ASSIGN\_CPU\_AFFINITY}]
  A boolean expression that defaults to \Expr{False}.
  When \Expr{True},
  each job under this startd is confined to using only as many
  cores as the slots has been configured for.  When using partitionable
  slots, each job will be bound to as many cores as it requests with
  request\_cpus.  When \Expr{True}, overrides any settings specified by
  \MacroNI{ENFORCE\_CPU\_AFFINITY}.

\label{param:EnforceCpuAffinity} 
\item[\Macro{ENFORCE\_CPU\_AFFINITY}]
  Deprecated by \MacroNI{ASSIGN\_CPU\_AFFINITY}.
  NOTE:  You most likely do not what to enabled this setting and the
  related setting below, unless you are using glidein, or other
  unusual setup!

  A boolean value that defaults to \Expr{False}.  When \Expr{False},
  the CPU affinity of processes in a job is not enforced.
  When \Expr{True}, the processes in a HTCondor job maintain their
  affinity to a CPU.  This means that this job will only run on
  that particular CPU, even if other CPU cores are idle.

  If this knob is \Expr{True}, and 
  \MacroNI{SLOT<N>\_CPU\_AFFINITY} is not set, the CPU the job
  is locked to is the same as slot id, minus one (SlotIds are
  one-based, cores are zero-based).
...
\item[\Macro{SLOT<N>\_CPU\_AFFINITY}]
  Deprecated by \MacroNI{ASSIGN\_CPU\_AFFINITY}.
  NOTE:  You most likely do not what to enabled this setting and the
  related setting above, unless you are using glidein, or other
  unusual setup!
...
\item[\Macro{ENCRYPT\_EXECUTE\_DIRECTORY}]
	The execute directory for jobs on Linux or Windows platforms will be
	encrypted by setting this configuration variable to \Expr{True}.  Defaults
	to \Expr{False}.  Note that even if this setting is \Expr{False}, the
	user can require encryption of the execute directory on a per-job basis by
	setting \SubmitCmd{encrypt\_execute\_directory} to \Expr{True} in the
	job submit description file given to \Condor{submit}.
	Enabling this functionality requires that the HTCondor service
	is run as user root (on Linux) or as a system service (on Windows).
	The method of encryption uses ecryptfs on Linux (and
	requires installation of the ecryptfs-utils package), or the EFS (Encrypted
	File System) feature of NTFS on Windows.
