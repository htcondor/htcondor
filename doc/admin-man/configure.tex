...
\item[\Macro{GANGLIA\_METRICS\_CONFIG\_DIR}]
  ...
  \item[name] The name of this metric.  Each metric must have a unique
  ...
  \item[daemon] A string containing a comma-separated list of daemon
  ...
  \item[group] A string specifying the metric group.  The default is
    determined by the daemon type.
  ...
  \item[type] The type of metric.  Possible values are \verb|"double"|
    and \verb|"string"|.  The default is \verb|"double"|.
  ...
  \item[regex] This string value specifies a regular expression that
    ...
    is the name of the matched attribute.  If the value of this
    attribute needs to be modified before publication, it can be
    referred to as an attribute named ``regex'' in the \verb|value|
    expression.  Regular expression groups defined by parentheses may
    be substitued into the values of string attributes such as the
    name and description.  This is done by putting macros in the
    string values.  ``\Bs\Bs{}1'' is replaced by the first group,
  ...
  \item[machine] The name of the host associated with this metric.
    The default is taken from the daemon ad.  If the daemon name is of
    the form \verb|extra@machine.name|, this may indicate that there
    are multiple instances of HTCondor running on the same machine.
    To avoid the metrics from these instances overwriting each other,
    the default machine name is set to the daemon name.  For aggregate
    metrics with no aggregate\_group defined, the default machine
    value will be the name of the collector host.

  \item[ip] A string containing the IP address of the host associated
    with this metric.  The default is determined from the daemon ad.
    This value must be unique for each machine published to ganglia.
    It need not be a valid IP address.  If the value of the machine
    attribute contains an \verb|"@"| sign, the default IP value
    will be set to the same value as the machine attribute in order
    to make the IP value unique to each instance of HTCondor running
    on the same host.
