...
\item[\Macro{STARTD\_RESOURCE\_PREFIX}] 
  ...
  If sites want to contine using \Expr{vm}, or prefer something other
...
\item[\Macro{USE\_VISIBLE\_DESKTOP}]
  This setting is only meaningful on Windows machines.  If True, HTCondor will
  ...
  application will not run under HTCondor.  If False, HTCondor uses the default
...
backward compatability.  In this example, the separator character is set
...
\item[\Macro{GRIDMANAGER\_JOB\_PROBE\_INTERVAL}]
  ...
  Intervals for specific grid types can be set by appending the
  grid type to the parameter name, like so:
  \begin{verbatim}
...
\item[\Macro{GRIDMANAGER\_JOB\_PROBE\_RATE}]
  ...
  The default is 5 probes per seconds.
  Rates for specific grid types can be set by appending the
  grid type to the parameter name, like so:
  \begin{verbatim}
  GRIDMANAGER_JOB_PROBE_RATE_GT5 = 300
  \end{verbatim}
...
\item[\Macro{GRIDMANAGER\_MAX\_SUBMITTED\_JOBS\_PER\_RESOURCE}]
  ...
  Limits for specific remote resources can be set by supplying
  a comma-separated list like so:
  \begin{verbatim}
  ...
  You can also set limits for specific grid-types by appending the 
  grid-type's name to the paramater, like so:
  \begin{verbatim}
