...
\item[\MacroB{<SUBSYS>\_USERID}] \index{SUBSYS\_USERID
  macro@\texttt{<SUBSYS>\_USERID} macro} This setting specifies the
  account name that should be used to run the \MacroNI{SUBSYS} process
  ...
  not have root priviledges.  The \Condor{master} must be running as
...
as the condor user with no root priviledges.  If we had specified some
account other than the condor user (\MacroNI{CONDOR\_IDS}), then we
...
other security method in which the daemon credential is owned by root,
