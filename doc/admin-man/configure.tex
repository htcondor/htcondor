...
\item[\Macro{COLLECTOR\_HOST}] \label{param:CollectorHost} The
  ...
  used to specify the network port that the \Condor{collector} should
  use.
  The port is seperated from the hostname by a ':' symbol.
  For example, \verb@COLLECTOR_HOST = $(CONDOR_HOST):1234@.
  If no port is specified, the default port of 9618 is used.
  Using the default port is recommended for most sites.
  This should only be modified if there is a conflict with another
  service listening on the same network port.
  For more information about specifying a non-standard port for the
  \Condor{collector}, see section~\ref{sec:Ports-NonStandard} on
...
\item[\Macro{NEGOTIATOR\_HOST}] \label{param:NegotiatorHost} The
  ...
  properly.
  This setting also be used to optionally define a network port for
  the \Condor{negotatior}.
  For more information, see the \MacroNI{COLLECTOR\_HOST} setting
  above.
