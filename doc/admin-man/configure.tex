...
\item[\Macro{STARTER\_ALLOW\_RUNAS\_OWNER}]
  ...
  account (true) or whether it will run as \MacroNI{VMx\_USER} or
...
\item[\Macro{DEDICATED\_EXECUTE\_ACCOUNT\_REGEXP}]
  ...
  you have configured \MacroNI{VMx\_USER} to be a \emph{different}
  account for each Condor VM, and no non-condor processes will ever be
  run by these accounts, then this pattern should match the names of
  all \MacroNI{VMx\_USER} accounts.  Jobs run under a dedicated
...
\item[\Macro{EXECUTE\_LOGIN\_IS\_DEDICATED}]
  ...
  \MacroNI{VMx\_USER}).  Therefore, when the job exits, all processes
