...
has not yet been defined.  This is particularly important if
configuration files are broken up
using the \Macro{LOCAL\_CONFIG\_FILE} setting
...
\begin{verbatim}
        $(macro_name)
\end{verbatim}
Macro definitions may contain references to \emph{previously-defined
macros}.  Nothing in a configuration file can reference
macros which have not yet been defined.  The example
\begin{verbatim}
        A = xxx
        C = $(A) 
\end{verbatim}
...
\Expr{xxx}.
Note that
\Macro{C} is actually bound to 
\MacroU{A}, not its value.
...
\Macro{C} is \Expr{yyy}.  However,
\begin{verbatim}
        A = $(C)
        C = xxx
\end{verbatim}
is not legal.
It is an example of a forward reference,
and it will result in the Condor daemons and tools exiting
when they try to parse their configuration files.
...
expanded.
\MacroU{A} is immediately expanded in line 3 of the example.
...
\end{verbatim}
\Note White space must appear between the expression name,
the ``:'' sign, and the expression definition.  Expressions may
contain constants, operators, other expressions, and macros.
Constants may be booleans (with a value of TRUE or FALSE),
signed integers, floating point values,
or strings enclosed in double quotes (").
All configuration file expressions are inserted into various
ClassAds.

Note that expressions which contain references to other expressions are
bound to the expression's definition, not its current value.
Expressions which contain macro invocations are bound to the current
value of the macro.  For example,
\begin{verbatim}
        X : "xxx"
        Y : X
        X : "yyy"
\end{verbatim}
will result in \Expr{Y} being evaluated as \Expr{"yyy"} at run time, but
\begin{verbatim}
        X = "xxx"
        Y : $(X)
        X = "yyy" 
\end{verbatim}
will result in \Expr{Y} having a run time value of \Expr{"xxx"}.
...
Other than macros and expressions, a Condor configuration
file can contain
comments or line continuations.  A comment is any line beginning with a
``\#'' character.
A continuation is any entry (either macro or expression) that
continues across multiples lines.  Line continuation
is accomplished by placing the
``$\backslash$'' character at the end of any line 
to be continued onto another.  Valid examples of line continuation are
\begin{verbatim}
...
Condor provides pre-defined macros and expressions that
help configure Condor.  Pre-defined macros are listed as
\MacroU{macro\_name}, while pre-defined expressions are listed as
...
\item[\Expr{CurrentTime}] \label{param:CurrentTime} An expression
  providing the current result of the system call \Cmd{time}{2}.  It
  is an integer containing the number of seconds elapsed
  since an arbitrary date defined by UNIX as the ``beginning of time'',
  referred to as the \Term{UNIX date}.
  
\item[\MacroU{FULL\_HOSTNAME}] \label{param:FullHostname} The
  fully qualified hostname of the local machine (hostname plus domain
  name).
  
\item[\MacroU{HOSTNAME}] \label{param:Hostname} The
  hostname of the local machine (no domain name).
  
\item[\MacroU{TILDE}] \label{param:Tilde} The full path to the
...
\item[\MacroU{SUBSYSTEM}] \label{param:Subsystem} The subsystem
...
This second set of macros and expressions
are entries whose default values are determined
automatically at runtime but which can be overwritten.  
\index{configuration file!macros}
\begin{description}

\item[\MacroU{ARCH}] \label{param:Arch} Defines the string
...
\item[\MacroU{OPSYS}] \label{param:OpSys} Defines the
...
\item[\MacroU{FILESYSTEM\_DOMAIN}] Defaults to the fully
...
\item[\MacroU{UID\_DOMAIN}] Defaults to the fully
  qualified hostname of the machine it is evaluated on.  See
...
Since \MacroU{ARCH} and \MacroU{OPSYS} will automatically be set to the
...
\item[\Macro{COLLECTOR\_HOST}] \label{param:CollectorHost} The
  ...
  \MacroU{CONDOR\_HOST} macro described above.
...
\item[\Macro{BIN}] \label{param:Bin} This directory points to the
  ...
  is usually defined relative to the \MacroU{RELEASE\_DIR} macro.
...
\item[\Macro{LIB}] \label{param:Lib} This directory points to the
  ...
  \MacroU{LIB} is usually defined relative to the
  \MacroU{RELEASE\_DIR} macro.
...
\item[\Dflag{BANDWIDTH}] \label{dflag:bandwidth} When this flag is
  set, the negotiator logs a message for every match. It 
...
\item[\Macro{SUBSYS\_LEVEL\_LOG}] \label{param:SubsysLevelLog} This is
  ...
  \MacroU{SUBSYS\_DEBUG}, then all messages of this debug level will be
  written both to the \MacroU{SUBSYS\_LOG} file and the
  \MacroU{SUBSYS\_LEVEL\_LOG} file.  For example,
  \MacroU{SHADOW\_SYSCALLS\_LOG} specifies a log file for all remote
  system call debug messages.

\item[\Macro{MAX\_SUBSYS\_LEVEL\_LOG}] \label{param:MaxSubsysLevelLog}
  Similar to \MacroU{MAX\_SUBSYS\_LOG}.

\item[\Macro{TRUNC\_SUBSYS\_LEVEL\_LOG\_ON\_OPEN}]
  \label{param:TruncSubsysLevelLogOnOpen} Similar to
  \MacroU{TRUNC\_SUBSYS\_LOG\_ON\_OPEN}.

\item[\Macro{SUBSYS\_LEVEL\_LOCK}] \label{param:SubsysLevelLock}
  Similar to \MacroU{SUBSYS\_LOCK}.
...
\item[\Macro{SUBSYS\_EXPRS}] \label{param:SubsysExprs}
  ...
  marks (") around strings.  Numeric values can go in directly,
  as can expressions or boolean macros.  For example, if 
  the startd is to advertise a string macro, a numeric macro,
  and a boolean expression, do something similar to:
...
\item[\Macro{SHADOW\_RENICE\_INCREMENT}]
  ...
  value can be any integer between 0 and 20, with a value of 19 being
...
\item[\Macro{FLOCK\_HOSTS}] \label{param:FlockHosts} This macro
  defines a list of central manager hostnames (not including the local
  \MacroU{CONDOR\_HOST} machine) for pools in which the schedd should
  ...
  \MacroU{HOSTALLOW\_NEGOTIATOR\_SCHEDD} (see
  section~\ref{param:HostAllow}) must also be configured to allow
  negotiators from all of the \MacroU{FLOCK\_HOSTS} to contact the
...
\item[\Expr{NEGOTIATOR\_TRAFFIC\_LIMIT}]
  \label{param:NegotiatorTrafficLimit} This macro specifies the
  maximum amount of network traffic (in Kbytes) that the negotiator may
  initiate per \Expr{NEGOTIATOR\_TRAFFIC\_INTERVAL} for job placement
  and preemption.  The negotiator uses the job \AdAttr{ImageSize} and
  \AdAttr{ExecutableSize} parameters to track network usage.  The negotiator
  will try to use bandwidth up to the limit, so if starting a large
  \AdAttr{ImageSize} job would put the negotiator over the limit, it will try
  to start a small \AdAttr{ImageSize} job in its place.  Thus, using traffic
  limits penalizes large \AdAttr{ImageSize} jobs for the load they place on the
...
\item[\Expr{NEGOTIATOR\_TRAFFIC\_INTERVAL}]
  \label{param:NegotiatorTrafficInterval} This macro specifies the
  interval (in seconds) to be used in maintaining the
  \Expr{NEGOTIATOR\_TRAFFIC\_LIMIT}.  This macro defaults to 0,
  which disables network usage management in the negotiator.  It is
  common to set this macro equal to \Expr{NEGOTIATOR\_INTERVAL}.
