...
\item[\Macro{MAX\_NEXT\_JOB\_START\_DELAY}] \label{param:MaxNextJobStartDelay}
  This integer is the maximum allowed value of the job attribute
  \AdAttr{NextJobStartDelay}.  It defaults to 10 minutes.
...
\begin{verbatim}
        ...
        SCHEDD_ROUND_ATTR_ExecutableSize = 5
        SCHEDD_ROUND_ATTR_DiskUsage = 5
        SCHEDD_ROUND_ATTR_NumCkpts = 4
\end{verbatim}
  Thus, the above attributes that are in Kbytes are rounded to the nearest
  100 Megabytes.  If your batch slots have less than 100MB of memory
  or disk, you should reduce the rounding to a smaller number, because
  job requirements will all be rounded up and will therefore not match.
