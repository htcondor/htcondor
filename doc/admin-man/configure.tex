...
\Macro{LOG\_ON\_NFS\_IS\_ERROR} is set to \Expr{true}, such
submit files will be rejected.  If \Macro{LOG\_ON\_NFS\_IS\_ERROR}
is set to \Expr{false}, submitting such a file will result in a
warning, but the job will be submitted.  If undefined,
\Macro{LOG\_ON\_NFS\_IS\_ERROR} defaults to \Expr{False}.
