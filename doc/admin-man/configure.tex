...
\item[\Macro{DAEMON\_SOCKET\_DIR}] This specifies the directory where
  ...
  The default value on Linux platforms is \Expr{auto},
  using anonymous sockets.
  The default value on non-Linux platforms is \Expr{\$(LOCK)/daemon\_sock}.
  If the resulting directory name is too long,
  the \Condor{master} attempts to create a directory in \File{/tmp}. 
