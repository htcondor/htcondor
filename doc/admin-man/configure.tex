...
\item[\Macro{BIND\_ALL\_INTERFACES}] \label{param:BindAllInterfaces}
  ...
  \MacroNI{BIND\_ALL\_INTERFACES} can be
...
\item[\Macro{MASTER\_NAME}] \label{param:MasterName}
  ...
  When this definition includes, but does not end with
  an \verb$@$ sign, Condor replaces whatever follows the \verb$@$
  sign with the fully qualified host name of the local machine.
  When this definition ends with an \verb$@$ sign,
  Condor does not modify the value.
