...
\label{param:FetchWorkDelay}
  ...
  If not defined, the \Condor{startd} will attempt to re-fetch every
  second.
...
\label{param:HibernateCheckInterval}
  ...
  disabled; otherwise, the \Expr{HIBERNATE} expression will be 
...
\label{param:Hibernate}
  A integer expression that, when non-zero, causes Condor to put the 
