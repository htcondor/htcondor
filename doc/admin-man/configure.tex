...
\label{param:ProcInfoNumAttempts}
\item[\Macro{PROC\_INFO\_NUM\_ATTEMPTS}]
  An integer value greater than or equal to 1.  Specifies the number of
  attempts made to obtain information about a process id.  Its value
  defaults to 5.  Setting this value smaller can reduce \Condor{master}
  blocking due to multiple failed attempts at querying defunct process
  ids which cannot succeed.
...
\item[\Macro{DAGMAN\_DEFAULT\_NODE\_LOG}]
  The name of a file to be used as a job event log by all node jobs
  of a DAG (by default, see \Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG}
  below).  Note
  that even if \Macro{DAGMAN\_ALWAYS\_USE\_NODE\_LOG} is set to
  false, the default log file will be used by any node jobs for which
  a log file is not defined in the job's submit file.
  The default value if not defined is \File{<DagFile>.nodes.log},
  where \verb@<DagFile>@ is replaced by the command line argument
  to \Condor{submit\_dag} that specifies the DAG input file.  (If
  more than one DAG file is specified, the name of the first is
