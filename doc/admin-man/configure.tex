...
\item[\Macro{GANGLIAD\_INTERVAL}]
  ...
  The ganglia heartbeat interval is 20 seconds. So, multiples of 20, between
  20 and 300 makes sense for this value.
...
\item[\Macro{GANGLIA\_GSTAT\_COMMAND}]
  ...
  by ganglia.
  For a \Condor{gangliad} running on a host whose local gmond does
  not know the list of monitored hosts, change the appropriate
  hostname and port in the from the default string.
  The default is 'gstat --all --mpifile --gmond\_ip=localhost --gmond\_port=8649'

\label{param:GangliaSendDataForAllHosts}
\item[\Macro{GANGLIA\_SEND\_DATA\_FOR\_ALL\_HOSTS}]
  Whether to send data to ganglia for hosts that it is not currently monitoring.
  The default is \Expr{False}.
