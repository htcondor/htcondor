...
\item[\Macro{DEBUG\_TIME\_FORMAT}] \label{param:DebugTimeFormat}
  This macro defines how to format the current time printed at the
  start of each line in the daemon log files.  The value is a format 
  string is passed to the C strftime function, so see that manual page for
  complete details on your system.  If not defined, the default
  value is \begin{verbatim} "%m/%d %H:%M:%S ".  
\end{verbatim}
