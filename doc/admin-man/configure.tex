...
\item[\Macro{MOUNT\_UNDER\_SCRATCH}]
  ...
  such as Red Hat Enterprise Linux 5.  A bind mount is like a symbolic link, but is
  not globally visible to all processes, it is only visible to the job and the
  job's child processes.
  As an example:
\begin{verbatim}
...
\end{verbatim}
  ...
  system will redirect the access. The resultant files 
  will actually end up in directories named tmp or var/tmp
  under the the job's temporary scratch directory. 
  This is useful, because the job's scratch directory will be cleaned up 
  after the job completes, two concurrent jobs will not interfere with each other,
