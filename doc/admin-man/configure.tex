...
\item[\Macro{KILL\_ORPHANS\_ON\_STARTUP}]
  \label{param:KillOrphansOnStartup} This macro
   determines if the master process should try to locate and hard
   kill orphaned processes from other instantiations of Condor. An
   administrative email is sent detailing the pids (along with their
   command lines) which had been killed. This entry is defined with a
   boolean value and defaults to TRUE.
