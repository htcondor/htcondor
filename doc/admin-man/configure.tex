...
\item[\Macro{GRAM\_VERSION\_DETECTION}]
  A boolean parameter that defaults to \Expr{True}.
  If set to \Expr{True}, the \Condor{gridmanager} treats grid-types
  \Expr{gt2} and \Expr{gt5} identically, and queries each server to
  determine which protocol it's using.
  If set to \Expr{False}, the \Condor{gridmanager} trusts the grid-type
  ...
  the server being overloaded if a large number of jobs are submitted.
