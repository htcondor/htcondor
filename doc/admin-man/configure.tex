...
\item[\Macro{FLOCK\_INCREMENT}]
  This configuration variable controls how quickly flocking to various
  pools will occur.  The variable defaults to 1, meaning that pools will
  be considered for flocking slowly: the first collector in
  \Macro{FLOCK\_COLLECTOR\_HOSTS} will be considered for flocking, then
  the second, and so on.  Setting this number to bigger than one will
  cause more than one collector to be considered for flocking; setting
  this number to 2, for example, will partition the
  \Macro{FLOCK\_COLLECTOR\_HOSTS} into sets of 2 collectors, and each
  set will be considered for flocking.
...
between retries of a failed call to glexec.  The default is 5.  The
actual delay to be used is determined by a random exponential backoff
