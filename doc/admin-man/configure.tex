...
\item[\Macro{GANGLIA\_METRICS\_CONFIG\_DIR}]
  ...
  the daemon ad that is being monitored.  The file containing the
  default metrics is a good source of examples.  The recognized metric
  ...
  \item[type] A string specifying the type of the metric.  Possible
    values are \verb|"double"| and \verb|"string"|.  The default is
    \verb|"double"|.
  ...
  \item[regex] This string value specifies a regular expression that
    ...
    referred to as an attribute named \Attr{regex} in the \Attr{value}
    expression.  Text matched by regular expression groups defined by
    parentheses within the regular expression may be substitued into
    the values of other string attributes such as \Attr{name} and
    \Attr{desc}.  This is done by putting macros in the string values.
    \verb|"\\1"| is replaced by the first group, \verb|"\\2"| by the
  ...
  \item[machine] The name of the host associated with this metric.
    ...
    the form \verb|name@hostname|, this may indicate that there are
    multiple instances of HTCondor running on the same machine.  To
    avoid the metrics from these instances overwriting each other, the
    default machine name is set to the daemon name.  For aggregate
    metrics with no \Attr{aggregate\_group} defined, the default
  ...
  \item[ip] A string containing the IP address of the host associated
    ...
    It need not be a valid IP address.  If the value of \Attr{machine}
    contains an \verb|"@"| sign, the default IP value will be set to
    the same value as \Attr{machine} in order to make the IP value
    unique to each instance of HTCondor running on the same host.
