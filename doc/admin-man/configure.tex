...
\item[\Macro{FLOCK\_NEGOTIATOR\_HOSTS}]
  This macro defines a list of negotiator host names for pools in which
  the \Condor{schedd} should attempt to run jobs.  If not set, the
  \Condor{schedd} will query the collectors for the addresses of the
  negotiators.  If set, then the negotiators must be specified in order,
  corresponding to the \MacroUNI{FLOCK\_COLLECTOR\_HOSTS} list.
  ...
  typically used as a macro for adding the negotiator to the relevant
  authorization lists.
