...
\index{configuration!condor\_shared\_port configuration variables}
 ...
 Condor daemons for which \Macro{USE\_SHARED\_PORT} is true (except
 for the \Condor{collector}, which always uses its own command port,
 and the \Condor{schedd}, which creates a command port for local use
 by the shadows).
