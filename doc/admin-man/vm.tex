...
The execution of a job within the docker universe causes the
instantiation of a Docker container on an execute host,
and runs that single HTCondor job within that container. 
...
The Docker service must be installed on each execute machine
...
for each machine that is capable of running Docker universe jobs.
