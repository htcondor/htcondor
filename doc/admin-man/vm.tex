...
Specifically: \Attr{HasVM} will be set to \Expr{False}; \Attr{VMOfflineReason}
will be set to a vaguely explanatory string; \Attr{VMOfflineTime} will be
set to the time of the failure to launch; and \Attr{OfflineUniverses} will be
adjusted to include \Expr{"VM"} and \Expr{13}.  By default, \Condor{submit}
will add \Expr{HasVM == True} to a VM universe job's requirements; thus,
normal VM universe jobs will not run on a machine which failed to launch
a VM.  (As usual, you can prevent \Condor{submit} from adding a default
...
second is to use \Condor{update\_machine\_ad} to set \Attr{HasVM}.  In
this case, the startd will automatically adjust the various Offline
attributes to match.  See the \Condor{update\_machine\_ad} man page for
