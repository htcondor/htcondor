...
The single, optional parameter, \texttt{classAd}, 
...
\\ \hline
\texttt{submit( ad, count = 1 )}

Submit one or more jobs to the \Condor{schedd} daemon.
Returns the newly created cluster ID.

This method requires the invoker to provide a full ClassAd for the new job;
such a ClassAd is much more complex than the contents of 
...
The submission does not perform any file staging.  
Thus, the current implementation's interface is only useful for submitting jobs 
to the local \Condor{schedd} daemon.
...
The optional parameter \texttt{pool} is a string with host:port pair specified.
...
\\ \hline
\texttt{locateAll( (Collector)arg1, (DaemonTypes)arg2 )}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\label{Python-Example} Sample Code using the Python Module}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This sample code illustrates interactions with the Python Module. 
