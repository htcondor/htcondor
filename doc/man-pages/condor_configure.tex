...
\begin{Options}
	...
	\OptItem{\Opt{---overwrite}} {
	  ...
	  ``--overwrite'' or ``--backup'' to tell \Condor{install}
	  what to do.

	  This prevents \Condor{install} from moving an ``sbin''
	  ...
	  For example: ``\Condor{install} --prefix=/usr'' will not move
	...
	\OptItem{\Opt{---backup}} {
	  ...
	  ``--backup'' to tell \Condor{install} what to do.
	...
	\OptItem{\Opt{---ignore-missing-libs}} {
	  ...
	  are deteted.  The \Opt{---ignore-missing-libs} will cause
	  \Condor{install} to not exit, and to proceed with the
	  installation if missing libraries are detected.
	}

	\OptItem{\Opt{---force}} {
	  This is equivielent to enabling both the \Opt{---overwrite} and
	...
	\OptItem{\Opt{---no-env-scripts}} {
	  ...
	  variables.  This option will prevent \Condor{configure} from
	...
	\OptItem{\Opt{---env-scripts-dir=$<$directory$>$}} {
	  ...
	  cauess \Condor{configure} to generate these scripts in the
	  specified directory.
	}
