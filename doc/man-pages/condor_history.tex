...
\oOpt{-backwards}
\oArg{-constraint expr \Bar cluster \Bar cluster.process \Bar owner \Bar
...
If no history files are specified (with the \Opt{-f} option), the local
...
\begin{Options}
  ...
  \OptItem{\OptArg{-match}{number}}{Limit the number of jobs displayed
    to \Arg{number}.}
  \OptItem{\OptArg{-name}{quill-name}}{Utilize the given Quill database
    for history information, instead of the history file.}
  \OptItem{\OptArg{-constraint}{expr}}{Display jobs that satisfy the
    expression.}
  \OptItem{\OptArg{-format}{formatSpec}{AttributeName}}{Display jobs
    with a custom format. See \Condor{q} man page for details.}
