\begin{ManPage}{\Condor{drain}}{man-condor-drain}{1}
...
unless the \Opt{-start} expression specifies otherwise.
...
If you specify \Opt{-graceful}, you may also specify \Opt{-start}.  On a
...
(see below).  The \Opt{-start} expression you supply replaces the draining
machine's normal \MacroNI{START} expression for the duration of the draining
...
\begin{Options}
  ...
  \OptItem{\OptArg{-start}{expr}}{The \MacroNI{START} expression to use
    while the machine is draining.  You can't reference the machine's
    existing \MacroNI{START} expression.}
