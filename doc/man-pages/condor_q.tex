...
\begin{Options}
  ...
  \OptItem{\Opt{-nobatch}}{(output option) Show a line for each job (turn
    off the \Opt{-batch} option).  Note that \Opt{-nobatch} is currently
    the default, but this option is included because \Opt{-batch}
    will become the default at some point in the 8.5 series.}
...
An example that shows the \Opt{-batch} option:
\footnotesize
\begin{verbatim}
$ condor_q -batch
