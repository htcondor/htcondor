...
\oOptArg{-machine}{name}
\oOptArg{-mconstraint}{expression}
\oOptArg{-slotads}{file}
\oOptArg{-userprios}{file}
\oOpt{-stream-results}
\oOpt{-wide}
...
The \Arg{-analyze} or \Arg{-better-analyze} options may be used to determine why certain jobs are not
...
\Macro{PREEMPTION\_REQUIREMENTS} expression.  If the \Arg{-verbose} option is specified 
along with the \Arg{-analyze} option, the reason for failure is displayed on a 
per machine basis. \Arg{-better-analyze} differs from \Arg{-analyze} in that it will
do matchmaking analyisis on jobs even if they are currently running, or if the reason
they are not running is not due to matchmaking.	 \Arg{-better-analyze} also produces
more thourough analysis of complex Requirements and shows the values of relevent job
ClassAd attributes.  When only a single machine is being analyzed via \Arg{-machine} or
\Arg{-mconstraint} the values of relevant attributes of the machine ClassAd are also
displayed.
...
\begin{Options}
  ...
  \OptItem{\Opt{-analyze}}{Perform an analysis why the requested jobs are not running.
    First a simple analysis determines if the job is not running because it isn't in
    a runnable state.  If the job is in a runnable state, then this option is equivalent
    to \Opt{-better-analyze}. }
  \OptItem{\Opt{-better-analyze}}{Perform an analysis to determine how
  ...
  \OptItem{\OptArg{-slotads}{file}}{When doing matchmaking analysis, use the
    machine ads from the file instead of the ones from the
    \Condor{collector} daemon. 
    This is most useful for debugging purposes. 
    The ClassAds appear as if \Condor{status} \Opt{-long} is used.}
  \OptItem{\OptArg{-machine}{name}}{When doing matchmaking analysis, analyze
    only machine ads that have slot or machine names that match the given name. }
  \OptItem{\OptArg{-mconstraint}{expression}}{When doing matchmaking analysis, 
    match only machine CladdAd which match the ClassAd expression constraint.}
  \OptItem{\OptArg{-userprios}{file}}{When doing matchmaking analysis with
  ...
  \OptItem{\Opt{-reverse}}{When used with \OptArg{-analyze} or \OptArg{-better-analyze}
    The Requirements expression of machines are analyzed instead of the default
    behavior which analyzes the Requirements of jobs.
    }
  \OptItem{\Opt{-verbose}}{When doing analysis, show progress and include
