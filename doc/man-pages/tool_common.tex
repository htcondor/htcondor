...
\oOptnm{-name hostname $|$ hostname $|$ -addr "\SinfulAny" $|$ "\SinfulAny" } \Opt{\Dots} \\
}

%% for checkpoint, off, on, reconfig, reschedule, restart and vacate
\newcommand{\ToolWhere}{
\Optnm{-pool centralmanagerhostname}\\
\Optnm{-name hostname $|$ hostname $|$ -addr "\SinfulAny" $|$ "\SinfulAny" } \Optnm{\Dots} \\
}

%% Alternative to locate the machines with which you want to mess.
%% Used by _rm, _hold, and _release.
\newcommand{\ToolLocate}{
\oOptnm{-pool centralmanagerhostname}
...
\OptItem{\OptArg{-pool}{centralmanagerhostname}}{Specify a pool by giving the
	central manager's hostname}
...
\OptItem{\Opt{-master}}{Send the command to the master (default)}
\OptItem{\Opt{-startd}}{Send the command to the startd daemon}
\OptItem{\Opt{-schedd}}{Send the command to the schedd daemon}
\OptItem{\Opt{-collector}}{Send the command to the collector daemon}
\OptItem{\Opt{-kbdd}}{Send the command to the kbdd daemon}
}
