...
\begin{Options}
    ...
    \OptItem{\Opt{-diagnose}}{(Help option) Print out query ad without 
      performing query}
    \OptItem{\Opt{-any}}{(Query option) Query all ads and display their type,
    ...
    \OptItem{\OptArg{-direct}{hostname}}{(Query option) Go directly to
      the given host name to get the ads to display}
    ...
    \OptItem{\Opt{-negotiator}}{(Query option) Query \Condor{negotiator} ads
    ...
    \OptItem{\Opt{-vm}}{(Query option) Query \Condor{startd} ads and display 
      only VM-enabled machines}
