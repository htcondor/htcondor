...
\index{job\_machine\_attrs}
\index{submit commands!job\_machine\_attrs}
\item[job\_machine\_attrs = $<$attr1,attr2,...$>$]
A comma and/or space separated list of machine attribute names that
should be recorded in the job ad in addition to the ones specified
by the \Condor{schedd} daemon's system configuration
...
attribute named \Attr{X} will be inserted into the job ad as an
...
will be recorded.  The value recorded in the job ad is the evaluation
of the machine attribute in the context of the job ad when
the \Condor{schedd} daemon initiates the startup of the job.  If the
