...
The vanilla universe has the ability to run prescripts and postscripts, before and
after the job, respectively.  Note that this is different from the PRE script and POST script in DAGman.  These commands can be executables or scripts which prepare or initialize
the job. The pre and post scripts aren't automatically transfered with the job,
as the main executable is, so they must be entered into the transfer\_input\_files
list, when file transfer is on.
