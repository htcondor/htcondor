...
Defaults to \Expr{False}. If set to \Expr{True}, then HTCondor will encrypt the
contents of
...
In addition, if enabled, \Condor{submit} will append by default
the expression
\begin{verbatim}
  && (TARGET.HasEncryptExecuteDirectory)
\end{verbatim}
to the \SubmitCmd{requirements} expression for the job to ensure
