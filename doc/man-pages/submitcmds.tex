...
megabytes of the input files that are transferred for a job.  This
expression does not apply to grid universe, standard universe, or
files transferred via file transfer plugins.  The expression may refer
...
hold code 32.  If the job passes this initial test, but the size of
...
This integer expression specifies the maximum allowed total size in
megabytes of the output files that are transferred for a job.  This
expression does not apply to grid universe, standard universe, or
files transferred via file transfer plugins.  The expression may refer
to attributes of the job.  The special value -1 indicates no limit.
If not yset, the system setting \Macro{MAX\_TRANSFER\_OUTPUT\_MB} is
used.  If the total size of the job's output files to be transferred
is larger than the limit, the job will be placed on hold with hold
code 33.  The output will be transferred up to the point when the
...
For grid type \SubmitCmd{ec2} jobs, 
specifies the Spot Instance bid, which is the most you're willing to pay
per hour to run this job.
...
is recorded on the job ad as \AdAttr{Group}.
...
\index{submit commands!group\_user}
...
recorded on the job ad as \AdAttr{GroupUser}.
