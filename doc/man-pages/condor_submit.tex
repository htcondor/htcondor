...
parallel job written with PVM 3.3. The \Arg{scheduler} is for a job that
...
the \Exec{globusrun} program for execution. 
...
\item[rendezvousdir = $<$pathname$>$] Used to specify the 
...
for GSS authentication. If included, the environment variables X509_USER_KEY,
X509_USER_CERT, and X509_CERT_DIR are automatically exported. If the default
