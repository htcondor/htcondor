...
\item[output = $<$pathname$>$] The \Opt{output} file name will capture
...
\item[error = $<$pathname$>$] The \Opt{error} file name will capture any
...
A \Opt{requirements} example that specifies only the \AdAttr{Disk} attribute:
\begin{verbatim}
...
completes (exits).
If not specified, \Arg{ONEXIT} is used as the default on NT machines.
If not specified, \Arg{NEVER} is used as the default on Unix machines.
...
%See section~\ref{sec:X509-Authentication} for more info.
...
When the ClassAd Expression evaluates to \Arg{True}, the job is
...
\Arg{false}, then Condor looks for the executable on the remote machine, and
