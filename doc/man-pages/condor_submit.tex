...
\index{submit commands!NextJobStartDelay}
This expression specifies the number of seconds after starting up
this job that the next job startup should be delayed.  The maximum
allowed delay is specified by the Condor configuration variable
\Macro{MAX\_NEXT\_JOB\_STARTUP\_DELAY}, which defaults to 10 minutes.
Currently, this command does not apply to scheduler or local universe
jobs.
...
\OptItem{\Opt{-dump}}{Sends all the class ads to a file instead of the
  \Condor{schedd}.}
