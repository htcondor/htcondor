...
\OptItem{\Opt{-interactive}}{The interactive flag to condor\_submit indicates
the user wants to run an interactive shell on some execute machine in the pool.
 This is equivalent to creating a submit file with a vanilla universe
 sleep job, then running condor\_ssh\_to\_job by hand.  Without any additional
arguments, \Condor{submit} with the -interactive flag creates a dummy
vanilla universe job that sleeps, submits it to the local scheduler, waits
for it to run, and launches condor\_ssh\_to\_job to run a shell.  If
the user would like to run the shell on a machine that matches a particular
Requirements expression, the -interactive feature can optionally take
the name of a job description file as an argument.  In that case, the
Requirements, Rank and other policy expressions specified in the job 
description file are honored, but the executable and universe are 
overwritten to be sleep and vanilla.  The job attribute InteractiveJob
is set to true for startd policy usage.
}
