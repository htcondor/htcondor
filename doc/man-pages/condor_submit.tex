...
file parameters. Used by grid-type gt2 and gt3 jobs.
...
file parameters. Used by grid-type gt4 jobs.
...
The \SubmitCmd{nordugrid\_resource} command is required when
the \SubmitCmd{universe} is \SubmitCmd{grid},
and \SubmitCmd{grid\_type} is \SubmitCmd{nordugrid}.
...
\index{submit commands!globus\_rsl}
string attributes which are not covered by regular submit description
file parameters. Used by grid-type nordugrid jobs.
...
The \SubmitCmd{unicore\_u\_site} command is required when
the \SubmitCmd{universe} is \SubmitCmd{grid},
...
The \SubmitCmd{unicore\_v\_site} command is required when
the \SubmitCmd{universe} is \SubmitCmd{grid},
and \SubmitCmd{grid\_type} is \SubmitCmd{unicore}.
...
Path to a java keystore file containing the certificate to be
used for a Unicore job.
Used with \SubmitCmd{keystore\_alias} and
\SubmitCmd{keystore\_passphrase\_file}.
...
Alias for the certificate in a java keystore file to be used for
a Unicore job.
Used with \SubmitCmd{keystore\_file} and
\SubmitCmd{keystore\_passphrase\_file}.
...
Path to a file containing the passphrase protecting a java keystore
file containing the certificate to be used for Unicore job.
Used with \SubmitCmd{keystore\_file} and
\SubmitCmd{keystore\_alias}.
