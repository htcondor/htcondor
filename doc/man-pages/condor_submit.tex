...
then the named executable must have been relinked with the Condor
libraries (such as via the \Condor{compile} command). If submitting into
the Vanilla Universe, then the named executable need not be relinked and
...
working directory for the Condor job. Should be a path to a prexisting
...
least 64 megs of RAM and has a MIPS performance rating greater than 45,
use: 
\begin{verbatim}
...
\begin{verbatim}
        <parameter> = <value>
\end{verbatim}
Multiple environment variables can be specified by seperating them with a
...
is the default, and tells Condor that this job has been relinked via
...
submitted in this fasion run only on machines which no other
...
submitted in this fasion have ``nice-user.'' pre-appended in front of
...
chracater are ignored by the submit-description file parser. 
