...
If only the \Arg{cluster} is given,
then the \Arg{process} defaults to the value 0, specifying a single job.
...
plus any changes made by the shell set up scripts
...
all job processes have exited until the \Prog{ssh} session is closed.

\Condor{ssh\_to\_job} stores ssh keys in temporary files within a newly
created and uniquely named directory.
An environment variable defines a value for \Env{TMPDIR}.
The newly created directory will be within the directory defined
by \Env{TMPDIR}.
When the ssh session is finished, this directory is removed.
...
Then the \Condor{starter} spawns \Prog{sshd} in inetd mode.
...
listening on the network for new connections or running with any privileges
...
This example shows upload or download with \Prog{scp}.
...
task of \Prog{rsync}.
Job id 32.0 is used in place of host name in this example.
\footnotesize
\begin{verbatim}
