...
If \Arg{classad-filename} is omitted or is ``-'', then the
ClassAds are read from standard input.

When \Opt{-multiple} is specified, multiple ads may be published.
Publishing many ads in a single invocation of \Condor{advertise} is
more efficient than invoking \Condor{advertise} once per ad.  The ads
are expected to be separated by one or more blank lines.  When
\Opt{-multiple} is not specified, blank lines are ignored (for
