...
%TEMPTEMP -- this is a really weird case -- condor_dagman actually defaults to a debug setting of 2, but condor_submit_dag defaults to setting it to 3 -- maybe I should just have dagman itself also default to 3
    ...
    \OptItem{\Opt{-PostIgnorePre}}{Causes \Condor{dagman} to ignore
       This will pass the \Opt{-PostIgnorePre} flag in the arguments to
       \Condor{dagman} in the submission file, which causes \Condor{dagman} to use the
       return value from a PRE script to determine whether a POST script will run.  By
       default, \Condor{dagman} runs the POST script regardless of the return value of
       the PRE script. Old versions of \Condor{dagman}, prior to 7.7.2, would not
       ignore the return value and would not run the POST script if the PRE script
       failed.}
