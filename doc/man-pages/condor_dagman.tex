...
\begin{Options}
    ...
    \OptItem{\Opt{-DontAlwaysRunPost}}{This option causes \Condor{dagman} to
       ...
       script fails.}
    \OptItem{\Opt{-Suppress\_notification}}{This option causes jobs submitted
       by \Condor{dagman} to not send email notification for events. The same effect
       can be achieved by setting \Macro{DAGMAN\_SUPPRESS\_NOTIFICATION} to True
       in a configuration file. This command line flag is independent of the
       ``-notification'' command line flag, which controls notification for the
       \Condor{dagman} job itself. This flag is generally superfluous, as
       \Macro{DAGMAN\_SUPPRESS\_NOTIFICATION} defaults to True.}
    \OptItem{\Opt{-Dont\_Suppress\_notification}}{This option causes jobs
       submitted by \Condor{dagman} to defer to the submit file when deciding to send
       email notification for events. The same effect can be achieved by setting
       \Macro{DAGMAN\_SUPPRESS\_NOTIFICATION} to False in a configuration file.
       This command line flag is independent of the ``-notification'' command line
       flag, which controls notification for the \Condor{dagman} job itself. If 
       -Dont\_Suppress\_notification and -Suppress\_notification are on the same 
