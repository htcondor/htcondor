...
running Condor instance.   This program uses \Condor{local\_stop} to
reliably shut down the running Condor instance.  Then it uses
\Condor{cleanup\_local} to create and store archives of the
\File{log}, \File{spool}, and \File{execute} directories.  After which
this program uninstalls the Condor binaries and libraries using
\Prog{cleanup\_release}.
...
\begin{Options}
  ...
  \OptItem{\OptArg{-execarchive}{archive}}{
    ...
    \File{execute} directory and stores it as \Opt{archive}.  The
    \Opt{archive} can be a file path or a grid-ftp url.
  }
  \OptItem{\Opt{-flock}}{
    The program will determine whether the Condor master has exited
  ...
  \OptItem{\OptArg{-logarchive}{archive}}{
    The program will create a tar'ed and gzip'ed archive of the
    \File{log} directory and stores it as \Opt{archive}.  The
    \Opt{archive} can be a file path or a grid-ftp url.
  }
  \OptItem{\Opt{-nogurl}}{
  ...
  \OptItem{\OptArg{-runlocalconfig}{file}}{
    Binds \Opt{file} into the configuration used by this instance of
    Condor.  This option should be provided here, if it was provided to
    \Condor{cold\_start}.
  }
  \OptItem{\OptArg{-spoolarchive}{archive}}{
    The program will create a tar'ed and gzip'ed archive of the
    \File{spool} directory and stores it as \Opt{archive}.  The
    \Opt{archive} can be a file path or a grid-ftp url.
  }
  \OptItem{\Opt{-stop}}{
