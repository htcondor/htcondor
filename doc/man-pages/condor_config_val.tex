...
\begin{Options}
  ...
  \OptItem{\Opt{use} \Arg{category}\oArg{:set name} \oOpt{-expand}}
    {Display information about metaknobs (see
    ~\ref{sec:Configuring-HTCondor-Metaknobs}).
    Specifying only a \Arg{category} will list the \Arg{set\_names}
    available for that category.
    Specifying a \Arg{category} and a \Arg{set\_name} will display
    the definition of that metaknob.
    Adding the \Opt{-expand} option will display the expanded
    definition (with macro substitutions).  (\Opt{-expand} has no
    effect if a \Arg{set\_name} is not specified.)
...
Getting a list of set\_names for the \Opt{role} category:
\footnotesize
\begin{verbatim}
...
Getting the definition of \Opt{role:personal} metaknob:
\footnotesize
\begin{verbatim}
