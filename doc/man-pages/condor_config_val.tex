%TEMPTEMP -- add "use" option
%TEMPTEMP -- doesn't seem to list which options are location and which are view...
...
%TEMPTEMP -- do location options work w/ use?
\SynProg{\Condor{config\_val}}
\Opt{use}
%TEMPTEMP -- should these be Opt or Arg?
\Opt{<category>}
\oOpt{:<set name>}
\oOpt{-expand}
...
\begin{Options}
  ...
  \OptItem{\Opt{use} \Opt{<category>} \oOpt{:<set name>} \oOpt{-expand}}
    {
    TEMPTEMP -- note no dash before use
    TEMPTEMP -- no spaces around ":"
    TEMPTEMP -- see metaknob documentation
    TEMPTEMP -- category only lists set names available for that category
    TEMPTEMP -- category:set\_name shows the definition
    TEMPTEMP -- category:set\_name -expand shows expanded definition
    }
