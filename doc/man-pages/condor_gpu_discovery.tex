...
\begin{Options}
  ...
  \OptItem{\Opt{-properties}} {
    ...
    will have an integer value after the \Arg{prefix string}.
  ...
  \OptItem{\OptArg{-device}{<N>}}{
    ...
    value for \Arg{<N>} except as indicated above.  (However, a device that
    is detected but not reported counts as having different attributes than
    every other device of that type.)
    This option may be specified more than once; additional items are
    listed along with the first.
  }
  \OptItem{\OptArg{-device-list}{<N[,M]*>}}{
    Display properties only for the listed GPU device indexes.  This option
    may be specified more than once; additional lists are appended to the
    first.  You may freely intermix \Opt{-device} and \Opt{-device-list}.
  }
  \OptItem{\Opt{-filter}{
    When computing \Attr{DetectedGPUs}, do not include devices not specified
    on the command-line.  Additionally, ignore \AdStr{CUDA\_VISIBLE\_DEVICES}
    when reporting.  The effect is to cause the \Opt{-device} and
    \Opt{-device-list} options to specify a whitelist of GPUs to report on
    using their original device index.  This may be of use when HTCondor is
    not managing all of the GPU resources on a system.
  }
