...
\begin{Options}
  ...
  \OptItem{\OptArg{-priority}{number}}{%
	Sets the minimum job priority of jobs submitted by the DAGMan running under
    the \Condor{dagman} submitted by this \Condor{submit\_dag} command.}
  \OptItem{\Opt{-dont\_use\_default\_node\_log}}{%
	This argument tells \Condor{dagman} to use the file specified by the JobAd
    UserLog attribute to monitor job status. If this argument is given to
    \Condor{submit\_dag}, then the user log file cannot be defined with a macro.
    This is necessary if one is using a \Condor{dagman} version of at least 7.9.0
    and submitting to a \Condor{schedd} daemon with version number 7.8 or lower.}
