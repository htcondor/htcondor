...
\begin{Options}
  ...
  \OptItem{\Opt{-no\_recurse}}{This optional argument causes
     ...
     nodes specified with the \MacroNI{SUBDAG EXTERNAL} keyword or
     with submit file names ending in \File{.condor.sub} are
     considered nested DAGs, and, by default, \Condor{submit\_dag}
     \emph{-no\_submit -update\_submit} is run on the corresponding
     DAG files.)
  }
  ...
  \OptItem{\Opt{-valgrind}}{This optional argument causes the submit
     ...
     \emph{Note: this argument is intended for testing only.  If you specify
     the \Opt{-valgrind} argument, your DAG will fail unless you have
     the Valgrind software installed on your system.  If your DAG does
     run, it will run many times more slowly than normal.}
  }
  \end{Options}
