...
hold and send you email explaining what went wrong and what you can do
about it, (unless you set ``Notification = Never'' in your submit
file).

\item All Condor daemons and tools now have a CondorPlatform string,
which shows which platform a given set of Condor binaries was built
for. 
In all places that you used to see CondorVersion, you will now see
both CondorVersion and CondorPlatform, such as in each daemon's
ClassAd, in the output to a \Opt{version} option (if supported), and
when running \Prog{ident} on a given Condor binary.
...
\item In a Condor linked job, \Syscall{getdomainname} did not work. This
has been fixed.
