...
\item In HP-UX 10.20, Condor is currently broken with the vendor C++
compilers. 
However, the GNU C/C++ and the vendor C compilers work just fine.

\item \Condor{q} \Opt{-io} has not yet been fixed in this release.
...
\item On IRIX 6.2 with the GNU C++ 2.7.2 compiler, global object constructors
are broken when linked with \Condor{compile}.

\item On Digital Unix 4.0, the threads.C test program does not behave correctly
when compiled with the vendor C++.

\item \Condor{q} \Opt{-format} does not work when querying remote
machines with the \Opt{-n} option.

\item \Condor{dagman} is currently broken but should be fixed in the next
release.
