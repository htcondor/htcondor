...
recomend you just upgrade SMP-Startd contrib module, not the entire
6.1.3 release. 
...
suspend vanilla jobs.
While we always killed all of the processes when we were trying to
vacate, if the vanilla job forked, the starter would sometimes not
...
This could show up in \Condor{status}, \Condor{q}, \Condor{history}, etc.
  
\item Various commands sent to the Condor daemons now have seperate
...
\item Added a \Opt{-run} option to \Condor{q}, which diplays
...
See the man pages (secion~\ref{command-reference} on
page~\pageref{command-reference}) or run any command with ``-help''
for details. 
...
\Condor{view} Java GUI, which is available as a seperate contrib
module.
