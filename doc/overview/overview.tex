...
where individuals throughout an organization own thier own resources.
The total computational power of the institution as a whole might rise
dramitcally as the result of such a change, but because of distributed
ownership individuals could not capitalize on the insitutional growth of
...
checkpoints the job and then mirgrates it over the network to a
...
Perhaps you have to run this 5 hour compute job 250 differnt times
...
(espcicially at night, for example) at any given moment running your
job. 
...
share filesystems (via NFS or AFS for example), machines across the
entire enterprise can run your job, including machines in different
adminstrative domains. Condor does not even require you to have an
...
files and sends them back over the network to be perfomed on the machine
...
ClassAds work in a fasion similar to the newspaper classified
...
\section{Distingushing Features}

\begin{description}
	\item[Checkpoint and Mirgration.] Users of Condor may be assured that
...
executing on.
	\item[No Changes Neccesary to User’s Souce Code.] No special programming
...
provided by Condor
and, if these facilities are desired, only requires the user to relink
...
instance, a User can require their job run a mahcine with 64 megs of
...
URL \Url{http://www.cs.wisc.edu/condor} or via anonymous ftp to \Url{ftp://ftp.cs.wisc.edu/condor}.  
...
subscibe or unsubscribe from the the list, follow the instructions at  
