...
checkpoints the job and migrates it over the network to a
...
So, for example, you submit a compute job that typically takes 5
...
when submitting a job, users can specify a \Term{resource request} ad which
...
owner returns, the job can be checkpointed. The job continues
by migrating to another machine. It makes progress toward
...
event of a system failure such as the machine being shutdown or a crash.
	\item[Remote System Calls.] Despite running jobs on remote machines,
