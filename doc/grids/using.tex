...
Condor supports both the \SubmitCmd{gt2} and \SubmitCmd{gt4}
protocols.  It does not support \SubmitCmd{gt3}.
\begin{description}
\item[gt2]
...
job queue as an addtional job. It will leave the queue when there are
no more \SubmitCmd{gt4} in the queue.
If the submit machine has a permanent \Prog{GridFTP} server running, you
can instruct Condor to use it by setting \Macro{GRIDFTP\_URL\_BASE}.
Here's an example:
\begin{verbatim}
