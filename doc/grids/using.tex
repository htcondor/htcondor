...
setting a grid-type of \SubmitCmd{gt2} in the \SubmitCmd{grid\_resource}
submit command.
...
The second field specifies the scheduling software to be used on the remote resource.
There is a specific jobmanager for each type of
...
the submit description file is much the same as for
\SubmitCmd{gt2} or \SubmitCmd{gt3} jobs.
\index{submit commands!grid\_resource}
The \SubmitCmd{grid\_resource} command is still required,
...
\footnotesize
\begin{verbatim}
grid_resource = gt4 [https://]hostname[:port][/wsrf/services/ManagedJobFactoryService] Scheduler
\end{verbatim}
\normalsize
...
\footnotesize
\begin{verbatim}
grid_resource = gt4 [https://]IPaddress[:port][/wsrf/services/ManagedJobFactoryService] Scheduler
\end{verbatim}
\normalsize
The portions of this syntax specification enclosed within
square brackets (\Lbr\ and \Rbr) are optional.

The last field of \SubmitCmd{grid\_resource} indicates which job execution
system should to be used on the remote system to execute the job. One of
these values should be subsitituted for \SubmitCmd{Scheduler}:
\begin{verbatim}
Fork
Condor
PBS
...
\SubmitCmd{grid}
\SubmitCmd{universe} jobs with \SubmitCmd{grid\_type}
\SubmitCmd{gt2}.
