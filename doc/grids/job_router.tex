...
\item[EditJobInPlace] A boolean expression that, when true, causes the
original job to be transformed in place rather than creating a new
transformed version of the job.  In this mode, the routing hook
...
only once, an attribute could be set in the job ad to prevent it from
...
into the job ad marking the time of the last transformation, and the
