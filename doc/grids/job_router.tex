...
The Condor Job Router permits vanilla universe jobs to be
matched either with local pool resources, allowed to flock,
or modified by Condor and sent (routed) to available grid sites. 
Jobs are specified to use the vanilla universe,
and identify themselves as potential candidates for job routing.
The configuration together with the \Condor{job\_router} daemon
implement the details of which jobs are routed under what
circumstances.
