...
The Condor Job Router is an add-on to \condor{schedd} that transforms
...
One example of how the Job Router can be used is the task of sending
excess vanilla universe jobs to one or more remote grid sites.  The
Job Router can transform the vanilla universe jobs into grid universe
...
The routing table is given using \Term{New ClassAds},
...
\SubmitCmd{output}, as well as those explicitly listed
...
The conversion of a vanilla job to a routed copy requires the
...
failures is \Expr{other.RemoteWallClockTime < 1800}.  A more flexible
...
use the boolean expression to only allow specific values of \Expr{other.Owner}
