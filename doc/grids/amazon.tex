...
To run an instance in a VPC, set \SubmitCmd{ec2\_vpc\_subnet} to the
the desired VPC's specification string.  You may also specify the instance's
IP address by using \SubmitCmd{ec2\_vpc\_id}.
...
using \SubmitCmd{ec2\_iam\_profile\_name} or \SubmitCmd{ec2\_iam\_profile\_arn}.
...
\SubmitCmd{ec2\_parameter\_names}, a space- or comma- separated list.  The
...
\SubmitCmd{ec\_parameter\_name\_}; dots in the parameter name must be
