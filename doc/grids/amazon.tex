...
mapped on to kernel device names in order to be used.  The
\SubmitCmd{ec2\_block\_device\_mapping} submit description file command
allows you to specify these maps.  A map is a device name followed by a
colon followed by kernel name; maps are separated by commas or space.  For
instance, to specify that the first ephemeral device should be \Expr{/dev/sdb}
