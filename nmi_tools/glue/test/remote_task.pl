...
print "Test for test environment variable GCBTARGET yields <$ENV{GCBTARGET}>\n";
...
chdir("$BaseDir/$testdir/$compiler") || c_die("Can't chdir($BaseDir/$testdir/$compiler): $!\n");
my $run_out       = "$testname.run.out";
my $run_out_path  = "$BaseDir/$testdir/$compiler/$run_out";
my $test_out      = "$testname.out";
my $test_out_path = "$BaseDir/$testdir/$compiler/$test_out";
my $test_err      = "$testname.err";
my $test_err_path = "$BaseDir/$testdir/$compiler/$test_err";
...
#my $local_out = "$BaseDir/$testdir/TestingPersonalCondor/condor_config.local";
...
#} else {
#    print "\n\nERROR: failed to open $local_out: $!\n";
#}
