...
GIT_DIR=/home/condorauto/condor.git NMI_BIN=/usr/local/nmi/bin CONDOR_BIN=/usr/bin ./nmi_tools/condor_nmi_submit --notify=gthain@cs.wisc.edu,nwp@cs.wisc.edu --build --git --tag=origin/master --module=FOO --desc="Continuous Build - Fedora" --platforms="x86_64_fedora_14"  --notify-fail-only --sha1=$sha1 > condor_nmi_submit.out
